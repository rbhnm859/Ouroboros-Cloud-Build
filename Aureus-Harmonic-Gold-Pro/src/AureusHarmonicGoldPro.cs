...
namespace cAlgo.Robots
{
    ...
    public class AureusHarmonicGoldPro : Robot
    {
        ...
        [Parameter("Min Net RR", DefaultValue = 2.00, MinValue = 1.0, MaxValue = 8.0, Group = "Risk")]
        public double MinimumRiskReward { get; set; }

        [Parameter("SL ATR Buffer", DefaultValue = 0.20, MinValue = 0.0, MaxValue = 2.0, Group = "Risk")]
        public double StopAtrBuffer { get; set; }

        [Parameter("Breakeven At R", DefaultValue = 1.00, MinValue = 0.5, MaxValue = 5.0, Group = "Risk")]
        public double BreakEvenAtR { get; set; }

        [Parameter("Trail Start R", DefaultValue = 1.50, MinValue = 0.5, MaxValue = 8.0, Group = "Risk")]
        public double TrailStartR { get; set; }

        [Parameter("Trail ATR", DefaultValue = 1.10, MinValue = 0.2, MaxValue = 5.0, Group = "Risk")]
        ...
        [Parameter("Pivot Left", DefaultValue = 4, MinValue = 2, MaxValue = 20, Group = "Patterns")]
        public int PivotLeft { get; set; }

        [Parameter("Pivot Right", DefaultValue = 4, MinValue = 2, MaxValue = 20, Group = "Patterns")]
        ...
        [Parameter("Max M15 Pattern Age", DefaultValue = 8, MinValue = 1, MaxValue = 50, Group = "Patterns")]
        public int MaxM15PatternAge { get; set; }

        [Parameter("Max H1 Pattern Age", DefaultValue = 8, MinValue = 1, MaxValue = 60, Group = "Patterns")]
        public int MaxH1PatternAge { get; set; }

        [Parameter("Max H4 Pattern Age", DefaultValue = 5, MinValue = 1, MaxValue = 40, Group = "Patterns")]
        public int MaxH4PatternAge { get; set; }

        [Parameter("Min Pattern Score %", DefaultValue = 86.0, MinValue = 60.0, MaxValue = 100.0, Group = "Patterns")]
        ...
        protected override void OnStart()
        {
            ...
            Print("AUREUS v1.0 | Symbol={0} TF={1} Trading={2}", SymbolName, TimeFrame, TradingEnabled);
            Print("Entry source: confirmed M15 harmonic pattern; H1/H4 are harmonic-only confluence/conflict filters.");
        }
        ...
        protected override void OnBarClosed()
        {
            ...
            if (_m15.Count < 150 || _h1.Count < 150 || _h4.Count < 150)
                return;
            ...
            if (_consumedSignals.Contains(m15.SignalKey))
                return;

            if (_tradesToday >= MaxTradesPerDay)
                return;

            if (_m15.Count - 1 - _lastTradeBar < CooldownBars)
                return;

            if (HasAnySameSymbolExposure())
            {
                if (DebugLogging)
                    Print("[BLOCK] Existing same-symbol exposure prevents duplicate or hedge.");
                return;
            }
        ...
        private void ExecutePatternTrade(PatternMatch m)
        {
            ...
            try
            {
                ...
                double structuralExtreme = m.Direction == TradeType.Buy
                    ? Math.Min(m.X.Price, m.D.Price)
                    : Math.Max(m.X.Price, m.D.Price);
                double slPrice = m.Direction == TradeType.Buy
                    ? structuralExtreme - atr * StopAtrBuffer
                    : structuralExtreme + atr * StopAtrBuffer;
                ...
                if (slPips <= ExecutionReservePips)
                    return;
                ...
                if (!IsFinitePositive(tpDistance))
                    return;
                ...
                {
                    if (DebugLogging)
                        Print("[BLOCK] Fibonacci target net RR {0:F2} < {1:F2}", netRr, MinimumRiskReward);
                    return;
                }
                ...
                {
                    if (DebugLogging)
                        Print("[BLOCK] Risk budget check failed estimated={0:F2} budget={1:F2}", estimatedRisk, riskCash);
                    return;
                }

                string comment = string.Format("{0}|risk={1:F8}|score={2:F1}", m.Definition.Name, stopDistance, m.Score);
                ...
                Print("[OPEN] {0} {1} score={2:F1} vol={3} SL={4:F1}p TP={5:F1}p netRR={6:F2}",
                    m.Definition.Name, m.Direction, m.Score, volume, slPips, tpPips, netRr);
            }
            finally
            {
        ...
        private void ManagePosition(Position p)
        {
            ...
            double currentSl = p.StopLoss.HasValue ? p.StopLoss.Value : double.NaN;
            ...
            if (r >= BreakEvenAtR)
            {
                ...
                if (!IsFinitePositive(Math.Abs(desired)) ||
                    (p.TradeType == TradeType.Buy && (double.IsNaN(desired) || be > desired)) ||
                    (p.TradeType == TradeType.Sell && (double.IsNaN(desired) || be < desired)))
                    desired = be;
            }
        ...
        private bool ResolveTimeframeConflict(PatternMatch m15, PatternMatch h1, PatternMatch h4)
        {
            ...
            {
                if (DebugLogging) Print("[MTF BLOCK] No same-direction H1/H4 harmonic agreement.");
                return false;
            }
        ...
        private PatternMatch FindBestPattern(Bars bars, int maxAge, double minScore)
        {
            ...
            {
                PivotPoint x = pivots[i];
                PivotPoint a = pivots[i + 1];
                PivotPoint b = pivots[i + 2];
                PivotPoint c = pivots[i + 3];
                PivotPoint d = pivots[i + 4];
                ...
                if (!IsFinitePositive(xa) || !IsFinitePositive(ab) || !IsFinitePositive(bc) || !IsFinitePositive(cd))
                    continue;
                ...
                foreach (PatternDefinition def in _patterns)
                {
                    ...
                    if (best == null || match.Score > best.Score || (Math.Abs(match.Score - best.Score) < 0.0001 && match.D.Index > best.D.Index))
                        best = match;
                }
            }
        ...
        private List<PivotPoint> BuildConfirmedPivots(Bars bars)
        {
            ...
            if (pivots.Count > 100)
                pivots = pivots.GetRange(pivots.Count - 100, 100);
        ...
        private bool PassRiskGovernor()
        {
            ...
            if (LossPercent(_dayStartEquity, Account.Equity) >= MaxDailyLossPercent && MaxDailyLossPercent > 0)
                return false;

            if (LossPercent(_weekStartEquity, Account.Equity) >= MaxWeeklyLossPercent && MaxWeeklyLossPercent > 0)
                return false;

            if (LossPercent(_monthStartEquity, Account.Equity) >= MaxMonthlyLossPercent && MaxMonthlyLossPercent > 0)
                return false;
        ...
        private sealed class PatternMatch
        {
            ...
            public string SignalKey
            {
                get
                {
                    return string.Format("{0}|{1}|{2:O}|{3:F5}", Definition.Name, Direction, DTime, D.Price);
                }
            }

            public PatternMatch(PatternDefinition definition, TradeType direction,
                PivotPoint x, PivotPoint a, PivotPoint b, PivotPoint c, PivotPoint d,
                double score, DateTime dTime)
            {
        ...
        private sealed class RatioRule
        {
            ...
            public RatioRule(double min, double max)
            {
                Min = min;
                Max = max;
            }
            ...
            public double Score(double value, double tolerancePercent)
            {
                ...
                double width = Math.Max((Max - Min) / 2.0, center * tolerancePercent / 100.0);
                double lo = Min - center * tolerancePercent / 100.0;
                double hi = Max + center * tolerancePercent / 100.0;
                ...
                if (value >= Min && value <= Max)
                {
                    if (Max - Min < 0.000001)
                        return 100;
                    double d = Math.Abs(value - center) / Math.Max(width, 0.000001);
                    return Math.Max(88.0, 100.0 - d * 12.0);
                }

                double edgeDistance = value < Min ? Min - value : value - Max;
                double tolerance = Math.Max(center * tolerancePercent / 100.0, 0.000001);
                return Math.Max(70.0, 88.0 - 18.0 * edgeDistance / tolerance);
            }
        }
        ...
        private sealed class PatternDefinition
        {
            ...
            public PatternDefinition(string name, RatioRule bXa, RatioRule cAb, RatioRule dXa, RatioRule cdBc, RatioRule cdAb)
            {
                Name = name;
                B_XA = bXa;
                C_AB = cAb;
            ...
            public double Score(double bXa, double cAb, double dXa, double cdBc, double cdAb, double tolerance)
            {
                double[] scores =
                {
                    B_XA.Score(bXa, tolerance),
                    C_AB.Score(cAb, tolerance),
                    D_XA.Score(dXa, tolerance),
                    CD_BC.Score(cdBc, tolerance),
                    CD_AB.Score(cdAb, tolerance)
                };
            ...
            public static List<PatternDefinition> Create()
            {
                return new List<PatternDefinition>
                {
                    new PatternDefinition("Gartley",
                        new RatioRule(0.618, 0.618),
                        new RatioRule(0.382, 0.886),
                        new RatioRule(0.786, 0.786),
                        new RatioRule(1.272, 1.618),
                        new RatioRule(1.000, 1.272)),

                    new PatternDefinition("Bat",
                        new RatioRule(0.382, 0.500),
                        new RatioRule(0.382, 0.886),
                        new RatioRule(0.886, 0.886),
                        new RatioRule(1.618, 2.618),
                        new RatioRule(1.000, 1.618)),

                    new PatternDefinition("Butterfly",
                        new RatioRule(0.786, 0.786),
                        new RatioRule(0.382, 0.886),
                        new RatioRule(1.270, 1.270),
                        new RatioRule(1.618, 2.240),
                        new RatioRule(1.270, 1.618)),

                    new PatternDefinition("Crab",
                        new RatioRule(0.382, 0.618),
                        new RatioRule(0.382, 0.886),
                        new RatioRule(1.618, 1.618),
                        new RatioRule(2.240, 3.618),
                        new RatioRule(1.618, 2.618)),
