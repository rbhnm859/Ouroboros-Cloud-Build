...
if policy.get('candidate_count') != 1:
    raise SystemExit('RC7 must remain one predeclared candidate')
...
if not (30 <= ttl_minutes <= 180):
    raise SystemExit('RC7 deferred TTL outside predeclared safety range')
if not (1 <= max_age_bars <= 6):
    raise SystemExit('RC7 deferred max-age invalid')
if not (0 <= blackout_min <= 5):
    raise SystemExit('RC7 H1 reservation blackout invalid')

main = out.read_text().replace('BTC-Harmonic-Guard-Commercial-RC6', 'BTC-Harmonic-Guard-Commercial-RC7')

# RC7 removes the legacy blind top-of-hour reservation and replaces it with
# deterministic arbitration only after a valid H1 candidate has passed Alpha/quality gates.
...
if direct_total != 1 or central != 1 or final_main.count('ExecuteMarketOrder(') != 0:
    raise SystemExit(f'RC7 single execution boundary failed direct={direct_total} central={central}')
...
if 'Round21ReserveH1Lane();' in final_main[final_main.find('protected override void OnTick()'):final_main.find('protected override void OnStop()')]:
    raise SystemExit('RC7 legacy blind reservation still active in OnTick')
if '[GROWTH3 SHADOW]' not in (base / 'Growth3.Architecture.cs').read_text():
    raise SystemExit('RC7 Growth3 shadow invariant failed')
if 'ABCD_NEGATIVE_CROSS_PERIOD_ATTRIBUTION' not in (base / 'Commercial.SignalPolicy.cs').read_text():
    raise SystemExit('RC7 ABCD shadow invariant failed')
print('Commercial RC7B generated: H1-core deterministic arbitration + deferred M30 intent/TTL + decision ledger; Alpha/base risk/gates unchanged')
