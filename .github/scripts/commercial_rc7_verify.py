...
for p in required:
    if not p.exists(): raise SystemExit(f'missing generated source {p}')
...
console=(ROOT/'commercial-rc7-validation-console.log').read_text(errors='replace') if (ROOT/'commercial-rc7-validation-console.log').exists() else ''
...
engineering={
    ...
    'h1_deterministic_arbitration': 'CommercialArbitrateH1BeforeExecution(best)' in main,
    ...
    'deferred_ttl': 'ExpiresUtc' in arb and 'EXPIRE_BARS' in arb,
...
full=summary['selected_3y']; recent=summary['recent']; h3=summary['harsh_3y']; hr=summary['harsh_recent']; annual=summary['annual']
