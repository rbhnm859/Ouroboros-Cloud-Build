...
G2 = ROOT / "g2"
POLICY = Path(".github/policies/commercial_rc6_policy.json")
OUT = ROOT / "Commercial-RC6-Gate.json"
...
def items(path: Path):
    data = load_json(path)
    return data.get("history", {}).get("items", [])
...
if policy.get("candidate_count") != 1:
    fail("RC6 must contain exactly one predeclared candidate")
...
for p in required_source:
    if not p.exists():
        fail(f"missing generated source: {p}")
...
# The immutable Store input alias can coexist in work/g2 and is excluded from the
# generated compile-set count. All live orders must cross the central boundary.
...
console_path = ROOT / "commercial-rc6-validation-console.log"
...
engineering = {
    ...
    "m30_policy_instrumented": "CommercialEvaluateM30Candidate" in signal_policy and "[COMMERCIAL POLICY]" in signal_policy,
    ...
    "reduction_only_governor": "Math.Min(1.0" in risk_governor and "CommercialRiskGovernorFactorV6" in risk_governor,
...
annual_rows = list(annual.values())
...
r3 = items(ROOT / "moderate-3y" / "report.json")
rr = items(ROOT / "recent-a" / "report.json")
...
payload = {
    "candidate": "BTC Harmonic Guard Commercial RC6A Policy Engine",
    ...
    "architecture": {
        ...
        "policy_decision_telemetry": "[COMMERCIAL POLICY]",
        "risk_shadow_telemetry": "[COMMERCIAL RISK SHADOW]",
    },
...
# Performance rejection is a valid experiment result and must still upload full
# evidence. Only engineering/harness failure makes the verifier step fail.
