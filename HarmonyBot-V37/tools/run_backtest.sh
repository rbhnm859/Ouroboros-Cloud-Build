...
CNAME="v37-$(echo "$RUN_NAME"|tr '[:upper:]_' '[:lower:]-')-$GITHUB_RUN_ID"
docker run --name "$CNAME" -v "$PWD/seal:/work" "$IMAGE" backtest "/work/${ALGO#seal/}" \
 ...
 --EnableM15ConfirmationGate=true --M15MinConfirmationScore=0.45 --EnableM5ExecutionRefinement=${M5REFINE:-false} --M5MinConfirmationScore=0.55 --EnableCapitalFeasibilityGate=false --EnableTransitionStateVeto=false \
 --EnableExhaustionEvidenceVeto=true --EnableQualifiedDynamicReroute=${QREROUTE:-false} --RerouteMinRobustness=0.62 --RerouteMinTimeSymmetry=0.35 --RerouteMinPivotQuality=0.35 \
...
PID=$!; DONE=0
echo "[V37-WATCHDOG] run=$RUN_NAME pid=$PID timeoutSeconds=$BACKTEST_TIMEOUT_SECONDS"
for ((i=0;i<BACKTEST_TIMEOUT_SECONDS;i+=5)); do
 if (( i % 60 == 0 )); then echo "[V37-WATCHDOG] run=$RUN_NAME elapsedSeconds=$i status=running"; fi
...
test "$DONE" = 1 || { echo "[V37-WATCHDOG-FAIL] run=$RUN_NAME"; tail -400 "seal/logs/$RUN_NAME.log" || true; exit 20; }
echo "[V37-WATCHDOG] run=$RUN_NAME status=complete"
