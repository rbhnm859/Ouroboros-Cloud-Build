...
rx=re.compile(r"\[V37-BASKET-CLOSED\].*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
pat=(r"\[V37-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
freq=re.findall(r"\[V37-FREQUENCY-SUMMARY\].*?schedulerDeferred=(\d+)\s+schedulerRecoveredExecutions=(\d+)\s+structuredRecallAdmitted=(\d+)\s+activeDeferred=(\d+)",t)
...
"broker_profile_present":"[V37-BROKER-PROFILE]" in t,
"transition_veto_events":len(re.findall(r"\[V37-ROUTE-VETO\] type=TRANSITION_STATE",t)),
"exhaustion_veto_events":len(re.findall(r"\[V37-ROUTE-VETO\] type=EXHAUSTION_EVIDENCE",t)),
"m5_refinement_events":len(re.findall(r"M5_CONFIRMATION",t)),
"recall_admit_events":len(re.findall(r"\[V37-RECALL-ADMIT\]",t)),
