...
rx=re.compile(r"\[V46-BASKET-CLOSED\].*?cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+subtype=(\S+)\s+route=(\S+)\s+dir=(\S+).*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)\s+reason=(\S+)")
b=[]
...
pat=(r"\[V46-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
im=re.findall(r"\[V46-INDEPENDENT-SETUP-SUMMARY\].*?duplicateSuppressed=(\d+)\s+uniqueExecuted=(\d+)\s+rescueAdmissions=(\d+)\s+transitionProofRejected=(\d+)\s+independentScaleCandidates=(\d+)",t)
...
cm=re.findall(r"\[V46-CONVERSION-SUMMARY\].*?scaleRouteRejected=(\d+)\s+temporalRescueAdmissions=(\d+)\s+armedGraceExtended=(\d+)\s+preExecutionRevalidationRejected=(\d+)",t)
conv={"scale_route_rejected":0,"temporal_rescue_admissions":0,"armed_grace_extended":0,"pre_execution_revalidation_rejected":0}
if cm:
 for k,z in zip(conv.keys(),map(int,cm[-1])): conv[k]=z

pipe_rx=re.compile(r"\[V46-PIPELINE\]\s+pattern=(.*?)\s+detected=(\d+)\s+validated=(\d+)\s+routed=(\d+)\s+prz=(\d+)\s+confirming=(\d+)\s+armed=(\d+)\s+basketPlanned=(\d+)\s+leg0=(\d+)\s+leg1=(\d+)\s+leg2=(\d+)\s+leg3=(\d+)\s+basketClosed=(\d+)\s+executed=(\d+)\s+expired=(\d+)\s+rejected=(\d+)\s+invalidated=(\d+)")
pipeline={}
...
"engineering_clean":clean,"summary_present":summary_present,"broker_profile_present":"[V46-BROKER-PROFILE]" in t,
"mean_mfe_r":statistics.mean([x["mfe"] for x in b]) if b else 0,"mean_mae_r":statistics.mean([x["mae"] for x in b]) if b else 0,
"basket_outcomes":b,"pattern_pipeline":pipeline,**ind,**conv,**c}
