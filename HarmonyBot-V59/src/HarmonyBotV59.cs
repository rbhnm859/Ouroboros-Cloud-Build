...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV59 : Robot
    {
        private const string Version = "HarmonyBot V59 — Family-Native Minimal-Coordinate Harmonic Manifold & Opportunity-Density / AB=CD Causal Repair Engine";
        private const string BotPrefix = "HB59";
        ...
        [Parameter("V59 Family Books", DefaultValue = true)]
        public bool EnableV59FamilyBooks { get; set; }

        [Parameter("V59 Canonical Manifold", DefaultValue = true)]
        public bool EnableV59CanonicalManifold { get; set; }

        [Parameter("V59 Pure Projected PRZ", DefaultValue = true)]
        public bool EnableV59PureProjectedPrz { get; set; }

        [Parameter("V59 Temporal Event DAG", DefaultValue = true)]
        public bool EnableV59TemporalEventDag { get; set; }

        [Parameter("V59 Excursion Capture Research", DefaultValue = true)]
        public bool EnableV59ExcursionCaptureResearch { get; set; }

        [Parameter("V59 Excursion Capture Live", DefaultValue = false)]
        public bool EnableV59ExcursionCaptureLive { get; set; }

        [Parameter("V59 Matched Control Attribution", DefaultValue = true)]
        public bool EnableV59MatchedControlAttribution { get; set; }

        [Parameter("V59 Max Projection Error ATR", DefaultValue = 0.55, MinValue = 0.20, MaxValue = 1.50)]
        public double V59MaxProjectionErrorAtr { get; set; }

        [Parameter("V59 Manifold Sigma", DefaultValue = 1.00, MinValue = 0.50, MaxValue = 2.50)]
        public double V59ManifoldSigma { get; set; }

        [Parameter("V59 Minimal Coordinate Manifold", DefaultValue = true)]
        public bool EnableV59MinimalCoordinateManifold { get; set; }

        [Parameter("V59 Opportunity Density Ledger", DefaultValue = true)]
        public bool EnableV59OpportunityDensityLedger { get; set; }

        [Parameter("V59 Hierarchical Partial Pooling", DefaultValue = true)]
        public bool EnableV59HierarchicalPartialPooling { get; set; }

        [Parameter("V59 Family Effective Sample Support", DefaultValue = 24, MinValue = 24, MaxValue = 240)]
        public int V59FamilyEffectiveSampleSupport { get; set; }

        [Parameter("V59 AB=CD Algebra Contract", DefaultValue = true)]
        public bool EnableV59AbcdAlgebraContract { get; set; }

        [Parameter("V59 AB=CD Terminality Research", DefaultValue = true)]
        public bool EnableV59AbcdTerminalityResearch { get; set; }

        [Parameter("V59 Universal Research Continuation", DefaultValue = true)]
        public bool EnableV59UniversalResearchContinuation { get; set; }

        [Parameter("V59 Virtual Parallel Execution", DefaultValue = true)]
        public bool EnableV59VirtualParallelExecution { get; set; }

        [Parameter("V59 Calibrated Capital Admission", DefaultValue = false)]
        public bool EnableV59CalibratedCapitalAdmission { get; set; }

        [Parameter("V59 Family Representative Arbitration", DefaultValue = true)]
        public bool EnableV59FamilyRepresentativeArbitration { get; set; }

        [Parameter("V59 Standalone AB=CD Capital", DefaultValue = false)]
        public bool EnableV59AbcdStandaloneCapital { get; set; }

        [Parameter("V59 Grid Execution", DefaultValue = true)]
        public bool EnableV59GridExecution { get; set; }

        [Parameter("V59 Min Calibrated Lower R", DefaultValue = 0.0, MinValue = -1.0, MaxValue = 1.0)]
        public double V59MinCalibratedLowerBoundR { get; set; }

        [Parameter("V59 Calibration Manifest", DefaultValue = "")]
        public string V59CalibrationManifest { get; set; }
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V59-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V59-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V59-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V59-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            InitializeV59FamilyBooks();
            ParseV59CalibrationManifest();
            Print("[V59-FAMILY-PORTFOLIO] familyBooks={0} virtualParallel={1} calibratedAdmission={2} representatives={3} abcdStandalone={4} calibrationCells={5}",
                EnableV59FamilyBooks, EnableV59VirtualParallelExecution, EnableV59CalibratedCapitalAdmission,
                EnableV59FamilyRepresentativeArbitration, EnableV59AbcdStandaloneCapital, _calibrationCells.Count);
            ...
            Print("[V59-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m1Bars), _profiles.Count);
            Print("[V59-MATH-ARCH] minimalCoordinates={0} abcdAlgebra={1} abcdTerminalityResearch={2} partialPooling={3} familyESS={4} opportunityDensity={5} rule=MAXIMIZE_RESEARCH_SUPPLY_WITHOUT_RELAXING_CAPITAL_RISK",
                EnableV59MinimalCoordinateManifold, EnableV59AbcdAlgebraContract, EnableV59AbcdTerminalityResearch,
                EnableV59HierarchicalPartialPooling, V59FamilyEffectiveSampleSupport, EnableV59OpportunityDensityLedger);
            Print("[V59-TIMEFRAME-AUDIT] primaryPattern=M15 execution=M1 macro=H4 intermediate=H1 allCompletedBars=true");
            Print("[V59-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
            Print("[V59-ALPHA-CONFIG] qualityObservation={0} legacyRegime={1} legacyEnhancedM1={2} capitalFeasibility={3} transitionVeto={4} exhaustionVeto={5} routeM1Veto={6}",
            ...
            Print("[V59-FREQUENCY-CONFIG] deferredRetention={0} agingPriority={1} ageBoost={2:F3} structuredRecall={3} recallGeometry={4:F3} recallPrz={5:F3} recallConfidence={6:F3}",
            ...
            Print("[V59-RESTORED-KERNEL] canonicalSetup={0} canonicalStandard={1} independentPivotGraph={2} transitionProof={3} m1Rescue={4} rescueBars={5} diversityScheduler={6} executionClock=M1 projectedD=false",
            ...
            Print("[V59-CONVERSION-ARCH] scaleRouteAdmission={0} temporalRescue={1} armedGrace={2} graceMinutes={3} preExecutionRevalidation={4}",
            ...
            Print("[V59-FAMILY-NATIVE] conversion={0} observation={1} principle=PATTERN_IDENTITY_NEQ_EXECUTION_IDENTITY", EnableFamilyNativeConversion, EnableFamilyNativeObservation);
            Print("[V59-COMPLETION-CONTRACT] canonicalContracts={0} familyCompletion={1} windowBars={2} provenLane=ABCD_SHARK_CYPHER_FROZEN",
                EnableCanonicalFamilyContracts, EnableFamilyCompletionContract, FamilyConfirmationWindowBars);
            Print("[V59-STRUCTURAL-GRID-CONTRACT] gridSpanV2={0} structuralInvalidationV2={1} rule=PRZ_LEGALITY_PLUS_RISK_RR_NOT_LEGACY_XA_MINSPAN",
                EnableGridSpanSemanticV2, EnableStructuralInvalidationV2);
            Print("[V59-MATH-GEOMETRY] jointGeometry={0} executionCorridor={1} entryAnchorForensics={2} rule=FAMILY_NATIVE_GEOMETRY_WITHOUT_RISK_RELAXATION",
                EnableFamilyNativeJointGeometry, EnableFamilyNativeExecutionCorridor, EnableEntryAnchorForensics);
            Print("[V59-DETECTOR-ARCH] identityReconstruction={0} boundedPivotGraph={1} maxMicroSkips={2} familyQuota={3} projectedPrz={4} truthLedger={5} rule=DETECT_MANY_EXECUTE_ONE",
            ...
            Print("[V59-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V46_SCALE_CONVERSION_FROZEN",
        ...
        {
            CloseOpenV59ShadowTradesAtStop();
            foreach (var book in _familyBooks.OrderBy(x => x.Key))
            {
                ...
                Print("[V59-FAMILY-BOOK] pattern={0} candidates={1} shadowStarted={2} shadowClosed={3}", book.Key, book.Value.Count, started, closed);
            }
            ...
            {
                var x = kv.Value;
                Print("[V59-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} nativeTemporalPass={6} armed={7} slotBlocked={8} parked={9} revalidated={10} revalidationRejected={11} basketPlanned={12} leg0={13} leg1={14} leg2={15} leg3={16} basketClosed={17} executed={18} expired={19} rejected={20} invalidated={21}",
            ...
            if (EnableEntryAnchorForensics)
            {
                ...
                {
                    Print("[V59-ENTRY-ANCHOR-FORENSICS] cid={0} setup={1} pattern={2} route={3} scale={4} completion={5} confirm={6} retest={7} completionMfeR={8:F3} completionMaeR={9:F3} confirmMfeR={10:F3} confirmMaeR={11:F3} retestMfeR={12:F3} retestMaeR={13:F3}",
            ...
            if (EnableDetectorTruthLedger)
            {
                foreach (var kv in _detectorTruth.OrderBy(x => x.Key))
                {
                    ...
                    Print("[V59-DETECTOR-TRUTH] pattern={0} stage={1} count={2}", pattern, stage, kv.Value);
                }
            }
            Print("[V59-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V59-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
                _alphaQualityRejected, _regimeRejected, _confirmationRejected, _capitalInfeasibleCandidates, _alphaPassed);
            Print("[V59-FREQUENCY-SUMMARY] schedulerDeferred={0} schedulerRecoveredExecutions={1} structuredRecallAdmitted={2} activeDeferred={3}",
                _schedulerDeferred, _schedulerRecoveredExecutions, _structuredRecallAdmitted, _deferredCandidates.Count);
            Print("[V59-INDEPENDENT-SETUP-SUMMARY] duplicateSuppressed={0} uniqueExecuted={1} rescueAdmissions={2} transitionProofRejected={3} independentScaleCandidates={4}",
            ...
            Print("[V59-CONVERSION-SUMMARY] scaleRouteRejected={0} temporalRescueAdmissions={1} armedGraceExtended={2} preExecutionRevalidationRejected={3}",
            ...
            Print("[V59-THROUGHPUT-SUMMARY] slotBlocked={0} parked={1} revalidated={2} revalidationRejected={3} recoveredExecutions={4} nativeTemporalPass={5} hardLifetimeExpired={6} decayRejected={7} avgSlotWaitMin={8:F2} medianSlotWaitMin={9:F2} p90SlotWaitMin={10:F2} avgBasketOccupancyMin={11:F2} missedPositive={12} avoidedNegative={13}",
            ...
            foreach (var p in _profiles.Select(x => x.Name).Distinct().OrderBy(x => x))
            {
                ...
                Print("[V59-FAMILY-CONTRACT-SUMMARY] pattern={0} pass={1} windowReject={2}", p, pass, reject);
            }
            foreach (var kv in _gridPlanRejectReasons.OrderBy(k => k.Key))
                Print("[V59-GRID-REJECT-SUMMARY] reason={0} count={1}", kv.Key, kv.Value);
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V59-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
            ...
            foreach (var signal in detected)
            {
                    ...
                    signal.PrzConfluence < Math.Max(MinPrzConfluence, signal.Profile.MinPrz))
                {
                    if (EnableV59UniversalResearchContinuation && EnableV59VirtualParallelExecution)
                    {
                        ...
                        Event(record, "V59_RESEARCH_CONTINUE_PATTERN_QUALITY");
                    }
                    else
                    {
                ...
                if (record.Route == HarmonicRoute.NO_TRADE)
                {
                    ...
                    {
                        record.ResearchOnlyHypothesis = true;
                        record.Route = V59ResearchRoute(signal, record.Conflict, regime);
                        if (record.Route == HarmonicRoute.NO_TRADE)
                            record.Route = HarmonicRoute.TRANSITION_REVERSAL;
                        Event(record, "V59_RESEARCH_CONTINUE_ROUTER_NO_TRADE");
                    }
                    else
                    {
                ...
                Print("[V59-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void ProcessNewM1Close()
        {
            ...
            UpdateV59FamilyShadowTrades(i, utc);
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    StartV59FamilyShadowTrade(c, i, utc);
                    ...
                    if (EnableV59CalibratedCapitalAdmission && !V59CalibratedCapitalEligible(c))
                    {
        ...
        private void TryScheduleAndExecute()
        {
            ...
            if (EnableV59FamilyRepresentativeArbitration)
            {
                armedQuery = armedQuery
                    .GroupBy(c => c.Signal == null ? "UNKNOWN" : c.Signal.PatternName)
                    .Select(g => g.OrderByDescending(c => V59FamilyCapitalScore(c, now)).ThenByDescending(c => c.Rank).First());
            }

            var armed = armedQuery
                .OrderByDescending(c => EnableV59FamilyRepresentativeArbitration ? V59FamilyCapitalScore(c, now) :
        ...
        private void MarkOpportunityTerminal(CandidateRecord c, string reason)
        {
            ...
            Print("[V59-OPPORTUNITY-LOSS] cid={0} setup={1} pattern={2} route={3} scale={4} reason={5} shadowMfeR={6:F3} shadowMaeR={7:F3} missedPositive={8} avoidedNegative={9}",
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            int routeMax = !EnableV59GridExecution ? 1 :
            ...
            Print("[V59-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V59-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V59-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void ReconcileAndManageBaskets()
        {
            ...
            foreach (var basket in _baskets.Values.Where(b => b.IsActive).ToList())
            {
                ...
                if (EnableV59ExcursionCaptureLive && basket.PeakR >= .75)
                {
                    ...
                    double lockPrice = basket.Direction == TradeDirection.Buy
                        ...
                        : basket.AverageEntry - span * lockR;
                    AdvanceBasketProtectionFrontier(basket, lockPrice, basket.PeakR >= 1.25 ? "V59_CAPTURE_TRAIL" : "V59_CAPTURE_PROTECT");
                }
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V59-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V59-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V59-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V59-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V59-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V59-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V59-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V59-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V59-BASKET-CLOSED] basket={0} cid={1} setup={2} pattern={3} subtype={4} route={5} dir={6} plannedLegs={7} filledLegs={8} anchor={9} avgEntry={10} entryImprovePips={11:F3} stop={12} target={13} initialRisk={14:F2} worstRisk={15:F2} mfeR={16:F3} maeR={17:F3} realizedR={18:F3} net={19:F2} reason={20}",
            ...
            Print("[V59-SLOT-OCCUPANCY] basket={0} cid={1} pattern={2} route={3} occupancyMinutes={4:F2} realizedR={5:F3} net={6:F2}",
        ...
        {
            Print("[V59-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
            ...
            double atr, DateTime completion, string timeframe, int pivotScale, out PatternSignal signal)
        {
            ...
            if (p.Mode == PatternMode.ABCD)
                ratioOk = V59AbcdDiscreteIdentity(abc, bcd, abcd);
            ...
            double geometry = EnableV59CanonicalManifold
                ? V59CanonicalManifoldScore(p, xab, abc, bcd, xad, abcd, xac, out manifoldDistance)
            ...
            double expectedD = EnableV59PureProjectedPrz
                ? V59PureProjectedPrzCenter(p, x, a, b, c, bullish, out projectionSpread)
            ...
            if (!EnableV59PureProjectedPrz && !(EnableFamilyIdentityReconstruction && EnableFamilyNativeProjectedPrz)) projectionSpread = 0;
            ...
            double projectionPurity = VClamp(Math.Exp(-0.5 * Math.Pow(projectionErrorAtr / Math.Max(.15, V59MaxProjectionErrorAtr), 2.0)));
            ...
            double przConfluence = EnableV59PureProjectedPrz
        ...
        private double[] V59AbcdCenters(string pattern)
        {
        ...
        private double V59NearestCanonical(double value, double[] centers)
        {
            return centers.OrderBy(v => Math.Abs(v - value)).First();
        }

        private bool V59AbcdDiscreteIdentity(double abc, double bcd, double abcd)
        {
            if (!InRange(abc, .382, .886) || bcd <= 0 || abcd <= 0) return false;

            // V59 algebra contract:
            ...
            double k = V59NearestCanonical(abcd, V59AbcdCenters("AB=CD"));
            ...
            return reciprocal && (!EnableV59AbcdAlgebraContract || algebra);
        }

        private double V59PureProjectedPrzCenter(PatternProfile p, PivotPoint x, PivotPoint a, PivotPoint b, PivotPoint c,
            bool bullish, out double halfSpread)
        {
            ...
            if (p.Mode == PatternMode.STANDARD)
            {
                ...
                double bestAbcd = V59AbcdCenters(p.Name).Select(k => c.Price + sign * k * ab).OrderBy(v => Math.Abs(v - anchor)).First();
            ...
            else if (p.Mode == PatternMode.ABCD)
            {
                ...
                // Standalone AB=CD remains research-only by default, so V59 emits the canonical
                ...
                foreach (double k in V59AbcdCenters("AB=CD"))
        ...
        {
            return V59NearestCanonical(observed, V59AbcdCenters(pattern));
        }
        ...
        private double FamilyProjectedPrzCenter(PatternProfile p, PivotPoint x, PivotPoint a, PivotPoint b, PivotPoint c, PivotPoint d,
            bool bullish, out double halfSpread)
        {
            return V59PureProjectedPrzCenter(p, x, a, b, c, bullish, out halfSpread);
        }
        ...
        private double V59CanonicalManifoldScore(PatternProfile p, double xab, double abc, double bcd, double xad, double abcd, double xac, out double distance)
        {
            // V59 minimal-coordinate rule: do not count algebraically dependent ratios as
            ...
            double sigma = Math.Max(.50, V59ManifoldSigma);
        ...
        private HarmonicRoute RouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (baseRoute == HarmonicRoute.NO_TRADE && EnableStructuredRecallExpansion)
            {
                ...
                if (baseRoute != HarmonicRoute.NO_TRADE)
                {
                    ...
                    Print("[V59-RECALL-ADMIT] pattern={0} direction={1} route={2} conflict={3} geometry={4:F3} prz={5:F3} confidence={6:F3} efficiency={7:F3} atrRatio={8:F3} extensionAtr={9:F3}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionProofGate)
            {
                ...
                if (!actualStructuralTransition)
                {
                    ...
                    Print("[V59-TRANSITION-PROOF-REJECT] pattern={0} conflict={1} transition={2} adxSlope={3:F2}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionStateVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V59-ROUTE-VETO] type=TRANSITION_STATE conflict={0} transition={1} adxSlope={2:F2}",
            ...
            if (baseRoute == HarmonicRoute.EXHAUSTION_REVERSAL && EnableExhaustionEvidenceVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V59-ROUTE-VETO] type=EXHAUSTION_EVIDENCE extensionAtr={0:F3} adxSlope={1:F2}",
        ...
        private void InitializeV59FamilyBooks()
        {
            if (!EnableV59FamilyBooks) return;
            foreach (var p in _profiles)
                if (!_familyBooks.ContainsKey(p.Name)) _familyBooks[p.Name]=new HashSet<string>();
        }

        private void AddToV59FamilyBook(CandidateRecord c)
        {
            if (!EnableV59FamilyBooks || c==null || c.Signal==null) return;
            HashSet<string> book;
        ...
        private string V59PatternToken(string pattern)
        {
            return (pattern??"UNKNOWN").Replace(" ","_");
        }

        private string V59RegimeBucket(CandidateRecord c)
        {
        ...
        private HarmonicRoute V59ResearchRoute(PatternSignal s,MtfConflict conflict,RegimeSnapshot r)
        {
            if(s==null||r==null)return HarmonicRoute.NO_TRADE;
            string p=s.PatternName??"";
            bool ext=p=="Alt Bat"||p=="Butterfly"||p=="Crab"||p=="Deep Crab";
        ...
        private string V59CalibrationKey(CandidateRecord c)
        {
            if(c==null||c.Signal==null)return "UNKNOWN";
            return V59PatternToken(c.Signal.PatternName)+"|"+c.Route+"|"+V59RegimeBucket(c);
        }

        private void ParseV59CalibrationManifest()
        {
            _calibrationCells.Clear();
            if(string.IsNullOrWhiteSpace(V59CalibrationManifest))return;
            foreach(var raw in V59CalibrationManifest.Split(new[]{';'},StringSplitOptions.RemoveEmptyEntries))
            {
        ...
        private CalibrationCell V59CalibrationCell(CandidateRecord c)
        {
            CalibrationCell x;
            return _calibrationCells.TryGetValue(V59CalibrationKey(c),out x)?x:null;
        }

        private bool V59CalibratedCapitalEligible(CandidateRecord c)
        {
            if(c==null||c.Signal==null)return false;
            if(c.ResearchOnlyHypothesis)return false;
            if(string.Equals(c.Signal.PatternName,"AB=CD",StringComparison.OrdinalIgnoreCase)&&!EnableV59AbcdStandaloneCapital)
                return false;
            var cell=V59CalibrationCell(c);
            ...
            return cell.LowerBoundR>V59MinCalibratedLowerBoundR && cell.Samples>0 && c.CalibratedRPerSlotHour>0;
        }

        private double V59FamilyCapitalScore(CandidateRecord c, DateTime now)
        {
            var cell=V59CalibrationCell(c);
            if(cell!=null)
            {
                double slot=cell.LowerBoundR/Math.Max(.25,cell.MeanHoldMinutes/60.0);
                return slot+.05*(EnableOpportunityDecayRanking?OpportunityScore(c,now):c.Rank);
            }
            return EnableOpportunityDecayRanking?OpportunityScore(c,now):c.Rank;
        }

        private void StartV59FamilyShadowTrade(CandidateRecord c,int i,DateTime utc)
        {
            if(!EnableV59VirtualParallelExecution||c==null||c.Signal==null||_familyShadowTrades.ContainsKey(c.CandidateId))return;
            ...
            {
                Print("[V59-SHADOW-REJECT] cid={0} pattern={1} route={2} regime={3} reason=NO_LEGAL_RR_TARGET",c.CandidateId,c.Signal.PatternName,c.Route,V59RegimeBucket(c));
                return;
            }
            double risk=Math.Abs(entry-stop);
            if(risk<=_symbol.PipSize)return;
            var x=new FamilyShadowTrade{
                CandidateId=c.CandidateId,SetupKey=c.SetupKey,Pattern=c.Signal.PatternName,Route=c.Route,Regime=V59RegimeBucket(c),
            ...
            Print("[V59-SHADOW-START] cid={0} setup={1} pattern={2} route={3} regime={4} entry={5} stop={6} target={7} netTargetR={8:F3}",
                x.CandidateId,x.SetupKey,x.Pattern,x.Route,x.Regime,x.Entry,x.Stop,x.Target,x.NetTargetR);
        }

        private void UpdateV59FamilyShadowTrades(int i,DateTime utc)
        {
            if(!EnableV59VirtualParallelExecution||_familyShadowTrades.Count==0)return;
            ...
            foreach(var x in _familyShadowTrades.Values.Where(z=>!z.Closed).ToList())
            {
                ...
                if(EnableV59ExcursionCaptureResearch)
                {
                    if(!x.Protect75Closed&&prevMfe>=.75&&V59ShadowFloorHit(x,lo,hi,.10))
                    { x.Protect75Closed=true;x.Protect75R=.10-costR; }
                    if(!x.Be1Closed&&prevMfe>=1.0&&V59ShadowFloorHit(x,lo,hi,.05))
                    { x.Be1Closed=true;x.Be1R=.05-costR; }
                    double trailFloor=Math.Max(.25,prevMfe-.75);
                    if(!x.Trail125Closed&&prevMfe>=1.25&&V59ShadowFloorHit(x,lo,hi,trailFloor))
                    ...
                    if(!x.HybridClosed&&!double.IsNaN(hybridFloor)&&V59ShadowFloorHit(x,lo,hi,hybridFloor))
                ...
                if(stopHit){CloseV59Shadow(x,utc,-1.0-costR,"SL_CONSERVATIVE");continue;}
                if(targetHit){CloseV59Shadow(x,utc,x.NetTargetR,"TP");continue;}
                if(utc>=x.ExpiryUtc) CloseV59Shadow(x,utc,currentR,"TTL_MARK_TO_MARKET");
            }
        }

        private bool V59ShadowFloorHit(FamilyShadowTrade x,double lo,double hi,double floorR)
        {
        ...
        private void CloseV59Shadow(FamilyShadowTrade x,DateTime utc,double realizedR,string reason)
        {
            ...
            Print("[V59-SHADOW-CLOSED] cid={0} setup={1} pattern={2} route={3} regime={4} mfeR={5:F4} maeR={6:F4} realizedR={7:F4} holdMin={8:F2} reason={9}",
                x.CandidateId,x.SetupKey,x.Pattern,x.Route,x.Regime,x.MfeR,x.MaeR,x.RealizedR,hold,reason);
            Print("[V59-CAPTURE-CLOSED] cid={0} pattern={1} route={2} regime={3} baselineR={4:F4} protect75R={5:F4} be1R={6:F4} trail125R={7:F4} timeDecayR={8:F4} hybridR={9:F4} mfeR={10:F4} captureRatio={11:F4}",
        ...
        private void CloseOpenV59ShadowTradesAtStop()
        {
            ...
            foreach(var x in _familyShadowTrades.Values.Where(z=>!z.Closed).ToList())
            {
                ...
                CloseV59Shadow(x,utc,r,"BOT_STOP_MARK_TO_MARKET");
            }
        }
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V59-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        {
            _candidateSeq++;
            return "V59-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private void Ledger(CandidateRecord c, CandidateState state, string reason)
        {
            ...
            Print("[V59-EVENT] cid={0} setup={1} pattern={2} subtype={3} scale={4} tf={5} dir={6} state={7} route={8} conflict={9} waitMin={10:F2} reason={11}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V59-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V59-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
