...
F=["V59_MANIFOLD_BASELINE_L0","V59_MANIFOLD_CAPTURE_L0","V59_MANIFOLD_CAPTURE_LEGACY_GRID"]
...
def agg(f):
 ...
 shadows=[r for x in xs for r in x.get("v59_shadow_outcomes",[])]; caps=[r for x in xs for r in x.get("v59_capture_outcomes",[])]
 ...
 for x in xs:
  ...
  for p,row in x.get("v59_family_books",{}).items():
...
capture=delta("V59_MANIFOLD_CAPTURE_L0","V59_MANIFOLD_BASELINE_L0")
grid=delta("V59_MANIFOLD_CAPTURE_LEGACY_GRID","V59_MANIFOLD_CAPTURE_L0")
for f,z in A.items():
 z["commercial_gate"]=gate(z)
 z["historical_superiority_gate"]=superior(z)
eligible=[]
if gate(A["V59_MANIFOLD_CAPTURE_L0"]) and superior(A["V59_MANIFOLD_CAPTURE_L0"]) and capture["pass"]:
 eligible.append("V59_MANIFOLD_CAPTURE_L0")
if gate(A["V59_MANIFOLD_CAPTURE_LEGACY_GRID"]) and superior(A["V59_MANIFOLD_CAPTURE_LEGACY_GRID"]) and capture["pass"] and grid["pass"]:
 eligible.append("V59_MANIFOLD_CAPTURE_LEGACY_GRID")
winner=max(eligible,key=lambda f:(A[f]["net"],A[f]["pf"],A[f]["frequency"])) if eligible else None
front={"version":"HarmonyBot V59","architecture":"CANONICAL_HARMONIC_MANIFOLD_EXCURSION_CAPTURE","commercial_minimum":COMM,"historical_superiority_reference":HIST,
...
(out/"V59_PERFORMANCE_FRONTIER.json").write_text(json.dumps(front,indent=2))
(out/"V59_CAPTURE_CAUSAL_ATTRIBUTION.json").write_text(json.dumps(capture,indent=2))
(out/"V59_GRID_CAUSAL_ATTRIBUTION.json").write_text(json.dumps(grid,indent=2))
(out/"candidate.txt").write_text(winner or "")
(out/"V59_FINAL_DEV4_DECISION.json").write_text(json.dumps({"version":"HarmonyBot V59","decision":"DEV4_SUPERIORITY_PASS" if winner else "HOLD_WITH_EVIDENCE","fresh_used":False},indent=2))
