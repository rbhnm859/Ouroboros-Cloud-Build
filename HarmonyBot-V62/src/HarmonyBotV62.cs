...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV62 : Robot
    {
        private const string Version = "HarmonyBot V62 — Alpha-Preserving Harmonic Execution & Opportunity Engine";
        private const string BotPrefix = "HB62";
        ...
        [Parameter("V62 Variant", DefaultValue = "V62_V51_EXACT_CONTROL")]
        public string V62Variant { get; set; }

        [Parameter("V62 Regime Survival Threshold", DefaultValue = 0.65, MinValue = 0.40, MaxValue = 0.75)]
        public double V62RegimeSurvivalThreshold { get; set; }

        [Parameter("V62 Rat Min Geometry", DefaultValue = 0.72, MinValue = 0.72, MaxValue = 0.72)]
        public double V62RatMinGeometry { get; set; }

        [Parameter("V62 Rat Min PRZ", DefaultValue = 0.72, MinValue = 0.72, MaxValue = 0.72)]
        public double V62RatMinPrz { get; set; }

        [Parameter("V62 Rat Min Confidence", DefaultValue = 0.68, MinValue = 0.68, MaxValue = 0.68)]
        public double V62RatMinConfidence { get; set; }

        [Parameter("V62 Runner Minimum R", DefaultValue = 3.0, MinValue = 3.0, MaxValue = 3.0)]
        public double V62RunnerMinimumR { get; set; }
        ...
        protected override void OnStop()
        {
            ...
            Print("[V62-SUMMARY] variant={0} regimeVetoed={1} ratExpansionAdmitted={2} abcdCapitalRejected={3} runnerAssigned={4} runnerFilled={5} runnerClosed={6} runnerSurvivedCanonical={7} conditionalArmed={8} conditionalRejected={9}",
                V62Variant, _v62RegimeVetoed, _v62RatExpansionAdmitted, _v62AbcdCapitalRejected, _v62RunnerAssigned, _v62RunnerFilled, _v62RunnerClosed, _v62RunnerSurvivedCanonicalTp, _v62ConditionalLegArmed, _v62ConditionalLegRejected);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                Print("[V62-GATE-PASS] cid={0} setup={1} stage=PatternQualification reason=PATTERN_VALIDATED", record.CandidateId, record.SetupKey);
                ...
                if (record.Route == HarmonicRoute.NO_TRADE && V62PatternNativeEnabled())
                {
                ...
                if (!V62ExactControlEnabled() && signal.PatternName == "AB=CD")
                {
                ...
                Print("[V62-GATE-PASS] cid={0} setup={1} stage=RegimeRouting reason=ROUTE_{2}", record.CandidateId, record.SetupKey, record.Route);
        ...
        private bool GridPlanReject(CandidateRecord c, string reason)
        {
            ...
            Print("[V62-GATE-REJECT] cid={0} setup={1} stage=RiskPlan reason={2}", c != null ? c.CandidateId : "", c != null ? c.SetupKey : "", reason);
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            int maxLegs;
            if (V62CanonicalGridEnabled())
            {
                V62RouteGridContract(c, out v62Fractions, out v62Weights);
            ...
            {
                double fraction = V62CanonicalGridEnabled() ? v62Fractions[leg] : p.GridFractions[leg];
                ...
                double riskWeight = V62CanonicalGridEnabled() ? v62Weights[leg] : (leg < p.GridRiskWeights.Length ? p.GridRiskWeights[leg] : 0);
            ...
            double targetBasis = V62ConditionalGridEnabled() ? anchor : plan.ExpectedWeightedEntry;
            double target, netRr;
            ...
            if (V62StructuralExitEnabled() && c.Signal.CanonicalTarget2 > 0 &&
                GeometryValid(c.Signal.Direction, targetBasis, plan.StructuralStop, c.Signal.CanonicalTarget2))
            {
                ...
                if (rr >= MinimumNetRR)
                {
                    ...
                    netRr = rr;
                    Print("[V62-STRUCTURAL-EXIT] cid={0} setup={1} target={2} netRR={3:F3}", c.CandidateId, c.SetupKey, target, netRr);
                }
            }

            plan.CanonicalTarget = target;
            plan.ExpectedNetRR = netRr;
            ConfigureV62SelectiveRunner(c, plan);
            c.GridPlan = plan;
            ...
            if (V62ConditionalGridEnabled())
            {
                double shadowL3 = c.Signal.Direction == TradeDirection.Buy ? anchor - .618 * executionUnit : anchor + .618 * executionUnit;
                Print("[V62-L3-SHADOW] cid={0} setup={1} pattern={2} route={3} fraction=0.618 price={4} capital=false",
                    c.CandidateId, c.SetupKey, c.Signal.PatternName, c.Route, shadowL3);
            }
            Print("[V62-GATE-PASS] cid={0} setup={1} stage=RiskPlan reason=GRID_PLAN_VALID", c.CandidateId, c.SetupKey);
        ...
        private void ExecuteFibonacciGridPlan(CandidateRecord c)
        {
            ...
            double l0Target = V62LegTarget(l0, plan.CanonicalTarget);
            ...
            Print("[V62-GATE-PASS] cid={0} setup={1} stage=Executed reason=LEG0_FILLED", c.CandidateId, c.SetupKey);
        ...
        private bool ExecuteImmediateRunnerLeg(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            double target = V62LegTarget(leg, basket.CanonicalTarget);
        ...
        private bool PlaceGridLimit(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            double legTarget = V62LegTarget(leg, basket.CanonicalTarget);
        ...
        private bool V62ExactControlEnabled()
        {
            return string.Equals(V62Variant, "V62_V51_EXACT_CONTROL", StringComparison.Ordinal);
        }
        ...
        {
            return string.Equals(V62Variant, "V62_CURRENT_V61_GRID", StringComparison.Ordinal);
        }

        private bool V62FrontLoadedGridEnabled()
        {
            return string.Equals(V62Variant, "V62_FRONT_LOADED_GRID", StringComparison.Ordinal);
        }

        private bool V62ConditionalGridEnabled()
        {
            return string.Equals(V62Variant, "V62_CONDITIONAL_GRID", StringComparison.Ordinal) ||
                   string.Equals(V62Variant, "V62_CONDITIONAL_STRUCTURAL_EXIT", StringComparison.Ordinal) ||
                   string.Equals(V62Variant, "V62_CONDITIONAL_RUNNER", StringComparison.Ordinal) ||
                   string.Equals(V62Variant, "V62_PATTERN_NATIVE", StringComparison.Ordinal);
        }

        private bool V62StructuralExitEnabled()
        {
            return string.Equals(V62Variant, "V62_CONDITIONAL_STRUCTURAL_EXIT", StringComparison.Ordinal);
        }

        private bool V62RunnerEnabled()
        {
            return string.Equals(V62Variant, "V62_CONDITIONAL_RUNNER", StringComparison.Ordinal) ||
                   string.Equals(V62Variant, "V62_PATTERN_NATIVE", StringComparison.Ordinal);
        }

        private bool V62PatternNativeEnabled()
        {
            return string.Equals(V62Variant, "V62_PATTERN_NATIVE", StringComparison.Ordinal);
        }

        private bool V62CanonicalGridEnabled()
        {
            return !V62ExactControlEnabled();
        }
        ...
        private bool V62RegimeSurvivalPass        private bool V62RegimeSurvivalPass(CandidateRecord c)
        {
            ...
            Print("[V62-REGIME-SURVIVAL] cid={0} setup={1} route={2} score={3:F3} threshold={4:F3} hard={5}",
                c.CandidateId, c.SetupKey, c.Route, score, V62RegimeSurvivalThreshold, routeHardPass);
            return routeHardPass && score >= V62RegimeSurvivalThreshold;
        }

        private bool V62TryRatControlledExpansion(PatternSignal sig, MtfConflict conflict, RegimeSnapshot r, out HarmonicRoute route)
        {
            route = HarmonicRoute.NO_TRADE;
            if (sig == null || r == null || sig.PatternName != "Rat") return false;
            if (sig.GeometryQuality < V62RatMinGeometry || sig.PrzConfluence < V62RatMinPrz || sig.Confidence < V62RatMinConfidence) return false;
        ...
        private void ConfigureV62SelectiveRunner(CandidateRecord c, FibonacciGridPlan plan)
        {
            ...
            double minimumRunner = c.Signal.Direction == TradeDirection.Buy ? leg.PlannedPrice + riskDistance * V62RunnerMinimumR : leg.PlannedPrice - riskDistance * V62RunnerMinimumR;
        ...
        private double V62LegTarget(FibonacciGridLeg leg, double fallback)
        {
            if (V62CommercialMaxEnabled() && leg != null && leg.RunnerEligible && leg.RunnerTarget > 0) return leg.RunnerTarget;
            return fallback;
        }

        private bool UpdateV62StageAwareFamilyCompletionEvidence(int i, CandidateRecord c, out double score)
        {
            score = 0;
            ...
            string p = sig.PatternName ?? ""; int prior = c.V62DagStage; bool advance = false;
            if (p == "Gartley" || p == "Bat" || p == "Deep Gartley" || p == "Rat" || p == "Cypher")
            {
                if (prior == 0 && (rejection || failedExtension)) advance = true;
                else if (prior == 1 && reclaim && i > c.V62DagAnchorBar) advance = true;
                else if (prior == 2 && (bos || displacement) && i > c.V62DagAnchorBar) advance = true;
                if (advance) { c.V62DagStage++; c.V62DagAnchorBar = i; }
                score = c.V62DagStage / 3.0; return c.V62DagStage >= 3;
            }
            if (p == "Alt Bat" || p == "Butterfly" || p == "Crab" || p == "Deep Crab" || p == "Shark")
            {
                if (prior == 0 && sweep) advance = true;
                else if (prior == 1 && failedExtension && i > c.V62DagAnchorBar) advance = true;
                else if (prior == 2 && (reclaim || insidePrz) && i > c.V62DagAnchorBar) advance = true;
                else if (prior == 3 && (bos || displacement) && i > c.V62DagAnchorBar) advance = true;
                if (advance) { c.V62DagStage++; c.V62DagAnchorBar = i; }
                score = c.V62DagStage / 4.0; return c.V62DagStage >= 4;
            }
            if (p == "5-0")
            {
                if (prior == 0 && failedExtension) advance = true;
                else if (prior == 1 && bos && i > c.V62DagAnchorBar) advance = true;
                else if (prior == 2 && retest && i > c.V62DagAnchorBar) advance = true;
                else if (prior == 3 && directional && i > c.V62DagAnchorBar) advance = true;
                if (advance) { c.V62DagStage++; c.V62DagAnchorBar = i; }
                score = c.V62DagStage / 4.0; return c.V62DagStage >= 4;
            }
            if (p == "AB=CD")
            {
                ...
                else if (prior == 2 && reclaim && i > c.V62DagAnchorBar) advance = true;
                else if (prior == 3 && (bos || displacement) && i > c.V62DagAnchorBar) advance = true;
                if (advance) { c.V62DagStage++; c.V62DagAnchorBar = i; }
                score = c.V62DagStage / 4.0; return c.V62DagStage >= 4;
            }
        ...
        private bool DeeperLegThesisEligible(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            if (!V62ConditionalGridEnabled()) return notAcceleratingAgainst;
            ...
            Print("[V62-CONDITIONAL-LEG-CHECK] basket={0} leg=L{1} pass={2} impulse={3} structure={4} contained={5} m1Evidence={6}", basket.BasketId, leg.Index, pass, impulse, structureStillValid, penetrationContained, m1ReversalEvidence);
            return pass;
        }
        ...
        private bool EnsurePostFillProtection(Position p, FibonacciBasket basket)
        {
            ...
            double protectionTarget = V62LegTarget(protectionLeg, basket.CanonicalTarget);
        ...
        {
            if (basket == null || !basket.IsActive || !V62ConditionalGridEnabled()) return;
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V62-ECONOMIC-ATTRIBUTION] basket={0} setup={1} pattern={2} route={3} mfeR={4:F3} maeR={5:F3} realizedR={6:F3} capture={7:F3} timeToMfeMin={8:F2} timeToMaeMin={9:F2}", basket.BasketId, setupKey, basket.Pattern, basket.Route, basket.PeakR, basket.MaxAdverseR, realizedR, captureRatio, timeToMfe, timeToMae);
        ...
        {
            if (!V62ExactControlEnabled())
                return UpdateV62StageAwareFamilyCompletionEvidence(i, c, out score);
            score = 0;
            if (i < 3 || i >= _m1Bars.Count || c == null || c.Signal == null) return false;
            var sig = c.Signal;
        ...
        {
            _candidateSeq++;
            return "V62-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private string V62GateStage(string reason)
        {
            string r=reason??"";
        ...
        private void Reject(CandidateRecord c, string reason)
        {
            ...
            Print("[V62-GATE-REJECT] cid={0} setup={1} stage={2} reason={3}",c.CandidateId,c.SetupKey,V62GateStage(reason),reason);
    ...
    public sealed class CandidateRecord
    {
        ...
        public int V62DagStage, V62DagAnchorBar = -1;
