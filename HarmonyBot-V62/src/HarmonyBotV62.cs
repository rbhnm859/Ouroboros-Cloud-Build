...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V62 — Alpha-Preserving Execution Conversion Rebuild";
        ...
        public double V61RatMinGeometry { get; set; }

        [Parameter("V61 Rat Min PRZ", DefaultValue = 0.72, MinValue = 0.72, MaxValue = 0.72)]
        public double V61RatMinPrz { get; set; }

        [Parameter("V61 Rat Min Confidence", DefaultValue = 0.68, MinValue = 0.68, MaxValue = 0.68)]
        public double V61RatMinConfidence { get; set; }

        [Parameter("V61 Runner Minimum R", DefaultValue = 3.0, MinValue = 3.0, MaxValue = 3.0)]
        public double V61RunnerMinimumR { get; set; }
        ...
        private int _v61RegimeVetoed;
        private int _v61RatExpansionAdmitted;
        private int _v61AbcdCapitalRejected;
        private int _v61RunnerAssigned;
        private int _v62ConditionalLegEligible;
        ...
        protected override void OnStop()
        {
            ...
            Print("[V62-SUMMARY] variant={0} regimeVetoed={1} ratExpansionAdmitted={2} abcdCapitalRejected={3} runnerAssigned={4} conditionalLegEligible={5} conditionalLegRejected={6} runnerFilled={7} runnerSurvivedCanonicalTp={8}",
                V62Variant, _v61RegimeVetoed, _v61RatExpansionAdmitted, _v61AbcdCapitalRejected, _v61RunnerAssigned,
                _v62ConditionalLegEligible, _v62ConditionalLegRejected, _v62RunnerFilled, _v62RunnerSurvivedCanonicalTp);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                if (record.Route == HarmonicRoute.NO_TRADE && V61CommercialMaxEnabled())
                {
                    HarmonicRoute ratRoute;
                    if (V61TryRatControlledExpansion(signal, record.Conflict, regime, out ratRoute))
                    {
                        record.Route = ratRoute;
                        _v61RatExpansionAdmitted++;
                        Print("[V61-RAT-EXPANSION] cid={0} setup={1} route={2} geometry={3:F3} prz={4:F3} confidence={5:F3}",
                            record.CandidateId, record.SetupKey, record.Route, signal.GeometryQuality, signal.PrzConfluence, signal.Confidence);
                    }
                }

                if (!V61ExactControlEnabled() && signal.PatternName == "AB=CD")
                {
                    _v61AbcdCapitalRejected++;
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (V61RegimeSurvivalEnabled() && !V61RegimeSurvivalPass(c))
                    {
                        _v61RegimeVetoed++;
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            double[] v61Fractions = null;
            double[] v61Weights = null;
            int maxLegs;
            if (V61CanonicalGridEnabled())
            {
                V61RouteGridContract(c.Route, out v61Fractions, out v61Weights);
                maxLegs = Math.Min(v61Fractions.Length, v61Weights.Length);
            }
            else
            {
            ...
            {
                double fraction = V61CanonicalGridEnabled() ? v61Fractions[leg] : p.GridFractions[leg];
                ...
                double riskWeight = V61CanonicalGridEnabled() ? v61Weights[leg] : (leg < p.GridRiskWeights.Length ? p.GridRiskWeights[leg] : 0);
            ...
            ConfigureV61SelectiveRunner(c, plan);
            c.GridPlan = plan;
        ...
        private void ExecuteFibonacciGridPlan(CandidateRecord c)
        {
            ...
            double l0Target = V61LegTarget(l0, plan.CanonicalTarget);
            ...
            foreach (var leg in plan.Legs.Where(x => x.Index > 0))
            {
                ...
                if (V62ConditionalGridEnabled()) { BasketEvent(basket, "V62_CONDITIONAL_LEG_ARMED_L" + leg.Index); continue; }
        ...
        private bool PlaceGridLimit(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            double legTarget = V61LegTarget(leg, basket.CanonicalTarget);
        ...
        private bool V61ExactControlEnabled()
        {
        ...
        private bool V61CanonicalGridEnabled() { return V62CurrentGridEnabled() || V62FrontLoadedGridEnabled() || V62ConditionalGridEnabled(); }
        private bool V61RegimeSurvivalEnabled() { return false; }
        private bool V61CommercialMaxEnabled()
        {
            return string.Equals(V62Variant, "V62_CONDITIONAL_RUNNER", StringComparison.Ordinal) ||
                   string.Equals(V62Variant, "V62_PATTERN_NATIVE", StringComparison.Ordinal);
        }

        private void V61RouteGridContract(HarmonicRoute route, out double[] fractions, out double[] weights)
        {
            if (V62CurrentGridEnabled())
            {
            ...
            if (V62FrontLoadedGridEnabled()) { fractions = new[] { 0.0, .236, .382 }; weights = new[] { .70, .20, .10 }; return; }
            if (V62ConditionalGridEnabled()) { fractions = new[] { 0.0, .236, .382, .618 }; weights = new[] { .60, .25, .15, 0.0 }; return; }
            fractions = Array.Empty<double>(); weights = Array.Empty<double>();
        }

        private bool V61RegimeSurvivalPass(CandidateRecord c)
        {
            ...
            Print("[V61-REGIME-SURVIVAL] cid={0} setup={1} route={2} score={3:F3} threshold={4:F3} hard={5}",
                c.CandidateId, c.SetupKey, c.Route, score, V61RegimeSurvivalThreshold, routeHardPass);
            return routeHardPass && score >= V61RegimeSurvivalThreshold;
        }

        private bool V61TryRatControlledExpansion(PatternSignal sig, MtfConflict conflict, RegimeSnapshot r, out HarmonicRoute route)
        {
            route = HarmonicRoute.NO_TRADE;
            if (sig == null || r == null || sig.PatternName != "Rat") return false;
            if (sig.GeometryQuality < V61RatMinGeometry || sig.PrzConfluence < V61RatMinPrz || sig.Confidence < V61RatMinConfidence) return false;
        ...
        private void ConfigureV61SelectiveRunner(CandidateRecord c, FibonacciGridPlan plan)
        {
            if (!V61CommercialMaxEnabled() || c == null || c.Signal == null || plan == null) return;
            ...
            double minimumRunner = c.Signal.Direction == TradeDirection.Buy
                ? leg.PlannedPrice + riskDistance * V61RunnerMinimumR
                : leg.PlannedPrice - riskDistance * V61RunnerMinimumR;
            double target = minimumRunner;
            double t2 = c.Signal.CanonicalTarget2;
            if (t2 > 0) target = c.Signal.Direction == TradeDirection.Buy ? Math.Max(minimumRunner, t2) : Math.Min(minimumRunner, t2);
            if (!GeometryValid(c.Signal.Direction, leg.PlannedPrice, plan.StructuralStop, target)) return;
            leg.RunnerEligible = true;
            leg.RunnerTarget = target;
            plan.SelectiveRunnerEnabled = true;
            plan.RunnerRiskWeight = leg.RiskWeight;
            _v61RunnerAssigned++;
            Print("[V61-RUNNER-ASSIGN] cid={0} setup={1} route={2} leg=L{3} weight={4:F3} target={5}", c.CandidateId, c.SetupKey, c.Route, leg.Index, leg.RiskWeight, target);
        }

        private double V61LegTarget(FibonacciGridLeg leg, double fallback)
        {
            if (V61CommercialMaxEnabled() && leg != null && leg.RunnerEligible && leg.RunnerTarget > 0) return leg.RunnerTarget;
            return fallback;
        }

        private bool UpdateV61StageAwareFamilyCompletionEvidence(int i, CandidateRecord c, out double score)
        {
            score = 0;
            ...
            string p = sig.PatternName ?? ""; int prior = c.V61DagStage; bool advance = false;
            if (p == "Gartley" || p == "Bat" || p == "Deep Gartley" || p == "Rat" || p == "Cypher")
            {
                if (prior == 0 && (rejection || failedExtension)) advance = true;
                else if (prior == 1 && reclaim && i > c.V61DagAnchorBar) advance = true;
                else if (prior == 2 && (bos || displacement) && i > c.V61DagAnchorBar) advance = true;
                if (advance) { c.V61DagStage++; c.V61DagAnchorBar = i; }
                score = c.V61DagStage / 3.0; return c.V61DagStage >= 3;
            }
            if (p == "Alt Bat" || p == "Butterfly" || p == "Crab" || p == "Deep Crab" || p == "Shark")
            {
                if (prior == 0 && sweep) advance = true;
                else if (prior == 1 && failedExtension && i > c.V61DagAnchorBar) advance = true;
                else if (prior == 2 && (reclaim || insidePrz) && i > c.V61DagAnchorBar) advance = true;
                else if (prior == 3 && (bos || displacement) && i > c.V61DagAnchorBar) advance = true;
                if (advance) { c.V61DagStage++; c.V61DagAnchorBar = i; }
                score = c.V61DagStage / 4.0; return c.V61DagStage >= 4;
            }
            if (p == "5-0")
            {
                if (prior == 0 && failedExtension) advance = true;
                else if (prior == 1 && bos && i > c.V61DagAnchorBar) advance = true;
                else if (prior == 2 && retest && i > c.V61DagAnchorBar) advance = true;
                else if (prior == 3 && directional && i > c.V61DagAnchorBar) advance = true;
                if (advance) { c.V61DagStage++; c.V61DagAnchorBar = i; }
                score = c.V61DagStage / 4.0; return c.V61DagStage >= 4;
            }
            if (p == "AB=CD")
            {
                ...
                else if (prior == 2 && reclaim && i > c.V61DagAnchorBar) advance = true;
                else if (prior == 3 && (bos || displacement) && i > c.V61DagAnchorBar) advance = true;
                if (advance) { c.V61DagStage++; c.V61DagAnchorBar = i; }
                score = c.V61DagStage / 4.0; return c.V61DagStage >= 4;
            }
        ...
        private void RegisterFilledPosition(FibonacciBasket basket, FibonacciGridLeg leg, Position p)
        {
            ...
            _positions[p.Id] = new PositionLedger
            {
                ...
                LegIndex = leg.Index,
                RunnerEligible = leg.RunnerEligible
            };
        }
        ...
        private bool DeeperLegThesisEligible(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            if (leg.Index >= 3) { _v62ConditionalLegRejected++; return false; }
            bool m1Evidence = basket.Candidate != null && basket.Candidate.Signal != null && RouteSpecificM1EvidencePass(i, basket.Candidate.Signal, basket.Route);
            double px = basket.Direction == TradeDirection.Buy ? _symbol.Bid : _symbol.Ask;
            double riskSpan = Math.Abs(basket.EntryAnchor - basket.StructuralStop);
            double penetration = riskSpan > _symbol.TickSize ? Math.Abs(px - basket.EntryAnchor) / riskSpan : 999;
            bool eligible = notAcceleratingAgainst && m1Evidence && penetration <= (leg.Index == 1 ? .50 : .72);
            if (eligible) _v62ConditionalLegEligible++; else _v62ConditionalLegRejected++;
            BasketEvent(basket, eligible ? "V62_CONDITIONAL_LEG_ELIGIBLE_L" + leg.Index : "V62_CONDITIONAL_LEG_REJECT_L" + leg.Index);
            return eligible;
        }
        ...
        private void RecordLegFillTelemetry(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            if (leg.RunnerEligible) { _v62RunnerFilled++; BasketEvent(basket, "V62_RUNNER_FILLED_L" + leg.Index); }
        ...
        private void ActivateV62ConditionalGridLegs()
        {
            if (!V62ConditionalGridEnabled()) return;
            foreach (var basket in _baskets.Values.Where(b => b.IsActive).ToList())
            foreach (var leg in basket.Plan.Legs.Where(x => x.Index > 0 && x.Physical && x.State == GridLegState.PLANNED).OrderBy(x => x.Index).ToList())
            {
                bool approached = basket.Direction == TradeDirection.Buy
                    ? _symbol.Ask <= leg.PlannedPrice + Math.Max(_symbol.TickSize, PipsToPrice(Math.Max(1.0, SpreadPips())))
                    : _symbol.Bid >= leg.PlannedPrice - Math.Max(_symbol.TickSize, PipsToPrice(Math.Max(1.0, SpreadPips())));
                if (!approached) continue;
                if (!DeeperLegThesisEligible(basket, leg)) { TransitionLegState(basket, leg, GridLegState.CANCELLED, "V62_CONDITIONAL_REPROOF_REJECT"); continue; }
                if (PlaceGridLimit(basket, leg)) { basket.State = FibonacciBasketState.GRID_PENDING; BasketEvent(basket, "V62_CONDITIONAL_SUBMIT_L" + leg.Index); }
            }
        }

        private void RevalidatePendingExposureGovernor()
        {
            ActivateV62ConditionalGridLegs();
            foreach (var basket in _baskets.Values.Where(b => b.IsActive).ToList())
            {
                foreach (var o in OwnPendingOrders().Where(x => LabelBasketId(x.Label) == basket.BasketId).ToList())
                {
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                if (args.Reason.ToString().IndexOf("TakeProfit", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    ...
                    if (V61CommercialMaxEnabled() && !l.RunnerEligible)
                    {
                        int survivors = 0;
                        foreach (var rp in OwnPositions().Where(x => LabelBasketId(x.Label) == basket.BasketId).ToList())
                        {
                            PositionLedger rl; if (!_positions.TryGetValue(rp.Id, out rl) || !rl.RunnerEligible) continue;
                            survivors++;
                            double be = rp.TradeType == TradeType.Buy ? rp.EntryPrice + PipsToPrice(Math.Max(.5, ModeledCostPips())) : rp.EntryPrice - PipsToPrice(Math.Max(.5, ModeledCostPips()));
                            double safe = BrokerSafeStop(rp.TradeType, be); if (safe > 0) rp.ModifyStopLossPrice(safe);
                        }
                        if (survivors > 0) { _v62RunnerSurvivedCanonicalTp += survivors; BasketEvent(basket, "V62_RUNNER_DETACHED_AFTER_CANONICAL_TP"); }
                    }
                }
            }
        ...
        {
            if (!V61ExactControlEnabled())
                return UpdateV61StageAwareFamilyCompletionEvidence(i, c, out score);
            score = 0;
            if (i < 3 || i >= _m1Bars.Count || c == null || c.Signal == null) return false;
            var sig = c.Signal;
    ...
    public sealed class CandidateRecord
    {
        ...
        public int V61DagStage, V61DagAnchorBar = -1;
