...
econ_rx=re.compile(r"\[V62-ECONOMIC-ATTRIBUTION\].*?basket=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+capture=([-0-9.]+)\s+timeToMfeMin=([-0-9.]+)\s+timeToMaeMin=([-0-9.]+)")
...
leg_rx=re.compile(r"\[V62-LEG-ECON\].*?basket=(\S+)\s+setup=(\S+)\s+leg=L(\d+)\s+fraction=([-0-9.]+)\s+weight=([-0-9.]+)\s+runner=(\S+)\s+mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
for m in re.finditer(r"\[V62-GATE-REJECT\].*?stage=(\S+)\s+reason=(\S+)",t):rej[m.group(1)]+=1
for m in re.finditer(r"\[V62-GATE-PASS\].*?stage=(\S+)\s+reason=(\S+)",t):pas[m.group(1)]+=1
...
v62rx=re.findall(r"\[V62-SUMMARY\].*?runnerAssigned=(\d+)\s+runnerFilled=(\d+)\s+runnerClosed=(\d+)\s+runnerSurvivedCanonical=(\d+)\s+conditionalArmed=(\d+)\s+conditionalRejected=(\d+)",t)
v62={"runner_assigned":0,"runner_filled":0,"runner_closed":0,"runner_survived_canonical":0,"conditional_armed":0,"conditional_rejected":0}
if v62rx:
 for k,z in zip(v62,map(int,v62rx[-1])):v62[k]=z
...
"gate_telemetry":gate_telemetry,"rejected_cohort_quality":rejected_cohort_quality,"v62_telemetry":v62,"basket_outcomes":b,"pattern_pipeline":pipeline,**c}
