...
case "$VAR" in V62_V51_EXACT_CONTROL|V62_DAG_SINGLE_ENTRY|V62_CURRENT_V61_GRID|V62_FRONT_LOADED_GRID|V62_CONDITIONAL_GRID|V62_CONDITIONAL_STRUCTURAL_EXIT|V62_CONDITIONAL_RUNNER|V62_PATTERN_NATIVE) ;; *) exit 31;; esac
C="$PWD/control"; W="$C/HarmonyBot-V62/window-$VAR-$WIN-B$BALANCE-S$SPREAD"; O="$C/HarmonyBot-V62/output-$VAR-$WIN-B$BALANCE-S$SPREAD"; rm -rf "$O"; mkdir -p "$W/seal/algo" "$W/seal/data" "$O/raw-logs"
cp "$C/HarmonyBot-V62/dist/HarmonyBot_V62_Alpha_Preserving_Execution_Conversion_Rebuild_RC.algo" "$W/seal/algo/"; N="V62-$VAR-$WIN-B$BALANCE-S$SPREAD"
