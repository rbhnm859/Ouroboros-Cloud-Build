...
equiv=v51["baskets"]==v62["baskets"] and abs(v51["net"]-v62["net"])<=.05 and abs(v51["pf"]-v62["pf"])<=.001 and abs(v51["expectancy"]-v62["expectancy"])<=.02 and abs(v51["win_rate"]-v62["win_rate"])<=.0005 and abs(v51["max_dd_pct"]-v62["max_dd_pct"])<=.01; ok=near(v51) and near(v62) and equiv
rep={"version":"HarmonyBot V62","expected_v51":expected,"immutable_v51":v51,"v62_exact_control":v62,"dynamic_equivalence":equiv,"pass":ok}; (out/"V62_V51_EXACT_CONTROL_REPLAY.json").write_text(json.dumps(rep,indent=2)); (out/"replay.out").write_text("true" if ok else "false"); print(json.dumps(rep,indent=2))
