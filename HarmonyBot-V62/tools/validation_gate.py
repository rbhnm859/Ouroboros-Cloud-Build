...
rep={"version":"HarmonyBot V62","validation_rows":rows,"validation_pass":ok,"no_tuning":True,"fresh_used":False}; (out/"V62_VALIDATION_GATE.json").write_text(json.dumps(rep,indent=2)); (out/"validation.out").write_text("true" if ok else "false"); print(json.dumps(rep,indent=2))
if not ok: raise SystemExit("V62 validation HOLD")
