        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                if (!qualificationPass)
                {
                    ...
                    ConversionTruth(signal.PatternName, "LIBERATION_QUALITY_VECTOR_CONTINUE");
                    Event(record, "LIBERATION_QUALITY_VECTOR_CONTINUE_" + familyQualificationReason);
                }
                ...
                if (EnableUniversalHarmonicLiberation && record.Route == HarmonicRoute.NO_TRADE)
                {
