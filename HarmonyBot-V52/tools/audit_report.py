...
rx=re.compile(r"\[V52-BASKET-CLOSED\].*?cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+subtype=(\S+)\s+route=(\S+)\s+dir=(\S+).*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)\s+reason=(\S+)")
b=[]
...
pat=(r"\[V52-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
evrx=re.compile(r"\[V52-EVENT\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+subtype=(\S+)\s+scale=(\d+)\s+tf=(\S+)\s+dir=(\S+)\s+state=(\S+)\s+route=(\S+)\s+conflict=(\S+)\s+waitMin=([-0-9.]+)\s+reason=(.*)$",re.M)
events=[]
...
oprx=re.compile(r"\[V52-OPPORTUNITY-LOSS\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+scale=(\d+)\s+reason=(\S+)\s+shadowMfeR=([-0-9.]+)\s+shadowMaeR=([-0-9.]+)\s+missedPositive=(True|False)\s+avoidedNegative=(True|False)")
opp=[]
...
sorx=re.compile(r"\[V52-SLOT-OCCUPANCY\].*?pattern=(.*?)\s+route=(\S+)\s+occupancyMinutes=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
"broker_profile_present":"[V52-BROKER-PROFILE]" in t,"mean_mfe_r":statistics.mean([x["mfe"] for x in b]) if b else 0,
...
anchor_rx=re.compile(r"\[V52-ENTRY-ANCHOR-FORENSICS\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+scale=(\d+)\s+completion=([-0-9.]+)\s+confirm=([-0-9.]+)\s+retest=([-0-9.]+)\s+completionMfeR=([-0-9.]+)\s+completionMaeR=([-0-9.]+)\s+confirmMfeR=([-0-9.]+)\s+confirmMaeR=([-0-9.]+)\s+retestMfeR=([-0-9.]+)\s+retestMaeR=([-0-9.]+)")
anchors=[]
...
truth_rx=re.compile(r"\[V52-DETECTOR-TRUTH\]\s+pattern=(.*?)\s+stage=(\S+)\s+count=(\d+)")
truth={}
