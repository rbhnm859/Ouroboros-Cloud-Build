...
s=pathlib.Path(sys.argv[1] if len(sys.argv)>1 else "HarmonyBot-V52/src/HarmonyBotV52.cs").read_text(errors="ignore")
checks={
"identity":"HarmonyBot V52" in s and "class HarmonyBotV52" in s and 'BotPrefix = "HB52"' in s,
...
"detector_truth":"V52-DETECTOR-TRUTH" in s and "DetectorTruth" in s,
...
out={"version":"HarmonyBot V52","architecture":"HARMONIC_FAMILY_IDENTITY_DETECTION_GRAPH_RECONSTRUCTION","checks":checks,"pass":all(checks.values())}
pathlib.Path("V52_ARCHITECTURE_AUDIT.json").write_text(json.dumps(out,indent=2))
