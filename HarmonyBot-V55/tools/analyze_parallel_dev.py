...
F=["V51_QUALITY_CORE_REPLAY","V52_LIBERATION_CONTROL","V55_LIBERATION_LEGACY_GRID","V55_LIBERATION_GRID_V3"]
...
grid_causal={"GRID_V3_VS_LEGACY":gd("V55_LIBERATION_GRID_V3","V55_LIBERATION_LEGACY_GRID")}
def cd(v55,v52):
 a,b=A[v55],A[v52]; aset={r["setup"] for r in a["rows"]}; bset={r["setup"] for r in b["rows"]}; add=[r for r in a["rows"] if r["setup"] not in bset]; rem=[r for r in b["rows"] if r["setup"] not in aset]
...
eligible=[f for f in ["V55_LIBERATION_LEGACY_GRID","V55_LIBERATION_GRID_V3"] if A[f]["commercial_gate"]]; winner=max(eligible,key=lambda f:(A[f]["net"],A[f]["pf"],A[f]["frequency"])) if eligible else None
front={"version":"HarmonyBot V55","architecture":"UNIVERSAL_HARMONIC_LIBERATION_AND_FIBONACCI_GRID_ALPHA_CORE","v52_grid_control":V51_REF,"commercial_minimum":COMM,"historical_control_reproduced":historical_control_reproduced,"current_snapshot_controls_valid":controls_ok,"data_snapshot_valid":data_snapshot_valid,"data_snapshot_by_window":data_snapshot_by_window,"families":A,"conversion_attribution":conversion,"grid_causal_attribution":grid_causal,"development_candidate":winner,"status":"COMMERCIAL_FREEZE_CANDIDATE_DEV_PASS" if winner else "HOLD_WITH_EVIDENCE","fresh_used":False,"next_stage":"CAPITAL_COMPATIBILITY" if winner else "STOP_DEV_HOLD"}
(out/"V55_PERFORMANCE_FRONTIER.json").write_text(json.dumps(front,indent=2)); (out/"V55_CONVERSION_TRUTH_LEDGER.json").write_text(json.dumps({f:A[f]["conversion_truth"] for f in F},indent=2)); (out/"V55_FAMILY_CONVERSION_ECONOMICS.json").write_text(json.dumps({f:{"pipeline":A[f]["pipeline"],"pattern_economics":A[f]["pattern_economics"],"starved_families":A[f]["starved_families"]} for f in F},indent=2)); (out/"V55_GRID_CAUSAL_ATTRIBUTION.json").write_text(json.dumps(grid_causal,indent=2)); (out/"candidate.txt").write_text(winner or ""); (out/"V55_FINAL_COMMERCIAL_DECISION.json").write_text(json.dumps({"version":"HarmonyBot V55","decision":"COMMERCIAL_FREEZE_CANDIDATE_DEV_PASS" if winner else "HOLD_WITH_EVIDENCE","stage":"DEV","fresh_used":False},indent=2)); print(json.dumps(front,indent=2))
