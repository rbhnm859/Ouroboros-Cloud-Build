...
RESEARCH="${RESEARCH:-true}"; EXPANSION="${EXPANSION:-false}"; RBOUNDED="${RBOUNDED:-true}"; RMICRO="${RMICRO:-2}"; RQUOTA="${RQUOTA:-4}"; GRIDV4="${GRIDV4:-false}"; EXPRR="${EXPRR:-2.5}"
ALGO="${ALGO:-seal/algo/HarmonyBot_V55_Champion_Core_Universal_Evidence_Admission_RC.algo}"
...
test "$DONE" = 1 || { echo "[V55-WATCHDOG-FAIL] run=$RUN_NAME"; tail -400 "seal/logs/$RUN_NAME.log" || true; exit 20; }
echo "[V55-WATCHDOG] run=$RUN_NAME status=complete"
