...
rx=re.compile(r"\[V351-BASKET-CLOSED\].*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
pat=(r"\[V351-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
"broker_profile_present":"[V351-BROKER-PROFILE]" in t,
"transition_veto_events":len(re.findall(r"\[V351-ROUTE-VETO\] type=TRANSITION_STATE",t)),
"exhaustion_veto_events":len(re.findall(r"\[V351-ROUTE-VETO\] type=EXHAUSTION_EVIDENCE",t)),
"m1_veto_events":len(re.findall(r"ROUTE_SPECIFIC_M1_VETO",t)),
"mean_mfe_r":statistics.mean([x["mfe"] for x in b]) if b else 0,
"mean_mae_r":statistics.mean([x["mae"] for x in b]) if b else 0,**c}
