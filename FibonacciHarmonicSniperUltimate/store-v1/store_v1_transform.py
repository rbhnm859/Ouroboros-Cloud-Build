...
# Checked close for H1 lane reservation.
...
# Pre-submit checks for H1/M30: hard single position, volume/risk, margin, broker stop/TP distances.
rep('''            var result = ExecuteMarketOrder(m.Direction, SymbolName, volume, BotLabel, slPips, tpPips, m.Definition.Name, false);\n''','''            double storeH1Risk = m.Direction == TradeType.Buy ? Round22H1BuyRiskPercent : Round22H1SellRiskPercent;\n            if (!StoreValidateOrder(m.Direction, volume, slPips, tpPips, storeH1Risk, "H1"))\n                return;\n\n            var result = ExecuteMarketOrder(m.Direction, SymbolName, volume, BotLabel, slPips, tpPips, m.Definition.Name, false);\n''','h1 preflight')
rep('''            string patternName=(round21Bypass ? "R21M30|" : "M30|")+m.Definition.Name;\n            var result=ExecuteMarketOrder(m.Direction,SymbolName,volume,BotLabel,slPips,tpPips,patternName,false);\n''','''            if (!StoreValidateOrder(m.Direction, volume, slPips, tpPips, riskPercent, round21Bypass ? "M30_BYPASS" : "M30"))\n                return;\n\n            string patternName=(round21Bypass ? "R21M30|" : "M30|")+m.Definition.Name;\n            var result=ExecuteMarketOrder(m.Direction,SymbolName,volume,BotLabel,slPips,tpPips,patternName,false);\n''','m30 preflight')

# Persist on stop. On close, persist after H1-health/state bookkeeping finishes.
...
namespace cAlgo.Robots
{
    public partial class FibonacciHarmonicSniperUltimate
    {
        ...
        private string StoreSanitizeKeyPart(string value)
        {
            if (string.IsNullOrWhiteSpace(value)) return "X";
            var chars = value.Where(char.IsLetterOrDigit).Take(12).ToArray();
            return chars.Length == 0 ? "X" : new string(chars);
        }

        private string StoreKey(string suffix)
        {
            // cTrader LocalStorage keys: alphanumeric + internal spaces only, <= 50 chars.
            string key = "BHG " + StoreSanitizeKeyPart(BotLabel) + " " + StoreSanitizeKeyPart(SymbolName) + " " + StoreSanitizeKeyPart(suffix);
            return key.Length <= 50 ? key : key.Substring(0, 50).Trim();
        }
        ...
        private void StoreValidateStartup()
        {
            ...
            if (Account.Equity < 3000)
                Print("[STORE WARN] Equity {0:F2} below validated recommended minimum USD 3000 equivalent; min-volume rounding may skip signals.", Account.Equity);
            ...
            Print("[STORE SAFETY] startupValid={0} dailyLocked={1} reconstructedTradesToday={2} dayStartEquity={3:F2} minVol={4} step={5} maxVol={6} minSL={7} minTP={8}",
                _storeStartupValid, _storeDailyLocked, _tradesToday, _dayStartEquity,
                Symbol.VolumeInUnitsMin, Symbol.VolumeInUnitsStep, Symbol.VolumeInUnitsMax,
                Symbol.MinStopLossDistance, Symbol.MinTakeProfitDistance);
        }
        ...
        private void StoreRestoreOrCreateDailyState()
        {
            ...
            string raw = LocalStorage.GetString(StoreKey("Daily"), LocalStorageScope.Type);
            bool restored = false;
        ...
        private void StorePersistDailyState()
        {
            ...
            LocalStorage.SetString(StoreKey("Daily"), raw, LocalStorageScope.Type);
            LocalStorage.Flush(LocalStorageScope.Type);
        }

        private void StoreRestoreRuntimeState()
        {
            string h1 = LocalStorage.GetString(StoreKey("H1Signals"), LocalStorageScope.Type);
            if (!string.IsNullOrWhiteSpace(h1))
                foreach (string k in h1.Split(new[]{'\n'}, StringSplitOptions.RemoveEmptyEntries))
                    _consumedSignals.Add(k);
            string m30 = LocalStorage.GetString(StoreKey("M30Signals"), LocalStorageScope.Type);
        ...
        private bool StoreCanExecute()
        {
            if (!_storeStartupValid || _storeDailyLocked)
                return false;
            // Hard no-hedging / no-pyramiding invariant, independent of public parameters.
            if (Round18OwnOpenPositions() >= 1)
                return false;
            return true;
        }

        private double StoreMinDistancePips(double rawDistance)
        {
            if (rawDistance <= 0) return 0;
            if (Symbol.MinDistanceType == SymbolMinDistanceType.Pips)
                return rawDistance;
            double mid = (Symbol.Ask + Symbol.Bid) * 0.5;
            if (mid <= 0 || Symbol.PipSize <= 0) return 0;
            return (mid * rawDistance / 100.0) / Symbol.PipSize;
        }

        private bool StoreValidateOrder(TradeType direction, double volume, double slPips, double tpPips, double riskPercent, string lane)
        {
            if (!StoreCanExecute())
            {
                Print("[STORE REJECT] lane={0} reason=execution_lock", lane);
                return false;
            }
            if (RiskMode != RiskSizingMode.RiskPercentEquity || riskPercent <= 0 || slPips <= 0 || tpPips <= 0)
            {
                Print("[STORE REJECT] lane={0} reason=invalid_risk_or_protection", lane);
                return false;
            }
            double normalized = Symbol.NormalizeVolumeInUnits(volume, RoundingMode.Down);
            if (double.IsNaN(normalized) || double.IsInfinity(normalized) || normalized < Symbol.VolumeInUnitsMin || normalized > Symbol.VolumeInUnitsMax || Math.Abs(normalized - volume) > 1e-8)
            {
                Print("[STORE REJECT] lane={0} reason=invalid_volume volume={1} normalized={2} min={3} max={4}", lane, volume, normalized, Symbol.VolumeInUnitsMin, Symbol.VolumeInUnitsMax);
                return false;
            }
            double budget = Account.Equity * riskPercent / 100.0;
            double estimatedRisk = Symbol.AmountRisked(volume, slPips);
            if (double.IsNaN(estimatedRisk) || double.IsInfinity(estimatedRisk) || estimatedRisk <= 0 || estimatedRisk > budget + Math.Max(0.01, budget * 0.0025))
            {
                Print("[STORE REJECT] lane={0} reason=risk_budget estimated={1:F4} budget={2:F4}", lane, estimatedRisk, budget);
                return false;
            }
            double estimatedMargin = Symbol.GetEstimatedMargin(direction, volume);
            if (double.IsNaN(estimatedMargin) || double.IsInfinity(estimatedMargin) || estimatedMargin < 0 || estimatedMargin > Account.FreeMargin)
            {
                Print("[STORE REJECT] lane={0} reason=margin estimated={1:F4} free={2:F4}", lane, estimatedMargin, Account.FreeMargin);
                return false;
            }
            double minSlPips = StoreMinDistancePips(Symbol.MinStopLossDistance);
            double minTpPips = StoreMinDistancePips(Symbol.MinTakeProfitDistance);
            if (minSlPips > 0 && slPips + 1e-8 < minSlPips)
            {
                Print("[STORE REJECT] lane={0} reason=min_sl sl={1:F4} brokerMin={2:F4}", lane, slPips, minSlPips);
                return false;
            }
            if (minTpPips > 0 && tpPips + 1e-8 < minTpPips)
            {
                Print("[STORE REJECT] lane={0} reason=min_tp tp={1:F4} brokerMin={2:F4}", lane, tpPips, minTpPips);
                return false;
            }
            return true;
        }
...
# Execution wrappers enforce store lock immediately before each alpha lane.
