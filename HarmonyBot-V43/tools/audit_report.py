...
fc=last_counts(r"\[V43-FREQUENCY-SUMMARY\].*?schedulerDeferred=(\d+)\s+schedulerRecoveredExecutions=(\d+)\s+structuredRecallAdmitted=(\d+)\s+activeDeferred=(\d+).*?routeDeferred=(\d+)\s+routeRecovered=(\d+)",
...
ec=last_counts(r"\[V43-EXECUTION-SUMMARY\].*?evidenceExecutable=(\d+)\s+survivalWaits=(\d+)\s+opportunityArbitrations=(\d+)\s+riskRenormalizations=(\d+)\s+riskRejects=(\d+)\s+shadowTargets=(\d+)\s+shadowStops=(\d+)\s+shadowUnresolved=(\d+)",
...
ac=last_counts(r"\[V43-ATTRIBUTION-SUMMARY\].*?observed=(\d+)\s+legacyRankFirstReleased=(\d+)\s+followThroughObserved=(\d+)\s+thesisFailureExits=(\d+)",
 ["attribution_observed","legacy_rank_first_released","follow_through_observed","thesis_failure_exits"])
mc=last_counts(r"\[V43-MARGINAL-ALPHA-SUMMARY\].*?coreAdmissions=(\d+)\s+rescueEvaluated=(\d+)\s+rescueAdmissions=(\d+)\s+rescueRejected=(\d+)",
 ["core_admissions","rescue_evaluated","rescue_admissions","rescue_rejected"])
xc=last_counts(r"\[V43-CROSS-REGIME-SUMMARY\].*?rejected=(\d+)\s+opportunityLossObserved=(\d+)\s+hypothesisResolutions=(\d+)",
...
# Correct V43 event parser: the V42 analyzer accidentally searched for literal backslashes.
evrx=re.compile(r"\[V43-EVENT\]\s+cid=(\S+)\s+pattern=(.*?)\s+tf=(\S+)\s+dir=(\S+)\s+state=(\S+)\s+route=(\S+)\s+conflict=(\S+)\s+reason=(.*)$",re.M)
attrition={}
...
opp=[]
opr=re.compile(r"\[V43-OPPORTUNITY-LOSS\]\s+cid=(\S+)\s+winner=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+rank=([-0-9.]+)\s+winnerRank=([-0-9.]+)")
