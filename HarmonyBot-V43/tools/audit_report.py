...
rx=re.compile(r"\[V43-BASKET-CLOSED\].*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
pat=(r"\[V43-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
freq=re.findall(r"\[V43-FREQUENCY-SUMMARY\].*?schedulerDeferred=(\d+)\s+schedulerRecoveredExecutions=(\d+)\s+structuredRecallAdmitted=(\d+)\s+activeDeferred=(\d+).*?routeDeferred=(\d+)\s+routeRecovered=(\d+)",t)
...
ex=re.findall(r"\[V43-EXECUTION-SUMMARY\].*?evidenceExecutable=(\d+)\s+survivalWaits=(\d+)\s+opportunityArbitrations=(\d+)\s+riskRenormalizations=(\d+)\s+riskRejects=(\d+)\s+shadowTargets=(\d+)\s+shadowStops=(\d+)\s+shadowUnresolved=(\d+)",t)
...
attr=re.findall(r"\[V43-ATTRIBUTION-SUMMARY\].*?observed=(\d+)\s+legacyRankFirstReleased=(\d+)\s+followThroughObserved=(\d+)\s+thesisFailureExits=(\d+)",t)
...
mar=re.findall(r"\[V43-MARGINAL-ALPHA-SUMMARY\].*?coreAdmissions=(\d+)\s+rescueEvaluated=(\d+)\s+rescueAdmissions=(\d+)\s+rescueRejected=(\d+)",t)
...
opp=re.findall(r"\[V43-OPPORTUNITY-SUMMARY\].*?gridAttempts=(\d+)\s+gridPasses=(\d+)\s+gridFails=(\d+)\s+slotBlocked=(\d+)\s+slotRecovered=(\d+)\s+patternNativeEvaluations=(\d+)\s+logicalFourLegPlans=(\d+)",t)
oc={"grid_plan_attempts":0,"grid_plan_passes":0,"grid_plan_fails":0,"slot_blocked_candidates":0,"slot_recovered_executions":0,
    "pattern_native_evaluations":0,"logical_four_leg_plans":0}
if opp:
 z=list(map(int,opp[-1]))
 for k,n in zip(oc,z): oc[k]=n

grid_failure_reasons={}
for q in re.finditer(r"\[V43-OPPORTUNITY-CONVERSION\].*?stage=GRID_PLAN\s+result=FAIL\s+reason=(\S+)",t):
 grid_failure_reasons[q.group(1)]=grid_failure_reasons.get(q.group(1),0)+1

rec=re.findall(r"\[V43-EXECUTION-RECOVERY-SUMMARY\].*?legacySpanBypasses=(\d+)\s+przSynchronousStarts=(\d+)",t)
recovery={"legacy_span_bypasses":0,"prz_synchronous_starts":0}
if rec:
 recovery["legacy_span_bypasses"],recovery["prz_synchronous_starts"]=map(int,rec[-1])

discovery={}
for q in re.finditer(r"\[V43-DISCOVERY-SUMMARY\]\s+pattern=(.*?)\s+attempts=(\d+)\s+rawMatches=(\d+)\s+selected=(\d+)",t):
 discovery[q.group(1)]={"attempts":int(q.group(2)),"raw_matches":int(q.group(3)),"selected":int(q.group(4))}
discovery_scales={}
for q in re.finditer(r"\[V43-DISCOVERY-SCALE\]\s+key=(\S+)\s+matches=(\d+)",t):
 discovery_scales[q.group(1)]=int(q.group(2))

pipe_rx=re.compile(r"\[V43-PIPELINE\]\s+pattern=(.*?)\s+detected=(\d+)\s+validated=(\d+)\s+routed=(\d+)\s+prz=(\d+)\s+confirming=(\d+)\s+armed=(\d+)\s+basketPlanned=(\d+)\s+leg0=(\d+)\s+leg1=(\d+)\s+leg2=(\d+)\s+leg3=(\d+)\s+basketClosed=(\d+)\s+executed=(\d+)\s+expired=(\d+)\s+rejected=(\d+)\s+invalidated=(\d+)")
pipeline={}
...
"broker_profile_present":"[V43-BROKER-PROFILE]" in t,"evidence_events":len(re.findall(r"\[V43-EVIDENCE\]",t)),
...
"attribution_outcomes":outcomes,"pattern_pipeline":pipeline,"grid_failure_reasons":grid_failure_reasons,
"pattern_discovery":discovery,"pattern_discovery_scales":discovery_scales,**recovery,**fc,**ec,**ac,**mc,**oc,**c}
