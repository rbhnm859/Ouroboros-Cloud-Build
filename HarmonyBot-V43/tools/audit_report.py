...
conv=re.findall(r"\[V43-CONVERSION-SUMMARY\].*?admissions=(\d+)\s+gridDeferred=(\d+)\s+capitalDeferred=(\d+)\s+executable=(\d+)\s+fallbackAttempts=(\d+)\s+fallbackExecutions=(\d+)",t)
cc={"conversion_admissions":0,"conversion_grid_deferred":0,"conversion_capital_deferred":0,"conversion_executable":0,
    "scheduler_fallback_attempts":0,"scheduler_fallback_executions":0}
if conv:
 z=list(map(int,conv[-1]))
 for k,n in zip(cc,z): cc[k]=n

sf=re.findall(r"\[V43-STRUCTURAL-FLOW-SUMMARY\].*?routeWatchAdmitted=(\d+)\s+routeWatchRecovered=(\d+)\s+routeRefreshes=(\d+)\s+sessionLifetimeExpired=(\d+)\s+thesisConsumedExpired=(\d+)\s+patternNativeEvidenceObserved=(\d+)",t)
sc={"route_watch_admitted":0,"route_watch_recovered":0,"route_refreshes":0,"session_lifetime_expired":0,
    "thesis_consumed_expired":0,"pattern_native_evidence_observed":0}
if sf:
 z=list(map(int,sf[-1]))
 for k,n in zip(sc,z): sc[k]=n
...
"attribution_outcomes":outcomes,"pattern_pipeline":pipeline,**fc,**ec,**ac,**mc,**cc,**sc,**c}
