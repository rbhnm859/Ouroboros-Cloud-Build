...
s=pathlib.Path(sys.argv[1] if len(sys.argv)>1 else "HarmonyBot-V43/src/HarmonyBotV43.cs").read_text(errors="ignore")
required={
...
out={"version":"HarmonyBot V43","checks":required,"pass":all(required.values())}
pathlib.Path("V43_GEOMETRY_CONTRACT_TEST.json").write_text(json.dumps(out,indent=2))
