...
GRIDREAL="${GRIDREAL:-true}"; QUEUE="${QUEUE:-true}"; PATTERNNATIVE="${PATTERNNATIVE:-true}"
MULTISCALE="${MULTISCALE:-false}"; DIVERSITY="${DIVERSITY:-false}"; STRUCTGRID="${STRUCTGRID:-false}"; PRZSYNC="${PRZSYNC:-false}"
TTL="${TTL:-8}"; MAXCAND="${MAXCAND:-12}"
ALGO="${ALGO:-seal/algo/HarmonyBot_V43_Multiscale_Harmonic_Discovery_Portfolio_RC.algo}"
...
CNAME="v43-$(echo "$RUN_NAME"|tr '[:upper:]_' '[:lower:]-')-$GITHUB_RUN_ID"
docker run --name "$CNAME" -v "$PWD/seal:/work" "$IMAGE" backtest "/work/${ALGO#seal/}" \
 ...
 --EnablePhysicalGridRealization="$GRIDREAL" --EnableOpportunityQueue="$QUEUE" --EnablePatternNativeExecution="$PATTERNNATIVE" --OpportunityHoldMinutes=90 \
 --EnableMultiScalePatternDiscovery="$MULTISCALE" --EnablePatternDiversityAdmission="$DIVERSITY" \
 --EnableStructuralGridEligibility="$STRUCTGRID" --EnablePrzSynchronousEvidence="$PRZSYNC" \
...
PID=$!; DONE=0
echo "[V43-WATCHDOG] run=$RUN_NAME pid=$PID timeoutSeconds=$BACKTEST_TIMEOUT_SECONDS"
for ((i=0;i<BACKTEST_TIMEOUT_SECONDS;i+=5)); do
 if (( i % 60 == 0 )); then echo "[V43-WATCHDOG] run=$RUN_NAME elapsedSeconds=$i status=running"; fi
...
test "$DONE" = 1 || { echo "[V43-WATCHDOG-FAIL] run=$RUN_NAME"; tail -400 "seal/logs/$RUN_NAME.log" || true; exit 20; }
echo "[V43-WATCHDOG] run=$RUN_NAME status=complete"
