...
ROUTEWATCH="${ROUTEWATCH:-true}"; EVENTLIFE="${EVENTLIFE:-true}"; NATIVEEVID="${NATIVEEVID:-true}"; ROUTEREFRESH="${ROUTEREFRESH:-true}"
TTL="${TTL:-8}"; MAXCAND="${MAXCAND:-12}"
ALGO="${ALGO:-seal/algo/HarmonyBot_V43_Pattern_Native_Structural_Routing_Portfolio_RC.algo}"
...
docker run --name "$CNAME" -v "$PWD/seal:/work" "$IMAGE" backtest "/work/${ALGO#seal/}" \
 ...
 --EnableOpportunityConversionLedger="$CONVERSION" --EnableAdmittedCandidateReplan="$REPLAN" --EnableSchedulerFallbackCascade="$FALLBACK" \
 --EnableStructuralRouteWatch="$ROUTEWATCH" --EnableEventDrivenCandidateLifetime="$EVENTLIFE" --EnablePatternNativeM5Evidence="$NATIVEEVID" --EnableCrossRegimeRouteRefresh="$ROUTEREFRESH" \
