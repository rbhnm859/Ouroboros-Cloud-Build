...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V43 — Multiscale Harmonic Discovery & Positive-Alpha Execution Portfolio";
        ...
        [Parameter("V43 Physical Grid Realization", DefaultValue = false)]
        public bool EnablePhysicalGridRealization { get; set; }

        [Parameter("V43 Opportunity Queue", DefaultValue = false)]
        public bool EnableOpportunityQueue { get; set; }

        [Parameter("V43 Pattern-Native Competition", DefaultValue = false)]
        public bool EnablePatternNativeExecution { get; set; }

        [Parameter("V43 Opportunity Hold Minutes", DefaultValue = 90, MinValue = 15, MaxValue = 180)]
        public int OpportunityHoldMinutes { get; set; }

        [Parameter("V43 Multi-Scale Pattern Discovery", DefaultValue = false)]
        public bool EnableMultiScalePatternDiscovery { get; set; }

        [Parameter("V43 Pattern Diversity Admission", DefaultValue = false)]
        public bool EnablePatternDiversityAdmission { get; set; }

        [Parameter("V43 Structural Grid Eligibility", DefaultValue = false)]
        public bool EnableStructuralGridEligibility { get; set; }

        [Parameter("V43 PRZ Synchronous Evidence", DefaultValue = false)]
        public bool EnablePrzSynchronousEvidence { get; set; }
        ...
        private int _gridPlanAttempts;
        private int _gridPlanPasses;
        private int _gridPlanFails;
        private int _slotBlockedCandidates;
        private int _slotRecoveredExecutions;
        private int _patternNativeEvaluations;
        private int _logicalFourLegPlans;
        private int _legacySpanBypasses;
        private int _przSynchronousStarts;
        private readonly Dictionary<string, int> _discoveryAttempts = new Dictionary<string, int>();
        private readonly Dictionary<string, int> _discoveryRawMatches = new Dictionary<string, int>();
        private readonly Dictionary<string, int> _discoverySelected = new Dictionary<string, int>();
        private readonly Dictionary<string, int> _discoveryScaleMatches = new Dictionary<string, int>();
        ...
        protected override void OnStart()
        {
            ...
            Print("[V43-OPPORTUNITY-CONFIG] physicalGrid={0} opportunityQueue={1} patternNative={2} holdMinutes={3}",
                EnablePhysicalGridRealization, EnableOpportunityQueue, EnablePatternNativeExecution, OpportunityHoldMinutes);
            Print("[V43-DISCOVERY-CONFIG] multiScale={0} diversityAdmission={1} structuralGridEligibility={2} przSynchronousEvidence={3}",
                EnableMultiScalePatternDiscovery, EnablePatternDiversityAdmission, EnableStructuralGridEligibility, EnablePrzSynchronousEvidence);
        }
        ...
        protected override void OnStop()
        {
            ...
            Print("[V43-OPPORTUNITY-SUMMARY] gridAttempts={0} gridPasses={1} gridFails={2} slotBlocked={3} slotRecovered={4} patternNativeEvaluations={5} logicalFourLegPlans={6}",
                _gridPlanAttempts, _gridPlanPasses, _gridPlanFails, _slotBlockedCandidates, _slotRecoveredExecutions,
                _patternNativeEvaluations, _logicalFourLegPlans);
            Print("[V43-EXECUTION-RECOVERY-SUMMARY] legacySpanBypasses={0} przSynchronousStarts={1}", _legacySpanBypasses, _przSynchronousStarts);
            foreach (var p in _profiles.OrderBy(x => x.Name))
                Print("[V43-DISCOVERY-SUMMARY] pattern={0} attempts={1} rawMatches={2} selected={3}",
                    p.Name, DictCount(_discoveryAttempts, p.Name), DictCount(_discoveryRawMatches, p.Name), DictCount(_discoverySelected, p.Name));
            foreach (var kv in _discoveryScaleMatches.OrderBy(k => k.Key))
                Print("[V43-DISCOVERY-SCALE] key={0} matches={1}", kv.Key, kv.Value);
        ...
        private void ProcessNewM15Close()
        {
            ...
            {
                string canonicalKey = BuildCanonicalCandidateKey(signal);
                ...
                var record = new CandidateRecord
                {
                    ...
                    Lane = "UNASSIGNED",
                    ExecutionArchetype = PatternExecutionArchetype(signal.Profile),
                    Signal = signal,
        ...
        private void AdvanceCandidateLifecycleOnBars(Bars bars, int i, DateTime utc, string executionTf, bool allowArm)
        {
            ...
            {
                bool opportunityHold = EnableOpportunityQueue && c.SlotBlocked && c.OpportunityHoldUntilUtc > utc;
                if (utc >= c.ExpiryUtc && !opportunityHold)
                {
                ...
                if (opportunityHold && utc >= c.ExpiryUtc)
                    Event(c, "OPPORTUNITY_QUEUE_HOLD");
                ...
                if (c.State == CandidateState.WAIT_PRZ)
                {
                    if (BarTouchesPrz(bars, i, c.Signal))
                    {
                        ...
                        if (!EnablePrzSynchronousEvidence) continue;
                    }
                    else continue;
                }
                ...
                if (c.State == CandidateState.PRZ_TOUCHED)
                {
                    ...
                    if (!c.PrzTouchUtc.HasValue || (utc <= c.PrzTouchUtc.Value && !EnablePrzSynchronousEvidence)) continue;
                    if (EnablePrzSynchronousEvidence && utc == c.PrzTouchUtc.Value) _przSynchronousStarts++;
                ...
                if (!c.PrzTouchUtc.HasValue || (utc <= c.PrzTouchUtc.Value && !EnablePrzSynchronousEvidence))
                    continue;
                ...
                {
                    _gridPlanFails++;
                    Print("[V43-OPPORTUNITY-CONVERSION] cid={0} key={1} pattern={2} lane={3} stage=GRID_PLAN result=FAIL reason={4}",
                        c.CandidateId, c.CanonicalCandidateKey, c.Signal.PatternName, c.Lane, c.LastReason ?? "UNKNOWN");
                    if (EnableCandidateSurvival)
                    {
                        _survivalWaits++;
                        c.GridPlan = null;
                ...
                _gridPlanPasses++;
                Print("[V43-OPPORTUNITY-CONVERSION] cid={0} key={1} pattern={2} lane={3} stage=GRID_PLAN result=PASS physicalDepth={4} logicalLegs={5}",
                    c.CandidateId, c.CanonicalCandidateKey, c.Signal.PatternName, c.Lane, c.GridPlan.PhysicalDepth, c.GridPlan.LogicalLegCount);

                c.AttributionScore = AttributionScore(c);
                c.PortfolioEdgeScore = c.AttributionScore;
                c.PatternNativeScore = EnablePatternNativeExecution ? PatternNativeOpportunityScore(c) : c.AttributionScore;
                _portfolioAccepted++;
        ...
        private void TryScheduleAndExecute()
        {
            ...
            if (OwnPositions().Any() || OwnPendingOrders().Any() || _baskets.Values.Any(b => b.IsActive))
            {
                if (EnableOpportunityQueue)
                {
                    DateTime now = Server.Time.ToUniversalTime();
                    foreach (var q in _candidates.Values.Where(c => c.IsActive &&
                        (c.State == CandidateState.EXECUTABLE || c.State == CandidateState.ARMED) && c.GridPlan != null))
                    {
                        if (!q.SlotBlocked)
                        {
                            q.SlotBlocked = true;
                            _slotBlockedCandidates++;
                            q.OpportunityHoldUntilUtc = q.ExpiryUtc.AddMinutes(Math.Max(15, OpportunityHoldMinutes));
                            Event(q, "OPPORTUNITY_QUEUE_SLOT_BLOCKED");
                            Print("[V43-OPPORTUNITY-CONVERSION] cid={0} key={1} pattern={2} lane={3} stage=SLOT result=BLOCKED holdUntil={4:o}",
                                q.CandidateId, q.CanonicalCandidateKey, q.Signal.PatternName, q.Lane, q.OpportunityHoldUntilUtc);
                        }
                    }
                }
                return;
            ...
            {
                if (winner.SlotBlocked)
                {
                    _slotRecoveredExecutions++;
                    Print("[V43-OPPORTUNITY-CONVERSION] cid={0} key={1} pattern={2} lane={3} stage=SLOT result=RECOVERED_EXECUTION",
                        winner.CandidateId, winner.CanonicalCandidateKey, winner.Signal.PatternName, winner.Lane);
                }
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            if (p == null || !p.GridEnabled) return GridPlanFailure(c, "PROFILE_GRID_DISABLED");
            if (!_initialCapitalEligible) return GridPlanFailure(c, "INITIAL_CAPITAL_INELIGIBLE");
            ...
            if (distance <= PipsToPrice(MinStopLossPips)) return GridPlanFailure(c, "STOP_DISTANCE");

            double xa = Math.Abs(c.Signal.A.Price - c.Signal.X.Price);
            double spanXa = xa > 0 ? distance / xa : 999;
            bool legacySpanEligible = spanXa >= p.MinimumGridSpanXa && spanXa <= p.MaximumGridSpanXa;
            if (!legacySpanEligible && !EnableStructuralGridEligibility) return GridPlanFailure(c, "SPAN_XA");
            if (!legacySpanEligible && EnableStructuralGridEligibility)
            {
                _legacySpanBypasses++;
                Print("[V43-EXECUTION-RECOVERY] cid={0} pattern={1} stage=SPAN_XA result=BYPASS legacySpan={2:F4} min={3:F4} max={4:F4}",
                    c.CandidateId, c.Signal.PatternName, spanXa, p.MinimumGridSpanXa, p.MaximumGridSpanXa);
            }
            ...
            int maxLegs = EnablePhysicalGridRealization
                ? Math.Min(4, p.GridFractions.Length)
                : Math.Min(Math.Min(routeMax, p.MaximumGridLegs), p.GridFractions.Length);
            if (maxLegs <= 0) return GridPlanFailure(c, "NO_LOGICAL_LEGS");
            ...
            var plan = new FibonacciGridPlan
            {
                ...
                RoutePhysicalDepthCap = Math.Max(1, routeMax)
            };
            if (plan.BasketRiskAmount <= 0) return GridPlanFailure(c, "NON_POSITIVE_RISK_BUDGET");
            ...
            if (plan.Legs.Count == 0 || plan.Legs[0].Index != 0) return GridPlanFailure(c, "NO_LEGAL_L0");
            plan.LogicalLegCount = plan.Legs.Count;
            if (plan.LogicalLegCount == 4) _logicalFourLegPlans++;

            if (!ConfigureCapitalExecution(plan))
            {
                _capitalRejectedBaskets++;
                return GridPlanFailure(c, "CAPITAL_EXECUTION_INFEASIBLE");
            }

            var physical = plan.Legs.Where(x => x.Physical && x.Volume > 0).ToList();
            if (physical.Count == 0 || physical[0].Index != 0) return GridPlanFailure(c, "NO_PHYSICAL_L0");
            ...
            if (!SelectCanonicalBasketTarget(c.Signal, plan.ExpectedWeightedEntry, plan.StructuralStop, out target, out netRr))
                return GridPlanFailure(c, "CANONICAL_TARGET");
        ...
        private bool GridPlanFailure(CandidateRecord c, string reason)
        {
            if (c != null) c.LastReason = reason;
            return false;
        }
        ...
        private bool ConfigureCapitalExecution(FibonacciGridPlan plan)
        {
            ...
            {
                int maxDepth = EnablePhysicalGridRealization
                    ? Math.Min(plan.Legs.Count, Math.Max(1, plan.RoutePhysicalDepthCap))
                    : plan.Legs.Count;
            ...
            int physicalDepth = 0;
            int maxPhysicalDepth = EnablePhysicalGridRealization
                ? Math.Min(plan.Legs.Count, Math.Max(1, plan.RoutePhysicalDepthCap))
                : plan.Legs.Count;
            foreach (var l in plan.Legs.Take(maxPhysicalDepth))
            {
                double slPips = PriceToPips(Math.Abs(l.PlannedPrice - plan.StructuralStop));
        ...
        {
            var empty = new List<PatternSignal>();
            if (bars == null || endIndex < 40) return empty;
            double atr = Atr(bars, 14, endIndex);
            if (atr <= 0) return empty;

            bool primaryM15 = string.Equals(timeframe, "M15", StringComparison.OrdinalIgnoreCase);
            int[] depths = primaryM15 && EnableMultiScalePatternDiscovery
                ? new[] { Math.Max(2, depth - 1), depth, Math.Min(8, depth + 2) }.Distinct().ToArray()
                : new[] { depth };

            var raw = new List<PatternSignal>();
            foreach (int detectorDepth in depths)
                CollectPatternMatchesForDepth(bars, endIndex, detectorDepth, lookback, atr, timeframe, raw, primaryM15);

            var dedup = raw
                .OrderByDescending(x => x.Confidence)
                .ThenByDescending(x => x.GeometryQuality)
                .GroupBy(SignalTopologyKey)
                .Select(g => g.First())
                .ToList();

            List<PatternSignal> selected;
            int cap = Math.Max(1, maxCandidates);
            if (primaryM15 && EnablePatternDiversityAdmission)
            {
                selected = dedup
                ...
                {
                    var keys = new HashSet<string>(selected.Select(SignalTopologyKey));
                    foreach (var x in dedup.OrderByDescending(x => x.Confidence).ThenByDescending(x => x.GeometryQuality))
                    {
                        string key = SignalTopologyKey(x);
            ...
            if (primaryM15)
                foreach (var x in selected)
                    IncCounter(_discoverySelected, x.PatternName);
            return selected;
        }

        private void CollectPatternMatchesForDepth(Bars bars, int endIndex, int depth, int lookback, double atr,
            string timeframe, List<PatternSignal> result, bool diagnostic)
        {
            var pivots = BuildConfirmedPivots(bars, endIndex, lookback, depth);
            if (pivots.Count < 5) return;
            ...
            for (int i = start; i <= pivots.Count - 5; i++)
            {
                ...
                {
                    if (diagnostic) IncCounter(_discoveryAttempts, profile.Name);
                    PatternSignal sig;
                    ...
                    sig.DiscoveryDepth = depth;
                    sig.DiscoveryTopology = "CONFIRMED_CONSECUTIVE_D" + depth.ToString(CultureInfo.InvariantCulture);
                    result.Add(sig);
                    if (diagnostic)
                    {
                        IncCounter(_discoveryRawMatches, profile.Name);
                        IncCounter(_discoveryScaleMatches, profile.Name + "|D" + depth.ToString(CultureInfo.InvariantCulture));
                    }
                }
            }
        }

        private string SignalTopologyKey(PatternSignal s)
        {
            if (s == null) return "INVALID";
            return s.PatternName + "|" + s.Direction + "|" + s.X.Index + "|" + s.A.Index + "|" + s.B.Index + "|" +
                   s.C.Index + "|" + s.D.Index;
        }

        private static void IncCounter(Dictionary<string, int> map, string key)
        {
            int n;
        ...
        private static int DictCount(Dictionary<string, int> map, string key)
        {
            int n;
            ...
            double tolerance, int ttlMinutes, double stopFibBuffer, string anchorRule, string targetPolicy)
        {
            ...
            if (p == null) return;
            p.GridEnabled = true;
            p.GridFractions = EnablePhysicalGridRealization ? new[] { 0.0, .236, .382, .618 } : fractions;
            p.MaximumGridLegs = EnablePhysicalGridRealization ? 4 : maxLegs;
        ...
        private string PatternExecutionArchetype(PatternProfile p)
        {
            if (p == null) return "UNKNOWN";
            if (p.Mode == PatternMode.ABCD) return "SYMMETRY";
            if (p.Mode == PatternMode.CYPHER) return "XC_RETRACE";
            if (p.Mode == PatternMode.SHARK || p.Mode == PatternMode.FIVEZERO) return "EXTREME_REVERSAL";
            if (p.Name == "Alt Bat" || p.Name == "Butterfly" || p.Name == "Crab" ||
                p.Name == "Deep Crab" || p.Name == "Rat") return "EXTENSION";
            return "RETRACEMENT";
        }

        private double PatternNativeOpportunityScore(CandidateRecord c)
        {
            if (c == null || c.Signal == null || c.Regime == null) return 0;
            _patternNativeEvaluations++;
            string a = string.IsNullOrWhiteSpace(c.ExecutionArchetype) ? PatternExecutionArchetype(c.Signal.Profile) : c.ExecutionArchetype;
            double geom = VClamp(c.Signal.GeometryQuality);
            double prz = VClamp(c.Signal.PrzConfluence);
            double time = VClamp(c.Signal.TimeSymmetry);
            double pivot = VClamp(c.Signal.PivotQuality);
            double ev = VClamp(Math.Max(c.EvidenceComposite, c.ConfirmationScore));
            double ft = VClamp(c.FollowThroughScore);
            double route = VClamp(c.RouteFitScore);
            double calm = 1.0 - VClamp(c.RegimeStressScore);
            if (a == "SYMMETRY") return VClamp(.40 * time + .20 * geom + .15 * pivot + .25 * ev);
            if (a == "EXTENSION") return VClamp(.30 * prz + .20 * geom + .25 * ft + .15 * route + .10 * calm);
            if (a == "XC_RETRACE") return VClamp(.25 * geom + .25 * prz + .25 * ft + .25 * route);
            if (a == "EXTREME_REVERSAL") return VClamp(.30 * prz + .25 * ft + .20 * pivot + .25 * calm);
            return VClamp(.30 * geom + .25 * prz + .20 * ev + .15 * route + .10 * time);
        }
        ...
        private double CandidateRank(CandidateRecord c)
        {
            ...
            {
                c.PatternNativeScore = PatternNativeOpportunityScore(c);
                baseRank = .70 * baseRank + .30 * c.PatternNativeScore;
            }
        ...
        private double OpportunityCostRank(CandidateRecord c)
        {
            ...
            {
                c.PatternNativeScore = PatternNativeOpportunityScore(c);
                edge = VClamp(.70 * edge + .30 * c.PatternNativeScore);
            }
            ...
            return edge * Math.Max(.10, readiness) * urgency * Math.Max(.10, structural) *
    ...
    public sealed class PatternSignal
    {
        ...
        public string DiscoveryTopology;
    }
    ...
    public sealed class CandidateRecord
    {
        ...
        public double PatternNativeScore;
    ...
    public sealed class FibonacciGridPlan
    {
        ...
        public int LogicalLegCount, PhysicalDepth, RoutePhysicalDepthCap;
