...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V43 — Cross-Validated Marginal Alpha Admission & Pattern Portfolio Engine";
        ...
        [Parameter("Opportunity Conversion Ledger", DefaultValue = true)]
        public bool EnableOpportunityConversionLedger { get; set; }

        [Parameter("Admitted Candidate Replan", DefaultValue = true)]
        public bool EnableAdmittedCandidateReplan { get; set; }

        [Parameter("Scheduler Fallback Cascade", DefaultValue = true)]
        public bool EnableSchedulerFallbackCascade { get; set; }

        [Parameter("Structural Route Watch", DefaultValue = true)]
        public bool EnableStructuralRouteWatch { get; set; }

        [Parameter("Event-Driven Candidate Lifetime", DefaultValue = true)]
        public bool EnableEventDrivenCandidateLifetime { get; set; }

        [Parameter("Pattern-Native M5 Evidence", DefaultValue = true)]
        public bool EnablePatternNativeM5Evidence { get; set; }

        [Parameter("Cross-Regime Route Refresh", DefaultValue = true)]
        public bool EnableCrossRegimeRouteRefresh { get; set; }
        ...
        private int _conversionAdmissions;
        private int _conversionGridDeferred;
        private int _conversionCapitalDeferred;
        private int _conversionExecutable;
        private int _schedulerFallbackAttempts;
        private int _schedulerFallbackExecutions;
        private int _routeWatchAdmitted;
        private int _routeWatchRecovered;
        private int _routeRefreshes;
        private int _sessionLifetimeExpired;
        private int _thesisConsumedExpired;
        private int _patternNativeEvidenceObserved;
        ...
        protected override void OnStop()
        {
            ...
            Print("[V43-CONVERSION-SUMMARY] admissions={0} gridDeferred={1} capitalDeferred={2} executable={3} fallbackAttempts={4} fallbackExecutions={5}",
                _conversionAdmissions, _conversionGridDeferred, _conversionCapitalDeferred, _conversionExecutable,
                _schedulerFallbackAttempts, _schedulerFallbackExecutions);
            Print("[V43-STRUCTURAL-FLOW-SUMMARY] routeWatchAdmitted={0} routeWatchRecovered={1} routeRefreshes={2} sessionLifetimeExpired={3} thesisConsumedExpired={4} patternNativeEvidenceObserved={5}",
                _routeWatchAdmitted, _routeWatchRecovered, _routeRefreshes, _sessionLifetimeExpired, _thesisConsumedExpired, _patternNativeEvidenceObserved);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            ReevaluateStructuralRouteWatch(h4State, h1State, regime, utc);
            RefreshPreTouchRoutes(h4State, h1State, regime, utc);
            ...
            foreach (var signal in detected)
            {
                ...
                var record = new CandidateRecord
                {
                    ...
                    ExpiryUtc = EnableEventDrivenCandidateLifetime
                        ? CandidateSessionExpiryUtc(Server.Time.ToUniversalTime())
                        : Server.Time.ToUniversalTime().AddMinutes(15.0 * Math.Max(2, Math.Min(CandidateTtlM15Bars, signal.Profile.MaxAgeM15Bars))),
                ...
                if (record.Route == HarmonicRoute.NO_TRADE && EnableStructuralRouteWatch)
                    record.Route = StructuralOpportunityRoute(signal, record.Conflict, regime);

                if (record.Route == HarmonicRoute.NO_TRADE)
                {
                    if (EnableRegimeContextGate) _regimeRejected++;
                    if (EnableStructuralRouteWatch)
                    {
                        record.RouteWatchStartedUtc = utc;
                        _routeWatchAdmitted++;
                        Transition(record, CandidateState.ROUTE_WATCH, "STRUCTURAL_ROUTE_WATCH");
                        continue;
                    }
        ...
        private void AdvanceCandidateLifecycleOnBars(Bars bars, int i, DateTime utc, string executionTf, bool allowArm)
        {
            ...
            {
                if (EnableEventDrivenCandidateLifetime && ThesisConsumedBeforeEntry(c))
                {
                    _thesisConsumedExpired++;
                    AddCounterfactualShadow(c, "THESIS_CONSUMED_BEFORE_ENTRY", utc);
                    Expire(c, "THESIS_CONSUMED_BEFORE_ENTRY");
                ...
                if (utc >= c.ExpiryUtc)
                {
                    string lifeReason = EnableEventDrivenCandidateLifetime ? "SESSION_LIFETIME_EXPIRED" : "TTL_EXPIRED";
                    if (EnableEventDrivenCandidateLifetime) _sessionLifetimeExpired++;
                    AddCounterfactualShadow(c, lifeReason, utc);
                    Expire(c, lifeReason);
                    _qualifiedRouteDeferredCandidates.Remove(c.CandidateId);
                    continue;
                }
                ...
                if (c.State == CandidateState.ADMITTED && allowArm)
                {
                    if (EnableAdmittedCandidateReplan)
                        TryConvertAdmittedCandidate(c, executionTf, c.EvidencePassedAtAdmission);
                    continue;
                }
                ...
                double barScore = EnablePatternNativeM5Evidence
                    ? PatternNativeExecutionScore(bars, i, c.Signal)
                    : ExecutionConfirmationScore(bars, i, c.Signal);
                if (EnablePatternNativeM5Evidence) _patternNativeEvidenceObserved++;
                ...
                if (EnableOpportunityConversionLedger || EnableAdmittedCandidateReplan)
                {
                    c.AdmittedUtc = utc;
                    _conversionAdmissions++;
                    Transition(c, CandidateState.ADMITTED, executionTf + "_" + c.Lane + "_ADMITTED");
                    TryConvertAdmittedCandidate(c, executionTf, evidencePass);
                    continue;
                }
        ...
        private bool TryConvertAdmittedCandidate(CandidateRecord c, string executionTf, bool evidencePass)
        {
            if (c == null || c.Signal == null || !c.IsActive || c.State != CandidateState.ADMITTED)
                return false;

            c.ConversionAttempts++;
            int capitalRejectsBefore = _capitalRejectedBaskets;
            bool gridOk = TryBuildFibonacciGridPlan(c);
            if (!gridOk)
            {
                bool capitalFailure = _capitalRejectedBaskets > capitalRejectsBefore;
                c.ConversionGridFeasible = false;
                c.ConversionCapitalFeasible = !capitalFailure;
                c.ConversionLastFailure = capitalFailure ? "CAPITAL_FEASIBILITY" : "GRID_FEASIBILITY";
                if (capitalFailure) _conversionCapitalDeferred++; else _conversionGridDeferred++;
                if (EnableOpportunityConversionLedger)
                    Print("[V43-CONVERSION] cid={0} key={1} lane={2} pattern={3} route={4} attempt={5} stage={6} status=DEFER evidence={7:F3} follow={8:F3}",
                        c.CandidateId, c.CanonicalCandidateKey, c.Lane, c.Signal.PatternName, c.Route, c.ConversionAttempts,
                        c.ConversionLastFailure, c.EvidenceComposite, c.FollowThroughScore);
                if (EnableCandidateSurvival)
                {
                    _survivalWaits++;
                    Event(c, "CONVERSION_DEFER_" + c.ConversionLastFailure);
                    return false;
                }
                Reject(c, "CONVERSION_REJECT_" + c.ConversionLastFailure);
                return false;
            }

            c.ConversionGridFeasible = true;
            c.ConversionCapitalFeasible = true;
            c.ConversionLastFailure = "NONE";
            c.AttributionScore = AttributionScore(c);
            c.PortfolioEdgeScore = c.AttributionScore;
            _portfolioAccepted++;
            c.Rank = CandidateRank(c);
            _alphaPassed++;
            _evidenceExecutable++;
            _conversionExecutable++;
            if (EnableAttributionLedger) PrintCandidateAttribution(c, executionTf, evidencePass);
            Transition(c, CandidateState.EXECUTABLE, executionTf + "_" + c.Lane + "_EXECUTABLE");
            CountPipeline(c.Signal.PatternName).Armed++;
            if (EnableOpportunityConversionLedger)
                Print("[V43-CONVERSION] cid={0} key={1} lane={2} pattern={3} route={4} attempt={5} stage=EXECUTABLE status=PASS rank={6:F4} netRR={7:F3} physicalDepth={8}",
                    c.CandidateId, c.CanonicalCandidateKey, c.Lane, c.Signal.PatternName, c.Route, c.ConversionAttempts,
                    c.Rank, c.NetRR, c.GridPlan == null ? 0 : c.GridPlan.PhysicalDepth);
            return true;
        }
        ...
        private HarmonicRoute StructuralOpportunityRoute(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            if (s == null || r == null) return HarmonicRoute.NO_TRADE;
            bool aligned = r.TrendDirection == s.Direction;
            bool neutral = r.TrendDirection == TradeDirection.Neutral;
            bool opposed = !neutral && !aligned;
            bool transition = r.Transition || conflict == MtfConflict.TRANSITION || neutral;

            // Classification, not an alpha veto: M5 evidence and the marginal gate decide admission.
            if (aligned && conflict != MtfConflict.CONFLICT)
                return HarmonicRoute.TREND_ALIGNED_REVERSAL;
            if (transition && conflict != MtfConflict.CONFLICT)
                return HarmonicRoute.TRANSITION_REVERSAL;
            // One ATR is a scale-normalized definition of structural extension, not a tuned DEV threshold.
            if (opposed && r.ExtensionAtr >= 1.0)
                return HarmonicRoute.EXHAUSTION_REVERSAL;
            return HarmonicRoute.NO_TRADE;
        }

        private DateTime CandidateSessionExpiryUtc(DateTime utc)
        {
            utc = DateTime.SpecifyKind(utc, DateTimeKind.Utc);
            DateTime ny = TimeZoneInfo.ConvertTimeFromUtc(utc, _newYorkTz);
            DateTime closeLocal = DateTime.SpecifyKind(ny.Date.AddHours(17), DateTimeKind.Unspecified);
            DateTime closeUtc = TimeZoneInfo.ConvertTimeToUtc(closeLocal, _newYorkTz);
            if (closeUtc <= utc)
            {
                closeLocal = DateTime.SpecifyKind(ny.Date.AddDays(1).AddHours(17), DateTimeKind.Unspecified);
                closeUtc = TimeZoneInfo.ConvertTimeToUtc(closeLocal, _newYorkTz);
            }
            return closeUtc;
        }

        private bool ThesisConsumedBeforeEntry(CandidateRecord c)
        {
            if (c == null || c.Signal == null) return true;
            // Once T1 has printed before entry, the original reversal opportunity has already paid out.
            return c.Signal.Direction == TradeDirection.Buy
                ? _symbol.Bid >= c.Signal.CanonicalTarget1
                : _symbol.Ask <= c.Signal.CanonicalTarget1;
        }

        private void ReevaluateStructuralRouteWatch(HarmonicState h4State, HarmonicState h1State, RegimeSnapshot regime, DateTime utc)
        {
            if (!EnableStructuralRouteWatch) return;
            foreach (var c in _candidates.Values.Where(x => x.IsActive && x.State == CandidateState.ROUTE_WATCH).ToList())
            {
                if (utc >= c.ExpiryUtc || PatternInvalidatedBeforeEntry(c.Signal) || ThesisConsumedBeforeEntry(c))
                    continue;
                ...
                var route = StructuralOpportunityRoute(c.Signal, c.Conflict, regime);
                if (route == HarmonicRoute.NO_TRADE)
                    continue;

                c.Route = route;
                c.LastRouteRefreshUtc = utc;
                _routeWatchRecovered++;
                Print("[V43-ROUTE-WATCH-RECOVER] cid={0} key={1} pattern={2} direction={3} conflict={4} route={5} regime={6:F3}",
                    c.CandidateId, c.CanonicalCandidateKey, c.Signal.PatternName, c.Signal.Direction, c.Conflict, c.Route, c.RegimeScore);
                Transition(c, CandidateState.ROUTED, "STRUCTURAL_ROUTE_RECOVERED_" + c.Route);
                CountPipeline(c.Signal.PatternName).Routed++;
                Transition(c, CandidateState.WAIT_PRZ, "WAIT_PRZ_AFTER_STRUCTURAL_ROUTE");
                CountPipeline(c.Signal.PatternName).PrzWaiting++;
            }
        }

        private void RefreshPreTouchRoutes(HarmonicState h4State, HarmonicState h1State, RegimeSnapshot regime, DateTime utc)
        {
            if (!EnableCrossRegimeRouteRefresh) return;
            foreach (var c in _candidates.Values.Where(x => x.IsActive && x.State == CandidateState.WAIT_PRZ && !x.PrzTouchUtc.HasValue).ToList())
            {
                if (utc >= c.ExpiryUtc || PatternInvalidatedBeforeEntry(c.Signal) || (EnableEventDrivenCandidateLifetime && ThesisConsumedBeforeEntry(c)))
                    continue;
                var conflict = ClassifyMtfConflict(c.Signal.Direction, h4State, h1State);
                var route = StructuralOpportunityRoute(c.Signal, conflict, regime);
                if (route == HarmonicRoute.NO_TRADE || route == c.Route) continue;
                var prior = c.Route;
                c.Conflict = conflict;
                c.Regime = regime;
                c.RegimeScore = RegimeContextScore(c.Signal, conflict, regime);
                c.Route = route;
                c.LastRouteRefreshUtc = utc;
                _routeRefreshes++;
                Print("[V43-ROUTE-REFRESH] cid={0} key={1} pattern={2} prior={3} next={4} conflict={5} regime={6:F3}",
                    c.CandidateId, c.CanonicalCandidateKey, c.Signal.PatternName, prior, route, conflict, c.RegimeScore);
                Event(c, "CROSS_REGIME_ROUTE_REFRESH_" + prior + "_TO_" + route);
            }
        }
        ...
        private void TryScheduleAndExecute()
        {
            ...
            foreach (var candidate in armed)
            {
                if (OwnPositions().Any() || OwnPendingOrders().Any() || _baskets.Values.Any(b => b.IsActive))
                    break;

                if (EnableOpportunityConversionLedger)
                    Print("[V43-OPPORTUNITY-AUCTION] cid={0} key={1} lane={2} pattern={3} route={4} rank={5:F4} conversionAttempts={6}",
                        candidate.CandidateId, candidate.CanonicalCandidateKey, candidate.Lane, candidate.Signal.PatternName,
                        candidate.Route, candidate.Rank, candidate.ConversionAttempts);

                ExecuteFibonacciGridPlan(candidate);
                if (candidate.State == CandidateState.EXECUTED)
                {
                    winner = candidate;
                    if (candidate != armed[0]) _schedulerFallbackExecutions++;
                    break;
                }
                ...
                if (!EnableSchedulerFallbackCascade)
                    break;

                _schedulerFallbackAttempts++;
            }
            ...
            foreach (var other in armed.Where(x => x != winner))
            {
        ...
        private double PatternNativeExecutionScore(Bars bars, int i, PatternSignal s)
        {
            if (bars == null || s == null || i < 2 || i >= bars.Count) return 0;
            double o = bars.OpenPrices[i], c = bars.ClosePrices[i], h = bars.HighPrices[i], l = bars.LowPrices[i];
            double pc = bars.ClosePrices[i - 1], ph = bars.HighPrices[i - 1], pl = bars.LowPrices[i - 1];
            double body = Math.Max(Math.Abs(c - o), _symbol.PipSize);
            double atr = Atr(bars, 14, i);
            ...
            bool displacement = atr > 0 && body >= atr * .25;
            string n = s.PatternName ?? "";
            bool extensionFamily = n == "Alt Bat" || n == "Butterfly" || n == "Crab" || n == "Deep Crab" || n == "Shark";
    ...
    public enum CandidateState { DETECTED, VALIDATED, ROUTE_WATCH, ROUTED, WAIT_PRZ, PRZ_TOUCHED, EVIDENCE_BUILDING, ADMITTED, EXECUTABLE, CONFIRMING, ARMED, EXECUTED, EXPIRED, REJECTED, INVALIDATED }
    ...
    public sealed class CandidateRecord
    {
        ...
        public bool ConversionGridFeasible;
        public bool ConversionCapitalFeasible;
        public string ConversionLastFailure;
        public DateTime? RouteWatchStartedUtc;
        public DateTime? LastRouteRefreshUtc;
