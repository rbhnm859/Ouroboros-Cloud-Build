...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV60 : Robot
    {
        private const string Version = "HarmonyBot V60 — Harmonic Alpha Conversion, Fibonacci Grid & Convex Profit-Capture Engine";
        private const string BotPrefix = "HB60";
        ...
        [Parameter("V60 Family Books", DefaultValue = true)]
        public bool EnableV60FamilyBooks { get; set; }

        [Parameter("V60 Canonical Manifold", DefaultValue = true)]
        public bool EnableV60CanonicalManifold { get; set; }

        [Parameter("V60 Pure Projected PRZ", DefaultValue = true)]
        public bool EnableV60PureProjectedPrz { get; set; }

        [Parameter("V60 Temporal Event DAG", DefaultValue = true)]
        public bool EnableV60TemporalEventDag { get; set; }

        [Parameter("V60 Excursion Capture Research", DefaultValue = true)]
        public bool EnableV60ExcursionCaptureResearch { get; set; }

        [Parameter("V60 Excursion Capture Live", DefaultValue = false)]
        public bool EnableV60ExcursionCaptureLive { get; set; }

        [Parameter("V60 Matched Control Attribution", DefaultValue = true)]
        public bool EnableV60MatchedControlAttribution { get; set; }

        [Parameter("V60 Max Projection Error ATR", DefaultValue = 0.55, MinValue = 0.20, MaxValue = 1.50)]
        public double V60MaxProjectionErrorAtr { get; set; }

        [Parameter("V60 Manifold Sigma", DefaultValue = 1.00, MinValue = 0.50, MaxValue = 2.50)]
        public double V60ManifoldSigma { get; set; }

        [Parameter("V60 Minimal Coordinate Manifold", DefaultValue = true)]
        public bool EnableV60MinimalCoordinateManifold { get; set; }

        [Parameter("V60 Opportunity Density Ledger", DefaultValue = true)]
        public bool EnableV60OpportunityDensityLedger { get; set; }

        [Parameter("V60 Hierarchical Partial Pooling", DefaultValue = true)]
        public bool EnableV60HierarchicalPartialPooling { get; set; }

        [Parameter("V60 Family Effective Sample Support", DefaultValue = 24, MinValue = 24, MaxValue = 240)]
        public int V60FamilyEffectiveSampleSupport { get; set; }

        [Parameter("V60 AB=CD Algebra Contract", DefaultValue = true)]
        public bool EnableV60AbcdAlgebraContract { get; set; }

        [Parameter("V60 AB=CD Terminality Research", DefaultValue = true)]
        public bool EnableV60AbcdTerminalityResearch { get; set; }

        [Parameter("V60 Universal Research Continuation", DefaultValue = true)]
        public bool EnableV60UniversalResearchContinuation { get; set; }

        [Parameter("V60 Virtual Parallel Execution", DefaultValue = true)]
        public bool EnableV60VirtualParallelExecution { get; set; }

        [Parameter("V60 Calibrated Capital Admission", DefaultValue = false)]
        public bool EnableV60CalibratedCapitalAdmission { get; set; }

        [Parameter("V60 Family Representative Arbitration", DefaultValue = true)]
        public bool EnableV60FamilyRepresentativeArbitration { get; set; }

        [Parameter("V60 Standalone AB=CD Capital", DefaultValue = false)]
        public bool EnableV60AbcdStandaloneCapital { get; set; }

        [Parameter("V60 Grid Execution", DefaultValue = true)]
        public bool EnableV60GridExecution { get; set; }

        [Parameter("V60 Variant", DefaultValue = "CORE_L0_BASELINE")]
        public string V60Variant { get; set; }

        [Parameter("V60 Cluster-Aware Evidence", DefaultValue = true)]
        public bool EnableV60ClusterAwareEvidence { get; set; }

        [Parameter("V60 Continuous Alpha Surface", DefaultValue = true)]
        public bool EnableV60ContinuousAlphaSurface { get; set; }

        [Parameter("V60 Convex Capture Research", DefaultValue = true)]
        public bool EnableV60ConvexCaptureResearch { get; set; }

        [Parameter("V60 Runner Research", DefaultValue = true)]
        public bool EnableV60RunnerResearch { get; set; }

        [Parameter("V60 Runner Fraction", DefaultValue = 0.25, MinValue = 0.10, MaxValue = 0.40)]
        public double V60RunnerFraction { get; set; }

        [Parameter("V60 Adaptive Fibonacci Grid", DefaultValue = false)]
        public bool EnableV60AdaptiveFibonacciGrid { get; set; }

        [Parameter("V60 Opportunity-Cost Scheduler", DefaultValue = true)]
        public bool EnableV60OpportunityCostScheduler { get; set; }

        [Parameter("V60 Min Calibrated Lower R", DefaultValue = 0.0, MinValue = -1.0, MaxValue = 1.0)]
        public double V60MinCalibratedLowerBoundR { get; set; }

        [Parameter("V60 Calibration Manifest", DefaultValue = "")]
        public string V60CalibrationManifest { get; set; }
        ...
        private readonly Dictionary<string, HashSet<string>> _familyBooks = new Dictionary<string, HashSet<string>>(StringComparer.OrdinalIgnoreCase);
        private readonly Dictionary<string, FamilyShadowTrade> _familyShadowTrades = new Dictionary<string, FamilyShadowTrade>();
        private readonly Dictionary<string, CalibrationCell> _calibrationCells = new Dictionary<string, CalibrationCell>(StringComparer.OrdinalIgnoreCase);
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V60-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V60-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V60-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V60-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            InitializeV60FamilyBooks();
            ParseV60CalibrationManifest();
            Print("[V60-FAMILY-PORTFOLIO] familyBooks={0} virtualParallel={1} calibratedAdmission={2} representatives={3} abcdStandalone={4} calibrationCells={5}",
                EnableV60FamilyBooks, EnableV60VirtualParallelExecution, EnableV60CalibratedCapitalAdmission,
                EnableV60FamilyRepresentativeArbitration, EnableV60AbcdStandaloneCapital, _calibrationCells.Count);
            ...
            Print("[V60-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m1Bars), _profiles.Count);
            Print("[V60-MATH-ARCH] minimalCoordinates={0} abcdAlgebra={1} abcdTerminalityResearch={2} partialPooling={3} familyESS={4} opportunityDensity={5} rule=MAXIMIZE_RESEARCH_SUPPLY_WITHOUT_RELAXING_CAPITAL_RISK",
                EnableV60MinimalCoordinateManifold, EnableV60AbcdAlgebraContract, EnableV60AbcdTerminalityResearch,
                EnableV60HierarchicalPartialPooling, V60FamilyEffectiveSampleSupport, EnableV60OpportunityDensityLedger);
            Print("[V60-ALPHA-CONVERSION] variant={0} clusterAware={1} continuousSurface={2} convexCaptureResearch={3} runnerResearch={4} runnerFraction={5:F2} gridEnabled={6} adaptiveGrid={7} scheduler={8}",
                V60Variant,EnableV60ClusterAwareEvidence,EnableV60ContinuousAlphaSurface,EnableV60ConvexCaptureResearch,
                EnableV60RunnerResearch,V60RunnerFraction,V60GridEnabledForVariant(),V60AdaptiveGridForVariant(),EnableV60OpportunityCostScheduler);
            Print("[V60-TIMEFRAME-AUDIT] primaryPattern=M15 execution=M1 macro=H4 intermediate=H1 allCompletedBars=true");
            Print("[V60-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
            Print("[V60-ALPHA-CONFIG] qualityObservation={0} legacyRegime={1} legacyEnhancedM1={2} capitalFeasibility={3} transitionVeto={4} exhaustionVeto={5} routeM1Veto={6}",
            ...
            Print("[V60-FREQUENCY-CONFIG] deferredRetention={0} agingPriority={1} ageBoost={2:F3} structuredRecall={3} recallGeometry={4:F3} recallPrz={5:F3} recallConfidence={6:F3}",
            ...
            Print("[V60-RESTORED-KERNEL] canonicalSetup={0} canonicalStandard={1} independentPivotGraph={2} transitionProof={3} m1Rescue={4} rescueBars={5} diversityScheduler={6} executionClock=M1 projectedD=false",
            ...
            Print("[V60-CONVERSION-ARCH] scaleRouteAdmission={0} temporalRescue={1} armedGrace={2} graceMinutes={3} preExecutionRevalidation={4}",
            ...
            Print("[V60-FAMILY-NATIVE] conversion={0} observation={1} principle=PATTERN_IDENTITY_NEQ_EXECUTION_IDENTITY", EnableFamilyNativeConversion, EnableFamilyNativeObservation);
            Print("[V60-COMPLETION-CONTRACT] canonicalContracts={0} familyCompletion={1} windowBars={2} provenLane=ABCD_SHARK_CYPHER_FROZEN",
                EnableCanonicalFamilyContracts, EnableFamilyCompletionContract, FamilyConfirmationWindowBars);
            Print("[V60-STRUCTURAL-GRID-CONTRACT] gridSpanV2={0} structuralInvalidationV2={1} rule=PRZ_LEGALITY_PLUS_RISK_RR_NOT_LEGACY_XA_MINSPAN",
                EnableGridSpanSemanticV2, EnableStructuralInvalidationV2);
            Print("[V60-MATH-GEOMETRY] jointGeometry={0} executionCorridor={1} entryAnchorForensics={2} rule=FAMILY_NATIVE_GEOMETRY_WITHOUT_RISK_RELAXATION",
                EnableFamilyNativeJointGeometry, EnableFamilyNativeExecutionCorridor, EnableEntryAnchorForensics);
            Print("[V60-DETECTOR-ARCH] identityReconstruction={0} boundedPivotGraph={1} maxMicroSkips={2} familyQuota={3} projectedPrz={4} truthLedger={5} rule=DETECT_MANY_EXECUTE_ONE",
            ...
            Print("[V60-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V46_SCALE_CONVERSION_FROZEN",
        ...
        {
            CloseOpenV60ShadowTradesAtStop();
            foreach (var book in _familyBooks.OrderBy(x => x.Key))
            {
                int started=_familyShadowTrades.Values.Count(x => string.Equals(x.Pattern, book.Key, StringComparison.OrdinalIgnoreCase));
                int closed=_familyShadowTrades.Values.Count(x => string.Equals(x.Pattern, book.Key, StringComparison.OrdinalIgnoreCase) && x.Closed);
                Print("[V60-FAMILY-BOOK] pattern={0} candidates={1} shadowStarted={2} shadowClosed={3}", book.Key, book.Value.Count, started, closed);
            }
            ...
            {
                var x = kv.Value;
                Print("[V60-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} nativeTemporalPass={6} armed={7} slotBlocked={8} parked={9} revalidated={10} revalidationRejected={11} basketPlanned={12} leg0={13} leg1={14} leg2={15} leg3={16} basketClosed={17} executed={18} expired={19} rejected={20} invalidated={21}",
            ...
            if (EnableEntryAnchorForensics)
            {
                ...
                {
                    Print("[V60-ENTRY-ANCHOR-FORENSICS] cid={0} setup={1} pattern={2} route={3} scale={4} completion={5} confirm={6} retest={7} completionMfeR={8:F3} completionMaeR={9:F3} confirmMfeR={10:F3} confirmMaeR={11:F3} retestMfeR={12:F3} retestMaeR={13:F3}",
            ...
            if (EnableDetectorTruthLedger)
            {
                foreach (var kv in _detectorTruth.OrderBy(x => x.Key))
                {
                    ...
                    Print("[V60-DETECTOR-TRUTH] pattern={0} stage={1} count={2}", pattern, stage, kv.Value);
                }
            }
            Print("[V60-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V60-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
                _alphaQualityRejected, _regimeRejected, _confirmationRejected, _capitalInfeasibleCandidates, _alphaPassed);
            Print("[V60-FREQUENCY-SUMMARY] schedulerDeferred={0} schedulerRecoveredExecutions={1} structuredRecallAdmitted={2} activeDeferred={3}",
                _schedulerDeferred, _schedulerRecoveredExecutions, _structuredRecallAdmitted, _deferredCandidates.Count);
            Print("[V60-INDEPENDENT-SETUP-SUMMARY] duplicateSuppressed={0} uniqueExecuted={1} rescueAdmissions={2} transitionProofRejected={3} independentScaleCandidates={4}",
            ...
            Print("[V60-CONVERSION-SUMMARY] scaleRouteRejected={0} temporalRescueAdmissions={1} armedGraceExtended={2} preExecutionRevalidationRejected={3}",
            ...
            Print("[V60-THROUGHPUT-SUMMARY] slotBlocked={0} parked={1} revalidated={2} revalidationRejected={3} recoveredExecutions={4} nativeTemporalPass={5} hardLifetimeExpired={6} decayRejected={7} avgSlotWaitMin={8:F2} medianSlotWaitMin={9:F2} p90SlotWaitMin={10:F2} avgBasketOccupancyMin={11:F2} missedPositive={12} avoidedNegative={13}",
            ...
            foreach (var p in _profiles.Select(x => x.Name).Distinct().OrderBy(x => x))
            {
                ...
                Print("[V60-FAMILY-CONTRACT-SUMMARY] pattern={0} pass={1} windowReject={2}", p, pass, reject);
            }
            foreach (var kv in _gridPlanRejectReasons.OrderBy(k => k.Key))
                Print("[V60-GRID-REJECT-SUMMARY] reason={0} count={1}", kv.Key, kv.Value);
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V60-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            {
                if (EnableV60AbcdTerminalityResearch && string.Equals(signal.PatternName, "AB=CD", StringComparison.OrdinalIgnoreCase))
                ...
                var record = new CandidateRecord
                {
                    ...
                    CapitalThesisId = identityKey
                };
                _candidates[id] = record;
                AddToV60FamilyBook(record);
                ...
                {
                    if (EnableV60UniversalResearchContinuation && EnableV60VirtualParallelExecution)
                    {
                        record.ResearchOnlyHypothesis = true;
                        Event(record, "V60_RESEARCH_CONTINUE_PATTERN_QUALITY");
                    }
                    else
                    {
                ...
                record.CapitalThesisId = record.SetupKey + "|" + signal.PatternName + "|" + record.Route + "|" + signal.PivotScale.ToString(CultureInfo.InvariantCulture);
                record.PreEntryAlphaScore = V60PreEntryAlphaScore(record);
                ...
                if (EnableV60AbcdTerminalityResearch && string.Equals(signal.PatternName, "AB=CD", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        record.ResearchOnlyHypothesis = true;
                        Event(record, "V60_ABCD_NESTED_PARENT_CONFLUENCE_ONLY");
                    }

                    Print("[V60-ABCD-CLASS] cid={0} subtype={1} terminality={2} scale={3} route={4} setup={5}",
                ...
                if (record.Route == HarmonicRoute.NO_TRADE)
                {
                    ...
                    {
                        record.ResearchOnlyHypothesis = true;
                        record.Route = V60ResearchRoute(signal, record.Conflict, regime);
                        if (record.Route == HarmonicRoute.NO_TRADE)
                            record.Route = HarmonicRoute.TRANSITION_REVERSAL;
                        Event(record, "V60_RESEARCH_CONTINUE_ROUTER_NO_TRADE");
                    }
                    else
                    {
                ...
                Print("[V60-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    StartV60FamilyShadowTrade(c, i, utc);
                    ...
                    if (EnableV60CalibratedCapitalAdmission && !V60CalibratedCapitalEligible(c))
                    {
                        Event(c, "CALIBRATION_RESEARCH_ONLY");
                        Reject(c, "CALIBRATION_RESEARCH_ONLY");
                        continue;
                    }
        ...
        private void TryScheduleAndExecute()
        {
            ...
            if (EnableV60FamilyRepresentativeArbitration)
            {
                armedQuery = armedQuery
                    .GroupBy(c => c.Signal == null ? "UNKNOWN" : c.Signal.PatternName)
                    .Select(g => g.OrderByDescending(c => V60FamilyCapitalScore(c, now)).ThenByDescending(c => c.Rank).First());
            }

            var armed = armedQuery
                .OrderByDescending(c => EnableV60FamilyRepresentativeArbitration ? V60FamilyCapitalScore(c, now) :
        ...
        private void MarkOpportunityTerminal(CandidateRecord c, string reason)
        {
            ...
            Print("[V60-OPPORTUNITY-LOSS] cid={0} setup={1} pattern={2} route={3} scale={4} reason={5} shadowMfeR={6:F3} shadowMaeR={7:F3} missedPositive={8} avoidedNegative={9}",
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            bool gridEnabled = V60GridEnabledForVariant();
            int routeMax = !gridEnabled ? 1 :
            ...
            double[] activeRiskWeights = V60ActiveGridRiskWeights(c, maxLegs);
            ...
            for (int leg = 0; leg < maxLegs; leg++)
            {
                ...
                double riskWeight = leg < activeRiskWeights.Length ? activeRiskWeights[leg] : 0;
            ...
            Print("[V60-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V60-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V60-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V60-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V60-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V60-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V60-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V60-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V60-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V60-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V60-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V60-BASKET-CLOSED] basket={0} cid={1} setup={2} pattern={3} subtype={4} route={5} dir={6} plannedLegs={7} filledLegs={8} anchor={9} avgEntry={10} entryImprovePips={11:F3} stop={12} target={13} initialRisk={14:F2} worstRisk={15:F2} mfeR={16:F3} maeR={17:F3} realizedR={18:F3} net={19:F2} reason={20}",
            ...
            Print("[V60-SLOT-OCCUPANCY] basket={0} cid={1} pattern={2} route={3} occupancyMinutes={4:F2} realizedR={5:F3} net={6:F2}",
        ...
        {
            Print("[V60-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
        ...
        private void RefreshHigherTimeframeContext()
        {
            ...
            if (h4i >= 40)
            {
                ...
                if (t > _lastH4ContextClosed)
                {
                    ...
                    Print("[V60-CONTEXT-CACHE] tf=H4 close={0:O} state={1}", t, _cachedH4State);
                }
            }
            ...
            if (h1i >= 40)
            {
                ...
                if (t > _lastH1ContextClosed)
                {
                    ...
                    Print("[V60-CONTEXT-CACHE] tf=H1 close={0:O} state={1}", t, _cachedH1State);
                }
            }
        }
            ...
            double atr, DateTime completion, string timeframe, int pivotScale, out PatternSignal signal)
        {
            ...
            if (p.Mode == PatternMode.ABCD)
                ratioOk = V60AbcdDiscreteIdentity(abc, bcd, abcd);
            else if (p.Mode == PatternMode.CYPHER)
                ratioOk = InRange(xab, .382, .618) && InRange(xac, 1.272, 1.414) && InRange(cd / Math.Max(xc, 1e-9), .760, .810);
            else if (p.Mode == PatternMode.SHARK)
                ratioOk = InRange(abc, 1.13, 1.618) && InRange(bcd, 1.13, 2.24) && InRange(xad, .886, 1.13);
            ...
            double manifoldDistance = 99;
            double geometry = EnableV60CanonicalManifold
                ? V60CanonicalManifoldScore(p, xab, abc, bcd, xad, abcd, xac, out manifoldDistance)
                : (EnableFamilyNativeJointGeometry ? FamilyNativeJointGeometryScore(p, xab, abc, bcd, xad, abcd, xac) : legacyGeometry);
            ...
            double expectedD = EnableV60PureProjectedPrz
                ? V60PureProjectedPrzCenter(p, x, a, b, c, bullish, out projectionSpread)
                : (EnableFamilyIdentityReconstruction && EnableFamilyNativeProjectedPrz ? FamilyProjectedPrzCenter(p, x, a, b, c, d, bullish, out projectionSpread) : d.Price);
            if (!EnableV60PureProjectedPrz && !(EnableFamilyIdentityReconstruction && EnableFamilyNativeProjectedPrz)) projectionSpread = 0;
            double przHalf = Math.Max(atr * p.PrzWidthAtr, projectionSpread);
            double projectionErrorAtr = atr > 0 ? Math.Abs(d.Price - expectedD) / atr : 99;
            double projectionConvergence = VClamp(Math.Exp(-0.5 * Math.Pow(projectionSpread / Math.Max(atr * .25, _symbol.PipSize), 2.0)));
            double projectionPurity = VClamp(Math.Exp(-0.5 * Math.Pow(projectionErrorAtr / Math.Max(.15, V60MaxProjectionErrorAtr), 2.0)));
            ...
            double legacyPrzConfluence = VClamp(1.0 - Math.Abs(xad - Mid(p.XadMin, p.XadMax)) / Math.Max(.15, p.XadMax - p.XadMin + .05));
            double przConfluence = EnableV60PureProjectedPrz
                ? VClamp(.55 * projectionConvergence + .45 * projectionPurity)
                : (p.Mode == PatternMode.STANDARD ? legacyPrzConfluence : VClamp((geometry + timeSym) / 2.0));
            ...
            signal = new PatternSignal
            {
                ...
                ExpectedD = expectedD, ProjectionSpreadAtr = projectionSpread / Math.Max(atr, _symbol.PipSize),
                ProjectionErrorAtr = projectionErrorAtr, ManifoldDistance = manifoldDistance,
        ...
        private double[] V60AbcdCenters(string pattern)
        {
            if (pattern == "Alt Bat") return new[] { 1.618 };
            if (pattern == "Crab") return new[] { 1.272, 1.618 };
            if (pattern == "Deep Crab") return new[] { 1.0, 1.272 };
            if (pattern == "Butterfly") return new[] { 1.0, 1.272, 1.618 };
            if (pattern == "Gartley" || pattern == "Bat" || pattern == "Deep Gartley" || pattern == "Rat") return new[] { 1.0, 1.272 };
            return new[] { 1.0, 1.272, 1.618 };
        }

        private double V60NearestCanonical(double value, double[] centers)
        {
            return centers.OrderBy(v => Math.Abs(v - value)).First();
        }

        private bool V60AbcdDiscreteIdentity(double abc, double bcd, double abcd)
        {
            if (!InRange(abc, .382, .886) || bcd <= 0 || abcd <= 0) return false;

            // V60 algebra contract:
            // abc = BC/AB, bcd = CD/BC, abcd = CD/AB, therefore abcd = abc*bcd.
            // Each canonical completion k in {1.000,1.272,1.618} owns its own BC->CD projection:
            // bcd_expected = k/abc.  V58 incorrectly centered every subtype on 1/abc.
            double k = V60NearestCanonical(abcd, V60AbcdCenters("AB=CD"));
            double completionTolerance = Math.Max(.06, k * .06);
            if (Math.Abs(abcd - k) > completionTolerance) return false;

            double expectedBcd = k / Math.Max(.30, abc);
            double reciprocalTolerance = Math.Max(.12, expectedBcd * .12);
            bool reciprocal = Math.Abs(bcd - expectedBcd) <= reciprocalTolerance;

            // Independent numerical sanity check. It should be nearly exact because the three
            // ratios are derived from the same legs; keep tolerance only for floating point noise.
            bool algebra = Math.Abs(abcd - abc * bcd) <= Math.Max(1e-8, Math.Abs(abcd) * 1e-6);
            return reciprocal && (!EnableV60AbcdAlgebraContract || algebra);
        }

        private double V60PureProjectedPrzCenter(PatternProfile p, PivotPoint x, PivotPoint a, PivotPoint b, PivotPoint c,
            bool bullish, out double halfSpread)
        {
            ...
            double xc = Math.Max(Math.Abs(c.Price - x.Price), _symbol.PipSize);
            double sign = bullish ? -1.0 : 1.0;
            var projections = new List<double>();

            if (p.Mode == PatternMode.STANDARD)
            {
                double dXa = a.Price + sign * Mid(p.XadMin, p.XadMax) * xa;
                double dBc = c.Price + sign * Mid(p.BcdMin, p.BcdMax) * bc;
                double anchor = .5 * (dXa + dBc);
                double bestAbcd = V60AbcdCenters(p.Name).Select(k => c.Price + sign * k * ab).OrderBy(v => Math.Abs(v - anchor)).First();
                projections.Add(dXa); projections.Add(dBc); projections.Add(bestAbcd);
            }
            else if (p.Mode == PatternMode.ABCD)
            {
                // Pure X/A/B/C projection.  Never use observed D here.
                // Standalone AB=CD remains research-only by default, so V60 emits the canonical
                // subtype projection set instead of pretending 1/ABC is a CD/AB completion.
                // The center is the median canonical completion; halfSpread below preserves the
                // full 1.000/1.272/1.618 hypothesis envelope for shadow attribution.
                foreach (double k in V60AbcdCenters("AB=CD"))
                    projections.Add(c.Price + sign * k * ab);
            }
            else if (p.Mode == PatternMode.CYPHER)
                projections.Add(c.Price + sign * .786 * xc);
            else if (p.Mode == PatternMode.SHARK)
            {
                projections.Add(a.Price + sign * 1.00 * xa);
                projections.Add(c.Price + sign * 1.618 * bc);
            }
            else
                projections.Add(c.Price + sign * Mid(p.BcdMin, p.BcdMax) * bc);

            double center = projections.Average();
            halfSpread = Math.Max(_symbol.PipSize, Math.Sqrt(projections.Select(v => (v - center) * (v - center)).Average()));
            return center;
        }

        private double NearestAbcdProjection(string pattern, double observed)
        {
            return V60NearestCanonical(observed, V60AbcdCenters(pattern));
        }
        ...
        private double FamilyProjectedPrzCenter(PatternProfile p, PivotPoint x, PivotPoint a, PivotPoint b, PivotPoint c, PivotPoint d,
            bool bullish, out double halfSpread)
        {
            return V60PureProjectedPrzCenter(p, x, a, b, c, bullish, out halfSpread);
        }
        ...
        private double V60CanonicalManifoldScore(PatternProfile p, double xab, double abc, double bcd, double xad, double abcd, double xac, out double distance)
        {
            // V60 minimal-coordinate rule: do not count algebraically dependent ratios as
            // independent dimensions.  Standard XABCD uses the independent leg ratios
            // (AB/XA, BC/AB, CD/BC), while XAD and CD/AB enter as one aggregate family
            // constraint residual.  AB=CD uses (BC/AB, canonical CD/AB subtype); CD/BC is
            // checked by the algebra contract rather than double-counted as another dimension.
            var z = new List<double>();
            ...
            if (p.Mode == PatternMode.STANDARD)
            {
                ...
                double xadResidual = RangeCoordinate(xad, p.XadMin, p.XadMax);
                double abcdResidual = CanonicalAbcdCoordinate(p.Name, abcd);
                double familyConstraint = Math.Sqrt(.5 * (xadResidual * xadResidual + abcdResidual * abcdResidual));
                z.Add(familyConstraint);
            }
            else if (p.Mode == PatternMode.ABCD)
            {
                double k = V60NearestCanonical(abcd, V60AbcdCenters("AB=CD"));
                double abcResidual = RangeCoordinate(abc, p.AbcMin, p.AbcMax);
                double completionResidual = (abcd - k) / Math.Max(.05, k * .06);
                double expectedBcd = k / Math.Max(.30, abc);
                double projectionResidual = (bcd - expectedBcd) / Math.Max(.10, expectedBcd * .12);

                z.Add(abcResidual);
                z.Add(completionResidual);
                // Projection consistency is a manifold constraint, not a third free coordinate.
                z.Add(Math.Abs(projectionResidual));
            }
            else if (p.Mode == PatternMode.CYPHER)
            {
                z.Add(RangeCoordinate(xab, .382, .618));
                z.Add(RangeCoordinate(xac, 1.272, 1.414));
            }
            else if (p.Mode == PatternMode.SHARK)
            {
                z.Add(RangeCoordinate(abc, 1.13, 1.618));
                z.Add(RangeCoordinate(bcd, 1.13, 2.24));
                z.Add(RangeCoordinate(xad, .886, 1.13));
            }
            else
            {
                z.Add(RangeCoordinate(xab, 1.13, 1.618));
            ...
            double sigma = Math.Max(.50, V60ManifoldSigma);
            return VClamp(Math.Exp(-.5 * q / (sigma * sigma)));
        }
        ...
        private void BuildPatternProfiles()
        {
            ...
            if (EnableCanonicalFamilyContracts)
            {
                ...
                AddStd("Butterfly", .770, .800, .382, .886, 1.618, 2.24, 1.24, 1.30, .14, .20, .618, 1.00, 7, .58, .58);
                AddStd("Crab", .382, .618, .382, .886, 2.618, 3.618, 1.58, 1.66, .15, .22, .618, 1.00, 6, .60, .60);
                AddStd("Deep Crab", .875, .900, .382, .886, 2.24, 3.618, 1.58, 1.66, .15, .22, .618, 1.00, 6, .60, .60);
            ...
            _profiles.Add(new PatternProfile { Name = "Cypher", Mode = PatternMode.CYPHER, AbcMin = 1.272, AbcMax = 1.414, BcdMin = .760, BcdMax = .810, XadMin = .760, XadMax = .810, PrzWidthAtr = .12, StopBufferAtr = .18, Target1Cd = .50, Target2Cd = .886, MaxAgeM15Bars = 6, MinGeometry = .60, MinPrz = .60 });
            ...
            _profiles.Add(new PatternProfile { Name = "AB=CD", Mode = PatternMode.ABCD, AbcMin = .382, AbcMax = .886, BcdMin = 1.13, BcdMax = 2.618, AbcDMin = .94, AbcDMax = 1.69, XadMin = .5, XadMax = 1.5, PrzWidthAtr = .12, StopBufferAtr = .18, Target1Cd = .618, Target2Cd = 1.00, MaxAgeM15Bars = 8, MinGeometry = .55, MinPrz = .55 });
        ...
        private HarmonicRoute RouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (baseRoute == HarmonicRoute.NO_TRADE && EnableStructuredRecallExpansion)
            {
                ...
                if (baseRoute != HarmonicRoute.NO_TRADE)
                {
                    ...
                    Print("[V60-RECALL-ADMIT] pattern={0} direction={1} route={2} conflict={3} geometry={4:F3} prz={5:F3} confidence={6:F3} efficiency={7:F3} atrRatio={8:F3} extensionAtr={9:F3}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionProofGate)
            {
                ...
                if (!actualStructuralTransition)
                {
                    ...
                    Print("[V60-TRANSITION-PROOF-REJECT] pattern={0} conflict={1} transition={2} adxSlope={3:F2}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionStateVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V60-ROUTE-VETO] type=TRANSITION_STATE conflict={0} transition={1} adxSlope={2:F2}",
            ...
            if (baseRoute == HarmonicRoute.EXHAUSTION_REVERSAL && EnableExhaustionEvidenceVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V60-ROUTE-VETO] type=EXHAUSTION_EVIDENCE extensionAtr={0:F3} adxSlope={1:F2}",
        ...
        private double CandidateRank(CandidateRecord c)
        {
            ...
            c.PreEntryAlphaScore = V60PreEntryAlphaScore(c);
            double baseRank = .20 * c.Signal.GeometryQuality + .12 * c.Signal.PrzConfluence + .12 * mtf + .12 * regime +
                              .12 * c.ConfirmationScore + .12 * VClamp(c.NetRR / 3.0) + .20 * c.PreEntryAlphaScore;
        ...
        private void InitializeV60FamilyBooks()
        {
            if (!EnableV60FamilyBooks) return;
            foreach (var p in _profiles)
                if (!_familyBooks.ContainsKey(p.Name)) _familyBooks[p.Name]=new HashSet<string>();
        }

        private void AddToV60FamilyBook(CandidateRecord c)
        {
            if (!EnableV60FamilyBooks || c==null || c.Signal==null) return;
            HashSet<string> book;
            if (!_familyBooks.TryGetValue(c.Signal.PatternName, out book))
            {
                book=new HashSet<string>();
                _familyBooks[c.Signal.PatternName]=book;
            }
            book.Add(c.CandidateId);
        }

        private string V60PatternToken(string pattern)
        {
            return (pattern??"UNKNOWN").Replace(" ","_");
        }

        private string V60RegimeBucket(CandidateRecord c)
        {
            var r=c==null?null:c.Regime;
            if(r==null)return "UNKNOWN";
            if(r.Transition)return "TRANSITION";
            if(r.ExtensionAtr>=1.25)return "EXHAUSTION_EXTENSION";
            if(r.AtrRatio>=1.25&&r.TrendStrength>=.50)return "TREND_EXPANSION";
            if(r.TrendStrength>=.50&&r.Efficiency>=.22)return "TREND_PULLBACK";
            if(r.AtrRatio<=.75)return "VOL_COMPRESSION";
            if(r.Efficiency<.18)return "MEAN_REVERTING_RANGE";
            return "BALANCED";
        }

        private HarmonicRoute V60ResearchRoute(PatternSignal s,MtfConflict conflict,RegimeSnapshot r)
        {
            if(s==null||r==null)return HarmonicRoute.NO_TRADE;
            string p=s.PatternName??"";
            bool ext=p=="Alt Bat"||p=="Butterfly"||p=="Crab"||p=="Deep Crab";
        ...
        private string V60CalibrationKey(CandidateRecord c)
        {
            if(c==null||c.Signal==null)return "UNKNOWN";
            return V60PatternToken(c.Signal.PatternName)+"|"+c.Route+"|"+V60RegimeBucket(c);
        }

        private void ParseV60CalibrationManifest()
        {
            _calibrationCells.Clear();
            if(string.IsNullOrWhiteSpace(V60CalibrationManifest))return;
            foreach(var raw in V60CalibrationManifest.Split(new[]{';'},StringSplitOptions.RemoveEmptyEntries))
            {
                var kv=raw.Split(new[]{'='},2);
                if(kv.Length!=2)continue;
                var vals=kv[1].Split(',');
                if(vals.Length<3)continue;
                double lower,hold; int n;
                if(!double.TryParse(vals[0],NumberStyles.Float,CultureInfo.InvariantCulture,out lower))continue;
                if(!double.TryParse(vals[1],NumberStyles.Float,CultureInfo.InvariantCulture,out hold))continue;
                if(!int.TryParse(vals[2],NumberStyles.Integer,CultureInfo.InvariantCulture,out n))continue;
                _calibrationCells[kv[0]]=new CalibrationCell{LowerBoundR=lower,MeanHoldMinutes=Math.Max(1,hold),Samples=Math.Max(0,n)};
            }
        }

        private CalibrationCell V60CalibrationCell(CandidateRecord c)
        {
            CalibrationCell x;
            return _calibrationCells.TryGetValue(V60CalibrationKey(c),out x)?x:null;
        }

        private bool V60CalibratedCapitalEligible(CandidateRecord c)
        {
            if(c==null||c.Signal==null)return false;
            if(c.ResearchOnlyHypothesis)return false;
            if(string.Equals(c.Signal.PatternName,"AB=CD",StringComparison.OrdinalIgnoreCase)&&!EnableV60AbcdStandaloneCapital)
                return false;
            var cell=V60CalibrationCell(c);
            if(cell==null)return false;
            c.CalibratedLowerBoundR=cell.LowerBoundR;
            c.CalibratedMeanHoldMinutes=cell.MeanHoldMinutes;
            c.CalibratedSamples=cell.Samples;
            c.CalibratedRPerSlotHour=cell.LowerBoundR/Math.Max(.25,cell.MeanHoldMinutes/60.0);
            return cell.LowerBoundR>V60MinCalibratedLowerBoundR && cell.Samples>0 && c.CalibratedRPerSlotHour>0;
        }

        private double V60FamilyCapitalScore(CandidateRecord c, DateTime now)
        {
            var cell=V60CalibrationCell(c);
            if(cell!=null)
            {
                double slot=cell.LowerBoundR/Math.Max(.25,cell.MeanHoldMinutes/60.0);
                return slot + (EnableV60OpportunityCostScheduler ? .08 * V60PreEntryAlphaScore(c) : 0) +
                       .03 * (EnableOpportunityDecayRanking?OpportunityScore(c,now):c.Rank);
            }
            return EnableOpportunityDecayRanking?OpportunityScore(c,now):c.Rank;
        }

        private void StartV60FamilyShadowTrade(CandidateRecord c,int i,DateTime utc)
        {
            if(!EnableV60VirtualParallelExecution||c==null||c.Signal==null||_familyShadowTrades.ContainsKey(c.CandidateId))return;
            double entry=_m1Bars.ClosePrices[i], stop=c.Signal.StructuralInvalidation, target,rr;
            if(!SelectCanonicalBasketTarget(c.Signal,entry,stop,out target,out rr))
            {
                Print("[V60-SHADOW-REJECT] cid={0} pattern={1} route={2} regime={3} reason=NO_LEGAL_RR_TARGET",c.CandidateId,c.Signal.PatternName,c.Route,V60RegimeBucket(c));
                return;
            }
            double risk=Math.Abs(entry-stop);
            if(risk<=_symbol.PipSize)return;
            c.PreEntryAlphaScore=V60PreEntryAlphaScore(c);
            var x=new FamilyShadowTrade{
                CandidateId=c.CandidateId,SetupKey=c.SetupKey,Pattern=c.Signal.PatternName,Route=c.Route,Regime=V60RegimeBucket(c),
                Direction=c.Signal.Direction,Entry=entry,Stop=stop,Target=target,RiskDistance=risk,NetTargetR=rr,
                StartUtc=utc,ExpiryUtc=c.ExpiryUtc,MinPrice=entry,MaxPrice=entry,
                GeometryQuality=c.Signal.GeometryQuality,ProjectionErrorAtr=c.Signal.ProjectionErrorAtr,
                PrzConfluence=c.Signal.PrzConfluence,RegimeScore=c.RegimeScore,ConfirmationScore=c.ConfirmationScore,
                PreEntryAlphaScore=c.PreEntryAlphaScore,RunnerFraction=Math.Max(.10,Math.Min(.40,V60RunnerFraction))
            };
            _familyShadowTrades[c.CandidateId]=x;
            Print("[V60-SHADOW-START] cid={0} setup={1} pattern={2} route={3} regime={4} entry={5} stop={6} target={7} netTargetR={8:F3} geometry={9:F4} projectionErrorAtr={10:F4} prz={11:F4} regimeScore={12:F4} confirmation={13:F4} alphaScore={14:F4}",
                x.CandidateId,x.SetupKey,x.Pattern,x.Route,x.Regime,x.Entry,x.Stop,x.Target,x.NetTargetR,
                x.GeometryQuality,x.ProjectionErrorAtr,x.PrzConfluence,x.RegimeScore,x.ConfirmationScore,x.PreEntryAlphaScore);
        }

        private void UpdateV60FamilyShadowTrades(int i,DateTime utc)
        {
            if(!EnableV60VirtualParallelExecution||_familyShadowTrades.Count==0)return;
            double hi=_m1Bars.HighPrices[i],lo=_m1Bars.LowPrices[i],close=_m1Bars.ClosePrices[i];
            foreach(var x in _familyShadowTrades.Values.Where(z=>!z.Closed || !z.ConvexClosed).ToList())
            {
                ...
                if(EnableV60ExcursionCaptureResearch)
                {
                    if(!x.Protect75Closed&&prevMfe>=.75&&V60ShadowFloorHit(x,lo,hi,.10))
                    { x.Protect75Closed=true;x.Protect75R=.10-costR; }
                    if(!x.Be1Closed&&prevMfe>=1.0&&V60ShadowFloorHit(x,lo,hi,.05))
                    { x.Be1Closed=true;x.Be1R=.05-costR; }
                    double trailFloor=Math.Max(.25,prevMfe-.75);
                    if(!x.Trail125Closed&&prevMfe>=1.25&&V60ShadowFloorHit(x,lo,hi,trailFloor))
                    ...
                    if(!x.HybridClosed&&!double.IsNaN(hybridFloor)&&V60ShadowFloorHit(x,lo,hi,hybridFloor))
                ...
                if(EnableV60ConvexCaptureResearch && !x.ConvexClosed)
                {
                    double protectTrigger,protectFloor,trailTrigger,trailDistance;
                    ...
                    {
                        double runner=EnableV60RunnerResearch?x.RunnerFraction:0.0;
                ...
                if(targetHit && !x.Closed)
                {
                    CloseV60Shadow(x,utc,x.NetTargetR,"TP");
                    if(!x.RunnerArmed || x.ConvexClosed)continue;
                }
                if(utc>=x.ExpiryUtc)
                {
                    if(!x.ConvexClosed)
        ...
        private void CloseV60Shadow(FamilyShadowTrade x,DateTime utc,double realizedR,string reason)
        {
            ...
            if(!EnableV60ConvexCaptureResearch && !x.ConvexClosed){x.ConvexClosed=true;x.ConvexR=realizedR;}
            else if(EnableV60ConvexCaptureResearch && !x.ConvexCoreClosed && !x.RunnerArmed && !x.ConvexClosed)
            { x.ConvexClosed=true; x.ConvexR=realizedR; }
            double hold=Math.Max(0,(utc-x.StartUtc).TotalMinutes);
            double capture=x.MfeR>1e-9?realizedR/x.MfeR:0;
            Print("[V60-SHADOW-CLOSED] cid={0} setup={1} pattern={2} route={3} regime={4} mfeR={5:F4} maeR={6:F4} realizedR={7:F4} holdMin={8:F2} reason={9} geometry={10:F4} projectionErrorAtr={11:F4} prz={12:F4} regimeScore={13:F4} confirmation={14:F4} alphaScore={15:F4}",
                x.CandidateId,x.SetupKey,x.Pattern,x.Route,x.Regime,x.MfeR,x.MaeR,x.RealizedR,hold,reason,
                x.GeometryQuality,x.ProjectionErrorAtr,x.PrzConfluence,x.RegimeScore,x.ConfirmationScore,x.PreEntryAlphaScore);
            Print("[V60-CAPTURE-CLOSED] cid={0} pattern={1} route={2} regime={3} baselineR={4:F4} protect75R={5:F4} be1R={6:F4} trail125R={7:F4} timeDecayR={8:F4} hybridR={9:F4} convexR={10:F4} runnerArmed={11} mfeR={12:F4} captureRatio={13:F4}",
                x.CandidateId,x.Pattern,x.Route,x.Regime,x.RealizedR,x.Protect75R,x.Be1R,x.Trail125R,x.TimeDecayR,x.HybridR,x.ConvexR,x.RunnerArmed,x.MfeR,capture);
        }

        private void CloseOpenV60ShadowTradesAtStop()
        {
            if(_m1Bars==null||_m1Bars.Count<2)return;
            int i=LastClosedIndex(_m1Bars); if(i<0)return;
            DateTime utc=DateTime.SpecifyKind(_m1Bars.OpenTimes[i],DateTimeKind.Utc);
            double close=_m1Bars.ClosePrices[i];
            foreach(var x in _familyShadowTrades.Values.Where(z=>!z.Closed || !z.ConvexClosed).ToList())
            {
                double costR=ModeledCostPips()/Math.Max(1.0,PriceToPips(x.RiskDistance));
        ...
        private double V60PreEntryAlphaScore(CandidateRecord c)
        {
            if(c==null||c.Signal==null)return 0;
            double projection=VClamp(Math.Exp(-Math.Max(0,c.Signal.ProjectionErrorAtr)));
            double regime=VClamp((c.RegimeScore+1.0)*.5);
            double rr=VClamp(c.NetRR/3.0);
            return VClamp(.30*c.Signal.GeometryQuality+.18*c.Signal.PrzConfluence+.18*projection+
                          .14*VClamp(c.ConfirmationScore)+.10*regime+.10*rr);
        }

        private bool V60GridEnabledForVariant()
        {
            if(!EnableV60GridExecution)return false;
            string v=(V60Variant??"").ToUpperInvariant();
            return v.Contains("GRID");
        }

        private bool V60AdaptiveGridForVariant()
        {
            string v=(V60Variant??"").ToUpperInvariant();
            return EnableV60AdaptiveFibonacciGrid || v.Contains("ADAPTIVE_FIB_GRID");
        }

        private double[] V60ActiveGridRiskWeights(CandidateRecord c,int maxLegs)
        {
            if(maxLegs<=0)return new double[0];
            if(!V60AdaptiveGridForVariant())
            {
                double[] legacy={3.0/7.0,2.0/7.0,1.0/7.0,1.0/7.0};
                return legacy.Take(maxLegs).ToArray();
            }
            double[] w;
            if(c.Route==HarmonicRoute.EXHAUSTION_REVERSAL)w=new[]{.65,.35};
            else if(c.Route==HarmonicRoute.TRANSITION_REVERSAL)w=new[]{.50,.30,.20};
            else w=new[]{.40,.30,.20,.10};
            w=w.Take(maxLegs).ToArray();
            double sum=w.Sum();
            if(sum<=0)return new double[0];
            for(int i=0;i<w.Length;i++)w[i]/=sum;
            for(int i=1;i<w.Length;i++)if(w[i]>w[i-1]+1e-12)throw new InvalidOperationException("V60 adaptive Grid risk must be monotonic non-increasing");
            return w;
        }

        private void V60ConvexPolicy(HarmonicRoute route,out double protectTrigger,out double protectFloor,out double trailTrigger,out double trailDistance)
        {
            if(route==HarmonicRoute.EXHAUSTION_REVERSAL){protectTrigger=.75;protectFloor=.10;trailTrigger=1.25;trailDistance=.75;return;}
            if(route==HarmonicRoute.TRANSITION_REVERSAL){protectTrigger=1.0;protectFloor=.10;trailTrigger=1.50;trailDistance=.90;return;}
            protectTrigger=1.50;protectFloor=.25;trailTrigger=2.00;trailDistance=1.25;
        }
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V60-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        {
            _candidateSeq++;
            return "V60-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private void Ledger(CandidateRecord c, CandidateState state, string reason)
        {
            ...
            Print("[V60-EVENT] cid={0} setup={1} pattern={2} subtype={3} scale={4} tf={5} dir={6} state={7} route={8} conflict={9} waitMin={10:F2} reason={11}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V60-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V60-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
    ...
    public sealed class CandidateRecord
    {
        ...
        public double PreEntryAlphaScore, ExpectedSlotHours;
        public string UnderlyingGeometryId, CapitalThesisId;
    ...
    public sealed class CalibrationCell
    {
        public double LowerBoundR;
        public double MeanHoldMinutes;
        public int Samples;
    }

    public sealed class FamilyShadowTrade
    {
        public string CandidateId,SetupKey,Pattern,Regime,ExitReason;
        public HarmonicRoute Route;
        public TradeDirection Direction;
        public double Entry,Stop,Target,RiskDistance,NetTargetR,MinPrice,MaxPrice,MfeR,MaeR,RealizedR;
        public double Protect75R,Be1R,Trail125R,TimeDecayR,HybridR,ConvexR,ConvexCoreR;
        public double GeometryQuality,ProjectionErrorAtr,PrzConfluence,RegimeScore,ConfirmationScore,PreEntryAlphaScore;
        public double RunnerFraction;
        public DateTime StartUtc,ExpiryUtc,ExitUtc;
        public bool Closed,Protect75Closed,Be1Closed,Trail125Closed,TimeDecayClosed,HybridClosed,ConvexClosed,ConvexCoreClosed,RunnerArmed;
    }
