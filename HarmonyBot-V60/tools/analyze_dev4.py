...
def agg(f):
 ...
 shadows=[r for x in xs for r in x.get("v60_shadow_outcomes",[])]; caps=[r for x in xs for r in x.get("v60_capture_outcomes",[])]
 ...
 for x in xs:
  ...
  for p,row in x.get("v60_family_books",{}).items():
...
capture=delta("V60_MANIFOLD_CAPTURE_L0","V60_MANIFOLD_BASELINE_L0")
grid=delta("V60_MANIFOLD_CAPTURE_LEGACY_GRID","V60_MANIFOLD_CAPTURE_L0")
for f,z in A.items():
