...
rx=re.compile(r"\[V60-BASKET-CLOSED\].*?cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+subtype=(\S+)\s+route=(\S+)\s+dir=(\S+).*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)\s+reason=(\S+)")
b=[]
...
pat=(r"\[V60-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
evrx=re.compile(r"\[V60-EVENT\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+subtype=(\S+)\s+scale=(\d+)\s+tf=(\S+)\s+dir=(\S+)\s+state=(\S+)\s+route=(\S+)\s+conflict=(\S+)\s+waitMin=([-0-9.]+)\s+reason=(.*)$",re.M)
events=[]
...
oprx=re.compile(r"\[V60-OPPORTUNITY-LOSS\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+scale=(\d+)\s+reason=(\S+)\s+shadowMfeR=([-0-9.]+)\s+shadowMaeR=([-0-9.]+)\s+missedPositive=(True|False)\s+avoidedNegative=(True|False)")
opp=[]
...
sorx=re.compile(r"\[V60-SLOT-OCCUPANCY\].*?pattern=(.*?)\s+route=(\S+)\s+occupancyMinutes=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
"broker_profile_present":"[V60-BROKER-PROFILE]" in t,"data_snapshot_sha":a.data_sha.strip(),"mean_mfe_r":statistics.mean([x["mfe"] for x in b]) if b else 0,
...
anchor_rx=re.compile(r"\[V60-ENTRY-ANCHOR-FORENSICS\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+scale=(\d+)\s+completion=([-0-9.]+)\s+confirm=([-0-9.]+)\s+retest=([-0-9.]+)\s+completionMfeR=([-0-9.]+)\s+completionMaeR=([-0-9.]+)\s+confirmMfeR=([-0-9.]+)\s+confirmMaeR=([-0-9.]+)\s+retestMfeR=([-0-9.]+)\s+retestMaeR=([-0-9.]+)")
anchors=[]
...
truth_rx=re.compile(r"\[V60-DETECTOR-TRUTH\]\s+pattern=(.*?)\s+stage=(\S+)\s+count=(\d+)")
truth={}
for q in truth_rx.finditer(t):
 truth.setdefault(q.group(1),{})[q.group(2)]=int(q.group(3))
out["detector_truth"]=truth
shadow_rx=re.compile(r"\[V60-SHADOW-CLOSED\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+regime=(\S+)\s+mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+holdMin=([-0-9.]+)\s+reason=(\S+)")
shadows=[]
...
out["v60_shadow_outcomes"]=shadows
cap_rx=re.compile(r"\[V60-CAPTURE-CLOSED\]\s+cid=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+regime=(\S+)\s+baselineR=([-0-9.]+)\s+protect75R=([-0-9.]+)\s+be1R=([-0-9.]+)\s+trail125R=([-0-9.]+)\s+timeDecayR=([-0-9.]+)\s+hybridR=([-0-9.]+)\s+mfeR=([-0-9.]+)\s+captureRatio=([-0-9.]+)")
out["v60_capture_outcomes"]=[{"cid":q.group(1),"pattern":q.group(2),"route":q.group(3),"regime":q.group(4),
...
book_rx=re.compile(r"\[V60-FAMILY-BOOK\]\s+pattern=(.*?)\s+candidates=(\d+)\s+shadowStarted=(\d+)\s+shadowClosed=(\d+)")
out["v60_family_books"]={q.group(1):{"candidates":int(q.group(2)),"shadow_started":int(q.group(3)),"shadow_closed":int(q.group(4))} for q in book_rx.finditer(t)}
