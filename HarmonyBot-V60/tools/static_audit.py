...
s=pathlib.Path(sys.argv[1] if len(sys.argv)>1 else "HarmonyBot-V60/src/HarmonyBotV60.cs").read_text(errors="ignore")
checks={
"identity":"HarmonyBot V60" in s and "class HarmonyBotV60" in s and 'BotPrefix = "HB60"' in s,
...
"detector_truth":"V60-DETECTOR-TRUTH" in s and "DetectorTruth" in s,
...
out={"version":"HarmonyBot V60","architecture":"CANONICAL_HARMONIC_MANIFOLD_EXCURSION_CAPTURE","checks":checks,"pass":all(checks.values())}
pathlib.Path("V60_ARCHITECTURE_AUDIT.json").write_text(json.dumps(out,indent=2))
