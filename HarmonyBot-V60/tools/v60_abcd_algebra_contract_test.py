...
for token in [
    ...
    'V60AbcdCenters("AB=CD")',
]:
    assert token in SRC, f"missing V60 AB=CD algebra token: {token}"
...
block = SRC[SRC.index("else if (p.Mode == PatternMode.ABCD)", SRC.index("V60PureProjectedPrzCenter")):]
block = block[:block.index("else if (p.Mode == PatternMode.CYPHER)")]
assert "d.Price" not in block
assert 'foreach (double k in V60AbcdCenters("AB=CD"))' in block
assert "c.Price + sign * k * ab" in block

print("V60 AB=CD algebra contract PASS")
