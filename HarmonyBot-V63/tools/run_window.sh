...
case "$VAR" in V63_GOLDEN_V51_CONTROL|V63_V51_CONDITIONAL|V63_CELL_POLICY_ROUTER|V63_CELL_POLICY_RUNNER|V63_POSITIVE_COHORT_RECOVERY|V63_OCCUPANCY_GOVERNOR) ;; *) exit 31;; esac
C="$PWD/control"; W="$C/HarmonyBot-V63/window-$VAR-$WIN-B$BALANCE-S$SPREAD"; O="$C/HarmonyBot-V63/output-$VAR-$WIN-B$BALANCE-S$SPREAD"; rm -rf "$O"; mkdir -p "$W/seal/algo" "$W/seal/data" "$O/raw-logs"
cp "$C/HarmonyBot-V63/dist/HarmonyBot_V63_Regime_Native_Alpha_Portfolio_Adaptive_Execution_Governor_RC.algo" "$W/seal/algo/"; N="V63-$VAR-$WIN-B$BALANCE-S$SPREAD"
