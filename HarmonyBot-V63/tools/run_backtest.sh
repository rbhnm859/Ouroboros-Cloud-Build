#!/usr/bin/env bash
set -euo pipefail
: "${CTRADER_PASSWORD:?}"; : "${CTRADER_CTID:?}"; : "${CTRADER_ACCOUNT:?}"; : "${RUN_NAME:?}"; : "${START_DATE:?}"; : "${END_DATE:?}"; : "${EVAL_DATE:?}"
BALANCE="${BALANCE:-10000}"; SPREAD="${SPREAD:-1}"; V63VARIANT="${V63VARIANT:-V63_CONDITIONAL_EXECUTION}"; V63_POLICY_MAP="${V63_POLICY_MAP:-}"; V63_RECOVERY_CELLS="${V63_RECOVERY_CELLS:-}"; V63_OCCUPANCY_CELLS="${V63_OCCUPANCY_CELLS:-}"
ALGO="${ALGO:-seal/algo/HarmonyBot_V63_Regime_Native_Alpha_Portfolio_Adaptive_Execution_Governor_RC.algo}"; IMAGE="${CTRADER_IMAGE:-ghcr.io/spotware/ctrader-console:5.9.11}"; BACKTEST_TIMEOUT_SECONDS="${BACKTEST_TIMEOUT_SECONDS:-2700}"
mkdir -p seal/{reports,logs,data}; if [ ! -s seal/ctrader.pwd ]; then printf '%s' "$CTRADER_PASSWORD" > seal/ctrader.pwd; chmod 600 seal/ctrader.pwd; fi
...
ACCT=$(python3 - <<'PY'
import json,os
a=json.load(open('seal/accounts.json',encoding='utf-8-sig')); e=os.environ['CTRADER_ACCOUNT'].strip(); m=next((x for x in a if str(x.get('Number',''))==e or str(x.get('Id',''))==e),None)
