...
v63rx=re.findall(r"\[V63-SUMMARY\].*?runnerAssigned=(\d+)\s+runnerFilled=(\d+)\s+runnerClosed=(\d+)\s+runnerSurvivedCanonical=(\d+)\s+conditionalArmed=(\d+)\s+conditionalRejected=(\d+)",t)
v63={"runner_assigned":0,"runner_filled":0,"runner_closed":0,"runner_survived_canonical":0,"conditional_armed":0,"conditional_rejected":0}
if v63rx:
...
"gate_telemetry":gate_telemetry,"rejected_cohort_quality":rejected_cohort_quality,"v63_telemetry":v63,"basket_outcomes":b,"pattern_pipeline":pipeline,**c}
