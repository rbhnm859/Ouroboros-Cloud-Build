...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV63 : Robot
    {
        ...
        [Parameter("V63 Variant", DefaultValue = "V63_GOLDEN_V51_CONTROL")]
        ...
        protected override void OnStop()
        {
            ...
            Print("[V63-SUMMARY] variant={0} regimeVetoed={1} ratExpansionAdmitted={2} abcdCapitalRejected={3} runnerAssigned={4} runnerFilled={5} runnerClosed={6} runnerSurvivedCanonical={7} conditionalArmed={8} conditionalRejected={9}",
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                if (record.Route == HarmonicRoute.NO_TRADE && V63PatternNativeEnabled())
                {
                    HarmonicRoute ratRoute;
                    if (V63TryRatControlledExpansion(signal, record.Conflict, regime, out ratRoute))
                    {
                        record.Route = ratRoute;
                        _v62RatExpansionAdmitted++;
                        Print("[V63-RAT-EXPANSION] cid={0} setup={1} route={2} geometry={3:F3} prz={4:F3} confidence={5:F3}",
                            record.CandidateId, record.SetupKey, record.Route, signal.GeometryQuality, signal.PrzConfluence, signal.Confidence);
                    }
                }

                if (!V63ExactControlEnabled() && signal.PatternName == "AB=CD")
                {
                    _v62AbcdCapitalRejected++;
                    Reject(record, "V63_ABCD_STANDALONE_CAPITAL_OFF");
                    continue;
                }
        ...
        private bool V63StructuralExitEnabled()
        {
        ...
        private bool V63PatternNativeEnabled()
        {
            return string.Equals(V63Variant, "V63_POSITIVE_COHORT_RECOVERY", StringComparison.Ordinal);
        }

        private bool V63CanonicalGridEnabled()
        {
            return !V63ExactControlEnabled();
        }

        private bool V63RegimeSurvivalEnabled()
        {
            return false;
        }

        private bool V63CommercialMaxEnabled()
        {
            return V63RunnerEnabled();
        }
        ...
        private void V63RouteGridContract(CandidateRecord c, out double[] fractions, out double[] weights)
        {
            ...
            if (V63DagSingleEntryEnabled())
            {
                fractions = new[] { 0.0 }; weights = new[] { 1.0 }; return;
            }

            if (V63CurrentV61GridEnabled())
            {
            ...
            if (V63RunnerEnabled() && route != HarmonicRoute.EXHAUSTION_REVERSAL)
            {
        ...
        private bool V63TryRatControlledExpansion(PatternSignal sig, MtfConflict conflict, RegimeSnapshot r, out HarmonicRoute route)
        {
            route = HarmonicRoute.NO_TRADE;
            if (sig == null || r == null || sig.PatternName != "Rat") return false;
            if (sig.GeometryQuality < V63RatMinGeometry || sig.PrzConfluence < V63RatMinPrz || sig.Confidence < V63RatMinConfidence) return false;
            if (conflict == MtfConflict.CONFLICT || r.AtrRatio < .50 || r.AtrRatio > 1.80) return false;
            bool aligned = r.TrendDirection == sig.Direction;
            bool opposed = r.TrendDirection != TradeDirection.Neutral && r.TrendDirection != sig.Direction;
            bool transition = r.Transition || conflict == MtfConflict.TRANSITION;
            if (aligned && r.Efficiency >= .12) route = HarmonicRoute.TREND_ALIGNED_REVERSAL;
            else if (opposed && r.ExtensionAtr >= 1.30 && r.AdxH1Slope <= 0) route = HarmonicRoute.EXHAUSTION_REVERSAL;
            else if (transition && r.AdxH1Slope <= 0) route = HarmonicRoute.TRANSITION_REVERSAL;
            return route != HarmonicRoute.NO_TRADE;
        }

        private void ConfigureV63SelectiveRunner(CandidateRecord c, FibonacciGridPlan plan)
        {
            if (!V63RunnerEnabled() || c == null || c.Signal == null || plan == null || c.Route == HarmonicRoute.EXHAUSTION_REVERSAL) return;
        ...
        private bool DeeperLegThesisEligible(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            if (leg.Fraction >= .618 - 1e-9) { Print("[V63-L3-SHADOW-ONLY] basket={0} leg=L{1}",basket.BasketId,leg.Index); return false; }
            ...
            double penetration=span>_symbol.TickSize?Math.Abs(close-basket.EntryAnchor)/span:999;
            bool m1=basket.Candidate!=null&&basket.Candidate.Signal!=null&&RouteSpecificM1EvidencePass(i,basket.Candidate.Signal,basket.Route);
            bool structure=basket.Direction==TradeDirection.Buy?close>basket.StructuralStop+_symbol.TickSize:close<basket.StructuralStop-_symbol.TickSize;
            double maxPen=leg.Fraction<=.2360001?.50:.72;
            bool pass=impulse>=-PipsToPrice(Math.Max(1.0,SpreadPips()))&&structure&&m1&&penetration<=maxPen;
            Print("[V63-DEPTH-EVIDENCE] basket={0} leg=L{1} pass={2} penetration={3:F3} max={4:F3} m1={5} structure={6}",basket.BasketId,leg.Index,pass,penetration,maxPen,m1,structure);
            return pass;
        }
        ...
        {
            if (!V63ExactControlEnabled())
                return UpdateV63StageAwareFamilyCompletionEvidence(i, c, out score);
            score = 0;
            if (i < 3 || i >= _m1Bars.Count || c == null || c.Signal == null) return false;
            var sig = c.Signal;
    ...
    public sealed class PipelineCounter
    {
        ...
        {
            if (c == null || c.Regime == null) return "R0_UNKNOWN";
            var r=c.Regime;
            if (r.Transition) return "R4_TRANSITION";
            if (r.TrendDirection != TradeDirection.Neutral && r.AdxH1Slope > 0 && r.Efficiency >= .20) return "R1_TREND_EXPANSION";
            if (r.TrendDirection != TradeDirection.Neutral && (r.ExtensionAtr >= 1.20 || r.AdxH1Slope <= 0)) return "R2_TREND_EXHAUSTION";
            if (r.AtrPercentile <= .35 && r.Efficiency < .20) return "R3_COMPRESSION";
            return "R4_TRANSITION";
        }

        private string V63CellKey(CandidateRecord c)
        {
            return (c.Signal == null ? "UNKNOWN" : c.Signal.PatternName) + "|" + c.Route + "|" + V63RegimeState(c);
        }

        private string V63PolicyForCell(CandidateRecord c)
        {
            string family=c.Signal==null?"":c.Signal.PatternName, regime=V63RegimeState(c);
            if (family=="AB=CD") return "SHADOW";
            if (family=="Rat" && c.Route==HarmonicRoute.TREND_ALIGNED_REVERSAL && regime=="R1_TREND_EXPANSION") return "SINGLE_RUNNER";
            if (family=="Rat" && c.Route==HarmonicRoute.EXHAUSTION_REVERSAL) return "CONDITIONAL_GRID";
            if ((family=="Shark"||family=="Cypher"||family=="5-0") && c.Route==HarmonicRoute.TREND_ALIGNED_REVERSAL) return "CONDITIONAL_RUNNER";
            if (c.Route==HarmonicRoute.TRANSITION_REVERSAL) return "SINGLE";
            return "SINGLE";
        }

        private double V63ShrinkageWeight(int n, double k=8.0) { return n<=0?0.0:n/(n+k); }

        private void V63RouteGridContract(CandidateRecord c, out double[] fractions, out double[] weights)
        {
            string policy = V63PolicyForCell(c);
            Print("[V63-CELL-POLICY] cid={0} setup={1} cell={2} policy={3}", c.CandidateId, c.SetupKey, V63CellKey(c), policy);
            if (policy=="SINGLE") { fractions=new[]{0.0}; weights=new[]{1.0}; return; }
            if (policy=="SINGLE_RUNNER") { fractions=new[]{0.0,0.0}; weights=new[]{.80,.20}; return; }
            if (policy=="CONDITIONAL_GRID") { fractions=new[]{0.0,.236,.382,.618}; weights=new[]{.60,.25,.15,0.0}; return; }
            if (policy=="CONDITIONAL_RUNNER") { fractions=new[]{0.0,0.0,.236,.382,.618}; weights=new[]{.60,.15,.15,.10,0.0}; return; }
            fractions=new[]{0.0}; weights=new[]{1.0};
        }
