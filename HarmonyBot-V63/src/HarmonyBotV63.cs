...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV63 : Robot
    {
        private const string Version = "HarmonyBot V63 — Alpha-Preserving Harmonic Execution & Opportunity Engine";
        private const string BotPrefix = "HB63";
        ...
        [Parameter("V63 Rat Min Geometry", DefaultValue = 0.72, MinValue = 0.72, MaxValue = 0.72)]
        public double V63RatMinGeometry { get; set; }

        [Parameter("V63 Rat Min PRZ", DefaultValue = 0.72, MinValue = 0.72, MaxValue = 0.72)]
        public double V63RatMinPrz { get; set; }

        [Parameter("V63 Rat Min Confidence", DefaultValue = 0.68, MinValue = 0.68, MaxValue = 0.68)]
        public double V63RatMinConfidence { get; set; }

        [Parameter("V63 Runner Minimum R", DefaultValue = 3.0, MinValue = 3.0, MaxValue = 3.0)]
        public double V63RunnerMinimumR { get; set; }
        ...
        protected override void OnStop()
        {
            ...
            Print("[V63-SUMMARY] variant={0} cohortRecovered={1} occupancyReleased={2} runnerAssigned={3} runnerFilled={4} runnerClosed={5} runnerSurvivedCanonical={6} conditionalArmed={7} conditionalRejected={8}",
                V63Variant,_v63CohortRecovered,_v63OccupancyReleased,_v63RunnerAssigned,_v63RunnerFilled,_v63RunnerClosed,_v63RunnerSurvivedCanonicalTp,_v63ConditionalLegArmed,_v63ConditionalLegRejected);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                Print("[V63-GATE-PASS] cid={0} setup={1} stage=PatternQualification reason=PATTERN_VALIDATED", record.CandidateId, record.SetupKey);
                ...
                if (record.Route == HarmonicRoute.NO_TRADE && V63CohortRecoveryEnabled())
                {
                    HarmonicRoute recoveredRoute;
                    if (V63TryPositiveCohortRecovery(signal,record.Conflict,regime,out recoveredRoute))
                    {
                        record.Route=recoveredRoute; _v63CohortRecovered++;
                        Print("[V63-COHORT-RECOVERY] cid={0} setup={1} pattern={2} route={3} regime={4} geometry={5:F3} prz={6:F3} confidence={7:F3}",
                            record.CandidateId,record.SetupKey,signal.PatternName,record.Route,V63ClassifyRegime(regime,record.Conflict,signal.Direction),signal.GeometryQuality,signal.PrzConfluence,signal.Confidence);
                    }
                }
                ...
                record.V63RegimeClass=V63ClassifyRegime(record);
                record.V63Policy=V63PolicyFor(record);
                Print("[V63-CELL] cid={0} setup={1} pattern={2} route={3} regime={4} policy={5}",record.CandidateId,record.SetupKey,signal.PatternName,record.Route,record.V63RegimeClass,record.V63Policy);
                Print("[V63-GATE-PASS] cid={0} setup={1} stage=RegimeRouting reason=ROUTE_{2}", record.CandidateId, record.SetupKey, record.Route);
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (V63RegimeSurvivalEnabled() && !V63RegimeSurvivalPass(c))
                    {
                        _v63RegimeVetoed++;
        ...
        private bool RevalidateCandidateForExecution(CandidateRecord c, DateTime now, bool finalCheck)
        {
            ...
            {
                _v63RegimeVetoed++;
                Event(c, "V63_REGIME_SURVIVAL_REVALIDATION_VETO");
                return false;
            }
        ...
        private bool GridPlanReject(CandidateRecord c, string reason)
        {
            ...
            Print("[V63-GATE-REJECT] cid={0} setup={1} stage=RiskPlan reason={2}", c != null ? c.CandidateId : "", c != null ? c.SetupKey : "", reason);
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            int maxLegs;
            if (V63CanonicalGridEnabled())
            {
                V63RouteGridContract(c, out v62Fractions, out v62Weights);
            ...
            {
                double fraction = V63CanonicalGridEnabled() ? v62Fractions[leg] : p.GridFractions[leg];
                ...
                double riskWeight = V63CanonicalGridEnabled() ? v62Weights[leg] : (leg < p.GridRiskWeights.Length ? p.GridRiskWeights[leg] : 0);
            ...
            double targetBasis = V63ConditionalGridEnabled() ? anchor : plan.ExpectedWeightedEntry;
            double target, netRr;
            ...
            if (V63StructuralExitEnabled() && c.Signal.CanonicalTarget2 > 0 &&
                GeometryValid(c.Signal.Direction, targetBasis, plan.StructuralStop, c.Signal.CanonicalTarget2))
            {
                ...
                if (rr >= MinimumNetRR)
                {
                    ...
                    netRr = rr;
                    Print("[V63-STRUCTURAL-EXIT] cid={0} setup={1} target={2} netRR={3:F3}", c.CandidateId, c.SetupKey, target, netRr);
                }
            }

            plan.CanonicalTarget = target;
            plan.ExpectedNetRR = netRr;
            ConfigureV63SelectiveRunner(c, plan);
            c.GridPlan = plan;
            ...
            if (V63ConditionalGridEnabled())
            {
                double shadowL3 = c.Signal.Direction == TradeDirection.Buy ? anchor - .618 * executionUnit : anchor + .618 * executionUnit;
                Print("[V63-L3-SHADOW] cid={0} setup={1} pattern={2} route={3} fraction=0.618 price={4} capital=false",
                    c.CandidateId, c.SetupKey, c.Signal.PatternName, c.Route, shadowL3);
            }
            Print("[V63-GATE-PASS] cid={0} setup={1} stage=RiskPlan reason=GRID_PLAN_VALID", c.CandidateId, c.SetupKey);
        ...
        private void ExecuteFibonacciGridPlan(CandidateRecord c)
        {
            ...
            double l0Target = V63LegTarget(l0, plan.CanonicalTarget);
            ...
            Print("[V63-GATE-PASS] cid={0} setup={1} stage=Executed reason=LEG0_FILLED", c.CandidateId, c.SetupKey);
        ...
        private bool ExecuteImmediateRunnerLeg(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            double target = V63LegTarget(leg, basket.CanonicalTarget);
        ...
        private bool PlaceGridLimit(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            double legTarget = V63LegTarget(leg, basket.CanonicalTarget);
        ...
        private bool V63ExactControlEnabled() { return false; }
        private bool V63DagSingleEntryEnabled() { return false; }
        private bool V63CurrentV61GridEnabled() { return false; }
        private bool V63FrontLoadedGridEnabled() { return false; }
        private bool V63PolicyRouterEnabled()
        {
        ...
        private bool V63ConditionalGridEnabled()
        {
            return string.Equals(V63Variant,"V63_CONDITIONAL_EXECUTION",StringComparison.Ordinal) || V63PolicyRouterEnabled();
        }
        private bool V63StructuralExitEnabled() { return false; }
        private bool V63RunnerEnabled()
        {
            return string.Equals(V63Variant,"V63_CELL_POLICY_RUNNER",StringComparison.Ordinal) ||
                   string.Equals(V63Variant,"V63_POSITIVE_COHORT_RECOVERY",StringComparison.Ordinal) ||
                   string.Equals(V63Variant,"V63_OCCUPANCY_GOVERNOR",StringComparison.Ordinal);
        }
        private bool V63PatternNativeEnabled() { return V63PolicyRouterEnabled(); }
        private bool V63CohortRecoveryEnabled()
        {
        ...
        private bool V63CanonicalGridEnabled() { return true; }
        private bool V63RegimeSurvivalEnabled() { return false; }
        private bool V63CommercialMaxEnabled() { return V63RunnerEnabled(); }

        private string V63ClassifyRegime(RegimeSnapshot r, MtfConflict conflict, TradeDirection direction)
        {
            if (r==null) return "R4_TRANSITION";
            bool aligned=r.TrendDirection==direction && conflict!=MtfConflict.CONFLICT;
            if (aligned && r.TrendStrength>=.45 && r.Efficiency>=.20 && r.AdxH1Slope>=-.25) return "R1_TREND_EXPANSION";
            if (r.ExtensionAtr>=1.20 && r.AtrPercentile>=.55) return "R2_TREND_EXHAUSTION";
            if (r.AtrPercentile<=.35 && r.Efficiency<=.25) return "R3_COMPRESSION";
            return "R4_TRANSITION";
        }
        private string V63ClassifyRegime(CandidateRecord c)
        {
        ...
        private string V63PolicyFor(CandidateRecord c)
        {
            ...
            if (p=="Rat" && c.Route==HarmonicRoute.EXHAUSTION_REVERSAL && r=="R2_TREND_EXHAUSTION") return "CONDITIONAL";
            if (p=="Shark" && c.Route!=HarmonicRoute.EXHAUSTION_REVERSAL && r=="R1_TREND_EXPANSION") return V63RunnerEnabled()?"CONDITIONAL_RUNNER":"CONDITIONAL";
            if (p=="Cypher" && (r=="R1_TREND_EXPANSION" || r=="R4_TRANSITION")) return V63RunnerEnabled()?"SINGLE_RUNNER":"SINGLE";
            if (p=="5-0" && c.Route!=HarmonicRoute.EXHAUSTION_REVERSAL && (r=="R2_TREND_EXHAUSTION" || r=="R4_TRANSITION")) return V63RunnerEnabled()?"CONDITIONAL_RUNNER":"CONDITIONAL";
            if (p=="AB=CD" && c.Route==HarmonicRoute.EXHAUSTION_REVERSAL && r=="R2_TREND_EXHAUSTION") return "SINGLE";
            return "CONDITIONAL";
        }
        private bool V63PolicyUsesRunner(CandidateRecord c)
        {
            string p=V63PolicyFor(c);
            return V63RunnerEnabled() && (p=="SINGLE_RUNNER" || p=="CONDITIONAL_RUNNER");
        }
        private void V63RouteGridContract(CandidateRecord c,out double[] fractions,out double[] weights)
        {
            string policy=V63PolicyFor(c);
            if (policy=="SINGLE"){fractions=new[]{0.0};weights=new[]{1.0};return;}
            if (policy=="SINGLE_RUNNER" && V63PolicyUsesRunner(c)){fractions=new[]{0.0,0.0};weights=new[]{.80,.20};return;}
            if (policy=="CONDITIONAL_RUNNER" && V63PolicyUsesRunner(c) && c.Route!=HarmonicRoute.EXHAUSTION_REVERSAL)
            {fractions=new[]{0.0,0.0,.236,.382};weights=new[]{.45,.15,.25,.15};return;}
        ...
        private bool V63RegimeSurvivalPass(CandidateRecord c)
        {
            ...
            double mtf = c.Conflict == MtfConflict.ALIGNED ? 1.0 :
                         ...
                         c.Conflict == MtfConflict.TRANSITION ? .55 : .10;
            double directionFit = r.TrendDirection == c.Signal.Direction ? 1.0 :
                                  r.TrendDirection == TradeDirection.Neutral ? .60 : .25;
            double atrHealth = 1.0 - Math.Min(1.0, Math.Abs(r.AtrPercentile - .55) / .55);
            double persistence = VClamp(.55 * r.Efficiency + .45 * r.TrendStrength);
            double confirmation = VClamp(c.ConfirmationScore);
            double riskSpan = Math.Abs(c.Signal.D.Price - c.Signal.StructuralInvalidation);
            double przCompression = riskSpan > _symbol.TickSize
                ? 1.0 - Math.Min(1.0, Math.Abs(c.Signal.PrzHigh - c.Signal.PrzLow) / riskSpan) : 0;
            double spreadHealth = 1.0 - Math.Min(1.0, CurrentSpreadPips() / Math.Max(MaxSpreadPips, 1e-9));
            double extensionFit = c.Route == HarmonicRoute.EXHAUSTION_REVERSAL
                ? VClamp(r.ExtensionAtr / 1.50)
                : c.Route == HarmonicRoute.TRANSITION_REVERSAL
                    ? (r.Transition || c.Conflict == MtfConflict.TRANSITION ? 1.0 : .20)
                    : directionFit;
            bool routeHardPass =
                c.Route == HarmonicRoute.TREND_ALIGNED_REVERSAL
                    ? c.Conflict != MtfConflict.CONFLICT && r.TrendDirection == c.Signal.Direction
                    : c.Route == HarmonicRoute.EXHAUSTION_REVERSAL
                        ? r.ExtensionAtr >= 1.20 && r.AdxH1Slope <= .50
                        : c.Route == HarmonicRoute.TRANSITION_REVERSAL
                            ? (r.Transition || c.Conflict == MtfConflict.TRANSITION)
                            : false;
            double score = .20 * mtf + .18 * directionFit + .16 * atrHealth + .14 * persistence +
                           .12 * extensionFit + .10 * confirmation + .05 * przCompression + .05 * spreadHealth;
            Print("[V63-REGIME-SURVIVAL] cid={0} setup={1} route={2} score={3:F3} threshold={4:F3} hard={5}",
                c.CandidateId, c.SetupKey, c.Route, score, V63RegimeSurvivalThreshold, routeHardPass);
            return routeHardPass && score >= V63RegimeSurvivalThreshold;
        }

        private bool V63TryPositiveCohortRecovery(PatternSignal sig,MtfConflict conflict,RegimeSnapshot r,out HarmonicRoute route)
        {
            route=HarmonicRoute.NO_TRADE;
            if(!V63CohortRecoveryEnabled()||sig==null||r==null||conflict==MtfConflict.CONFLICT)return false;
            string regime=V63ClassifyRegime(r,conflict,sig.Direction);
            bool high=sig.GeometryQuality>=.68&&sig.PrzConfluence>=.68&&sig.Confidence>=.64;
            if(!high)return false;
            if(sig.PatternName=="Rat"&&regime=="R1_TREND_EXPANSION"&&r.TrendDirection==sig.Direction) route=HarmonicRoute.TREND_ALIGNED_REVERSAL;
        ...
        {
            if (c == null || c.Signal == null || plan == null || !V63PolicyUsesRunner(c) || c.Route == HarmonicRoute.EXHAUSTION_REVERSAL) return;
            ...
            double minimumRunner = c.Signal.Direction == TradeDirection.Buy ? leg.PlannedPrice + riskDistance * V63RunnerMinimumR : leg.PlannedPrice - riskDistance * V63RunnerMinimumR;
            ...
            Print("[V63-RUNNER-ASSIGN] cid={0} setup={1} route={2} leg=L{3} weight={4:F3} target={5}", c.CandidateId, c.SetupKey, c.Route, leg.Index, leg.RiskWeight, target);
        }

        private double V63LegTarget(FibonacciGridLeg leg, double fallback)
        {
            if (V63CommercialMaxEnabled() && leg != null && leg.RunnerEligible && leg.RunnerTarget > 0) return leg.RunnerTarget;
            return fallback;
        }

        private bool UpdateV63StageAwareFamilyCompletionEvidence(int i, CandidateRecord c, out double score)
        {
            score = 0;
            ...
            string p = sig.PatternName ?? ""; int prior = c.V63DagStage; bool advance = false;
            if (p == "Gartley" || p == "Bat" || p == "Deep Gartley" || p == "Rat" || p == "Cypher")
            {
                if (prior == 0 && (rejection || failedExtension)) advance = true;
                else if (prior == 1 && reclaim && i > c.V63DagAnchorBar) advance = true;
                else if (prior == 2 && (bos || displacement) && i > c.V63DagAnchorBar) advance = true;
                if (advance) { c.V63DagStage++; c.V63DagAnchorBar = i; }
                score = c.V63DagStage / 3.0; return c.V63DagStage >= 3;
            }
            if (p == "Alt Bat" || p == "Butterfly" || p == "Crab" || p == "Deep Crab" || p == "Shark")
            {
                if (prior == 0 && sweep) advance = true;
                else if (prior == 1 && failedExtension && i > c.V63DagAnchorBar) advance = true;
                else if (prior == 2 && (reclaim || insidePrz) && i > c.V63DagAnchorBar) advance = true;
                else if (prior == 3 && (bos || displacement) && i > c.V63DagAnchorBar) advance = true;
                if (advance) { c.V63DagStage++; c.V63DagAnchorBar = i; }
                score = c.V63DagStage / 4.0; return c.V63DagStage >= 4;
            }
            if (p == "5-0")
            {
                if (prior == 0 && failedExtension) advance = true;
                else if (prior == 1 && bos && i > c.V63DagAnchorBar) advance = true;
                else if (prior == 2 && retest && i > c.V63DagAnchorBar) advance = true;
                else if (prior == 3 && directional && i > c.V63DagAnchorBar) advance = true;
                if (advance) { c.V63DagStage++; c.V63DagAnchorBar = i; }
                score = c.V63DagStage / 4.0; return c.V63DagStage >= 4;
            }
            if (p == "AB=CD")
            {
                ...
                else if (prior == 2 && reclaim && i > c.V63DagAnchorBar) advance = true;
                else if (prior == 3 && (bos || displacement) && i > c.V63DagAnchorBar) advance = true;
                if (advance) { c.V63DagStage++; c.V63DagAnchorBar = i; }
                score = c.V63DagStage / 4.0; return c.V63DagStage >= 4;
            }
        ...
        private void ReconcileAndManageBaskets()
        {
            ...
            foreach (var basket in _baskets.Values.Where(b => b.IsActive).ToList())
            {
                ...
                if (V63OccupancyGovernorEnabled() && v63OccupancyCellEligible && age >= 150.0 && basket.PeakR < .20 && currentR <= -.10 && _parkedCandidateIds.Count > 0)
                {
                    ...
                    CancelBasketPending(basket,basket.ExitOverride); CloseBasketPositions(basket,basket.ExitOverride); continue;
                }
        ...
        private bool DeeperLegThesisEligible(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            if (!V63ConditionalGridEnabled()) return notAcceleratingAgainst;
            ...
            Print("[V63-CONDITIONAL-LEG-CHECK] basket={0} leg=L{1} pass={2} impulse={3} structure={4} contained={5} m1Evidence={6}", basket.BasketId, leg.Index, pass, impulse, structureStillValid, penetrationContained, m1ReversalEvidence);
            return pass;
        }
        ...
        private bool EnsurePostFillProtection(Position p, FibonacciBasket basket)
        {
            ...
            double protectionTarget = V63LegTarget(protectionLeg, basket.CanonicalTarget);
        ...
        private void RecordLegFillTelemetry(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            if (V63ConditionalGridEnabled() && leg.Fraction > 1e-9) TryArmNextConditionalLeg(basket, leg.Index);
        }

        private void TryArmNextConditionalLeg(FibonacciBasket basket, int filledIndex)
        {
            if (basket == null || !basket.IsActive || !V63ConditionalGridEnabled()) return;
            ...
            { basket.State = FibonacciBasketState.GRID_PENDING; _v63ConditionalLegArmed++; Print("[V63-CONDITIONAL-LEG-ARMED] basket={0} prior=L{1} next=L{2}", basket.BasketId, filledIndex, next.Index); }
        }
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V63-LEG-ECON] basket={0} setup={1} leg=L{2} fraction={3:F3} weight={4:F3} runner={5} mfeR={6:F3} maeR={7:F3} realizedR={8:F3} net={9:F2} reason={10}", basket.BasketId, basket.Candidate != null ? basket.Candidate.SetupKey : "", l.LegIndex, spec != null ? spec.Fraction : -1, spec != null ? spec.RiskWeight : 0, runner, l.PeakR, l.MaxAdverseR, legRealizedR, p.NetProfit, args.Reason);
                if (runner) { _v63RunnerClosed++; Print("[V63-RUNNER-CLOSED] basket={0} leg=L{1} realizedR={2:F3} net={3:F2} reason={4}", basket.BasketId, l.LegIndex, legRealizedR, p.NetProfit, args.Reason); }
                if (args.Reason.ToString().IndexOf("TakeProfit", StringComparison.OrdinalIgnoreCase) >= 0 && !runner)
                {
                    ...
                    if (openRunner) { _v63RunnerSurvivedCanonicalTp++; Print("[V63-RUNNER-SURVIVED-CANONICAL] basket={0}", basket.BasketId); }
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V63-ECONOMIC-ATTRIBUTION] basket={0} setup={1} pattern={2} route={3} mfeR={4:F3} maeR={5:F3} realizedR={6:F3} capture={7:F3} timeToMfeMin={8:F2} timeToMaeMin={9:F2}", basket.BasketId, setupKey, basket.Pattern, basket.Route, basket.PeakR, basket.MaxAdverseR, realizedR, captureRatio, timeToMfe, timeToMae);
            Print("[V63-CELL-OUTCOME] setup={0} pattern={1} route={2} regime={3} policy={4} realizedR={5:F3} net={6:F2} mfeR={7:F3} maeR={8:F3}",setupKey,basket.Pattern,basket.Route,basket.Candidate!=null?basket.Candidate.V63RegimeClass:"UNKNOWN",basket.Candidate!=null?basket.Candidate.V63Policy:"UNKNOWN",realizedR,basket.RealizedNet,basket.PeakR,basket.MaxAdverseR);
        ...
        {
            // V63 preserves immutable V51 completion semantics in every candidate lane; the V62/V63 stage-aware DAG is observation-only.
            score = 0;
            if (i < 3 || i >= _m1Bars.Count || c == null || c.Signal == null) return false;
            var sig = c.Signal;
        ...
        {
            _candidateSeq++;
            return "V63-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private string V63GateStage(string reason)
        {
            string r=reason??"";
        ...
        private void Reject(CandidateRecord c, string reason)
        {
            ...
            Print("[V63-GATE-REJECT] cid={0} setup={1} stage={2} reason={3}",c.CandidateId,c.SetupKey,V63GateStage(reason),reason);
    ...
    public sealed class CandidateRecord
    {
        ...
        public int V63DagStage, V63DagAnchorBar = -1;
        ...
        public string V63RegimeClass = "UNKNOWN";
        public string V63Policy = "UNKNOWN";
