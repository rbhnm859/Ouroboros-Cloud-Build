...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV39 : Robot
    {
        private const string Version = "HarmonyBot V39 — Regime-Conditioned Harmonic Portfolio Architecture";
        private const string BotPrefix = "HB39";
        ...
        [Parameter("Regime Portfolio Selector", DefaultValue = true)]
        public bool EnableRegimePortfolioSelector { get; set; }

        [Parameter("Route Specialization", DefaultValue = true)]
        public bool EnableRouteSpecialization { get; set; }

        [Parameter("Stress Quarantine", DefaultValue = true)]
        public bool EnableStressQuarantine { get; set; }

        [Parameter("Portfolio Min Edge Score", DefaultValue = 0.54, MinValue = 0.40, MaxValue = 0.80)]
        public double PortfolioMinEdgeScore { get; set; }

        [Parameter("Portfolio Max Stress Score", DefaultValue = 0.62, MinValue = 0.30, MaxValue = 0.90)]
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V39-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V39-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V39-WARMUP-PENDING] H4={0} H1={1} M15={2} M5={3}",
            ...
            {
                Print("[V39-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            Print("[V39-START] version={0} symbol={1} H4={2} H1={3} M15={4} M5={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m5Bars), _profiles.Count);
            Print("[V39-TIMEFRAME-AUDIT] primaryPattern=M15 thesis=M15 primaryExecution=M5 evidenceAccumulation=true regimePortfolioSelector=true macro=H4 intermediate=H1 m1StrategyDependency=false allCompletedBars=true candidateSurvival=true opportunityArbitration=true");
            Print("[V39-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
            Print("[V39-ALPHA-CONFIG] qualityObservation={0} regimeGate={1} m15Gate={2} m5Refinement={3} capitalFeasibility={4} transitionVeto={5} exhaustionVeto={6}",
            ...
            Print("[V39-FREQUENCY-CONFIG] deferredRetention={0} agingPriority={1} ageBoost={2:F3} structuredRecall={3} qualifiedReroute={4} recallGeometry={5:F3} recallPrz={6:F3} recallConfidence={7:F3}",
            ...
            Print("[V39-EXECUTION-CONFIG] evidence={0} window={1} minBars={2} threshold={3:F3} survival={4} opportunityArbitration={5} shadowMinutes={6}",
            ...
            Print("[V39-PORTFOLIO-CONFIG] selector={0} routeSpecialization={1} stressQuarantine={2} minEdge={3:F3} maxStress={4:F3}",
        ...
        protected override void OnStop()
        {
            ...
            {
                var x = kv.Value;
                Print("[V39-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} armed={6} basketPlanned={7} leg0={8} leg1={9} leg2={10} leg3={11} basketClosed={12} executed={13} expired={14} rejected={15} invalidated={16}",
            ...
            Print("[V39-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V39-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
                _alphaQualityRejected, _regimeRejected, _confirmationRejected, _capitalInfeasibleCandidates, _alphaPassed);
            Print("[V39-FREQUENCY-SUMMARY] schedulerDeferred={0} schedulerRecoveredExecutions={1} structuredRecallAdmitted={2} activeDeferred={3} routeDeferred={4} routeRecovered={5}",
            ...
            Print("[V39-EXECUTION-SUMMARY] evidenceExecutable={0} survivalWaits={1} opportunityArbitrations={2} riskRenormalizations={3} riskRejects={4} shadowTargets={5} shadowStops={6} shadowUnresolved={7}",
            ...
            Print("[V39-PORTFOLIO-SUMMARY] evaluated={0} accepted={1} rejected={2} routeSpecializationRejected={3} stressQuarantineRejected={4}",
            ...
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V39-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                Print("[V39-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void AdvanceCandidateLifecycleOnBars(Bars bars, int i, DateTime utc, string executionTf, bool allowArm)
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                Print("[V39-EVIDENCE] cid={0} tf={1} barScore={2:F3} composite={3:F3} bars={4} threshold={5:F3} pass={6}",
                    c.CandidateId, executionTf, barScore, c.EvidenceComposite, c.EvidenceBarsObserved, threshold, evidencePass);
                ...
                c.PortfolioEdgeScore = PortfolioEdgeScore(c);
                ...
                _alphaPassed++;
                _evidenceExecutable++;
                Transition(c, CandidateState.EXECUTABLE, executionTf + "_PORTFOLIO_EXECUTABLE");
        ...
        private bool PortfolioExecutionEligible(CandidateRecord c)
        {
            ...
            if (EnableStressQuarantine)
            {
                ...
                if (hardStress)
                {
                    ...
                    Print("[V39-PORTFOLIO-REJECT] cid={0} reason=STRESS score={1:F3} edge={2:F3} route={3}",
            ...
            if (EnableRouteSpecialization && !RouteSpecializationEligible(c))
            {
                ...
                Print("[V39-PORTFOLIO-REJECT] cid={0} reason=ROUTE_SPECIALIZATION stress={1:F3} edge={2:F3} route={3}",
                    c.CandidateId, c.RegimeStressScore, c.PortfolioEdgeScore, c.Route);
                return false;
            }
            ...
            {
                Print("[V39-PORTFOLIO-REJECT] cid={0} reason=EDGE_SCORE stress={1:F3} edge={2:F3} route={3}",
                    c.CandidateId, c.RegimeStressScore, c.PortfolioEdgeScore, c.Route);
                return false;
            }

            Print("[V39-PORTFOLIO-ACCEPT] cid={0} pattern={1} route={2} edge={3:F3} stress={4:F3} evidence={5:F3} netRR={6:F3}",
        ...
        private void ReevaluateQualifiedRoutes(HarmonicState h4State, HarmonicState h1State, RegimeSnapshot regime, DateTime utc)
        {
            ...
            foreach (var id in _qualifiedRouteDeferredCandidates.ToList())
            {
                ...
                _qualifiedRouteDeferredCandidates.Remove(id);
                Print("[V39-QUALIFIED-REROUTE] cid={0} pattern={1} direction={2} conflict={3} route={4} quality={5:F3} regime={6:F3}",
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            Print("[V39-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V39-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V39-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V39-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V39-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V39-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V39-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V39-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V39-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V39-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V39-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            double entryImprovementPips = basket.AverageEntry > 0
                ...
                : 0;
            Print("[V39-BASKET-CLOSED] basket={0} cid={1} pattern={2} route={3} dir={4} plannedLegs={5} filledLegs={6} anchor={7} avgEntry={8} entryImprovePips={9:F3} stop={10} target={11} initialRisk={12:F2} worstRisk={13:F2} mfeR={14:F3} maeR={15:F3} realizedR={16:F3} net={17:F2} reason={18}",
        ...
        {
            Print("[V39-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
        ...
        private HarmonicRoute RouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (baseRoute == HarmonicRoute.NO_TRADE && EnableStructuredRecallExpansion)
            {
                ...
                if (baseRoute != HarmonicRoute.NO_TRADE)
                {
                    ...
                    Print("[V39-RECALL-ADMIT] pattern={0} direction={1} route={2} conflict={3} geometry={4:F3} prz={5:F3} confidence={6:F3} efficiency={7:F3} atrRatio={8:F3} extensionAtr={9:F3}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionStateVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V39-ROUTE-VETO] type=TRANSITION_STATE conflict={0} transition={1} adxSlope={2:F2}",
            ...
            if (baseRoute == HarmonicRoute.EXHAUSTION_REVERSAL && EnableExhaustionEvidenceVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V39-ROUTE-VETO] type=EXHAUSTION_EVIDENCE extensionAtr={0:F3} adxSlope={1:F2}",
        ...
        private double CandidateRank(CandidateRecord c)
        {
            ...
            double baseRank = .20 * c.Signal.GeometryQuality + .12 * c.Signal.PrzConfluence + .13 * mtf + .13 * regime +
        ...
        private double OpportunityCostRank(CandidateRecord c)
        {
            ...
            double edge = VClamp(.20 * c.AlphaQualityScore + .15 * c.RegimeScore + .20 * readiness +
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V39-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        private void AddCounterfactualShadow(CandidateRecord c, string reason, DateTime utc)
        {
            ...
            Print("[V39-SHADOW-START] cid={0} pattern={1} reason={2} entry={3} stop={4} target={5}",
        ...
        private void UpdateCounterfactualShadows(Bars bars, int i, DateTime utc)
        {
            ...
            foreach (var sh in _counterfactualShadows.Values.Where(x => !x.Resolved).ToList())
            {
                ...
                if (stopHit || targetHit)
                {
                    ...
                    Print("[V39-SHADOW-RESOLVE] cid={0} outcome={1} mfeR={2:F3} maeR={3:F3} reason={4}",
                ...
                if (utc >= sh.ExpiryUtc)
                {
                    ...
                    sh.ResolvedUtc = utc;
                    _shadowUnresolved++;
                    Print("[V39-SHADOW-RESOLVE] cid={0} outcome=UNRESOLVED mfeR={1:F3} maeR={2:F3} reason={3}",
        ...
        {
            _candidateSeq++;
            return "V39-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        {
            Print("[V39-EVENT] cid={0} pattern={1} tf={2} dir={3} state={4} route={5} conflict={6} reason={7}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V39-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V39-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
