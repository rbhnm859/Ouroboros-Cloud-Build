...
rx=re.compile(r"\[V39-BASKET-CLOSED\].*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
pat=(r"\[V39-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
freq=re.findall(r"\[V39-FREQUENCY-SUMMARY\].*?schedulerDeferred=(\d+)\s+schedulerRecoveredExecutions=(\d+)\s+structuredRecallAdmitted=(\d+)\s+activeDeferred=(\d+).*?routeDeferred=(\d+)\s+routeRecovered=(\d+)",t)
...
ex=re.findall(r"\[V39-EXECUTION-SUMMARY\].*?evidenceExecutable=(\d+)\s+survivalWaits=(\d+)\s+opportunityArbitrations=(\d+)\s+riskRenormalizations=(\d+)\s+riskRejects=(\d+)\s+shadowTargets=(\d+)\s+shadowStops=(\d+)\s+shadowUnresolved=(\d+)",t)
...
port=re.findall(r"\[V39-PORTFOLIO-SUMMARY\].*?evaluated=(\d+)\s+accepted=(\d+)\s+rejected=(\d+)\s+routeSpecializationRejected=(\d+)\s+stressQuarantineRejected=(\d+)",t)
...
"broker_profile_present":"[V39-BROKER-PROFILE]" in t,
"exhaustion_veto_events":len(re.findall(r"\[V39-ROUTE-VETO\] type=EXHAUSTION_EVIDENCE",t)),
"evidence_events":len(re.findall(r"\[V39-EVIDENCE\]",t)),
...
"mean_mae_r":statistics.mean([x["mae"] for x in b]) if b else 0,**fc,**ec,**pc,**c}
