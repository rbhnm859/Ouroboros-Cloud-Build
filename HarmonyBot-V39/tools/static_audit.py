...
p=pathlib.Path(sys.argv[1] if len(sys.argv)>1 else "HarmonyBot-V39/src/HarmonyBotV39.cs")
s=p.read_text(errors="ignore")
checks={
 "identity":"HarmonyBot V39" in s and "class HarmonyBotV39" in s and 'BotPrefix = "HB39"' in s,
 ...
 "regime_portfolio_selector":"EnableRegimePortfolioSelector" in s and "PortfolioExecutionEligible" in s and "PortfolioEdgeScore" in s,
 "route_specialization":"EnableRouteSpecialization" in s and "RouteSpecializationEligible" in s,
 "stress_quarantine":"EnableStressQuarantine" in s and "RegimeStressScore" in s,
 "evidence_accumulation":"EnableEvidenceAccumulation" in s and "EvidenceCompositeScore" in s,
 ...
 "no_old_prefix":'"HB38-' not in s and '"HB37-' not in s
}
out={"version":"HarmonyBot V39","architecture":"REGIME_CONDITIONED_HARMONIC_PORTFOLIO","checks":checks,"pass":all(checks.values())}
pathlib.Path("V39_ARCHITECTURE_AUDIT.json").write_text(json.dumps(out,indent=2))
