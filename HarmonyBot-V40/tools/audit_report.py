...
rx=re.compile(r"\[V40-BASKET-CLOSED\].*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
pat=(r"\[V40-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
freq=re.findall(r"\[V40-FREQUENCY-SUMMARY\].*?schedulerDeferred=(\d+)\s+schedulerRecoveredExecutions=(\d+)\s+structuredRecallAdmitted=(\d+)\s+activeDeferred=(\d+).*?routeDeferred=(\d+)\s+routeRecovered=(\d+)",t)
...
ex=re.findall(r"\[V40-EXECUTION-SUMMARY\].*?evidenceExecutable=(\d+)\s+survivalWaits=(\d+)\s+opportunityArbitrations=(\d+)\s+riskRenormalizations=(\d+)\s+riskRejects=(\d+)\s+shadowTargets=(\d+)\s+shadowStops=(\d+)\s+shadowUnresolved=(\d+)",t)
...
port=re.findall(r"\[V40-PORTFOLIO-SUMMARY\].*?evaluated=(\d+)\s+accepted=(\d+)\s+rejected=(\d+)\s+routeSpecializationRejected=(\d+)\s+stressQuarantineRejected=(\d+)",t)
pc={"portfolio_evaluated":0,"portfolio_accepted":0,"portfolio_rejected":0,"route_specialization_rejected":0,"stress_quarantine_rejected":0}
if port:
 z=list(map(int,port[-1]))
 for k,n in zip(pc,z): pc[k]=n
attr=re.findall(r"\[V40-ATTRIBUTION-SUMMARY\].*?observed=(\d+)\s+rankFirstReleased=(\d+)\s+followThroughObserved=(\d+)\s+thesisFailureExits=(\d+)",t)
ac={"attribution_observed":0,"rank_first_released":0,"follow_through_observed":0,"thesis_failure_exits":0}
if attr:
 z=list(map(int,attr[-1]))
 for k,n in zip(ac,z): ac[k]=n
orx=re.compile(r"\[V40-ATTRIBUTION-OUTCOME\].*?cid=(\S+)\s+pattern=(\S+)\s+route=(\S+)\s+dir=(\S+)\s+conflict=(\S+)\s+geom=([-0-9.]+)\s+prz=([-0-9.]+)\s+time=([-0-9.]+)\s+pivot=([-0-9.]+)\s+alpha=([-0-9.]+)\s+regime=([-0-9.]+)\s+atrRatio=([-0-9.]+)\s+efficiency=([-0-9.]+)\s+extensionAtr=([-0-9.]+)\s+adxSlope=([-0-9.]+)\s+evidence=([-0-9.]+)\s+follow=([-0-9.]+)\s+routeFit=([-0-9.]+)\s+stress=([-0-9.]+)\s+netRR=([-0-9.]+)\s+attr=([-0-9.]+)\s+mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)\s+reason=(\S+)")
outcomes=[]
for q in orx.finditer(t):
 outcomes.append({"cid":q.group(1),"pattern":q.group(2),"route":q.group(3),"direction":q.group(4),"conflict":q.group(5),
 "geometry":float(q.group(6)),"prz":float(q.group(7)),"time_symmetry":float(q.group(8)),"pivot_quality":float(q.group(9)),
 "alpha":float(q.group(10)),"regime":float(q.group(11)),"atr_ratio":float(q.group(12)),"efficiency":float(q.group(13)),
 "extension_atr":float(q.group(14)),"adx_slope":float(q.group(15)),"evidence":float(q.group(16)),"follow":float(q.group(17)),
 "route_fit":float(q.group(18)),"stress":float(q.group(19)),"net_rr":float(q.group(20)),"attribution":float(q.group(21)),
 "mfe_r":float(q.group(22)),"mae_r":float(q.group(23)),"realized_r":float(q.group(24)),"net":float(q.group(25)),"reason":q.group(26)})
...
"broker_profile_present":"[V40-BROKER-PROFILE]" in t,
"exhaustion_veto_events":len(re.findall(r"\[V40-ROUTE-VETO\] type=EXHAUSTION_EVIDENCE",t)),
"evidence_events":len(re.findall(r"\[V40-EVIDENCE\]",t)),
...
"mean_mae_r":statistics.mean([x["mae"] for x in b]) if b else 0,"attribution_outcomes":outcomes,**fc,**ec,**pc,**ac,**c}
