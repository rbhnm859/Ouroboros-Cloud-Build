...
CNAME="v40-$(echo "$RUN_NAME"|tr '[:upper:]_' '[:lower:]-')-$GITHUB_RUN_ID"
docker run --name "$CNAME" -v "$PWD/seal:/work" "$IMAGE" backtest "/work/${ALGO#seal/}" \
 ...
 --EnableRegimePortfolioSelector="$REGIMESEL" --EnableRouteSpecialization="$ROUTESPEC" --EnableStressQuarantine="$STRESSQ" \
 --PortfolioMinEdgeScore=0.54 --PortfolioMaxStressScore=0.62 \
 --EnableAttributionLedger=true --EnableRankFirstPortfolio="$RANKFIRST" --EnableFollowThroughEngine="$FOLLOW" --EnableThesisFailureExit="$THESISEXIT" \
 ...
 --EnableCapitalFeasibilityGate=false --EnableTransitionStateVeto=false \
 --EnableExhaustionEvidenceVeto=true --EnableQualifiedDynamicReroute=false --RerouteMinRobustness=0.62 --RerouteMinTimeSymmetry=0.35 --RerouteMinPivotQuality=0.35 \
...
PID=$!; DONE=0
echo "[V40-WATCHDOG] run=$RUN_NAME pid=$PID timeoutSeconds=$BACKTEST_TIMEOUT_SECONDS"
for ((i=0;i<BACKTEST_TIMEOUT_SECONDS;i+=5)); do
 if (( i % 60 == 0 )); then echo "[V40-WATCHDOG] run=$RUN_NAME elapsedSeconds=$i status=running"; fi
