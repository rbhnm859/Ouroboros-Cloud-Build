...
namespace cAlgo.Robots
{
    ...
    public class FibonacciHarmonicSniperUltimate : Robot
    {
        ...
        [Parameter("Max Entry Distance ATR", DefaultValue = 2.00, MinValue = 0.1, MaxValue = 10.0, Group = "Confirmation")]
        ...
        [Parameter("Minimum RR", DefaultValue = 2.00, MinValue = 0.2, MaxValue = 20.0, Group = "Risk")]
        ...
        [Parameter("Execution Cost Reserve (pips)", DefaultValue = 2.0, MinValue = 0.0, Group = "Risk")]
            ...
            for (int i = firstWindow; i <= pivots.Count - 5; i++)
            {
                ...
                foreach (var def in _patterns)
                {
                    ...
                    if (RejectInsufficientFibTarget && TpMode != TakeProfitMode.RiskReward)
                    {
                        ...
                        if (tp - ExecutionCostReservePips < (sl + ExecutionCostReservePips) * MinimumRiskReward)
                        {
        ...
        private void ExecutePatternTrade(PatternMatch m)
        {
            ...
            if (tpPips - ExecutionCostReservePips < (slPips + ExecutionCostReservePips) * MinimumRiskReward)
            {
                Reject("net_reward_insufficient");
                return;
            }
            // Explicit allowance for commission and execution uncertainty, not a guaranteed loss cap.
            double volume = CalculateVolume(slPips + ExecutionCostReservePips);
