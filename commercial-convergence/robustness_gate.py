...
expected={"BASE","THRESH_MINUS10","THRESH_PLUS10","SPREAD_150","COMMISSION_150","SLIPPAGE_150","COMBINED_COST"}
matrix_ok=bool(
    set(by)==expected
    and all(x["pf"]>=0.90 and x["max_dd_pct"]<=10 and x["execution_errors"]==0 for x in rows)
    and statistics.median([x["pf"] for x in rows])>1.0
    and by["BASE"]["pf"]>1.0 and by["BASE"]["expectancy"]>0
)
nets=by.get("BASE",{}).get("basket_nets",[])
random.seed(2956); dd95=999.0; ruin=1.0; p5_total=-1e99
if nets:
    dds=[]; totals=[]; ruins=0
    for _ in range(5000):
