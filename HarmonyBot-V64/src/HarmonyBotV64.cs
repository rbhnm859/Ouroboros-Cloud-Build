...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV64 : Robot
    {
        private const string Version = "HarmonyBot V64 — XAUUSD Harmonic Product Convergence";
        private const string BotPrefix = "HB64";
        ...
        [Parameter("V64 Mode", DefaultValue = "V64_PRODUCT")]
        public string V64Variant { get; set; }

        [Parameter("V64 Runner Minimum R", DefaultValue = 3.0, MinValue = 3.0, MaxValue = 3.0)]
        public double V64RunnerMinimumR { get; set; }
        ...
        private int _v64RunnerAssigned;
        private int _v64RunnerFilled;
        private int _v64RunnerClosed;
        private int _v64RunnerSurvivedCanonicalTp;
        private int _v64ConditionalLegArmed;
        private int _v64ConditionalLegRejected;
        ...
        protected override void OnStart()
        {
            ...
            Print("[V64-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
        ...
        protected override void OnStop()
        {
            ...
            Print("[V64-RISK-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V64-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                Print("[V64-GATE-PASS] cid={0} setup={1} stage=PatternQualification reason=PATTERN_VALIDATED", record.CandidateId, record.SetupKey);
                ...
                Print("[V64-THESIS] cid={0} setup={1} pattern={2} route={3} geometry={4:F3} prz={5:F3} confirmation={6:F3}",
                ...
                Print("[V64-GATE-PASS] cid={0} setup={1} stage=RegimeRouting reason=ROUTE_{2}", record.CandidateId, record.SetupKey, record.Route);
        ...
        private bool GridPlanReject(CandidateRecord c, string reason)
        {
            ...
            Print("[V64-GATE-REJECT] cid={0} setup={1} stage=RiskPlan reason={2}", c != null ? c.CandidateId : "", c != null ? c.SetupKey : "", reason);
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            int maxLegs;
            if (V64CanonicalGridEnabled())
            {
                V64RouteGridContract(c, out v62Fractions, out v62Weights);
            ...
            {
                double fraction = V64CanonicalGridEnabled() ? v62Fractions[leg] : p.GridFractions[leg];
                ...
                double riskWeight = V64CanonicalGridEnabled() ? v62Weights[leg] : (leg < p.GridRiskWeights.Length ? p.GridRiskWeights[leg] : 0);
            ...
            double targetBasis = V64ConditionalGridEnabled() ? anchor : plan.ExpectedWeightedEntry;
            double target, netRr;
            ...
            if (V64StructuralExitEnabled() && c.Signal.CanonicalTarget2 > 0 &&
                GeometryValid(c.Signal.Direction, targetBasis, plan.StructuralStop, c.Signal.CanonicalTarget2))
            {
                ...
                if (rr >= MinimumNetRR)
                {
                    ...
                    netRr = rr;
                    Print("[V64-STRUCTURAL-EXIT] cid={0} setup={1} target={2} netRR={3:F3}", c.CandidateId, c.SetupKey, target, netRr);
                }
            }

            plan.CanonicalTarget = target;
            plan.ExpectedNetRR = netRr;
            ConfigureV64SelectiveRunner(c, plan);
            c.GridPlan = plan;
            c.SelectedTarget = target;
            c.NetRR = netRr;

            Print("[V64-EXECUTION-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V64-EXECUTION-LEG] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
            ...
            if (V64ConditionalGridEnabled())
            {
                double shadowL3 = c.Signal.Direction == TradeDirection.Buy ? anchor - .618 * executionUnit : anchor + .618 * executionUnit;
                Print("[V64-L3-SHADOW] cid={0} setup={1} pattern={2} route={3} fraction=0.618 price={4} capital=false",
                    c.CandidateId, c.SetupKey, c.Signal.PatternName, c.Route, shadowL3);
            }
            Print("[V64-GATE-PASS] cid={0} setup={1} stage=RiskPlan reason=GRID_PLAN_VALID", c.CandidateId, c.SetupKey);
        ...
        private void ExecuteFibonacciGridPlan(CandidateRecord c)
        {
            ...
            double l0Target = V64LegTarget(l0, plan.CanonicalTarget);
            ...
            Print("[V64-GATE-PASS] cid={0} setup={1} stage=Executed reason=LEG0_FILLED", c.CandidateId, c.SetupKey);
            ...
            if (V64ConditionalGridEnabled())
            {
                ...
                if (firstDeep != null)
                {
                    ...
                    {
                        if (PlaceGridLimit(basket, firstDeep)) { basket.State = FibonacciBasketState.GRID_PENDING; _v64ConditionalLegArmed++; }
                    }
                    else { TransitionLegState(basket, firstDeep, GridLegState.CANCELLED, "CONDITIONAL_GRID_INITIAL_REJECT"); _v64ConditionalLegRejected++; }
                }
            }
            else
            {
        ...
        private bool ExecuteImmediateRunnerLeg(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            double target = V64LegTarget(leg, basket.CanonicalTarget);
        ...
        private bool PlaceGridLimit(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            double legTarget = V64LegTarget(leg, basket.CanonicalTarget);
        ...
        private bool V64CommercialMaxEnabled() { return V64ProductEnabled(); }

        // V64 deliberately uses route-native execution only. Harmonic family identity remains in
        // the Alpha Engine; execution does not create a second pattern-selection system.
        private string V64PolicyFor(CandidateRecord c)
        {
            if (!V64ProductEnabled() || c == null) return "LEGACY_V51";
        ...
        private bool V64PolicyUsesRunner(CandidateRecord c)
        {
            return V64ProductEnabled() && V64PolicyFor(c) == "CONDITIONAL_RUNNER";
        }

        private void V64RouteGridContract(CandidateRecord c, out double[] fractions, out double[] weights)
        {
            string policy = V64PolicyFor(c);
            if (policy == "SINGLE")
            {
                fractions = new[] { 0.0 };
                weights = new[] { 1.0 };
        ...
        private void ConfigureV64SelectiveRunner(CandidateRecord c, FibonacciGridPlan plan)
        {
            if (c == null || c.Signal == null || plan == null || !V64PolicyUsesRunner(c) || c.Route == HarmonicRoute.EXHAUSTION_REVERSAL) return;
            ...
            double minimumRunner = c.Signal.Direction == TradeDirection.Buy ? leg.PlannedPrice + riskDistance * V64RunnerMinimumR : leg.PlannedPrice - riskDistance * V64RunnerMinimumR;
            ...
            leg.RunnerEligible = true; leg.RunnerTarget = target; plan.SelectiveRunnerEnabled = true; plan.RunnerRiskWeight = leg.RiskWeight; _v64RunnerAssigned++;
            Print("[V64-RUNNER-ASSIGN] cid={0} setup={1} route={2} leg=L{3} weight={4:F3} target={5}", c.CandidateId, c.SetupKey, c.Route, leg.Index, leg.RiskWeight, target);
        }

        private double V64LegTarget(FibonacciGridLeg leg, double fallback)
        {
        ...
        private bool DeeperLegThesisEligible(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            if (!V64ConditionalGridEnabled()) return notAcceleratingAgainst;
            ...
            Print("[V64-CONDITIONAL-LEG-CHECK] basket={0} leg=L{1} pass={2} impulse={3} structure={4} contained={5} m1Evidence={6}", basket.BasketId, leg.Index, pass, impulse, structureStillValid, penetrationContained, m1ReversalEvidence);
            return pass;
        }
        ...
        private bool EnsurePostFillProtection(Position p, FibonacciBasket basket)
        {
            ...
            double protectionTarget = V64LegTarget(protectionLeg, basket.CanonicalTarget);
        ...
        private void RecordLegFillTelemetry(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            if (leg.RunnerEligible) _v64RunnerFilled++;
            BasketEvent(basket, "GRID_LEG_FILLED_L" + leg.Index);
            if (V64ConditionalGridEnabled() && leg.Fraction > 1e-9) TryArmNextConditionalLeg(basket, leg.Index);
        }

        private void TryArmNextConditionalLeg(FibonacciBasket basket, int filledIndex)
        {
            if (basket == null || !basket.IsActive || !V64ConditionalGridEnabled()) return;
            ...
            { TransitionLegState(basket, next, GridLegState.CANCELLED, "CONDITIONAL_GRID_REVALIDATION_REJECT"); _v64ConditionalLegRejected++; return; }
            if (PlaceGridLimit(basket, next))
            { basket.State = FibonacciBasketState.GRID_PENDING; _v64ConditionalLegArmed++; Print("[V64-CONDITIONAL-LEG-ARMED] basket={0} prior=L{1} next=L{2}", basket.BasketId, filledIndex, next.Index); }
        }
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V64-LEG-ECON] basket={0} setup={1} leg=L{2} fraction={3:F3} weight={4:F3} runner={5} mfeR={6:F3} maeR={7:F3} realizedR={8:F3} net={9:F2} reason={10}", basket.BasketId, basket.Candidate != null ? basket.Candidate.SetupKey : "", l.LegIndex, spec != null ? spec.Fraction : -1, spec != null ? spec.RiskWeight : 0, runner, l.PeakR, l.MaxAdverseR, legRealizedR, p.NetProfit, args.Reason);
                if (runner) { _v64RunnerClosed++; Print("[V64-RUNNER-CLOSED] basket={0} leg=L{1} realizedR={2:F3} net={3:F2} reason={4}", basket.BasketId, l.LegIndex, legRealizedR, p.NetProfit, args.Reason); }
                if (args.Reason.ToString().IndexOf("TakeProfit", StringComparison.OrdinalIgnoreCase) >= 0 && !runner)
                {
                    ...
                    if (openRunner) { _v64RunnerSurvivedCanonicalTp++; Print("[V64-RUNNER-SURVIVED-CANONICAL] basket={0}", basket.BasketId); }
        ...
        {
            // V64 preserves immutable V51 completion semantics in every candidate lane; the V62/V64 stage-aware DAG is observation-only.
            score = 0;
            if (i < 3 || i >= _m1Bars.Count || c == null || c.Signal == null) return false;
            var sig = c.Signal;
        ...
        {
            _candidateSeq++;
            return "V64-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private string V64GateStage(string reason)
        {
            string r=reason??"";
        ...
        private void Reject(CandidateRecord c, string reason)
        {
            ...
            Print("[V64-GATE-REJECT] cid={0} setup={1} stage={2} reason={3}",c.CandidateId,c.SetupKey,V64GateStage(reason),reason);
