...
econ_rx=re.compile(r"\[V64-ECONOMIC-ATTRIBUTION\].*?basket=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+capture=([-0-9.]+)\s+timeToMfeMin=([-0-9.]+)\s+timeToMaeMin=([-0-9.]+)")
...
leg_rx=re.compile(r"\[V64-LEG-ECON\].*?basket=(\S+)\s+setup=(\S+)\s+leg=L(\d+)\s+fraction=([-0-9.]+)\s+weight=([-0-9.]+)\s+runner=(\S+)\s+mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
prod_rx=re.findall(r"\[V64-PRODUCT-SUMMARY\].*?mode=(\S+)\s+runnerAssigned=(\d+)\s+runnerFilled=(\d+)\s+runnerClosed=(\d+)\s+runnerSurvivedCanonical=(\d+)\s+conditionalArmed=(\d+)\s+conditionalRejected=(\d+)",t)
...
out={
 "version":"HarmonyBot V64","mode":a.mode,"window":a.window,"starting_balance":a.balance,"spread":a.spread,"data_snapshot_sha":a.data_sha,
 ...
 "product_execution":product,"leg_economics":leg_economics,"family_performance":family,"route_performance":route,
 "basket_outcomes":b,**c
}
