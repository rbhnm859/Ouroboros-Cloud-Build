...
helper = '''        private bool TryGetPositionLifecycleFromHistory(int positionId, out double netProfit, out double weightedPips, out double totalClosedVolume)\n        {\n            netProfit = 0;\n            weightedPips = 0;\n            totalClosedVolume = 0;\n            try\n            {\n                var deals = History.Where(h => h.PositionId == positionId).ToArray();\n                if (deals == null || deals.Length == 0) return false;\n\n                double pipVol = 0;\n                foreach (var h in deals)\n                {\n                    if (h == null || h.SymbolName != SymbolName) continue;\n                    double v = Math.Max(0.0, h.VolumeInUnits);\n                    netProfit += h.NetProfit;\n                    pipVol += h.Pips * v;\n                    totalClosedVolume += v;\n                }\n                if (totalClosedVolume <= 0) return false;\n                weightedPips = pipVol / totalClosedVolume;\n                return true;\n            }\n            catch\n            {\n                netProfit = 0;\n                weightedPips = 0;\n                totalClosedVolume = 0;\n                return false;\n            }\n        }\n\n'''
if marker not in s:
    raise SystemExit('history-accounting helper marker missing')
s = s.replace(marker, helper + marker, 1)

for token in ['TryGetPositionLifecycleFromHistory', 'History.Where(h => h.PositionId == positionId)', 'h.NetProfit']:
    if token not in s:
