...
# Geometry must be calculated inside MatchPattern, where x/a/b/c/d, atrPrice and
# geometry are in scope. Persist it on PatternMatch and copy it to Signal later.
needle='''        public string PatternName { get; set; }\n        public TradeDirection Direction { get; set; }\n'''
if needle in s:
    insert=needle+'''        public double GeometryQuality { get; set; }\n        public double PrzConfluence { get; set; }\n        public double TimeSymmetry { get; set; }\n        public double PivotQuality { get; set; }\n'''
    s=s.replace(needle,insert,1)
else:
    # PatternMatch property order differs in some payloads; anchor on its class.
    cls=s.find('class PatternMatch')
    if cls < 0: raise SystemExit('PatternMatch class missing')
    brace=s.find('{',cls)
    s=s[:brace+1]+'''\n        public double GeometryQuality { get; set; }\n        public double PrzConfluence { get; set; }\n        public double TimeSymmetry { get; set; }\n        public double PivotQuality { get; set; }\n'''+s[brace+1:]

# Find the PatternMatch construction in MatchPattern and add in-scope geometry.
match_anchor='''            return new PatternMatch\n            {\n'''
idx=s.find(match_anchor)
if idx < 0: raise SystemExit('PatternMatch construction missing')
end=s.find('            };',idx)
if end < 0: raise SystemExit('PatternMatch construction end missing')
block=s[idx:end]
if 'GeometryQuality =' not in block:
    addition='''                GeometryQuality = GeometryQualityScore(x, a, b, c, d, atrPrice, geometry, out double przScore, out double timeScore, out double pivotScore),\n                PrzConfluence = przScore,\n                TimeSymmetry = timeScore,\n                PivotQuality = pivotScore,\n'''
    # Insert immediately after opening initializer; C# permits any property order.
    s=s[:idx+len(match_anchor)]+addition+s[idx+len(match_anchor):]
...
for token in ['Geometry Quality Engine','GeometryQualityScore(SwingPoint','PrzConfluence','TimeSymmetry','PivotQuality','_diagGeometryValidated','_diagGeometryRejectedQuality','EnableFibGrid','capitalInfeasible={11}','geometryValid={12}','geometryQualityRejected={13}','GeometryQuality = best.GeometryQuality']:
...
# Regression guard: signal construction must not reference MatchPattern locals.
signal_start=s.find('Confidence = finalScore')
signal_end=s.find('};',signal_start)
if signal_start >= 0 and 'GeometryQualityScore(x, a, b, c, d' in s[signal_start:signal_end]:
    raise SystemExit('out-of-scope geometry calculation remains in Signal construction')

p.write_text(s,encoding='utf-8')
print('Applied V28.1 independent Harmonic Geometry Quality Engine with in-scope PatternMatch scoring')
