...
# Stage4: memoize pure EMA/ATR helpers using their actual generated signatures.
# EMA uses DataSeries; ATR uses Bars. Keys include object identity/count + arguments,
# so values invalidate automatically on new bars. Trading rules unchanged.
...
def find_method(name, first_type):
    pat=re.compile(r'(?m)^(\s*)private\s+double\s+'+re.escape(name)+r'\s*\(\s*'+re.escape(first_type)+r'\s+(\w+)\s*,\s*int\s+(\w+)\s*,\s*int\s+(\w+)\s*\)\s*\n\s*\{')
    m=pat.search(s)
    if not m:
        decls=re.findall(r'(?m)^\s*(?:private|public|protected|internal)[^\n]*\b'+re.escape(name)+r'\s*\([^\n]*',s)
        raise SystemExit(name+' signature missing; declarations='+repr(decls[:5]))
    return m

def instrument(name, first_type, cache, hits, misses):
    global s
    m=find_method(name, first_type)
    indent,obj,period,endidx=m.group(1),m.group(2),m.group(3),m.group(4)
...
{0}    {6}++;
'''.format(indent,obj,period,endidx,cache,hits,misses)
    original_end=m.end(); s=s[:original_end]+body+s[original_end:]
...
instrument('CalculateEma','DataSeries','_stage4EmaCache','_stage4EmaHits','_stage4EmaMisses')
instrument('CalculateAtr','Bars','_stage4AtrCache','_stage4AtrHits','_stage4AtrMisses')
...
for token in ['EnableFibGrid','GeometryQualityScore','SmallAccountMinSLPips','capitalInfeasible','[PERF-STAGE4]','_stage4EmaCache','_stage4AtrCache','DataSeries','Bars']:
    if token not in s: raise SystemExit('Stage4 integrity failure: '+token)
p.write_text(s,encoding='utf-8')
print('Applied Stage4 actual-signature EMA/ATR memoization; trading rules unchanged')
