...
# v28 release audit: correct ABCD direction and prevent repeated entries
...
# 6) Reject a pattern only after it has ACTUALLY produced a successful order.
...
# 7) Mark the harmonic completion only after broker accepted the order.
...
# 8) Small bounded helper set; completion bar + pattern + direction defines one setup.
...
# 9) Add dedupe count to commercial diagnostics.
...
print('Applied v28 ABCD direction fix, signal dedupe and release audit hardening')
