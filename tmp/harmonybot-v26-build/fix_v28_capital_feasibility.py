...
# Commercial execution hardening reserves slippage plus estimated round-turn
# commission inside the same risk budget.
# v28 deadlock fix: dynamic risk reduction must not permanently block broker
# minimum volume when that minimum-volume all-in risk still fits the ORIGINAL
# configured RiskPercent and configured fallback cap.
...
for token in ['EstimateExecutionReservePips', 'allInRiskPips', 'executionReserve=', 'EstimateRequiredEquityForRisk', '[CAPITAL-FEASIBILITY]', '_diagCapitalInfeasible', '[RISK-FALLBACK]', 'smallAccountForFallback']:
    if token not in s:
...
print('Fixed CS0103 regression: fallback account class uses existing SmallAccount parameters')