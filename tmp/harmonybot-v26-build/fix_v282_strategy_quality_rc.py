...
# V28.2 Strategy Quality RC: preserve trading decisions while adding the observability
# needed to calibrate geometry quality, small-capital feasibility and Grid attribution.
# No RiskPercent inflation, pattern disable, Grid disable, or structural-SL weakening.

# Version marker is deliberately source-visible for build/artifact audits.
...
# Add quality-bucket attribution at the existing GeometryQualityScore assignment/use site.
# This is intentionally diagnostic-only: thresholds and entry decisions remain unchanged.
needle='GeometryQualityScore'
pos=s.find(needle)
if pos < 0:
    raise SystemExit('GeometryQualityScore missing')

# Add reusable diagnostic helpers before OnStop when available, otherwise before class end.
...
# Extend OnStop with an iteration-level audit line without altering order/position state.
onstop=re.search(r'(protected\s+override\s+void\s+OnStop\s*\([^)]*\)\s*\{)',s)
if nonstop:
    ...
    s=s[:nonstop.end()]+audit+s[nonstop.end():]
else:
    raise SystemExit('OnStop missing')

# Integrity gates: V28.2 must not silently change the commercial risk contract.
...
print('Preserved all harmonic families, Fibonacci Grid, structural SL and base RiskPercent')
