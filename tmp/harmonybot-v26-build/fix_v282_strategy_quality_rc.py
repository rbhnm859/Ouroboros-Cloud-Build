...
# V28.2 diagnostic-only observability. No trading/risk decision is changed.
if 'V28.2-Strategy-Quality-RC' not in s:
...
if '[V282-SUMMARY]' not in s:
    ...
    s = s[:onstop_body.end()] + audit + s[onstop_body.end():]

required = ['EnableFibGrid','GeometryQualityScore','SmallAccountMode','SmallAccountMinSLPips','RiskPercent','capitalInfeasible','MinRR','Gartley','Bat','Butterfly','Crab','Cypher','Shark','FiveZero','V28.2-Strategy-Quality-RC','[V282-SUMMARY]']
missing = [x for x in required if x not in s]
if missing:
    raise SystemExit('V28.2 integrity missing: ' + ','.join(missing))

p.write_text(s, encoding='utf-8')
print('V28.2 diagnostic patch applied and integrity validated')
