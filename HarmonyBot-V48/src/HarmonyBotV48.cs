...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV48 : Robot
    {
        private const string Version = "HarmonyBot V48 — Family-Native Harmonic Portfolio Reform Engine";
        private const string BotPrefix = "HB48";
        ...
        [Parameter("Family Structural Geometry Repair", DefaultValue = true)]
        public bool EnableFamilyStructuralGeometryRepair { get; set; }

        [Parameter("Family Grid Envelope", DefaultValue = true)]
        public bool EnableFamilyGridEnvelope { get; set; }

        [Parameter("Family Route Compatibility", DefaultValue = true)]
        public bool EnableFamilyRouteCompatibility { get; set; }

        [Parameter("Family Native Execution Reform", DefaultValue = true)]
        public bool EnableFamilyNativeExecutionReform { get; set; }

        [Parameter("Family Native Max M1 Bars", DefaultValue = 6, MinValue = 4, MaxValue = 8)]
        public int FamilyNativeMaxM1Bars { get; set; }

        [Parameter("Dormant Family Shadow Ledger", DefaultValue = true)]
        public bool EnableDormantFamilyShadowLedger { get; set; }
        ...
        private int _familyRouteRejected;
        private int _familyGridEnvelopeAccepted;
        private int _familyGridEnvelopeRejected;
        private int _familyNativePass;
        private int _familyStructuralExtensionRepairs;
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V48-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V48-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V48-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V48-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            Print("[V48-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m1Bars), _profiles.Count);
            Print("[V48-TIMEFRAME-AUDIT] primaryPattern=M15 execution=M1 macro=H4 intermediate=H1 allCompletedBars=true");
            Print("[V48-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
            Print("[V48-ALPHA-CONFIG] qualityObservation={0} legacyRegime={1} legacyEnhancedM1={2} capitalFeasibility={3} transitionVeto={4} exhaustionVeto={5} routeM1Veto={6}",
            ...
            Print("[V48-FREQUENCY-CONFIG] deferredRetention={0} agingPriority={1} ageBoost={2:F3} structuredRecall={3} recallGeometry={4:F3} recallPrz={5:F3} recallConfidence={6:F3}",
            ...
            Print("[V48-RESTORED-KERNEL] canonicalSetup={0} canonicalStandard={1} independentPivotGraph={2} transitionProof={3} m1Rescue={4} rescueBars={5} diversityScheduler={6} executionClock=M1 projectedD=false",
            ...
            Print("[V48-CONVERSION-ARCH] scaleRouteAdmission={0} temporalRescue={1} armedGrace={2} graceMinutes={3} preExecutionRevalidation={4}",
            ...
            Print("[V48-FAMILY-REFORM] structuralGeometryRepair={0} familyGridEnvelope={1} familyRouteCompatibility={2} familyNativeExecution={3} familyNativeMaxM1Bars={4} dormantShadow={5}",
                EnableFamilyStructuralGeometryRepair, EnableFamilyGridEnvelope, EnableFamilyRouteCompatibility,
                EnableFamilyNativeExecutionReform, FamilyNativeMaxM1Bars, EnableDormantFamilyShadowLedger);
            Print("[V48-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V46_SCALE_CONVERSION_FROZEN",
        ...
        protected override void OnStop()
        {
            ...
            {
                var x = kv.Value;
                Print("[V48-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} nativeTemporalPass={6} armed={7} slotBlocked={8} parked={9} revalidated={10} revalidationRejected={11} basketPlanned={12} leg0={13} leg1={14} leg2={15} leg3={16} basketClosed={17} executed={18} expired={19} rejected={20} invalidated={21}",
            ...
            Print("[V48-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V48-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
                _alphaQualityRejected, _regimeRejected, _confirmationRejected, _capitalInfeasibleCandidates, _alphaPassed);
            Print("[V48-FREQUENCY-SUMMARY] schedulerDeferred={0} schedulerRecoveredExecutions={1} structuredRecallAdmitted={2} activeDeferred={3}",
                _schedulerDeferred, _schedulerRecoveredExecutions, _structuredRecallAdmitted, _deferredCandidates.Count);
            Print("[V48-INDEPENDENT-SETUP-SUMMARY] duplicateSuppressed={0} uniqueExecuted={1} rescueAdmissions={2} transitionProofRejected={3} independentScaleCandidates={4}",
            ...
            Print("[V48-CONVERSION-SUMMARY] scaleRouteRejected={0} temporalRescueAdmissions={1} armedGraceExtended={2} preExecutionRevalidationRejected={3}",
                _scaleRouteRejected, _temporalRescueAdmissions, _armedGraceExtended, _preExecutionRevalidationRejected);
            Print("[V48-FAMILY-REFORM-SUMMARY] routeRejected={0} gridEnvelopeAccepted={1} gridEnvelopeRejected={2} nativePass={3} structuralExtensionRepairs={4}",
                _familyRouteRejected, _familyGridEnvelopeAccepted, _familyGridEnvelopeRejected, _familyNativePass, _familyStructuralExtensionRepairs);
            ...
            Print("[V48-THROUGHPUT-SUMMARY] slotBlocked={0} parked={1} revalidated={2} revalidationRejected={3} recoveredExecutions={4} nativeTemporalPass={5} hardLifetimeExpired={6} decayRejected={7} avgSlotWaitMin={8:F2} medianSlotWaitMin={9:F2} p90SlotWaitMin={10:F2} avgBasketOccupancyMin={11:F2} missedPositive={12} avoidedNegative={13}",
            ...
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V48-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                if (!FamilyRouteCompatible(signal, record.Route, record.Regime))
                {
                    _familyRouteRejected++;
                    Reject(record, "FAMILY_ROUTE_INCOMPATIBLE");
                    continue;
                }

                // V48 expands only genuinely independent secondary-scale setups.
                ...
                Print("[V48-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    bool confirmationPass = legacyScore >= legacyRequired;

                    if (!confirmationPass && EnableFamilyNativeExecutionReform && IsDormantStandardFamily(c.Signal.PatternName))
                    {
                        c.FamilyNativeBarsObserved++;
                        double familyScore;
                        if (c.FamilyNativeBarsObserved <= Math.Max(4, Math.Min(8, FamilyNativeMaxM1Bars)) &&
                            UpdateFamilyNativeM1Evidence(i, c, out familyScore))
                        {
                            ...
                            Event(c, "FAMILY_NATIVE_M1_PASS_" + familyScore.ToString("F2", CultureInfo.InvariantCulture));
                        }
                    }

                    if (!confirmationPass && EnablePatternNativeM1Expansion)
                    {
        ...
        private void MarkOpportunityTerminal(CandidateRecord c, string reason)
        {
            ...
            Print("[V48-OPPORTUNITY-LOSS] cid={0} setup={1} pattern={2} route={3} scale={4} reason={5} shadowMfeR={6:F3} shadowMaeR={7:F3} missedPositive={8} avoidedNegative={9}",
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            double legalMinSpanXa, legalMaxSpanXa;
            FamilyGridSpanEnvelope(p, out legalMinSpanXa, out legalMaxSpanXa);
            if (spanXa < legalMinSpanXa || spanXa > legalMaxSpanXa)
            {
                if (EnableFamilyGridEnvelope && p.Mode == PatternMode.STANDARD) _familyGridEnvelopeRejected++;
                return false;
            }
            if (EnableFamilyGridEnvelope && p.Mode == PatternMode.STANDARD) _familyGridEnvelopeAccepted++;
            ...
            Print("[V48-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V48-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V48-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V48-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V48-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V48-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V48-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V48-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V48-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V48-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V48-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V48-BASKET-CLOSED] basket={0} cid={1} setup={2} pattern={3} subtype={4} route={5} dir={6} plannedLegs={7} filledLegs={8} anchor={9} avgEntry={10} entryImprovePips={11:F3} stop={12} target={13} initialRisk={14:F2} worstRisk={15:F2} mfeR={16:F3} maeR={17:F3} realizedR={18:F3} net={19:F2} reason={20}",
            ...
            Print("[V48-SLOT-OCCUPANCY] basket={0} cid={1} pattern={2} route={3} occupancyMinutes={4:F2} realizedR={5:F3} net={6:F2}",
        ...
        {
            Print("[V48-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
        ...
        private string PatternFamily(string pattern)
        {
            if (pattern == "Gartley" || pattern == "Bat" || pattern == "Deep Gartley" || pattern == "Rat") return "RETRACEMENT";
            if (pattern == "Alt Bat" || pattern == "Butterfly" || pattern == "Crab" || pattern == "Deep Crab") return "EXTENSION";
            if (pattern == "Shark" || pattern == "5-0") return "TRANSITION";
            if (pattern == "Cypher") return "XC_RETRACE";
            if (pattern == "AB=CD") return "COMPLETION_SYMMETRY";
            return "OTHER";
        }

        private bool IsDormantStandardFamily(string pattern)
        {
            string f = PatternFamily(pattern);
            return f == "RETRACEMENT" || f == "EXTENSION";
        }

        private void FamilyGridSpanEnvelope(PatternProfile p, out double minSpanXa, out double maxSpanXa)
        {
            minSpanXa = p.MinimumGridSpanXa;
            maxSpanXa = p.MaximumGridSpanXa;
            if (!EnableFamilyGridEnvelope || p == null || p.Mode != PatternMode.STANDARD) return;

            double b = Math.Max(.010, p.StructuralStopFibBuffer);
            if (p.XadMax <= 1.0)
            {
                minSpanXa = Math.Max(.015, (1.0 - p.XadMax) + b * .50);
                maxSpanXa = Math.Min(.75, Math.Max(minSpanXa + .04, (1.0 - p.XadMin) + b * 2.0));
            }
            else
            {
                minSpanXa = Math.Max(.015, b * .50);
                maxSpanXa = Math.Min(.75, Math.Max(minSpanXa + .04, (p.XadMax - p.XadMin) + b * 2.0));
            }
        }

        private bool FamilyRouteCompatible(PatternSignal sig, HarmonicRoute route, RegimeSnapshot r)
        {
            if (!EnableFamilyRouteCompatibility || sig == null) return true;
            string f = PatternFamily(sig.PatternName);

            if (sig.PatternName == "AB=CD" || sig.PatternName == "Shark" || sig.PatternName == "Cypher")
                return true;

            if (f == "RETRACEMENT")
            {
                if (route == HarmonicRoute.TREND_ALIGNED_REVERSAL) return true;
                if (route == HarmonicRoute.TRANSITION_REVERSAL) return r != null && r.Transition;
                if (route == HarmonicRoute.EXHAUSTION_REVERSAL)
                    return r != null && r.ExtensionAtr >= 1.50 && sig.GeometryQuality >= .72 && sig.PrzConfluence >= .68;
                return false;
            }

            if (f == "EXTENSION")
            {
                if (route == HarmonicRoute.EXHAUSTION_REVERSAL) return true;
                if (route == HarmonicRoute.TRANSITION_REVERSAL) return r != null && r.Transition;
                return false;
            }

            if (sig.PatternName == "5-0")
        ...
        private double PatternStructuralInvalidation(PatternProfile p, PivotPoint x, PivotPoint a, PivotPoint b, PivotPoint c, PivotPoint d, bool bullish)
        {
            ...
            {
                if (!EnableFamilyStructuralGeometryRepair)
                {
                    if (p.XadMax <= 1.0)
                        return bullish ? x.Price - buffer : x.Price + buffer;

                    double legacyExtreme = bullish ? x.Price - p.XadMax * xa : x.Price + p.XadMax * xa;
                    return bullish ? legacyExtreme - buffer : legacyExtreme + buffer;
                }

                if (p.XadMax <= 1.0)
                    return bullish ? x.Price - buffer : x.Price + buffer;

                double deepestD = a.Price + p.XadMax * (x.Price - a.Price);
                _familyStructuralExtensionRepairs++;
                return bullish ? deepestD - buffer : deepestD + buffer;
            }
        ...
        private HarmonicRoute RouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (baseRoute == HarmonicRoute.NO_TRADE && EnableStructuredRecallExpansion)
            {
                ...
                if (baseRoute != HarmonicRoute.NO_TRADE)
                {
                    ...
                    Print("[V48-RECALL-ADMIT] pattern={0} direction={1} route={2} conflict={3} geometry={4:F3} prz={5:F3} confidence={6:F3} efficiency={7:F3} atrRatio={8:F3} extensionAtr={9:F3}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionProofGate)
            {
                ...
                if (!actualStructuralTransition)
                {
                    ...
                    Print("[V48-TRANSITION-PROOF-REJECT] pattern={0} conflict={1} transition={2} adxSlope={3:F2}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionStateVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V48-ROUTE-VETO] type=TRANSITION_STATE conflict={0} transition={1} adxSlope={2:F2}",
            ...
            if (baseRoute == HarmonicRoute.EXHAUSTION_REVERSAL && EnableExhaustionEvidenceVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V48-ROUTE-VETO] type=EXHAUSTION_EVIDENCE extensionAtr={0:F3} adxSlope={1:F2}",
        ...
        private bool UpdateFamilyNativeM1Evidence(int i, CandidateRecord c, out double score)
        {
            score = 0;
            ...
            double ph2 = Math.Max(ph, _m1Bars.HighPrices[i - 2]);
            double pl2 = Math.Min(pl, _m1Bars.LowPrices[i - 2]);
            double body = Math.Max(Math.Abs(cl - o), _symbol.PipSize);
            double atr = Atr(_m1Bars, 14, i);
            bool bullish = sig.Direction == TradeDirection.Buy;
            bool directional = bullish ? cl > o : cl < o;
            bool reclaim = bullish ? (cl > sig.PrzLow && cl >= pc) : (cl < sig.PrzHigh && cl <= pc);
            bool bos1 = bullish ? cl > ph : cl < pl;
            bool bos2 = bullish ? cl > ph2 : cl < pl2;
            bool rejection = bullish ? Math.Max(0, Math.Min(o, cl) - l) >= body * .45 : Math.Max(0, h - Math.Max(o, cl)) >= body * .45;
            ...
            c.FamilySawDirectional |= directional;
            c.FamilySawReclaim |= reclaim;
            c.FamilySawBos1 |= bos1;
            c.FamilySawBos2 |= bos2;
            c.FamilySawRejection |= rejection;
            c.FamilySawFailedExtension |= failedExtension;
            c.FamilySawSweep |= sweep;
            c.FamilySawInsidePrz |= insidePrz;
            c.FamilySawDisplacement |= displacement;

            string family = PatternFamily(sig.PatternName);
            if (family == "RETRACEMENT")
            {
                score = (c.FamilySawRejection ? .20 : 0) + (c.FamilySawReclaim ? .25 : 0) +
                        (c.FamilySawBos1 ? .25 : 0) + (c.FamilySawDirectional ? .10 : 0) +
                        (c.FamilySawDisplacement ? .20 : 0);
                return c.FamilyNativeBarsObserved >= 2 && c.FamilySawReclaim &&
                       (c.FamilySawRejection || c.FamilySawFailedExtension) &&
                       c.FamilySawBos1 && (c.FamilySawDirectional || c.FamilySawDisplacement) && score >= .70;
            }
            if (family == "EXTENSION")
            {
                score = (c.FamilySawSweep ? .20 : 0) + (c.FamilySawFailedExtension ? .25 : 0) +
                        ((c.FamilySawInsidePrz || c.FamilySawReclaim) ? .20 : 0) +
                        (c.FamilySawBos1 ? .25 : 0) + (c.FamilySawDisplacement ? .10 : 0);
                return c.FamilyNativeBarsObserved >= 2 && (c.FamilySawSweep || c.FamilySawFailedExtension) &&
                       c.FamilySawFailedExtension && (c.FamilySawInsidePrz || c.FamilySawReclaim) &&
                       c.FamilySawBos1 && score >= .70;
            }
            return false;
        }
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V48-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        {
            _candidateSeq++;
            return "V48-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private void Ledger(CandidateRecord c, CandidateState state, string reason)
        {
            ...
            Print("[V48-EVENT] cid={0} setup={1} pattern={2} subtype={3} scale={4} tf={5} dir={6} state={7} route={8} conflict={9} waitMin={10:F2} reason={11}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V48-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V48-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
    ...
    public sealed class CandidateRecord
    {
        ...
        public int FamilyNativeBarsObserved;
        public bool FamilySawDirectional, FamilySawReclaim, FamilySawBos1, FamilySawBos2, FamilySawRejection, FamilySawFailedExtension, FamilySawSweep, FamilySawInsidePrz, FamilySawDisplacement;
