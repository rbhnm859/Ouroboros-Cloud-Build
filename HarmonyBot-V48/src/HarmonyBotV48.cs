...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V48 — Alpha-Preserving Throughput & Serial Opportunity Engine";
        ...
        [Parameter("Retracement Family Activation", DefaultValue = false)]
        public bool EnableRetracementFamilyActivation { get; set; }

        [Parameter("Extension Family Activation", DefaultValue = false)]
        public bool EnableExtensionFamilyActivation { get; set; }

        [Parameter("Dormant Family Evidence Bars", DefaultValue = 6, MinValue = 6, MaxValue = 6)]
        public int DormantFamilyEvidenceBars { get; set; }
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (!confirmationPass && DormantFamilyActivationEnabled(c.Signal.PatternName))
                    {
                        c.NativeM1BarsObserved++;
                        double familyScore;
                        if (c.NativeM1BarsObserved <= DormantFamilyEvidenceBars &&
                            UpdateDormantFamilyEvidence(i, c, out familyScore))
                        {
                            c.ConfirmationScore = Math.Max(c.ConfirmationScore, familyScore);
                            confirmationPass = true;
                            _nativeTemporalPass++;
                            CountPipeline(c.Signal.PatternName).NativeTemporalPass++;
                            Event(c, "DORMANT_FAMILY_ACTIVATED_" + familyScore.ToString("F2", CultureInfo.InvariantCulture));
                        }
                    }
        ...
        private bool DormantFamilyActivationEnabled(string pattern)
        {
            if (string.IsNullOrWhiteSpace(pattern)) return false;
            bool retracement = pattern == "Gartley" || pattern == "Bat" || pattern == "Deep Gartley" || pattern == "Rat";
            bool extension = pattern == "Alt Bat" || pattern == "Butterfly" || pattern == "Crab" || pattern == "Deep Crab";
            return (retracement && EnableRetracementFamilyActivation) || (extension && EnableExtensionFamilyActivation);
        }

        // V48 does not lower the legacy M1 threshold. It gives historically dormant, canonically
        // detected families a separate completed-M1 evidence contract. Evidence may accumulate
        // across six closed M1 bars, but the terminal bar must still move in the pattern direction.
        private bool UpdateDormantFamilyEvidence(int i, CandidateRecord c, out double score)
        {
            score = 0;
            if (i < 3 || c == null || c.Signal == null) return false;
            var sig = c.Signal;
            ...
            bool rejection = buy ? Math.Max(0, Math.Min(o, cl) - l) >= body * .5
                                 : Math.Max(0, h - Math.Max(o, cl)) >= body * .5;
            ...
            if (retracement)
            {
                score = (c.TemporalRejection ? .20 : 0) + (c.TemporalReclaim ? .20 : 0) +
                        (c.TemporalBos1 ? .20 : 0) + (c.TemporalDirectional ? .15 : 0) +
                        (c.TemporalFailedExtension ? .15 : 0) + (c.TemporalDisplacement ? .10 : 0);
                return directional && c.TemporalRejection && c.TemporalReclaim &&
                       (c.TemporalBos1 || c.TemporalFailedExtension) && score >= .70;
            }
            if (extension)
            {
                c.TemporalFailedExtension |= sweep && failedExtension;
                score = (c.TemporalFailedExtension ? .25 : 0) + (c.TemporalRejection ? .20 : 0) +
                        (c.TemporalReclaim ? .20 : 0) + (c.TemporalBos1 ? .15 : 0) +
                        (c.TemporalDirectional ? .10 : 0) + (c.TemporalDisplacement ? .10 : 0);
                return directional && c.TemporalFailedExtension && c.TemporalRejection &&
                       c.TemporalReclaim && (c.TemporalBos1 || c.TemporalDisplacement) && score >= .75;
            }
            return false;
        }
