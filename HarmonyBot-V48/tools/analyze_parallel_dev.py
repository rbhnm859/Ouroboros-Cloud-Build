...
fams=["V46_SCALE_CONTROL","RETRACEMENT_ACTIVATION","EXTENSION_ACTIVATION","FULL_V48_FAMILY_PORTFOLIO"]
...
(out/"V48_PATTERN_FUNNEL.json").write_text(json.dumps({"version":"HarmonyBot V48","by_pattern_route_scale":funnel_report},indent=2))
...
(out/"V48_SLOT_OCCUPANCY_REPORT.json").write_text(json.dumps({"version":"HarmonyBot V48","families":slot_report},indent=2))
...
(out/"V48_ARMED_RETRACEMENT_ACTIVATION.json").write_text(json.dumps({"version":"HarmonyBot V48","families":queue},indent=2))
(out/"V48_OPPORTUNITY_LOSS_LEDGER.json").write_text(json.dumps({"version":"HarmonyBot V48","families":opportunity},indent=2))
...
(out/"V48_PATTERN_ROUTE_SCALE_ATTRIBUTION.json").write_text(json.dumps({"version":"HarmonyBot V48","families":attrib},indent=2))

marginal_report={f:A[f]["marginal_vs_control"] for f in fams if f!=control}
(out/"V48_MARGINAL_FREQUENCY_REPORT.json").write_text(json.dumps({"version":"HarmonyBot V48","control":control,"families":marginal_report},indent=2))
...
frontier={"version":"HarmonyBot V48","architecture":"HARMONIC_FAMILY_SPECIALIZATION_DORMANT_ALPHA",
...
(out/"V48_CAPITAL_COMPATIBILITY.json").write_text(json.dumps({"version":"HarmonyBot V48","status":"PENDING" if winner else "NOT_RUN_GATE_BLOCKED","candidate":winner},indent=2))
if not winner:
 (out/"V48_FINAL_COMMERCIAL_DECISION.json").write_text(json.dumps({"version":"HarmonyBot V48","decision":"HOLD_WITH_EVIDENCE",
