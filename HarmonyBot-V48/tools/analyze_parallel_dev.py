...
F=["V46_SCALE_CONTROL","FAMILY_NATIVE_OBSERVATION","FAMILY_NATIVE_CONVERSION","FULL_V48_COMMERCIAL"]
...
front={"version":"HarmonyBot V48","architecture":"PATTERN_FAMILY_NATIVE_CONVERSION","baseline":BASE,"v36":V36,"commercial_minimum":COMM,"baseline_reproduction_pass":c["baseline_reproduction_pass"],"families":A,"development_candidate":winner,"status":"COMMERCIAL_FREEZE_CANDIDATE_DEV_PASS" if winner else "HOLD_WITH_EVIDENCE","fresh_used":False}
(out/"V48_PERFORMANCE_FRONTIER.json").write_text(json.dumps(front,indent=2)); (out/"candidate.txt").write_text(winner or "")
(out/"V48_PATTERN_FAMILY_EFFECTIVENESS.json").write_text(json.dumps({f:A[f]["pattern_economics"] for f in F},indent=2))
(out/"V48_FINAL_COMMERCIAL_DECISION.json").write_text(json.dumps({"decision":"COMMERCIAL_FREEZE_CANDIDATE_PASS" if winner else "HOLD_WITH_EVIDENCE","stage":"DEV","fresh_used":False},indent=2))
