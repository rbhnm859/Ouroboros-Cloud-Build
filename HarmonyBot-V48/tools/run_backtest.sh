...
REVALIDATE="${REVALIDATE:-false}"; RETRACT="${RETRACT:-false}"; EXTEND="${EXTEND:-false}"
ALGO="${ALGO:-seal/algo/HarmonyBot_V48_Alpha_Preserving_Throughput_Serial_Opportunity_RC.algo}"
...
test "$DONE" = 1 || { echo "[V48-WATCHDOG-FAIL] run=$RUN_NAME"; tail -400 "seal/logs/$RUN_NAME.log" || true; exit 20; }
echo "[V48-WATCHDOG] run=$RUN_NAME status=complete"
