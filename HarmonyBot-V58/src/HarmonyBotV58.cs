...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV58 : Robot
    {
        ...
        [Parameter("V58 Family Books", DefaultValue = true)]
        public bool EnableV58FamilyBooks { get; set; }

        [Parameter("V58 Canonical Manifold", DefaultValue = true)]
        public bool EnableV58CanonicalManifold { get; set; }

        [Parameter("V58 Pure Projected PRZ", DefaultValue = true)]
        public bool EnableV58PureProjectedPrz { get; set; }

        [Parameter("V58 Temporal Event DAG", DefaultValue = true)]
        public bool EnableV58TemporalEventDag { get; set; }
        ...
        [Parameter("V58 Max Projection Error ATR", DefaultValue = 0.55, MinValue = 0.20, MaxValue = 1.50)]
        public double V58MaxProjectionErrorAtr { get; set; }

        [Parameter("V58 Manifold Sigma", DefaultValue = 1.00, MinValue = 0.50, MaxValue = 2.50)]
        public double V58ManifoldSigma { get; set; }

        [Parameter("V58 Universal Research Continuation", DefaultValue = true)]
        public bool EnableV58UniversalResearchContinuation { get; set; }

        [Parameter("V58 Virtual Parallel Execution", DefaultValue = true)]
        public bool EnableV58VirtualParallelExecution { get; set; }

        [Parameter("V58 Calibrated Capital Admission", DefaultValue = false)]
        public bool EnableV58CalibratedCapitalAdmission { get; set; }

        [Parameter("V58 Family Representative Arbitration", DefaultValue = true)]
        public bool EnableV58FamilyRepresentativeArbitration { get; set; }

        [Parameter("V58 Standalone AB=CD Capital", DefaultValue = false)]
        public bool EnableV58AbcdStandaloneCapital { get; set; }

        [Parameter("V58 Grid Execution", DefaultValue = true)]
        public bool EnableV58GridExecution { get; set; }

        [Parameter("V58 Min Calibrated Lower R", DefaultValue = 0.0, MinValue = -1.0, MaxValue = 1.0)]
        public double V58MinCalibratedLowerBoundR { get; set; }

        [Parameter("V58 Calibration Manifest", DefaultValue = "")]
        public string V58CalibrationManifest { get; set; }
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V58-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V58-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V58-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V58-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            InitializeV58FamilyBooks();
            ParseV58CalibrationManifest();
            Print("[V58-FAMILY-PORTFOLIO] familyBooks={0} virtualParallel={1} calibratedAdmission={2} representatives={3} abcdStandalone={4} calibrationCells={5}",
                EnableV58FamilyBooks, EnableV58VirtualParallelExecution, EnableV58CalibratedCapitalAdmission,
                EnableV58FamilyRepresentativeArbitration, EnableV58AbcdStandaloneCapital, _calibrationCells.Count);
            ...
            Print("[V58-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m1Bars), _profiles.Count);
            Print("[V58-TIMEFRAME-AUDIT] primaryPattern=M15 execution=M1 macro=H4 intermediate=H1 allCompletedBars=true");
            Print("[V58-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
            Print("[V58-ALPHA-CONFIG] qualityObservation={0} legacyRegime={1} legacyEnhancedM1={2} capitalFeasibility={3} transitionVeto={4} exhaustionVeto={5} routeM1Veto={6}",
            ...
            Print("[V58-FREQUENCY-CONFIG] deferredRetention={0} agingPriority={1} ageBoost={2:F3} structuredRecall={3} recallGeometry={4:F3} recallPrz={5:F3} recallConfidence={6:F3}",
            ...
            Print("[V58-RESTORED-KERNEL] canonicalSetup={0} canonicalStandard={1} independentPivotGraph={2} transitionProof={3} m1Rescue={4} rescueBars={5} diversityScheduler={6} executionClock=M1 projectedD=false",
            ...
            Print("[V58-CONVERSION-ARCH] scaleRouteAdmission={0} temporalRescue={1} armedGrace={2} graceMinutes={3} preExecutionRevalidation={4}",
            ...
            Print("[V58-FAMILY-NATIVE] conversion={0} observation={1} principle=PATTERN_IDENTITY_NEQ_EXECUTION_IDENTITY", EnableFamilyNativeConversion, EnableFamilyNativeObservation);
            Print("[V58-COMPLETION-CONTRACT] canonicalContracts={0} familyCompletion={1} windowBars={2} provenLane=ABCD_SHARK_CYPHER_FROZEN",
                EnableCanonicalFamilyContracts, EnableFamilyCompletionContract, FamilyConfirmationWindowBars);
            Print("[V58-STRUCTURAL-GRID-CONTRACT] gridSpanV2={0} structuralInvalidationV2={1} rule=PRZ_LEGALITY_PLUS_RISK_RR_NOT_LEGACY_XA_MINSPAN",
                EnableGridSpanSemanticV2, EnableStructuralInvalidationV2);
            Print("[V58-MATH-GEOMETRY] jointGeometry={0} executionCorridor={1} entryAnchorForensics={2} rule=FAMILY_NATIVE_GEOMETRY_WITHOUT_RISK_RELAXATION",
                EnableFamilyNativeJointGeometry, EnableFamilyNativeExecutionCorridor, EnableEntryAnchorForensics);
            Print("[V58-DETECTOR-ARCH] identityReconstruction={0} boundedPivotGraph={1} maxMicroSkips={2} familyQuota={3} projectedPrz={4} truthLedger={5} rule=DETECT_MANY_EXECUTE_ONE",
            ...
            Print("[V58-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V46_SCALE_CONVERSION_FROZEN",
        ...
        {
            CloseOpenV58ShadowTradesAtStop();
            foreach (var book in _familyBooks.OrderBy(x => x.Key))
            {
                ...
                Print("[V58-FAMILY-BOOK] pattern={0} candidates={1} shadowStarted={2} shadowClosed={3}", book.Key, book.Value.Count, started, closed);
            }
            ...
            {
                var x = kv.Value;
                Print("[V58-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} nativeTemporalPass={6} armed={7} slotBlocked={8} parked={9} revalidated={10} revalidationRejected={11} basketPlanned={12} leg0={13} leg1={14} leg2={15} leg3={16} basketClosed={17} executed={18} expired={19} rejected={20} invalidated={21}",
            ...
            if (EnableEntryAnchorForensics)
            {
                ...
                {
                    Print("[V58-ENTRY-ANCHOR-FORENSICS] cid={0} setup={1} pattern={2} route={3} scale={4} completion={5} confirm={6} retest={7} completionMfeR={8:F3} completionMaeR={9:F3} confirmMfeR={10:F3} confirmMaeR={11:F3} retestMfeR={12:F3} retestMaeR={13:F3}",
            ...
            if (EnableDetectorTruthLedger)
            {
                foreach (var kv in _detectorTruth.OrderBy(x => x.Key))
                {
                    ...
                    Print("[V58-DETECTOR-TRUTH] pattern={0} stage={1} count={2}", pattern, stage, kv.Value);
                }
            }
            Print("[V58-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V58-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
                _alphaQualityRejected, _regimeRejected, _confirmationRejected, _capitalInfeasibleCandidates, _alphaPassed);
            Print("[V58-FREQUENCY-SUMMARY] schedulerDeferred={0} schedulerRecoveredExecutions={1} structuredRecallAdmitted={2} activeDeferred={3}",
                _schedulerDeferred, _schedulerRecoveredExecutions, _structuredRecallAdmitted, _deferredCandidates.Count);
            Print("[V58-INDEPENDENT-SETUP-SUMMARY] duplicateSuppressed={0} uniqueExecuted={1} rescueAdmissions={2} transitionProofRejected={3} independentScaleCandidates={4}",
            ...
            Print("[V58-CONVERSION-SUMMARY] scaleRouteRejected={0} temporalRescueAdmissions={1} armedGraceExtended={2} preExecutionRevalidationRejected={3}",
            ...
            Print("[V58-THROUGHPUT-SUMMARY] slotBlocked={0} parked={1} revalidated={2} revalidationRejected={3} recoveredExecutions={4} nativeTemporalPass={5} hardLifetimeExpired={6} decayRejected={7} avgSlotWaitMin={8:F2} medianSlotWaitMin={9:F2} p90SlotWaitMin={10:F2} avgBasketOccupancyMin={11:F2} missedPositive={12} avoidedNegative={13}",
            ...
            foreach (var p in _profiles.Select(x => x.Name).Distinct().OrderBy(x => x))
            {
                ...
                Print("[V58-FAMILY-CONTRACT-SUMMARY] pattern={0} pass={1} windowReject={2}", p, pass, reject);
            }
            foreach (var kv in _gridPlanRejectReasons.OrderBy(k => k.Key))
                Print("[V58-GRID-REJECT-SUMMARY] reason={0} count={1}", kv.Key, kv.Value);
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V58-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                {
                    if (EnableV58UniversalResearchContinuation && EnableV58VirtualParallelExecution)
                    {
                        record.ResearchOnlyHypothesis = true;
                        Event(record, "V58_RESEARCH_CONTINUE_PATTERN_QUALITY");
                    }
                    else
                    {
                ...
                if (record.Route == HarmonicRoute.NO_TRADE)
                {
                    ...
                    {
                        record.ResearchOnlyHypothesis = true;
                        record.Route = V58ResearchRoute(signal, record.Conflict, regime);
                        if (record.Route == HarmonicRoute.NO_TRADE)
                            record.Route = HarmonicRoute.TRANSITION_REVERSAL;
                        Event(record, "V58_RESEARCH_CONTINUE_ROUTER_NO_TRADE");
                    }
                    else
                    {
                ...
                Print("[V58-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void ProcessNewM1Close()
        {
            ...
            UpdateV58FamilyShadowTrades(i, utc);
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    StartV58FamilyShadowTrade(c, i, utc);
                    ...
                    if (EnableV58CalibratedCapitalAdmission && !V58CalibratedCapitalEligible(c))
                    {
        ...
        private void TryScheduleAndExecute()
        {
            ...
            if (EnableV58FamilyRepresentativeArbitration)
            {
                armedQuery = armedQuery
                    .GroupBy(c => c.Signal == null ? "UNKNOWN" : c.Signal.PatternName)
                    .Select(g => g.OrderByDescending(c => V58FamilyCapitalScore(c, now)).ThenByDescending(c => c.Rank).First());
            }

            var armed = armedQuery
                .OrderByDescending(c => EnableV58FamilyRepresentativeArbitration ? V58FamilyCapitalScore(c, now) :
        ...
        private void MarkOpportunityTerminal(CandidateRecord c, string reason)
        {
            ...
            Print("[V58-OPPORTUNITY-LOSS] cid={0} setup={1} pattern={2} route={3} scale={4} reason={5} shadowMfeR={6:F3} shadowMaeR={7:F3} missedPositive={8} avoidedNegative={9}",
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            int routeMax = !EnableV58GridExecution ? 1 :
            ...
            Print("[V58-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V58-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V58-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V58-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V58-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V58-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V58-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V58-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V58-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V58-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V58-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V58-BASKET-CLOSED] basket={0} cid={1} setup={2} pattern={3} subtype={4} route={5} dir={6} plannedLegs={7} filledLegs={8} anchor={9} avgEntry={10} entryImprovePips={11:F3} stop={12} target={13} initialRisk={14:F2} worstRisk={15:F2} mfeR={16:F3} maeR={17:F3} realizedR={18:F3} net={19:F2} reason={20}",
            ...
            Print("[V58-SLOT-OCCUPANCY] basket={0} cid={1} pattern={2} route={3} occupancyMinutes={4:F2} realizedR={5:F3} net={6:F2}",
        ...
        {
            Print("[V58-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
            ...
            double atr, DateTime completion, string timeframe, int pivotScale, out PatternSignal signal)
        {
            ...
            if (p.Mode == PatternMode.ABCD)
                ratioOk = V58AbcdDiscreteIdentity(abc, bcd, abcd);
            ...
            double geometry = EnableV58CanonicalManifold
                ? V58CanonicalManifoldScore(p, xab, abc, bcd, xad, abcd, xac, out manifoldDistance)
            ...
            double expectedD = EnableV58PureProjectedPrz
                ? V58PureProjectedPrzCenter(p, x, a, b, c, bullish, out projectionSpread)
            ...
            if (!EnableV58PureProjectedPrz && !(EnableFamilyIdentityReconstruction && EnableFamilyNativeProjectedPrz)) projectionSpread = 0;
            ...
            double projectionPurity = VClamp(Math.Exp(-0.5 * Math.Pow(projectionErrorAtr / Math.Max(.15, V58MaxProjectionErrorAtr), 2.0)));
            ...
            double przConfluence = EnableV58PureProjectedPrz
        ...
        private double[] V58AbcdCenters(string pattern)
        {
        ...
        private double V58NearestCanonical(double value, double[] centers)
        {
            return centers.OrderBy(v => Math.Abs(v - value)).First();
        }

        private bool V58AbcdDiscreteIdentity(double abc, double bcd, double abcd)
        {
            if (!InRange(abc, .382, .886) || !InRange(bcd, 1.13, 2.618)) return false;
            double expected = V58NearestCanonical(1.0 / Math.Max(.30, abc), new[] { 1.0, 1.272, 1.618, 2.0, 2.24, 2.618 });
            bool reciprocal = Math.Abs(bcd - expected) <= Math.Max(.12, expected * .12);
            bool completion = V58AbcdCenters("AB=CD").Any(x => Math.Abs(abcd - x) <= Math.Max(.06, x * .06));
            return reciprocal && completion;
        }

        private double V58PureProjectedPrzCenter(PatternProfile p, PivotPoint x, PivotPoint a, PivotPoint b, PivotPoint c,
            bool bullish, out double halfSpread)
        {
            ...
            if (p.Mode == PatternMode.STANDARD)
            {
                ...
                double bestAbcd = V58AbcdCenters(p.Name).Select(k => c.Price + sign * k * ab).OrderBy(v => Math.Abs(v - anchor)).First();
                projections.Add(dXa); projections.Add(dBc); projections.Add(bestAbcd);
            }
            else if (p.Mode == PatternMode.ABCD)
            {
                double desired = V58NearestCanonical(1.0 / Math.Max(.30, bc / ab), V58AbcdCenters("AB=CD"));
                projections.Add(c.Price + sign * desired * ab);
            }
            else if (p.Mode == PatternMode.CYPHER)
        ...
        {
            return V58NearestCanonical(observed, V58AbcdCenters(pattern));
        }
        ...
        private double FamilyProjectedPrzCenter(PatternProfile p, PivotPoint x, PivotPoint a, PivotPoint b, PivotPoint c, PivotPoint d,
            bool bullish, out double halfSpread)
        {
            return V58PureProjectedPrzCenter(p, x, a, b, c, bullish, out halfSpread);
        }
        ...
        private double V58CanonicalManifoldScore(PatternProfile p, double xab, double abc, double bcd, double xad, double abcd, double xac, out double distance)
        {
            var z = new List<double>();
            if (p.Mode == PatternMode.STANDARD)
            {
                ...
                z.Add(RangeCoordinate(xad, p.XadMin, p.XadMax));
                z.Add(CanonicalAbcdCoordinate(p.Name, abcd));
                double reciprocalTarget = V58NearestCanonical(1.0 / Math.Max(.30, abc), new[] { 1.13, 1.272, 1.414, 1.618, 2.0, 2.24, 2.618, 3.618 });
                z.Add((bcd - reciprocalTarget) / Math.Max(.10, reciprocalTarget * .12));
            }
            else if (p.Mode == PatternMode.ABCD)
            {
                double reciprocalTarget = V58NearestCanonical(1.0 / Math.Max(.30, abc), new[] { 1.0, 1.272, 1.618, 2.0, 2.24, 2.618 });
                z.Add((bcd - reciprocalTarget) / Math.Max(.10, reciprocalTarget * .12));
                z.Add(V58AbcdCenters("AB=CD").Min(k => Math.Abs(abcd - k) / Math.Max(.05, k * .06)));
            }
            else if (p.Mode == PatternMode.CYPHER)
            {
                z.Add(RangeCoordinate(xab, .382, .618));
            ...
            double sigma=Math.Max(.50,V58ManifoldSigma);
        ...
        private HarmonicRoute RouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (baseRoute == HarmonicRoute.NO_TRADE && EnableStructuredRecallExpansion)
            {
                ...
                if (baseRoute != HarmonicRoute.NO_TRADE)
                {
                    ...
                    Print("[V58-RECALL-ADMIT] pattern={0} direction={1} route={2} conflict={3} geometry={4:F3} prz={5:F3} confidence={6:F3} efficiency={7:F3} atrRatio={8:F3} extensionAtr={9:F3}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionProofGate)
            {
                ...
                if (!actualStructuralTransition)
                {
                    ...
                    Print("[V58-TRANSITION-PROOF-REJECT] pattern={0} conflict={1} transition={2} adxSlope={3:F2}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionStateVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V58-ROUTE-VETO] type=TRANSITION_STATE conflict={0} transition={1} adxSlope={2:F2}",
            ...
            if (baseRoute == HarmonicRoute.EXHAUSTION_REVERSAL && EnableExhaustionEvidenceVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V58-ROUTE-VETO] type=EXHAUSTION_EVIDENCE extensionAtr={0:F3} adxSlope={1:F2}",
        ...
        private void InitializeV58FamilyBooks()
        {
            if (!EnableV58FamilyBooks) return;
            foreach (var p in _profiles)
                if (!_familyBooks.ContainsKey(p.Name)) _familyBooks[p.Name]=new HashSet<string>();
        }

        private void AddToV58FamilyBook(CandidateRecord c)
        {
            if (!EnableV58FamilyBooks || c==null || c.Signal==null) return;
            HashSet<string> book;
        ...
        private string V58PatternToken(string pattern)
        {
            return (pattern??"UNKNOWN").Replace(" ","_");
        }

        private string V58RegimeBucket(CandidateRecord c)
        {
        ...
        private HarmonicRoute V58ResearchRoute(PatternSignal s,MtfConflict conflict,RegimeSnapshot r)
        {
            if(s==null||r==null)return HarmonicRoute.NO_TRADE;
            string p=s.PatternName??"";
            bool ext=p=="Alt Bat"||p=="Butterfly"||p=="Crab"||p=="Deep Crab";
        ...
        private string V58CalibrationKey(CandidateRecord c)
        {
            if(c==null||c.Signal==null)return "UNKNOWN";
            return V58PatternToken(c.Signal.PatternName)+"|"+c.Route+"|"+V58RegimeBucket(c);
        }

        private void ParseV58CalibrationManifest()
        {
            _calibrationCells.Clear();
            if(string.IsNullOrWhiteSpace(V58CalibrationManifest))return;
            foreach(var raw in V58CalibrationManifest.Split(new[]{';'},StringSplitOptions.RemoveEmptyEntries))
            {
        ...
        private CalibrationCell V58CalibrationCell(CandidateRecord c)
        {
            CalibrationCell x;
            return _calibrationCells.TryGetValue(V58CalibrationKey(c),out x)?x:null;
        }

        private bool V58CalibratedCapitalEligible(CandidateRecord c)
        {
            if(c==null||c.Signal==null)return false;
            if(c.ResearchOnlyHypothesis)return false;
            if(string.Equals(c.Signal.PatternName,"AB=CD",StringComparison.OrdinalIgnoreCase)&&!EnableV58AbcdStandaloneCapital)
                return false;
            var cell=V58CalibrationCell(c);
            ...
            return cell.LowerBoundR>V58MinCalibratedLowerBoundR && cell.Samples>0 && c.CalibratedRPerSlotHour>0;
        }

        private double V58FamilyCapitalScore(CandidateRecord c, DateTime now)
        {
            var cell=V58CalibrationCell(c);
            if(cell!=null)
            {
        ...
        private void StartV58FamilyShadowTrade(CandidateRecord c,int i,DateTime utc)
        {
            if(!EnableV58VirtualParallelExecution||c==null||c.Signal==null||_familyShadowTrades.ContainsKey(c.CandidateId))return;
            ...
            {
                Print("[V58-SHADOW-REJECT] cid={0} pattern={1} route={2} regime={3} reason=NO_LEGAL_RR_TARGET",c.CandidateId,c.Signal.PatternName,c.Route,V58RegimeBucket(c));
                return;
            }
            double risk=Math.Abs(entry-stop);
            if(risk<=_symbol.PipSize)return;
            var x=new FamilyShadowTrade{
                CandidateId=c.CandidateId,SetupKey=c.SetupKey,Pattern=c.Signal.PatternName,Route=c.Route,Regime=V58RegimeBucket(c),
            ...
            Print("[V58-SHADOW-START] cid={0} setup={1} pattern={2} route={3} regime={4} entry={5} stop={6} target={7} netTargetR={8:F3}",
                x.CandidateId,x.SetupKey,x.Pattern,x.Route,x.Regime,x.Entry,x.Stop,x.Target,x.NetTargetR);
        }

        private void UpdateV58FamilyShadowTrades(int i,DateTime utc)
        {
            if(!EnableV58VirtualParallelExecution||_familyShadowTrades.Count==0)return;
        ...
        private void CloseOpenV58ShadowTradesAtStop()
        {
            ...
            foreach(var x in _familyShadowTrades.Values.Where(z=>!z.Closed).ToList())
            {
                ...
                CloseV58Shadow(x,utc,r,"BOT_STOP_MARK_TO_MARKET");
            }
        }
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V58-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        {
            _candidateSeq++;
            return "V58-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private void Ledger(CandidateRecord c, CandidateState state, string reason)
        {
            ...
            Print("[V58-EVENT] cid={0} setup={1} pattern={2} subtype={3} scale={4} tf={5} dir={6} state={7} route={8} conflict={9} waitMin={10:F2} reason={11}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V58-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V58-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
