...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV42 : Robot
    {
        ...
        [Parameter("V42 Physical Grid Realization", DefaultValue = false)]
        public bool EnablePhysicalGridRealization { get; set; }

        [Parameter("V42 Opportunity Queue", DefaultValue = false)]
        public bool EnableOpportunityQueue { get; set; }

        [Parameter("V42 Pattern-Native Competition", DefaultValue = false)]
        public bool EnablePatternNativeExecution { get; set; }

        [Parameter("V42 Opportunity Hold Minutes", DefaultValue = 90, MinValue = 15, MaxValue = 180)]
        ...
        protected override void OnStart()
        {
            ...
            Print("[V42-OPPORTUNITY-CONFIG] physicalGrid={0} opportunityQueue={1} patternNative={2} holdMinutes={3}",
        ...
        protected override void OnStop()
        {
            ...
            Print("[V42-OPPORTUNITY-SUMMARY] gridAttempts={0} gridPasses={1} gridFails={2} slotBlocked={3} slotRecovered={4} patternNativeEvaluations={5} logicalFourLegPlans={6}",
        ...
        private void AdvanceCandidateLifecycleOnBars(Bars bars, int i, DateTime utc, string executionTf, bool allowArm)
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                {
                    _gridPlanFails++;
                    Print("[V42-OPPORTUNITY-CONVERSION] cid={0} key={1} pattern={2} lane={3} stage=GRID_PLAN result=FAIL reason={4}",
                ...
                _gridPlanPasses++;
                Print("[V42-OPPORTUNITY-CONVERSION] cid={0} key={1} pattern={2} lane={3} stage=GRID_PLAN result=PASS physicalDepth={4} logicalLegs={5}",
        ...
        private void TryScheduleAndExecute()
        {
                ...
                if (EnableOpportunityQueue)
                {
                        ...
                        (c.State == CandidateState.EXECUTABLE || c.State == CandidateState.ARMED) && c.GridPlan != null))
                    {
                        if (!q.SlotBlocked)
                        {
                            ...
                            Print("[V42-OPPORTUNITY-CONVERSION] cid={0} key={1} pattern={2} lane={3} stage=SLOT result=BLOCKED holdUntil={4:o}",
            ...
            if (winner.State == CandidateState.EXECUTED)
            {
                if (winner.SlotBlocked)
                {
                    ...
                    Print("[V42-OPPORTUNITY-CONVERSION] cid={0} key={1} pattern={2} lane={3} stage=SLOT result=RECOVERED_EXECUTION",
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            if (spanXa < p.MinimumGridSpanXa || spanXa > p.MaximumGridSpanXa) return GridPlanFailure(c, "SPAN_XA");
