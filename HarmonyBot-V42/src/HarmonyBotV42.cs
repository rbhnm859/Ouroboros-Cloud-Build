...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV42 : Robot
    {
        ...
        protected override void OnStop()
        {
            ...
            Print("[V42-CONVERSION-SUMMARY] admissions={0} gridDeferred={1} capitalDeferred={2} executable={3} fallbackAttempts={4} fallbackExecutions={5}",
        ...
        private bool TryConvertAdmittedCandidate(CandidateRecord c, string executionTf, bool evidencePass)
        {
            ...
            if (!gridOk)
            {
                ...
                if (EnableOpportunityConversionLedger)
                    Print("[V42-CONVERSION] cid={0} key={1} lane={2} pattern={3} route={4} attempt={5} stage={6} status=DEFER evidence={7:F3} follow={8:F3}",
            ...
            CountPipeline(c.Signal.PatternName).Armed++;
            if (EnableOpportunityConversionLedger)
                Print("[V42-CONVERSION] cid={0} key={1} lane={2} pattern={3} route={4} attempt={5} stage=EXECUTABLE status=PASS rank={6:F4} netRR={7:F3} physicalDepth={8}",
                    c.CandidateId, c.CanonicalCandidateKey, c.Lane, c.Signal.PatternName, c.Route, c.ConversionAttempts,
        ...
        private void TryScheduleAndExecute()
        {
            ...
            foreach (var candidate in armed)
            {
                ...
                if (EnableOpportunityConversionLedger)
                    Print("[V42-OPPORTUNITY-AUCTION] cid={0} key={1} lane={2} pattern={3} route={4} rank={5:F4} conversionAttempts={6}",
    ...
    public enum CandidateState { DETECTED, VALIDATED, ROUTED, WAIT_PRZ, PRZ_TOUCHED, EVIDENCE_BUILDING, ADMITTED, EXECUTABLE, CONFIRMING, ARMED, EXECUTED, EXPIRED, REJECTED, INVALIDATED }
