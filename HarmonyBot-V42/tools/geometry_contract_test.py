...
s=pathlib.Path(sys.argv[1] if len(sys.argv)>1 else "HarmonyBot-V42/src/HarmonyBotV42.cs").read_text(errors="ignore")
required={
...
out={"version":"HarmonyBot V42","checks":required,"pass":all(required.values())}
pathlib.Path("V42_GEOMETRY_CONTRACT_TEST.json").write_text(json.dumps(out,indent=2))
