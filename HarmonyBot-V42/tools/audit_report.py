...
# Pattern x terminal reason attrition matrix.
evrx=re.compile(r"\\[V42-EVENT\\]\\s+cid=(\\S+)\\s+pattern=(.*?)\\s+tf=(\\S+)\\s+dir=(\\S+)\\s+state=(\\S+)\\s+route=(\\S+)\\s+conflict=(\\S+)\\s+reason=(.*)$",re.M)
attrition={}
for q in evrx.finditer(t):
 reason=q.group(8).strip()
 if reason.startswith(("REJECTED:","EXPIRED:","INVALIDATED:")) or "DEFERRED" in reason or "SCHEDULER" in reason:
  k=q.group(2)+"|"+reason
...
"attribution_outcomes":outcomes,"pattern_pipeline":pipeline,"pattern_attrition":attrition,**fc,**ec,**ac,**mc,**c}
