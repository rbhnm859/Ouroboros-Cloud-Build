...
rx=re.compile(r"\[V42-BASKET-CLOSED\].*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
pat=(r"\[V42-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
freq=re.findall(r"\[V42-FREQUENCY-SUMMARY\].*?schedulerDeferred=(\d+)\s+schedulerRecoveredExecutions=(\d+)\s+structuredRecallAdmitted=(\d+)\s+activeDeferred=(\d+).*?routeDeferred=(\d+)\s+routeRecovered=(\d+)",t)
...
ex=re.findall(r"\[V42-EXECUTION-SUMMARY\].*?evidenceExecutable=(\d+)\s+survivalWaits=(\d+)\s+opportunityArbitrations=(\d+)\s+riskRenormalizations=(\d+)\s+riskRejects=(\d+)\s+shadowTargets=(\d+)\s+shadowStops=(\d+)\s+shadowUnresolved=(\d+)",t)
...
attr=re.findall(r"\[V42-ATTRIBUTION-SUMMARY\].*?observed=(\d+)\s+legacyRankFirstReleased=(\d+)\s+followThroughObserved=(\d+)\s+thesisFailureExits=(\d+)",t)
...
mar=re.findall(r"\[V42-MARGINAL-ALPHA-SUMMARY\].*?coreAdmissions=(\d+)\s+rescueEvaluated=(\d+)\s+rescueAdmissions=(\d+)\s+rescueRejected=(\d+)",t)
...
opp=re.findall(r"\[V42-OPPORTUNITY-SUMMARY\].*?gridAttempts=(\d+)\s+gridPasses=(\d+)\s+gridFails=(\d+)\s+slotBlocked=(\d+)\s+slotRecovered=(\d+)\s+patternNativeEvaluations=(\d+)\s+logicalFourLegPlans=(\d+)",t)
...
for q in re.finditer(r"\[V42-OPPORTUNITY-CONVERSION\].*?stage=GRID_PLAN\s+result=FAIL\s+reason=(\S+)",t):
 grid_failure_reasons[q.group(1)]=grid_failure_reasons.get(q.group(1),0)+1

pipe_rx=re.compile(r"\[V42-PIPELINE\]\s+pattern=(.*?)\s+detected=(\d+)\s+validated=(\d+)\s+routed=(\d+)\s+prz=(\d+)\s+confirming=(\d+)\s+armed=(\d+)\s+basketPlanned=(\d+)\s+leg0=(\d+)\s+leg1=(\d+)\s+leg2=(\d+)\s+leg3=(\d+)\s+basketClosed=(\d+)\s+executed=(\d+)\s+expired=(\d+)\s+rejected=(\d+)\s+invalidated=(\d+)")
pipeline={}
...
"broker_profile_present":"[V42-BROKER-PROFILE]" in t,"evidence_events":len(re.findall(r"\[V42-EVIDENCE\]",t)),
...
"attribution_outcomes":outcomes,"pattern_pipeline":pipeline,"grid_failure_reasons":grid_failure_reasons,**fc,**ec,**ac,**mc,**oc,**c}
