...
conv=re.findall(r"\[V42-CONVERSION-SUMMARY\].*?admissions=(\d+)\s+gridDeferred=(\d+)\s+capitalDeferred=(\d+)\s+executable=(\d+)\s+fallbackAttempts=(\d+)\s+fallbackExecutions=(\d+)",t)
...
"attribution_outcomes":outcomes,"pattern_pipeline":pipeline,**fc,**ec,**ac,**mc,**cc,**c}
