...
GRIDREAL="${GRIDREAL:-false}"; QUEUE="${QUEUE:-false}"; PATTERNNATIVE="${PATTERNNATIVE:-false}"
TTL="${TTL:-8}"; MAXCAND="${MAXCAND:-12}"
ALGO="${ALGO:-seal/algo/HarmonyBot_V42_Marginal_Alpha_Opportunity_Portfolio_RC.algo}"
...
CNAME="v42-$(echo "$RUN_NAME"|tr '[:upper:]_' '[:lower:]-')-$GITHUB_RUN_ID"
