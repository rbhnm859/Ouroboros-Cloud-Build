...
s=pathlib.Path(sys.argv[1] if len(sys.argv)>1 else "HarmonyBot-V47/src/HarmonyBotV47.cs").read_text(errors="ignore")
checks={
 "identity":"HarmonyBot V47" in s and "class HarmonyBotV47" in s and 'BotPrefix = "HB47"' in s,
 ...
 "shadow_ledger":"V47-OPPORTUNITY-LOSS" in s and "ShadowMfeR" in s and "ShadowMaeR" in s,
 "slot_occupancy":"V47-SLOT-OCCUPANCY" in s and "_basketOccupancyMinutes" in s,
...
out={"version":"HarmonyBot V47","architecture":"ALPHA_PRESERVING_THROUGHPUT_SERIAL_OPPORTUNITY","checks":checks,"pass":all(checks.values())}
pathlib.Path("V47_ARCHITECTURE_AUDIT.json").write_text(json.dumps(out,indent=2))
