...
CFAUDIT="${CFAUDIT:-false}"; NATIVELANES="${NATIVELANES:-false}"; EXTNATIVE="${EXTNATIVE:-false}"; EVIDENCEBARS="${EVIDENCEBARS:-12}"
ALGO="${ALGO:-seal/algo/HarmonyBot_V49_Harmonic_Family_Confirmation_Kernel_RC.algo}"
...
CNAME="v49-$(echo "$RUN_NAME"|tr '[:upper:]_' '[:lower:]-')-$GITHUB_RUN_ID"
docker run --name "$CNAME" -v "$PWD/seal:/work" "$IMAGE" backtest "/work/${ALGO#seal/}" \
 ...
 --EnableEnhancedM1Confirmation=false --EnableCapitalFeasibilityGate=false --EnableTransitionStateVeto=false \
 --EnableExhaustionEvidenceVeto=true --EnableRouteSpecificM1Veto=false \
 ...
 --EnableCanonicalSetupIdentity=true --EnableCanonicalStandardCoordinates=true --EnableIndependentPivotGraph=true \
 --EnableTransitionProofGate=true --EnableM1RescueLane=false --M1RescueMaxBars=3 --EnableDiversityScheduler=true \
 --EnableScaleRouteAdmission=true --EnableM1TemporalRescue=false --EnableArmedExecutionGrace=false --ArmedGraceMinutes=90 \
 --EnablePreExecutionGridRevalidation=false \
 --EnablePersistentArmedQueue=false --EnableEventDrivenSerialHandoff=false \
 --EnablePatternNativeM1Expansion=false --PatternNativeM1MaxBars=4 --EnableOpportunityDecayRanking=false --ParkedHardLifetimeMinutes=180 \
 --EnableFamilyNativeConversion=false --EnableFamilyNativeObservation=false \
 --EnableCounterfactualConfirmationAudit="$CFAUDIT" --EnableV49NativeEvidenceLanes="$NATIVELANES" \
 --EnableV49ExtendedNativeResearch="$EXTNATIVE" --V49EvidenceWindowBars="$EVIDENCEBARS" \
...
PID=$!; DONE=0
echo "[V49-WATCHDOG] run=$RUN_NAME pid=$PID timeoutSeconds=$BACKTEST_TIMEOUT_SECONDS"
for ((i=0;i<BACKTEST_TIMEOUT_SECONDS;i+=5)); do
 if (( i % 60 == 0 )); then echo "[V49-WATCHDOG] run=$RUN_NAME elapsedSeconds=$i status=running"; fi
...
test "$DONE" = 1 || { echo "[V49-WATCHDOG-FAIL] run=$RUN_NAME"; tail -400 "seal/logs/$RUN_NAME.log" || true; exit 20; }
echo "[V49-WATCHDOG] run=$RUN_NAME status=complete"
