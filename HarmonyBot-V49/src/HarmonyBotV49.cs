...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV49 : Robot
    {
        ...
        [Parameter("Counterfactual Confirmation Audit", DefaultValue = false)]
        public bool EnableCounterfactualConfirmationAudit { get; set; }

        [Parameter("V49 Native Evidence Lanes", DefaultValue = false)]
        public bool EnableV49NativeEvidenceLanes { get; set; }

        [Parameter("V49 Extended Native Research", DefaultValue = false)]
        public bool EnableV49ExtendedNativeResearch { get; set; }

        [Parameter("V49 Evidence Window Bars", DefaultValue = 12, MinValue = 12, MaxValue = 12)]
        public int V49EvidenceWindowBars { get; set; }
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V49-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V49-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V49-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V49-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            Print("[V49-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m1Bars), _profiles.Count);
            Print("[V49-TIMEFRAME-AUDIT] primaryPattern=M15 execution=M1 macro=H4 intermediate=H1 allCompletedBars=true");
            Print("[V49-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
            Print("[V49-ALPHA-CONFIG] qualityObservation={0} legacyRegime={1} legacyEnhancedM1={2} capitalFeasibility={3} transitionVeto={4} exhaustionVeto={5} routeM1Veto={6}",
            ...
            Print("[V49-FREQUENCY-CONFIG] deferredRetention={0} agingPriority={1} ageBoost={2:F3} structuredRecall={3} recallGeometry={4:F3} recallPrz={5:F3} recallConfidence={6:F3}",
            ...
            Print("[V49-RESTORED-KERNEL] canonicalSetup={0} canonicalStandard={1} independentPivotGraph={2} transitionProof={3} m1Rescue={4} rescueBars={5} diversityScheduler={6} executionClock=M1 projectedD=false",
            ...
            Print("[V49-CONVERSION-ARCH] scaleRouteAdmission={0} temporalRescue={1} armedGrace={2} graceMinutes={3} preExecutionRevalidation={4}",
        ...
        protected override void OnStop()
        {
            ...
            {
                var x = kv.Value;
                Print("[V49-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} evidenceSeen={6} nativePass={7} cfPositive={8} cfNegative={9} nativeTemporalPass={10} armed={11} slotBlocked={12} parked={13} revalidated={14} revalidationRejected={15} basketPlanned={16} leg0={17} leg1={18} leg2={19} leg3={20} basketClosed={21} executed={22} expired={23} rejected={24} invalidated={25}",
                    kv.Key, x.Detected, x.Validated, x.Routed, x.PrzWaiting, x.Confirming, x.EvidenceSeen, x.NativePass,
                    x.CounterfactualPositive, x.CounterfactualNegative, x.NativeTemporalPass, x.Armed,
            ...
            Print("[V49-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V49-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
                _alphaQualityRejected, _regimeRejected, _confirmationRejected, _capitalInfeasibleCandidates, _alphaPassed);
            Print("[V49-FREQUENCY-SUMMARY] schedulerDeferred={0} schedulerRecoveredExecutions={1} structuredRecallAdmitted={2} activeDeferred={3}",
                _schedulerDeferred, _schedulerRecoveredExecutions, _structuredRecallAdmitted, _deferredCandidates.Count);
            Print("[V49-INDEPENDENT-SETUP-SUMMARY] duplicateSuppressed={0} uniqueExecuted={1} rescueAdmissions={2} transitionProofRejected={3} independentScaleCandidates={4}",
            ...
            Print("[V49-CONVERSION-SUMMARY] scaleRouteRejected={0} temporalRescueAdmissions={1} armedGraceExtended={2} preExecutionRevalidationRejected={3}",
            ...
            Print("[V49-THROUGHPUT-SUMMARY] slotBlocked={0} parked={1} revalidated={2} revalidationRejected={3} recoveredExecutions={4} nativeTemporalPass={5} hardLifetimeExpired={6} decayRejected={7} avgSlotWaitMin={8:F2} medianSlotWaitMin={9:F2} p90SlotWaitMin={10:F2} avgBasketOccupancyMin={11:F2} missedPositive={12} avoidedNegative={13}",
            ...
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V49-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                Print("[V49-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.WAIT_PRZ)
                {
                    ...
                    {
                        c.PrzTouchUtc = utc;
                        if (EnableCounterfactualConfirmationAudit || EnableV49NativeEvidenceLanes || EnableV49ExtendedNativeResearch)
                            InitializeV49Counterfactual(c, i);
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    c.CfLegacyBestScore = Math.Max(c.CfLegacyBestScore, legacyScore);
                    ...
                    if (c.CfActive)
                    {
                        ...
                        {
                            c.CfNativePass = true;
                            c.CfNativePassUtc = utc;
                            CountPipeline(c.Signal.PatternName).NativePass++;
                            Event(c, "V49_NATIVE_EVIDENCE_PASS_" + nativeEvidenceScore.ToString("F2", CultureInfo.InvariantCulture));
                        }
                        if (!confirmationPass && nativeEvidencePass && V49NativeTradingEnabled(c))
                        {
                            c.ConfirmationScore = Math.Max(c.ConfirmationScore, nativeEvidenceScore);
                            confirmationPass = true;
                            c.CfNativeAdmitted = true;
                            Event(c, "V49_NATIVE_EVIDENCE_ADMITTED_" + nativeEvidenceScore.ToString("F2", CultureInfo.InvariantCulture));
                        }
                    }
        ...
        private void MarkOpportunityTerminal(CandidateRecord c, string reason)
        {
            ...
            Print("[V49-OPPORTUNITY-LOSS] cid={0} setup={1} pattern={2} route={3} scale={4} reason={5} shadowMfeR={6:F3} shadowMaeR={7:F3} missedPositive={8} avoidedNegative={9}",
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            Print("[V49-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V49-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V49-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V49-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V49-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V49-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V49-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V49-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V49-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V49-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V49-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V49-BASKET-CLOSED] basket={0} cid={1} setup={2} pattern={3} subtype={4} route={5} dir={6} plannedLegs={7} filledLegs={8} anchor={9} avgEntry={10} entryImprovePips={11:F3} stop={12} target={13} initialRisk={14:F2} worstRisk={15:F2} mfeR={16:F3} maeR={17:F3} realizedR={18:F3} net={19:F2} reason={20}",
            ...
            Print("[V49-SLOT-OCCUPANCY] basket={0} cid={1} pattern={2} route={3} occupancyMinutes={4:F2} realizedR={5:F3} net={6:F2}",
        ...
        {
            Print("[V49-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
        ...
        private HarmonicRoute RouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (baseRoute == HarmonicRoute.NO_TRADE && EnableStructuredRecallExpansion)
            {
                ...
                if (baseRoute != HarmonicRoute.NO_TRADE)
                {
                    ...
                    Print("[V49-RECALL-ADMIT] pattern={0} direction={1} route={2} conflict={3} geometry={4:F3} prz={5:F3} confidence={6:F3} efficiency={7:F3} atrRatio={8:F3} extensionAtr={9:F3}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionProofGate)
            {
                ...
                if (!actualStructuralTransition)
                {
                    ...
                    Print("[V49-TRANSITION-PROOF-REJECT] pattern={0} conflict={1} transition={2} adxSlope={3:F2}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionStateVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V49-ROUTE-VETO] type=TRANSITION_STATE conflict={0} transition={1} adxSlope={2:F2}",
            ...
            if (baseRoute == HarmonicRoute.EXHAUSTION_REVERSAL && EnableExhaustionEvidenceVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V49-ROUTE-VETO] type=EXHAUSTION_EVIDENCE extensionAtr={0:F3} adxSlope={1:F2}",
        ...
        private void InitializeV49Counterfactual(CandidateRecord c, int i)
        {
            if (c == null || c.Signal == null || !c.PrzTouchUtc.HasValue) return;
            c.CfActive = true;
            c.CfEntryAnchor = c.Signal.Direction == TradeDirection.Buy ? _symbol.Ask : _symbol.Bid;
            ...
            Print("[V49-CF-START] cid={0} setup={1} pattern={2} subtype={3} route={4} scale={5} dir={6} przTouch={7:o} anchor={8:F5} stop={9:F5} riskDistance={10:F5}",
                ...
                c.Signal.StructuralInvalidation, c.CfRiskDistance);
        }
        ...
        private void UpdateV49CounterfactualShadow(CandidateRecord c, int i, DateTime utc)
        {
            ...
            if (c.Signal.Direction == TradeDirection.Buy)
            {
                fav = (_m1Bars.HighPrices[i] - c.CfEntryAnchor) / c.CfRiskDistance;
                adv = (c.CfEntryAnchor - _m1Bars.LowPrices[i]) / c.CfRiskDistance;
                if (!c.CfTarget1Utc.HasValue && _m1Bars.HighPrices[i] >= c.Signal.CanonicalTarget1) c.CfTarget1Utc = utc;
                if (!c.CfTarget2Utc.HasValue && _m1Bars.HighPrices[i] >= c.Signal.CanonicalTarget2) c.CfTarget2Utc = utc;
            }
            else
            {
                fav = (c.CfEntryAnchor - _m1Bars.LowPrices[i]) / c.CfRiskDistance;
                adv = (_m1Bars.HighPrices[i] - c.CfEntryAnchor) / c.CfRiskDistance;
        ...
        {
            score = 0;
            if (i < 3 || c == null || c.Signal == null || !c.PrzTouchUtc.HasValue) return false;
            c.CfBarsObserved++;
            var s = c.Signal;
            double o = _m1Bars.OpenPrices[i], cl = _m1Bars.ClosePrices[i], h = _m1Bars.HighPrices[i], l = _m1Bars.LowPrices[i];
            double pc = _m1Bars.ClosePrices[i - 1], ph1 = _m1Bars.HighPrices[i - 1], pl1 = _m1Bars.LowPrices[i - 1];
            double ph2 = Math.Max(ph1, _m1Bars.HighPrices[i - 2]), pl2 = Math.Min(pl1, _m1Bars.LowPrices[i - 2]);
            ...
            bool bos1 = bullish ? cl > ph1 : cl < pl1;
            ...
            bool sweep = bullish ? l < pl1 : h > ph1;
            bool failedExtension = bullish ? (l < pl1 && cl > pl1) : (h > ph1 && cl < ph1);
            bool displacement = atr > 0 && body >= atr * .30;
            bool closeBackInside = bullish ? cl > pl1 : cl < ph1;

            bool firstEvidence = !(c.CfDirectional || c.CfReclaim || c.CfBos1 || c.CfBos2 || c.CfRejection || c.CfFailedExtension || c.CfSweep || c.CfDisplacement || c.CfCloseBackInside);
            if (directional) { c.CfDirectional = true; if (!c.CfDirectionalUtc.HasValue) c.CfDirectionalUtc = utc; }
            if (reclaim) { c.CfReclaim = true; if (!c.CfReclaimUtc.HasValue) c.CfReclaimUtc = utc; }
            if (bos1) { c.CfBos1 = true; if (!c.CfBos1Utc.HasValue) c.CfBos1Utc = utc; }
            if (bos2) { c.CfBos2 = true; if (!c.CfBos2Utc.HasValue) c.CfBos2Utc = utc; }
            if (rejection) { c.CfRejection = true; if (!c.CfRejectionUtc.HasValue) c.CfRejectionUtc = utc; }
            if (failedExtension) { c.CfFailedExtension = true; if (!c.CfFailedExtensionUtc.HasValue) c.CfFailedExtensionUtc = utc; }
            if (sweep) { c.CfSweep = true; if (!c.CfSweepUtc.HasValue) c.CfSweepUtc = utc; }
            if (displacement) { c.CfDisplacement = true; if (!c.CfDisplacementUtc.HasValue) c.CfDisplacementUtc = utc; }
            if (closeBackInside) { c.CfCloseBackInside = true; if (!c.CfCloseBackInsideUtc.HasValue) c.CfCloseBackInsideUtc = utc; }
            bool nowAny = c.CfDirectional || c.CfReclaim || c.CfBos1 || c.CfBos2 || c.CfRejection || c.CfFailedExtension || c.CfSweep || c.CfDisplacement || c.CfCloseBackInside;
            if (firstEvidence && nowAny) CountPipeline(s.PatternName).EvidenceSeen++;
            ...
            bool fiveZero = p == "5-0";
            bool shark = p == "Shark";
            bool inWindow = c.CfBarsObserved <= Math.Max(12, V49EvidenceWindowBars);
            bool pass = false;
            if (inWindow && retracement)
                pass = c.CfRejection && c.CfReclaim && (c.CfBos1 || c.CfDisplacement);
            else if (inWindow && extension)
                pass = c.CfSweep && c.CfCloseBackInside && c.CfFailedExtension && (c.CfBos1 || c.CfDisplacement);
            else if (inWindow && cypher)
                pass = c.CfRejection && c.CfReclaim && (c.CfBos1 || c.CfDisplacement);
            else if (inWindow && fiveZero)
                pass = c.CfSweep && c.CfFailedExtension && c.CfReclaim && (c.CfBos1 || c.CfDisplacement);
            else if (inWindow && shark)
                pass = c.CfFailedExtension && c.CfReclaim && (c.CfBos1 || c.CfDisplacement);

            score = (c.CfReclaim ? .20 : 0) + (c.CfBos1 ? .20 : 0) + (c.CfBos2 ? .10 : 0) +
                    (c.CfRejection ? .15 : 0) + (c.CfFailedExtension ? .15 : 0) + (c.CfSweep ? .05 : 0) +
                    (c.CfDisplacement ? .10 : 0) + (c.CfDirectional ? .05 : 0);

            Print("[V49-CONFIRM-BAR] cid={0} setup={1} time={2:o} pattern={3} subtype={4} route={5} scale={6} dir={7} bar={8} legacy={9:F2} directional={10} reclaim={11} bos1={12} bos2={13} rejection={14} failedExtension={15} sweep={16} displacement={17} closeBackInside={18} nativePass={19}",
                c.CandidateId, c.SetupKey ?? "", utc, s.PatternName, s.HarmonicSubtype ?? s.PatternName, c.Route, s.PivotScale, s.Direction,
                c.CfBarsObserved, legacyScore, directional, reclaim, bos1, bos2, rejection, failedExtension, sweep, displacement, closeBackInside, pass);
            return pass;
        }

        private bool V49NativeTradingEnabled(CandidateRecord c)
        {
            if (!EnableV49NativeEvidenceLanes || c == null || c.Signal == null) return false;
            string p = c.Signal.PatternName ?? "";
            if (p == "Gartley" || p == "Bat" || p == "Deep Gartley" || p == "Rat" ||
                p == "Alt Bat" || p == "Butterfly" || p == "Crab" || p == "Deep Crab")
                return true;
            if (EnableV49ExtendedNativeResearch && (p == "Cypher" || p == "5-0"))
                return true;
            return false; // AB=CD and Shark stay on immutable incumbent lane; Shark alternate is shadow-only.
        }

        private string V49Ts(DateTime? x)
        {
            return x.HasValue ? x.Value.ToString("o", CultureInfo.InvariantCulture) : "-";
        }

        private void FinalizeV49Counterfactual(CandidateRecord c, string reason)
        {
            if (c == null || !c.CfActive || c.CfLogged || c.CfEverArmed) return;
            c.CfLogged = true;
            bool pos = c.CfTwoRUtc.HasValue && (!c.CfSlUtc.HasValue || c.CfTwoRUtc.Value < c.CfSlUtc.Value);
            bool neg = c.CfSlUtc.HasValue && (!c.CfTwoRUtc.HasValue || c.CfSlUtc.Value < c.CfTwoRUtc.Value);
            string cls = pos ? "POSITIVE_2R_BEFORE_SL" : neg ? "NEGATIVE_SL_BEFORE_2R" :
                         (c.CfTwoRUtc.HasValue && c.CfSlUtc.HasValue && c.CfTwoRUtc.Value == c.CfSlUtc.Value) ? "AMBIGUOUS_SAME_BAR" : "UNRESOLVED";
            if (pos) CountPipeline(c.Signal.PatternName).CounterfactualPositive++;
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V49-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        private void Ledger(CandidateRecord c, CandidateState state, string reason)
        {
            ...
            Print("[V49-EVENT] cid={0} setup={1} pattern={2} subtype={3} scale={4} tf={5} dir={6} state={7} route={8} conflict={9} waitMin={10:F2} reason={11}",
                c.CandidateId, c.SetupKey ?? "", c.Signal.PatternName, c.Signal.HarmonicSubtype ?? c.Signal.PatternName,
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V49-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V49-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
    ...
    {
        public long Detected, Validated, Routed, PrzWaiting, Confirming, EvidenceSeen, NativePass, CounterfactualPositive, CounterfactualNegative, NativeTemporalPass, Armed, SlotBlocked, Parked, Revalidated, RevalidationRejected, BasketPlanned;
