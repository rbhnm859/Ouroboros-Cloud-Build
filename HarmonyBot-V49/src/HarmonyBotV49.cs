...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V49 — Pattern-Family Native Conversion Engine";
        ...
        protected override void OnStart()
        {
            ...
            Print("[V49-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V46_SCALE_CONVERSION_FROZEN",
        ...
        protected override void OnStop()
        {
            ...
            {
                var x = kv.Value;
                Print("[V49-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} nativeTemporalPass={6} armed={7} slotBlocked={8} parked={9} revalidated={10} revalidationRejected={11} basketPlanned={12} leg0={13} leg1={14} leg2={15} leg3={16} basketClosed={17} executed={18} expired={19} rejected={20} invalidated={21}",
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    bool incumbentFamily = c.Signal.PatternName == "AB=CD" || c.Signal.PatternName == "Shark";
                    bool nativePrimaryFamily = EnablePatternNativeM1Expansion && !incumbentFamily;
                    bool nativePass = false;
                    bool confirmationPass = !nativePrimaryFamily && legacyScore >= legacyRequired;

                    // V49 root-cause correction: dormant harmonic families no longer have to pass
                    // the AB=CD/Shark-shaped single-bar legacy gate before their own temporal
                    // completion semantics can mature. Incumbent positive-alpha families retain
                    // the frozen V46 legacy lane.
                    if (nativePrimaryFamily)
                    {
                        c.NativeM1BarsObserved++;
                        double nativeScore;
                        int nativeHorizon = Math.Max(4, Math.Min(6, PatternNativeM1MaxBars + 2));
                        if (c.NativeM1BarsObserved <= nativeHorizon &&
                            UpdatePatternNativeM1State(i, c, out nativeScore))
                        {
                            ...
                            Event(c, "PATTERN_NATIVE_PRIMARY_PASS_" + nativeScore.ToString("F2", CultureInfo.InvariantCulture));
                        }
                        else if (c.NativeM1BarsObserved > nativeHorizon)
                        {
                            Event(c, "PATTERN_NATIVE_HORIZON_EXHAUSTED_STAGE_" + c.NativeStage.ToString(CultureInfo.InvariantCulture));
                        }
                    }
                    else if (!confirmationPass && EnablePatternNativeM1Expansion)
                    {
                        // Incumbent lane may observe native evidence, but native semantics cannot
                        // displace the frozen V46 admission contract for AB=CD/Shark.
                        c.NativeM1BarsObserved++;
                        double nativeScore;
                        UpdatePatternNativeM1State(i, c, out nativeScore);
                    }
                    ...
                    // A completed pattern-native temporal proof is itself the execution
                    // evidence contract. Reapplying the generic single-bar route veto here was
                    // the structural family bias found in V48 (Confirming but zero Armed).
                    if (EnableRouteSpecificM1Veto && !nativePass && !RouteSpecificM1EvidencePass(i, c.Signal, c.Route))
                    {
