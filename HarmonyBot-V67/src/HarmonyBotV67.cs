...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV67 : Robot
    {
        ...
        public string V67Mode { get; set; }

        [Parameter("V67 Family Native Alpha", DefaultValue = true)]
        public bool V67FamilyNativeAlphaEnabled { get; set; }

        [Parameter("V67 Family Fair Scheduler", DefaultValue = true)]
        public bool V67FamilyFairSchedulerEnabled { get; set; }

        [Parameter("V67 Family Grid", DefaultValue = true)]
        public bool V67FamilyGridEnabled { get; set; }

        [Parameter("V67 Allow Broad AB=CD Capital", DefaultValue = false)]
        public bool V67AllowBroadAbcdCapital { get; set; }
        ...
        private readonly Dictionary<string, int> _v67FamilyExecutions = new Dictionary<string, int>();
        private readonly Dictionary<string, int> _v67FamilySelections = new Dictionary<string, int>();
        private int _v67BroadAbcdShadowRejected;
        private int _v67FamilyQualityRejected;
        private int _v67FamilyRouteRejected;
        ...
        private bool V67ControlEnabled()
        {
            return string.Equals(V67Mode, "V67_CONTROL", StringComparison.Ordinal);
        }

        private bool V67ProductEnabled()
        {
            return string.Equals(V67Mode, "V67_PRODUCT", StringComparison.Ordinal);
        }

        private bool V67ModeValid()
        {
            return V67ControlEnabled() || V67ProductEnabled();
        }
        ...
        protected override void OnStart()
        {
            if (!V67ModeValid())
            {
                Print("[V67-FATAL] INVALID_MODE {0}", V67Mode);
                Stop();
                return;
            }
            ...
            if (_symbol == null)
            {
                Print("[V67-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V67-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V67-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V67-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            Print("[V67-COMMERCIAL-REBASE] mode={0} parent=V52_FAMILY_IDENTITY_RECONSTRUCTION familyNativeAlpha={1} familyFairScheduler={2} familyGrid={3} allowBroadABCD={4} riskCapPct={5:F2}",
                V67Mode, V67FamilyNativeAlphaEnabled, V67FamilyFairSchedulerEnabled, V67FamilyGridEnabled,
                V67AllowBroadAbcdCapital, BasketRiskPercent);
            ...
            Print("[V67-FREQUENCY-CONFIG] deferredRetention={0} agingPriority={1} ageBoost={2:F3} structuredRecall={3} recallGeometry={4:F3} recallPrz={5:F3} recallConfidence={6:F3}",
            ...
            Print("[V67-RESTORED-KERNEL] canonicalSetup={0} canonicalStandard={1} independentPivotGraph={2} transitionProof={3} m1Rescue={4} rescueBars={5} diversityScheduler={6} executionClock=M1 projectedD=false",
            ...
            Print("[V67-CONVERSION-ARCH] scaleRouteAdmission={0} temporalRescue={1} armedGrace={2} graceMinutes={3} preExecutionRevalidation={4}",
            ...
            Print("[V67-FAMILY-NATIVE] conversion={0} observation={1} principle=PATTERN_IDENTITY_NEQ_EXECUTION_IDENTITY", EnableFamilyNativeConversion, EnableFamilyNativeObservation);
            Print("[V67-COMPLETION-CONTRACT] canonicalContracts={0} familyCompletion={1} windowBars={2} provenLane=ABCD_SHARK_CYPHER_FROZEN",
                EnableCanonicalFamilyContracts, EnableFamilyCompletionContract, FamilyConfirmationWindowBars);
            Print("[V67-STRUCTURAL-GRID-CONTRACT] gridSpanV2={0} structuralInvalidationV2={1} rule=PRZ_LEGALITY_PLUS_RISK_RR_NOT_LEGACY_XA_MINSPAN",
                EnableGridSpanSemanticV2, EnableStructuralInvalidationV2);
            Print("[V67-MATH-GEOMETRY] jointGeometry={0} executionCorridor={1} entryAnchorForensics={2} rule=FAMILY_NATIVE_GEOMETRY_WITHOUT_RISK_RELAXATION",
                EnableFamilyNativeJointGeometry, EnableFamilyNativeExecutionCorridor, EnableEntryAnchorForensics);
            Print("[V67-DETECTOR-ARCH] identityReconstruction={0} boundedPivotGraph={1} maxMicroSkips={2} familyQuota={3} projectedPrz={4} truthLedger={5} rule=DETECT_MANY_EXECUTE_ONE",
            ...
            Print("[V67-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V46_SCALE_CONVERSION_FROZEN",
        ...
        protected override void OnStop()
        {
            ...
            {
                var x = kv.Value;
                Print("[V67-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} nativeTemporalPass={6} armed={7} slotBlocked={8} parked={9} revalidated={10} revalidationRejected={11} basketPlanned={12} leg0={13} leg1={14} leg2={15} leg3={16} basketClosed={17} executed={18} expired={19} rejected={20} invalidated={21}",
            ...
            if (EnableEntryAnchorForensics)
            {
                ...
                {
                    Print("[V67-ENTRY-ANCHOR-FORENSICS] cid={0} setup={1} pattern={2} route={3} scale={4} completion={5} confirm={6} retest={7} completionMfeR={8:F3} completionMaeR={9:F3} confirmMfeR={10:F3} confirmMaeR={11:F3} retestMfeR={12:F3} retestMaeR={13:F3}",
            ...
            if (EnableDetectorTruthLedger)
            {
                foreach (var kv in _detectorTruth.OrderBy(x => x.Key))
                {
                    ...
                    Print("[V67-DETECTOR-TRUTH] pattern={0} stage={1} count={2}", pattern, stage, kv.Value);
                }
            }
            foreach (var kv in _v67FamilySelections.OrderBy(x => x.Key))
            {
                int exec = _v67FamilyExecutions.ContainsKey(kv.Key) ? _v67FamilyExecutions[kv.Key] : 0;
                Print("[V67-FAMILY-SELECTION] pattern={0} selected={1} executed={2}", kv.Key, kv.Value, exec);
            }
            Print("[V67-FAMILY-ARCH-SUMMARY] broadAbcdShadowRejected={0} familyQualityRejected={1} familyRouteRejected={2}",
                _v67BroadAbcdShadowRejected, _v67FamilyQualityRejected, _v67FamilyRouteRejected);
            ...
            Print("[V67-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
                _alphaQualityRejected, _regimeRejected, _confirmationRejected, _capitalInfeasibleCandidates, _alphaPassed);
            Print("[V67-FREQUENCY-SUMMARY] schedulerDeferred={0} schedulerRecoveredExecutions={1} structuredRecallAdmitted={2} activeDeferred={3}",
                _schedulerDeferred, _schedulerRecoveredExecutions, _structuredRecallAdmitted, _deferredCandidates.Count);
            Print("[V67-INDEPENDENT-SETUP-SUMMARY] duplicateSuppressed={0} uniqueExecuted={1} rescueAdmissions={2} transitionProofRejected={3} independentScaleCandidates={4}",
            ...
            Print("[V67-CONVERSION-SUMMARY] scaleRouteRejected={0} temporalRescueAdmissions={1} armedGraceExtended={2} preExecutionRevalidationRejected={3}",
            ...
            Print("[V67-THROUGHPUT-SUMMARY] slotBlocked={0} parked={1} revalidated={2} revalidationRejected={3} recoveredExecutions={4} nativeTemporalPass={5} hardLifetimeExpired={6} decayRejected={7} avgSlotWaitMin={8:F2} medianSlotWaitMin={9:F2} p90SlotWaitMin={10:F2} avgBasketOccupancyMin={11:F2} missedPositive={12} avoidedNegative={13}",
            ...
            foreach (var p in _profiles.Select(x => x.Name).Distinct().OrderBy(x => x))
            {
                ...
                Print("[V67-FAMILY-CONTRACT-SUMMARY] pattern={0} pass={1} windowReject={2}", p, pass, reject);
            }
            foreach (var kv in _gridPlanRejectReasons.OrderBy(k => k.Key))
                Print("[V67-GRID-REJECT-SUMMARY] reason={0} count={1}", kv.Key, kv.Value);
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V67-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                bool familyQualityPass = V67ProductEnabled() && V67FamilyNativeAlphaEnabled
                    ? V67FamilyQualityEligible(signal)
                ...
                {
                    _v67FamilyQualityRejected += V67ProductEnabled() ? 1 : 0;
                ...
                if (record.Route == HarmonicRoute.NO_TRADE)
                {
                    ...
                    if (V67ProductEnabled()) _v67FamilyRouteRejected++;
                ...
                Print("[V67-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void TryScheduleAndExecute()
        {
            ...
            if (V67ProductEnabled() && V67FamilyFairSchedulerEnabled)
                armedQuery = armedQuery.GroupBy(c => c.Signal != null ? c.Signal.PatternName : "UNKNOWN")
                    .Select(g => g.OrderByDescending(c => EnableOpportunityDecayRanking ? OpportunityScore(c, now) : c.Rank).First());
            ...
            var winner = armed[0];
            if (V67ProductEnabled() && winner.Signal != null)
                IncrementCounter(_v67FamilySelections, winner.Signal.PatternName);
            ...
            {
                if (V67ProductEnabled() && winner.Signal != null)
                    IncrementCounter(_v67FamilyExecutions, winner.Signal.PatternName);
        ...
        private void MarkOpportunityTerminal(CandidateRecord c, string reason)
        {
            ...
            Print("[V67-OPPORTUNITY-LOSS] cid={0} setup={1} pattern={2} route={3} scale={4} reason={5} shadowMfeR={6:F3} shadowMaeR={7:F3} missedPositive={8} avoidedNegative={9}",
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            Print("[V67-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V67-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V67-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V67-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V67-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V67-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V67-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V67-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V67-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V67-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V67-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V67-BASKET-CLOSED] basket={0} cid={1} setup={2} pattern={3} subtype={4} route={5} dir={6} plannedLegs={7} filledLegs={8} anchor={9} avgEntry={10} entryImprovePips={11:F3} stop={12} target={13} initialRisk={14:F2} worstRisk={15:F2} mfeR={16:F3} maeR={17:F3} realizedR={18:F3} net={19:F2} reason={20}",
            ...
            Print("[V67-SLOT-OCCUPANCY] basket={0} cid={1} pattern={2} route={3} occupancyMinutes={4:F2} realizedR={5:F3} net={6:F2}",
        ...
        {
            Print("[V67-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
        ...
        private void BuildPatternProfiles()
        {
            ...
            if (V67ProductEnabled() && V67FamilyGridEnabled)
                ConfigureV67FamilyGridProfiles();
        }
        ...
        private bool V67FamilyQualityEligible(PatternSignal s)
        {
            if (s == null || s.Profile == null) return false;
            if (s.PatternName == "AB=CD" && s.HarmonicSubtype == "ABCD_LEGACY_BROAD" && !V67AllowBroadAbcdCapital)
            {
                _v67BroadAbcdShadowRejected++;
                DetectorTruth(s.PatternName, "V67_ABCD_BROAD_SHADOW_ONLY");
                return false;
            }

            double legacyGeom = Math.Max(MinGeometryQuality, s.Profile.MinGeometry);
            double legacyPrz = Math.Max(MinPrzConfluence, s.Profile.MinPrz);

            if (s.PatternName == "Shark" || s.PatternName == "Rat" || s.PatternName == "Cypher" || s.PatternName == "5-0" || s.PatternName == "AB=CD")
                return s.GeometryQuality >= legacyGeom && s.PrzConfluence >= legacyPrz;

            double familyGeom = Math.Max(.48, Math.Max(MinGeometryQuality - .05, s.Profile.MinGeometry - .06));
            double familyPrz = Math.Max(.48, Math.Max(MinPrzConfluence - .05, s.Profile.MinPrz - .06));
            return s.GeometryQuality >= familyGeom && s.PrzConfluence >= familyPrz &&
                   s.Confidence >= .52 && s.TimeSymmetry >= .20 && s.PivotQuality >= .20;
        }

        private double V67FamilyEvidenceAdjustment(PatternSignal s)
        {
            if (s == null) return 0;
            if (s.PatternName == "Shark") return .10;
            if (s.PatternName == "Rat") return .08;
            if (s.PatternName == "Cypher") return .06;
            if (s.PatternName == "AB=CD") return -.06;
            if (s.PatternName == "5-0") return -.04;
            if (s.PatternName == "Gartley") return -.02;
            return .02;
        }
        ...
        private HarmonicRoute V67FamilyRouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (r.AtrRatio < .50 || r.AtrRatio > 1.85) return HarmonicRoute.NO_TRADE;
            ...
            bool conflictHard = conflict == MtfConflict.CONFLICT;
            bool strong = s.GeometryQuality >= .58 && s.PrzConfluence >= .55 && s.Confidence >= .55;
            ...
            if (retracement)
            {
                if (aligned && !conflictHard && r.Efficiency >= .12)
                    return HarmonicRoute.TREND_ALIGNED_REVERSAL;
                if (p == "Rat" && opposed && r.ExtensionAtr >= 1.15 && strong && r.AdxH1Slope <= .75)
                    return HarmonicRoute.EXHAUSTION_REVERSAL;
                if (transition && strong && r.AdxH1Slope <= 1.25)
            ...
            if (extension)
            {
                if (opposed && r.ExtensionAtr >= .90 && strong && r.AdxH1Slope <= 1.0)
                    return HarmonicRoute.EXHAUSTION_REVERSAL;
                if (transition && r.ExtensionAtr >= .70 && strong && r.AdxH1Slope <= 1.25)
                    return HarmonicRoute.TRANSITION_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            }
            ...
            if (p == "Shark")
            {
                if (opposed && r.ExtensionAtr >= .75 && strong && r.AdxH1Slope <= 1.0)
                    return HarmonicRoute.EXHAUSTION_REVERSAL;
                if (aligned && !conflictHard && r.Efficiency >= .12)
                    return HarmonicRoute.TREND_ALIGNED_REVERSAL;
            ...
            if (p == "Cypher")
            {
                if (aligned && !conflictHard && r.Efficiency >= .10)
                    return HarmonicRoute.TREND_ALIGNED_REVERSAL;
                if (transition && strong)
                    return HarmonicRoute.TRANSITION_REVERSAL;
                if (opposed && r.ExtensionAtr >= 1.30 && strong && r.AdxH1Slope <= .50)
                    return HarmonicRoute.EXHAUSTION_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            }
            ...
            if (p == "5-0")
            {
                if (opposed && r.ExtensionAtr >= .90 && strong && r.AdxH1Slope <= .50)
                    return HarmonicRoute.EXHAUSTION_REVERSAL;
                if (transition && strong && r.AdxH1Slope <= .75)
                    return HarmonicRoute.TRANSITION_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            }
            ...
            if (p == "AB=CD")
            {
                if (!strong) return HarmonicRoute.NO_TRADE;
                return RouteSignalV34(s, conflict, r);
            }

            return RouteSignalV34(s, conflict, r);
        }

        private void ConfigureV67FamilyGridProfiles()
        {
            V67SetGrid("Gartley", new[] { 0.0, .236, .382, .618 }, new[] { .42, .28, .20, .10 });
            V67SetGrid("Bat", new[] { 0.0, .236, .382, .618 }, new[] { .42, .28, .20, .10 });
            V67SetGrid("Deep Gartley", new[] { 0.0, .236, .382, .618 }, new[] { .42, .28, .20, .10 });
            V67SetGrid("Rat", new[] { 0.0, .236, .382, .618 }, new[] { .40, .30, .20, .10 });

            V67SetGrid("Alt Bat", new[] { 0.0, .236, .382 }, new[] { .55, .30, .15 });
            V67SetGrid("Butterfly", new[] { 0.0, .236, .382 }, new[] { .55, .30, .15 });
            V67SetGrid("Crab", new[] { 0.0, .236 }, new[] { .70, .30 });
            V67SetGrid("Deep Crab", new[] { 0.0, .236 }, new[] { .70, .30 });

            V67SetGrid("Cypher", new[] { 0.0, .236, .382 }, new[] { .55, .30, .15 });
            V67SetGrid("Shark", new[] { 0.0, .236 }, new[] { .72, .28 });
            V67SetGrid("5-0", new[] { 0.0, .236, .382 }, new[] { .60, .25, .15 });
            V67SetGrid("AB=CD", new[] { 0.0, .236, .382 }, new[] { .55, .30, .15 });
        }

        private void V67SetGrid(string name, double[] fractions, double[] weights)
        {
            var p = _profiles.FirstOrDefault(x => x.Name == name);
            if (p == null) return;
            p.GridEnabled = true;
            p.GridFractions = fractions;
            p.MaximumGridLegs = fractions.Length;
            p.GridRiskWeights = weights;
        }
        ...
        {
            if (V67ProductEnabled() && V67FamilyNativeAlphaEnabled &&
                s != null && s.PatternName == "AB=CD" && s.HarmonicSubtype == "ABCD_LEGACY_BROAD" && !V67AllowBroadAbcdCapital)
                return HarmonicRoute.NO_TRADE;

            HarmonicRoute baseRoute = V67ProductEnabled() && V67FamilyNativeAlphaEnabled
                ? V67FamilyRouteSignal(s, conflict, r)
                : RouteSignalV34(s, conflict, r);
            if (baseRoute == HarmonicRoute.NO_TRADE && EnableStructuredRecallExpansion &&
                !(V67ProductEnabled() && V67FamilyNativeAlphaEnabled))
            {
                baseRoute = StructuredRecallRoute(s, conflict, r);
                if (baseRoute != HarmonicRoute.NO_TRADE)
                {
                    _structuredRecallAdmitted++;
                    Print("[V67-RECALL-ADMIT] pattern={0} direction={1} route={2} conflict={3} geometry={4:F3} prz={5:F3} confidence={6:F3} efficiency={7:F3} atrRatio={8:F3} extensionAtr={9:F3}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionProofGate)
            {
                ...
                if (!actualStructuralTransition)
                {
                    ...
                    Print("[V67-TRANSITION-PROOF-REJECT] pattern={0} conflict={1} transition={2} adxSlope={3:F2}",
                        s.PatternName, conflict, r.Transition, r.AdxH1Slope);
                    return HarmonicRoute.NO_TRADE;
                }
            }

            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionStateVeto && !V67ProductEnabled())
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V67-ROUTE-VETO] type=TRANSITION_STATE conflict={0} transition={1} adxSlope={2:F2}",
                        conflict, r.Transition, r.AdxH1Slope);
                    return HarmonicRoute.NO_TRADE;
                }
            }

            if (baseRoute == HarmonicRoute.EXHAUSTION_REVERSAL && EnableExhaustionEvidenceVeto && !V67ProductEnabled())
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V67-ROUTE-VETO] type=EXHAUSTION_EVIDENCE extensionAtr={0:F3} adxSlope={1:F2}",
        ...
        private bool UpdateFamilyCompletionEvidence(int i, CandidateRecord c, out double score)
        {
            ...
            bool deceleration = body <= prevBody * .90;
            ...
            if (retracement)
            {
                ...
                double need = V67ProductEnabled() && p != "Rat" ? .70 : .75;
                return c.FamilyReclaim && (c.FamilyRejection || c.FamilyFailedExtension) &&
                       (c.FamilyBos || c.FamilyDisplacement) && score >= need;
            }

            if (extension)
            {
                score = (c.FamilySweep ? .15 : 0) + (c.FamilyFailedExtension ? .25 : 0) +
                        ((c.FamilyReclaim || c.FamilyInsidePrz) ? .25 : 0) +
                        (c.FamilyBos ? .20 : 0) + (c.FamilyDisplacement ? .15 : 0);
                if (V67ProductEnabled())
                    return c.FamilyFailedExtension && (c.FamilyReclaim || c.FamilyInsidePrz) &&
                           (c.FamilyBos || c.FamilyDisplacement) && score >= .65;
            ...
            if (p == "Shark")
            {
                score = (c.FamilyReclaim ? .25 : 0) + ((c.FamilyFailedExtension || c.FamilyRejection) ? .25 : 0) +
                        (c.FamilyBos ? .25 : 0) + (c.FamilyDisplacement ? .15 : 0) + (c.FamilySweep ? .10 : 0);
            ...
            if (p == "Cypher")
            {
                score = (c.FamilyReclaim ? .30 : 0) + (c.FamilyRejection ? .20 : 0) +
                        (c.FamilyBos ? .30 : 0) + (c.FamilyDisplacement ? .20 : 0);
                return c.FamilyReclaim && c.FamilyBos && (c.FamilyRejection || c.FamilyDisplacement) && score >= .70;
            }

            if (p == "AB=CD")
            {
                if (V67ProductEnabled() && sig.HarmonicSubtype == "ABCD_LEGACY_BROAD" && !V67AllowBroadAbcdCapital)
                    return false;
                score = (deceleration ? .20 : 0) + ((c.FamilyFailedExtension || c.FamilyRejection) ? .25 : 0) +
                        (c.FamilyReclaim ? .25 : 0) + (c.FamilyBos ? .20 : 0) + (c.FamilyDisplacement ? .10 : 0);
                return deceleration && (c.FamilyFailedExtension || c.FamilyRejection) && c.FamilyReclaim &&
                       (c.FamilyBos || c.FamilyDisplacement) && score >= .70;
            }
        ...
        private double CandidateRank(CandidateRecord c)
        {
            ...
            if (V67ProductEnabled() && V67FamilyFairSchedulerEnabled)
                baseRank += V67FamilyEvidenceAdjustment(c.Signal);
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V67-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        {
            _candidateSeq++;
            return "V67-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private void Ledger(CandidateRecord c, CandidateState state, string reason)
        {
            ...
            Print("[V67-EVENT] cid={0} setup={1} pattern={2} subtype={3} scale={4} tf={5} dir={6} state={7} route={8} conflict={9} waitMin={10:F2} reason={11}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V67-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V67-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
