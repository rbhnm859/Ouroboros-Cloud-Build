...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V67 — V52 Throughput Family-Native Commercial Rebase";
        ...
        [Parameter("V67 Family Arbitration", DefaultValue = true)]
        public bool V67FamilyArbitrationEnabled { get; set; }

        [Parameter("V67 Family Native Capital", DefaultValue = true)]
        public bool V67FamilyNativeCapitalEnabled { get; set; }

        [Parameter("V67 DMS Context Telemetry", DefaultValue = true)]
        public bool V67DmsContextTelemetryEnabled { get; set; }
        ...
        protected override void OnStart()
        {
            ...
            Print("[V67-FAMILY-COMMERCIAL] mode={0} arbitration={1} nativeCapital={2} dmsTelemetry={3} projectedPrzMustRemainFalse={4}",
                V67Variant, V67FamilyArbitrationEnabled, V67FamilyNativeCapitalEnabled, V67DmsContextTelemetryEnabled,
                !EnableFamilyNativeProjectedPrz);
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                if (V67DmsContextTelemetryEnabled)
                    Print("[V67-FAMILY-THESIS] cid={0} setup={1} family={2} subtype={3} tier={4} route={5} conflict={6} dmiH1={7:F4} dmiH4={8:F4} geometry={9:F3} prz={10:F3} confidence={11:F3} abcdConfluence={12}",
                        record.CandidateId, record.SetupKey, signal.PatternName, signal.HarmonicSubtype, record.V67FamilyTier,
                        record.Route, record.Conflict, record.V67DmiBiasH1, record.V67DmiBiasH4,
                        signal.GeometryQuality, signal.PrzConfluence, signal.Confidence, V67AbcdCompletionConfluence(signal));

                if (V67ProductEnabled() && V67FamilyNativeCapitalEnabled &&
                    record.Route != HarmonicRoute.NO_TRADE && !V67FamilyRouteEligible(record))
                {
                    Reject(record, "V67_FAMILY_ROUTE_REJECT");
                    continue;
                }
        ...
        private void TryScheduleAndExecute()
        {
            ...
            if (V67ProductEnabled() && V67FamilyArbitrationEnabled)
                Print("[V67-FAMILY-ARBITRATION] setup={0} winnerFamily={1} subtype={2} route={3} tier={4} rank={5:F4} competing={6}",
                    winner.SetupKey, winner.Signal.PatternName, winner.Signal.HarmonicSubtype, winner.Route,
                    winner.V67FamilyTier, winner.Rank, armed.Count);
        ...
        private RegimeSnapshot BuildRegimeSnapshot()
        {
            ...
            V67DirectionalMovement(_h1Bars, 14, h1, out r.DiPlusH1, out r.DiMinusH1);
            V67DirectionalMovement(_h4Bars, 14, h4, out r.DiPlusH4, out r.DiMinusH4);
            return r;
        }

        private bool V67ControlEnabled()
        {
            return string.Equals(V67Variant, "V67_CONTROL", StringComparison.Ordinal);
        }
        ...
        private string V67FamilyTier(PatternSignal s)
        {
            if (s == null) return "UNKNOWN";
            if (s.PatternName == "Shark") return "PRIMARY";
            if (s.PatternName == "Rat") return "PRIMARY";
            if (s.PatternName == "Cypher") return "APPROVED";
            if (s.PatternName == "AB=CD")
            {
                if (s.HarmonicSubtype == "ABCD_EXACT") return "SELECTIVE_EXACT";
                if (s.HarmonicSubtype == "ABCD_NEAR_127") return "SELECTIVE_NEAR127";
                return "COMPLETION_PRIMITIVE";
            }
            if (s.PatternName == "Gartley" || s.PatternName == "Bat" || s.PatternName == "Deep Gartley")
                return "RETRACEMENT_EXPANSION";
        ...
        private bool V67FamilyRouteEligible(CandidateRecord c)
        {
            ...
            if (p == "Shark") return r == HarmonicRoute.TREND_ALIGNED_REVERSAL || r == HarmonicRoute.EXHAUSTION_REVERSAL;
            if (p == "Rat" || p == "Gartley" || p == "Bat" || p == "Deep Gartley")
            ...
            if (p == "AB=CD")
            {
                bool selective = c.Signal.HarmonicSubtype == "ABCD_EXACT" || c.Signal.HarmonicSubtype == "ABCD_NEAR_127";
        ...
        {
            return s != null && s.AbCd >= .80 && s.AbCd <= 1.25;
        }

        private double V67SignedDmiBias(TradeDirection direction, double diPlus, double diMinus)
        {
        ...
        private void V67DirectionalMovement(Bars bars, int period, int end, out double diPlus, out double diMinus)
        {
            diPlus = 0; diMinus = 0;
        ...
        {
            bool legacy = pattern == "Gartley" || pattern == "Bat" || pattern == "Alt Bat" ||
                          pattern == "Butterfly" || pattern == "Crab" || pattern == "Deep Crab" ||
                          pattern == "Deep Gartley" || pattern == "Rat" || pattern == "5-0";
            if (!V67ProductEnabled()) return legacy;
            return legacy || pattern == "Shark" || pattern == "Cypher" || pattern == "AB=CD";
        }
        ...
        private double CandidateRank(CandidateRecord c)
        {
            ...
            double rank = .25 * c.Signal.GeometryQuality + .15 * c.Signal.PrzConfluence + .15 * mtf + .15 * regime +
            ...
            if (EnableFrequencyAgingPriority && CandidateAgeRankBoost > 0)
            {
                ...
                rank += CandidateAgeRankBoost * VClamp(age / ttl);
            }
            return rank;
        }
    ...
    public sealed class CandidateRecord
    {
        ...
        public string V67FamilyTier = "UNKNOWN";
        public double V67DmiBiasH1, V67DmiBiasH4;
        public bool V67FamilyDeceleration;
