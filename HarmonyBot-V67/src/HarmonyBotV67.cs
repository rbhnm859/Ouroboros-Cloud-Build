...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V67 — V52 Throughput Core Family-Native Grid Commercial Breakthrough";
        ...
        public bool V67SuppressParentAbcd { get; set; }

        [Parameter("V67 Normalize Grid Risk", DefaultValue = true)]
        public bool V67NormalizeGridRisk { get; set; }
        ...
        private int _v67ParentAbcdSuppressed;
        ...
        protected override void OnStart()
        {
            ...
            Print("[V67-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V52_FAMILY_IDENTITY_THROUGHPUT",
            ...
            Print("[V67-PRODUCT-CONFIG] mode={0} familyNativeBars={1} parentAbcdSuppression={2} normalizeGridRisk={3} abcdRole=COMPLETION_PRIMITIVE_FALLBACK",
                V67Variant, V67FamilyNativeConfirmationBars, V67SuppressParentAbcd, V67NormalizeGridRisk);
        }
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (V67ProductEnabled())
                    {
                        ...
                        if (confirmationPass)
                        {
                            _v67FamilyNativeConfirmed++;
                            _nativeTemporalPass++;
                            CountPipeline(c.Signal.PatternName).NativeTemporalPass++;
                            Event(c, "V67_FAMILY_NATIVE_CONFIRM_" + nativeScore.ToString("F2", CultureInfo.InvariantCulture));
                        }
                        else if (c.NativeM1BarsObserved >= Math.Max(4, V67FamilyNativeConfirmationBars))
                        {
                            _v67FamilyNativeExpired++;
                            Reject(c, "V67_FAMILY_NATIVE_CONFIRMATION_EXPIRED");
                            continue;
                        }
                    }
                    ...
                    if (!V67ProductEnabled() && !familyContractLane && !confirmationPass && EnablePatternNativeM1Expansion)
                    {
                        c.NativeM1BarsObserved++;
                        double nativeScore;
                    ...
                    if (!V67ProductEnabled() && !confirmationPass && EnableM1RescueLane)
                    {
        ...
        private void TryScheduleAndExecute()
        {
            ...
            if (EnableDiversityScheduler)
                armedQuery = armedQuery.GroupBy(c => string.IsNullOrWhiteSpace(c.SetupKey) ? c.CandidateId : c.SetupKey)
                    ...
                    {
                        var q = g.ToList();
                        if (V67ProductEnabled() && q.Any(x => x.Signal != null && x.Signal.PatternName != "AB=CD"))
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            if (V67ProductEnabled() && V67NormalizeGridRisk)
            {
        ...
        private List<PatternSignal> DetectPatternCandidates(Bars bars, int endIndex, int depth, int lookback, int maxCandidates, string timeframe)
        {
            ...
            if (V67ProductEnabled() && V67SuppressParentAbcd)
            {
        ...
        {
            return V67ProductEnabled() || V67ControlEnabled();
        }

        private bool V67IsRetracementFamily(string p)
        {
            return p == "Gartley" || p == "Bat" || p == "Deep Gartley" || p == "Rat";
        }

        private bool V67IsExtensionFamily(string p)
        {
        ...
        private int V67FamilyDetectionQuotaFor(string pattern, int defaultQuota)
        {
            if (!V67ProductEnabled()) return defaultQuota;
