...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V67 — Harmonic Family Identity & Detection Graph Reconstruction Kernel";
        ...
        protected override void OnStart()
        {
            ...
            if (EnableV67FamilyNativeGrid) ApplyV67FamilyGridContracts();
            ...
            Print("[V67-FAMILY-TRADE-CONTRACTS] enabled={0} detectorFrontier={1} familyGrid={2} evidenceGuard={3} controlledExpansion={4} contracts={5} principle=FAMILY_IDENTITY_ROUTE_CONFIRMATION_GRID_SEPARATED",
            ...
            {
                var fc = kv.Value;
                Print("[V67-FAMILY-POLICY] pattern={0} role={1} routes=T{2}/E{3}/X{4} confirm={5} gridLegs={6} runner={7} expiryBars={8}",
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                bool familyQualityPass = EnableV67FamilyTradeContracts
                    ? V67FamilyQualityPass(signal)
                ...
                {
                    Reject(record, EnableV67FamilyTradeContracts ? "V67_FAMILY_QUALITY" : "PATTERN_QUALITY");
                    continue;
                }
                ...
                record.Route = EnableV67FamilyTradeContracts ? RouteSignalFamilyNativeV67(signal, record.Conflict, regime) : RouteSignal(signal, record.Conflict, regime);
                ...
                if (!EnableV67FamilyTradeContracts && EnableScaleRouteAdmission && signal.PivotScale != M15SwingDepth &&
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (familyContractLane)
                    {
                        ...
                        double familyScore;
                        confirmationPass = EnableV67FamilyTradeContracts
                        ...
                        if (confirmationPass)
                        {
                            ...
                            Event(c, (EnableV67FamilyTradeContracts ? "V67_FAMILY_TRADE_CONTRACT_PASS_" : "FAMILY_COMPLETION_CONTRACT_PASS_") + familyScore.ToString("F2", CultureInfo.InvariantCulture));
                        }
                        else if (c.FamilyConfirmationBarsObserved >= (EnableV67FamilyTradeContracts ? V67FamilyConfirmationWindow(c.Signal) : Math.Max(6, FamilyConfirmationWindowBars)))
                        {
                    ...
                    if (EnableRouteSpecificM1Veto && !(EnableV67FamilyTradeContracts ? V67FamilyRouteEvidencePass(i, c) : RouteSpecificM1EvidencePass(i, c.Signal, c.Route)))
                    {
