...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV67 : Robot
    {
        ...
        [Parameter("V67 Variant", DefaultValue = "V67_PRODUCT")]
        public string V67Variant { get; set; }

        [Parameter("V67 Family-Native Thesis", DefaultValue = true)]
        public bool EnableV67FamilyNativeThesis { get; set; }

        [Parameter("V67 Family-Native Completion", DefaultValue = true)]
        public bool EnableV67FamilyNativeCompletion { get; set; }

        [Parameter("V67 Family-Aware Grid", DefaultValue = true)]
        public bool EnableV67FamilyAwareGrid { get; set; }

        [Parameter("V67 AB=CD Standalone Capital", DefaultValue = false)]
        public bool EnableV67AbcdStandaloneCapital { get; set; }
        ...
        private int _preventedRiskRejections;
        private int _v67FamilyRouteRejected;
        private int _v67AbcdStandaloneSuppressed;
        private int _v67FamilyCompletionRejected;
        ...
        protected override void OnStart()
        {
            if (!V67ModeValid())
            {
                Print("[V67-FATAL] INVALID_MODE {0}", V67Variant);
                Stop();
                return;
            }
            ...
            Print("[V67-COMMERCIAL-REBASE] variant={0} familyThesis={1} familyCompletion={2} familyGrid={3} abcdStandalone={4} parent=V52_FULL",
                V67Variant, EnableV67FamilyNativeThesis, EnableV67FamilyNativeCompletion, EnableV67FamilyAwareGrid, EnableV67AbcdStandaloneCapital);
        ...
        protected override void OnStop()
        {
            ...
            Print("[V67-COMMERCIAL-SEMANTICS] preventedRiskRejections={0} actualBasketRiskViolations={1} familyRouteRejected={2} abcdStandaloneSuppressed={3} familyCompletionRejected={4}",
                _preventedRiskRejections, _actualBasketRiskViolations, _v67FamilyRouteRejected, _v67AbcdStandaloneSuppressed, _v67FamilyCompletionRejected);
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                if (V67ProductEnabled() && EnableV67FamilyNativeThesis)
                {
                    var legacyRoute = record.Route;
                    record.Route = V67FamilyNativeRoute(signal, record.Conflict, regime, legacyRoute);
                    Print("[V67-FAMILY-THESIS] cid={0} setup={1} pattern={2} legacyRoute={3} familyRoute={4} geom={5:F3} prz={6:F3} conf={7:F3} efficiency={8:F3} extension={9:F3}",
                        record.CandidateId, record.SetupKey, signal.PatternName, legacyRoute, record.Route,
                        signal.GeometryQuality, signal.PrzConfluence, signal.Confidence, regime.Efficiency, regime.ExtensionAtr);
                    if (legacyRoute != HarmonicRoute.NO_TRADE && record.Route == HarmonicRoute.NO_TRADE)
                ...
                if ((!V67ProductEnabled() || EnableV67AbcdStandaloneCapital) &&
                    EnableScaleRouteAdmission && signal.PivotScale != M15SwingDepth &&
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (familyContractLane)
                    {
                        ...
                        else if (c.FamilyConfirmationBarsObserved >= Math.Max(6, FamilyConfirmationWindowBars))
                        {
                            ...
                            if (V67ProductEnabled()) _v67FamilyCompletionRejected++;
        ...
        {
            return string.Equals(V67Variant, "V67_V52_CONTROL", StringComparison.Ordinal);
        }

        private bool V67ProductEnabled()
        {
            return string.Equals(V67Variant, "V67_PRODUCT", StringComparison.Ordinal);
        }
        ...
        private double V67FamilyDetectorPriority(PatternSignal s)
        {
            if (s == null) return -999;
            double prior = 0;
            if (s.PatternName == "Shark") prior = .08;
            else if (s.PatternName == "Cypher") prior = .04;
            else if (s.PatternName == "Rat") prior = .03;
            else if (s.PatternName == "5-0") prior = .01;
            else if (s.PatternName == "AB=CD") prior = -.08;
            return s.Confidence + prior;
        }

        private HarmonicRoute V67FamilyNativeRoute(PatternSignal s, MtfConflict conflict, RegimeSnapshot r, HarmonicRoute legacyRoute)
        {
            ...
            bool contextClean = conflict != MtfConflict.CONFLICT && r.AtrRatio >= .50 && r.AtrRatio <= 1.85;

            if (p == "AB=CD")
            {
                if (!EnableV67AbcdStandaloneCapital)
                {
                    _v67AbcdStandaloneSuppressed++;
                    return HarmonicRoute.NO_TRADE;
                }
                bool terminal = s.HarmonicSubtype == "ABCD_EXACT" || s.HarmonicSubtype == "ABCD_NEAR_127";
                return terminal && contextClean ? legacyRoute : HarmonicRoute.NO_TRADE;
            }

            if (p == "Shark")
                return contextClean && aligned ? HarmonicRoute.TREND_ALIGNED_REVERSAL : HarmonicRoute.NO_TRADE;

            if (p == "Cypher")
            {
                if (contextClean && aligned) return HarmonicRoute.TREND_ALIGNED_REVERSAL;
                if (contextClean && transition && s.GeometryQuality >= .60 && s.PrzConfluence >= .60)
            ...
            if (p == "5-0")
            {
                if (contextClean && opposed && r.ExtensionAtr >= 1.00) return HarmonicRoute.EXHAUSTION_REVERSAL;
                if (contextClean && transition) return HarmonicRoute.TRANSITION_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            }
            ...
            if (retracement)
            {
                if (contextClean && aligned && r.Efficiency >= .12) return HarmonicRoute.TREND_ALIGNED_REVERSAL;
                if (contextClean && transition && s.GeometryQuality >= .60) return HarmonicRoute.TRANSITION_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            }
        ...
        private double[] V67FamilyGridWeights(string name, int maxLegs)
        {
            ...
            if (maxLegs == 2) return new[] { .65, .35 };
            if (maxLegs == 3)
            {
                if (name == "Shark" || name == "Cypher") return new[] { .55, .30, .15 };
                return new[] { .50, .30, .20 };
            }
            return new[] { .40, .30, .20, .10 };
        }

        private double V67FamilyEvidencePrior(CandidateRecord c)
        {
            if (c == null || c.Signal == null) return 0;
            string p = c.Signal.PatternName ?? "";
            if (p == "Shark" && c.Route == HarmonicRoute.TREND_ALIGNED_REVERSAL) return .08;
            if (p == "Cypher" && c.Route == HarmonicRoute.TREND_ALIGNED_REVERSAL) return .04;
            if (p == "Rat" && c.Route == HarmonicRoute.TREND_ALIGNED_REVERSAL) return .03;
            if (p == "5-0" && c.Route == HarmonicRoute.EXHAUSTION_REVERSAL) return .02;
            if (p == "AB=CD") return -.08;
            return 0;
        }

        private bool V67HasAbcdConfluence(CandidateRecord c)
        {
            if (c == null || c.Signal == null || c.Signal.PatternName == "AB=CD") return false;
            return _candidates.Values.Any(x => x != null && x.Signal != null &&
                x.Signal.PatternName == "AB=CD" && x.SetupKey == c.SetupKey &&
                x.Signal.Direction == c.Signal.Direction);
        }
        ...
        private void ConfigureFibonacciGridProfiles()
        {
            ...
            ConfigureGrid("Shark", V67ProductEnabled() && EnableV67FamilyAwareGrid ? new[] { 0.0, .236, .382 } : new[] { 0.0, .236 },
                V67ProductEnabled() && EnableV67FamilyAwareGrid ? 3 : 2, .02, .60, .42, 75, .050, "EXTREME_PRZ_CONFIRM", "T1_THEN_T2");
            ConfigureGrid("5-0", new[] { 0.0, .236 }, 2, .02, .60, .26, 60, .050, "REVERSAL_PRZ_CONFIRM", "T1_THEN_T2");
            ConfigureGrid("AB=CD", V67ProductEnabled() && !EnableV67AbcdStandaloneCapital ? new[] { 0.0 } : new[] { 0.0, .236, .382, .618 },
                V67ProductEnabled() && !EnableV67AbcdStandaloneCapital ? 1 : 4, .02, .80, .65, 90, .050, "ABCD_COMPLETION_CONFIRM", "T1_THEN_T2");
            ConfigureGrid("Deep Gartley", new[] { 0.0, .236, .382 }, 3, .70, 1.20, .42, 75, .030, "DEEP_PRZ_CONFIRM", "T1_THEN_T2");
            ConfigureGrid("Rat", V67ProductEnabled() && EnableV67FamilyAwareGrid ? new[] { 0.0, .236, .382, .618 } : new[] { 0.0, .236, .382 },
                V67ProductEnabled() && EnableV67FamilyAwareGrid ? 4 : 3, .02, 1.40, .65, 90, .035, "RATIO_PRZ_CONFIRM", "T1_THEN_T2");
        }
            ...
            double tolerance, int ttlMinutes, double stopFibBuffer, string anchorRule, string targetPolicy)
        {
            ...
            p.GridRiskWeights = V67ProductEnabled() && EnableV67FamilyAwareGrid
        ...
        {
            if (V67ProductEnabled() && EnableV67FamilyNativeCompletion)
                return pattern == "Gartley" || pattern == "Bat" || pattern == "Alt Bat" ||
                       pattern == "Butterfly" || pattern == "Crab" || pattern == "Deep Crab" ||
                       pattern == "Deep Gartley" || pattern == "Rat" || pattern == "Cypher" ||
                       pattern == "Shark" || pattern == "5-0" || pattern == "AB=CD";
            return pattern == "Gartley" || pattern == "Bat" || pattern == "Alt Bat" ||
                   pattern == "Butterfly" || pattern == "Crab" || pattern == "Deep Crab" ||
        ...
        private bool UpdateFamilyCompletionEvidence(int i, CandidateRecord c, out double score)
        {
            ...
            if (p == "Cypher")
            {
                score = (c.FamilyReclaim ? .30 : 0) + (c.FamilyBos ? .30 : 0) +
                        (c.FamilyRetest ? .20 : 0) + (c.FamilyDirectional ? .20 : 0);
                return c.FamilyReclaim && c.FamilyBos && c.FamilyRetest && c.FamilyDirectional && score >= .80;
            }
            if (p == "Shark")
            {
                score = (c.FamilySweep ? .25 : 0) + (c.FamilyFailedExtension ? .25 : 0) +
                        (c.FamilyReclaim ? .25 : 0) + ((c.FamilyBos || c.FamilyDisplacement) ? .25 : 0);
                return c.FamilySweep && c.FamilyFailedExtension && c.FamilyReclaim &&
                       (c.FamilyBos || c.FamilyDisplacement) && score >= .80;
            }
            if (p == "AB=CD")
            {
                bool terminal = sig.HarmonicSubtype == "ABCD_EXACT" || sig.HarmonicSubtype == "ABCD_NEAR_127";
                score = (terminal ? .20 : 0) + (c.FamilyReclaim ? .25 : 0) +
                        ((c.FamilyRejection || c.FamilyFailedExtension) ? .25 : 0) +
                        (c.FamilyBos ? .30 : 0);
                return terminal && c.FamilyReclaim && (c.FamilyRejection || c.FamilyFailedExtension) &&
                       c.FamilyBos && score >= .80;
            }
            return false;
        }
        ...
        private double CandidateRank(CandidateRecord c)
        {
            ...
            {
                baseRank += V67FamilyEvidencePrior(c);
                if (V67HasAbcdConfluence(c)) baseRank += .04;
            }
