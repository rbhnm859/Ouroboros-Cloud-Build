...
pat=(r"\[V67-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
evrx=re.compile(r"\[V67-EVENT\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+subtype=(\S+)\s+scale=(\d+)\s+tf=(\S+)\s+dir=(\S+)\s+state=(\S+)\s+route=(\S+)\s+conflict=(\S+)\s+waitMin=([-0-9.]+)\s+reason=(.*)$",re.M)
events=[]
...
oprx=re.compile(r"\[V67-OPPORTUNITY-LOSS\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+scale=(\d+)\s+reason=(\S+)\s+shadowMfeR=([-0-9.]+)\s+shadowMaeR=([-0-9.]+)\s+missedPositive=(True|False)\s+avoidedNegative=(True|False)")
opp=[]
...
# V67 explicit family economics, route economics, and concentration.
fam=collections.defaultdict(list); route=collections.defaultdict(list)
for x in b:
    fam[x["pattern"]].append(x); route[x["route"]].append(x)
def econ(q):
    v=[x["net"] for x in q]; gp=sum(x for x in v if x>0); gl=abs(sum(x for x in v if x<0))
    return {"trades":len(q),"net":sum(v),"pf":gp/gl if gl else (999 if gp else 0),
            "expectancy":sum(v)/len(v) if v else 0,"win_rate":sum(x>0 for x in v)/len(v) if v else 0}
family_performance={k:econ(q) for k,q in sorted(fam.items())}
route_performance={k:econ(q) for k,q in sorted(route.items())}
top_family_share=max([len(q) for q in fam.values()] or [0]) / max(1,len(b))
