...
rx=re.compile(r"\[V56-BASKET-CLOSED\].*?cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+subtype=(\S+)\s+route=(\S+)\s+dir=(\S+).*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)\s+reason=(\S+)")
b=[]
...
pat=(r"\[V56-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
evrx=re.compile(r"\[V56-EVENT\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+subtype=(\S+)\s+scale=(\d+)\s+tf=(\S+)\s+dir=(\S+)\s+state=(\S+)\s+route=(\S+)\s+conflict=(\S+)\s+waitMin=([-0-9.]+)\s+reason=(.*)$",re.M)
events=[]
...
oprx=re.compile(r"\[V56-OPPORTUNITY-LOSS\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+scale=(\d+)\s+reason=(\S+)\s+shadowMfeR=([-0-9.]+)\s+shadowMaeR=([-0-9.]+)\s+missedPositive=(True|False)\s+avoidedNegative=(True|False)")
opp=[]
...
sorx=re.compile(r"\[V56-SLOT-OCCUPANCY\].*?pattern=(.*?)\s+route=(\S+)\s+occupancyMinutes=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
"broker_profile_present":"[V56-BROKER-PROFILE]" in t,"data_snapshot_sha":a.data_sha.strip(),"mean_mfe_r":statistics.mean([x["mfe"] for x in b]) if b else 0,
...
anchor_rx=re.compile(r"\[V56-ENTRY-ANCHOR-FORENSICS\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+scale=(\d+)\s+completion=([-0-9.]+)\s+confirm=([-0-9.]+)\s+retest=([-0-9.]+)\s+completionMfeR=([-0-9.]+)\s+completionMaeR=([-0-9.]+)\s+confirmMfeR=([-0-9.]+)\s+confirmMaeR=([-0-9.]+)\s+retestMfeR=([-0-9.]+)\s+retestMaeR=([-0-9.]+)")
anchors=[]
...
truth_rx=re.compile(r"\[V56-DETECTOR-TRUTH\]\s+pattern=(.*?)\s+stage=(\S+)\s+count=(\d+)")
truth={}
for q in truth_rx.finditer(t):
 truth.setdefault(q.group(1),{})[q.group(2)]=int(q.group(3))
out["detector_truth"]=truth
conv_rx=re.compile(r"\[V56-CONVERSION-TRUTH\]\s+pattern=(.*?)\s+stage=(\S+)\s+count=(\d+)")
conv={}
for q in conv_rx.finditer(t): conv.setdefault(q.group(1),{})[q.group(2)]=int(q.group(3))
out["conversion_truth"]=conv
ev56_rx=re.compile(r"\[V56-EVIDENCE\]\s+cid=(\S+)\s+pattern=(.*?)\s+core=(True|False)\s+score=([-0-9.]+)\s+expR=([-0-9.]+)\s+margin2=([-0-9.]+)\s+hazard=([-0-9.]+)\s+rPerHour=([-0-9.]+)\s+admitted=(True|False)")
out["v56_evidence"]=[{"cid":q.group(1),"pattern":q.group(2),"core":q.group(3)=="True","score":float(q.group(4)),"expected_r_proxy":float(q.group(5)),"commercial_margin_proxy":float(q.group(6)),"core_arrival_hazard":float(q.group(7)),"expected_r_per_slot_hour":float(q.group(8)),"admitted":q.group(9)=="True"} for q in ev56_rx.finditer(t)]
gmeta_rx=re.compile(r"\[V56-BASKET-CLOSED\].*?cid=(\S+).*?plannedLegs=(\d+)\s+filledLegs=(\d+).*?entryImprovePips=([-0-9.]+)")
gmeta={q.group(1):{"planned_legs":int(q.group(2)),"filled_legs":int(q.group(3)),"entry_improve_pips":float(q.group(4))} for q in gmeta_rx.finditer(t)}
for row in out["basket_outcomes"]: row.update(gmeta.get(row["cid"],{"planned_legs":1,"filled_legs":1,"entry_improve_pips":0.0}))
out["grid_execution_enabled"]="[V56-GRID-CAUSAL-MODE] enabled=True" in t
