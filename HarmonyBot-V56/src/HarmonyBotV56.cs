...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV56 : Robot
    {
        private const string Version = "HarmonyBot V56 — Universal Harmonic High-Frequency Alpha Purification & Core-Protected Capital Engine";
        private const string BotPrefix = "HB56";
        ...
        [Parameter("Universal Harmonic Liberation", DefaultValue = false)]
        public bool EnableUniversalHarmonicLiberation { get; set; }

        [Parameter("Core Alpha Preservation", DefaultValue = false)]
        public bool EnableCoreAlphaPreservation { get; set; }

        [Parameter("Family Grid Alpha Core V3", DefaultValue = false)]
        public bool EnableFamilyGridAlphaCoreV3 { get; set; }

        [Parameter("State Aware Grid V3", DefaultValue = false)]
        public bool EnableStateAwareGridV3 { get; set; }

        [Parameter("Grid Leg Attribution", DefaultValue = true)]
        public bool EnableGridLegAttribution { get; set; }

        [Parameter("V56 Market-State Evidence Engine", DefaultValue = false)]
        public bool EnableV56MarketStateEvidence { get; set; }

        [Parameter("V56 Core-Protected Capital", DefaultValue = false)]
        public bool EnableV56CoreProtectedCapital { get; set; }

        [Parameter("V56 Min Evidence Score", DefaultValue = 0.64, MinValue = 0.50, MaxValue = 0.90)]
        public double V56MinEvidenceScore { get; set; }

        [Parameter("V56 Min Expected R Proxy", DefaultValue = 0.10, MinValue = 0.0, MaxValue = 1.0)]
        public double V56MinExpectedRProxy { get; set; }

        [Parameter("V56 Max Core Arrival Hazard", DefaultValue = 0.68, MinValue = 0.20, MaxValue = 0.95)]
        public double V56MaxCoreArrivalHazard { get; set; }

        [Parameter("V56 AB=CD Standalone Evidence", DefaultValue = 0.76, MinValue = 0.60, MaxValue = 0.95)]
        public double V56AbcdStandaloneEvidence { get; set; }

        [Parameter("V56 Evidence Grid V4", DefaultValue = false)]
        public bool EnableV56EvidenceGridV4 { get; set; }
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V56-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V56-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V56-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V56-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            Print("[V56-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m1Bars), _profiles.Count);
            Print("[V56-TIMEFRAME-AUDIT] primaryPattern=M15 execution=M1 macro=H4 intermediate=H1 allCompletedBars=true");
            Print("[V56-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
            Print("[V56-ALPHA-CONFIG] qualityObservation={0} legacyRegime={1} legacyEnhancedM1={2} capitalFeasibility={3} transitionVeto={4} exhaustionVeto={5} routeM1Veto={6}",
            ...
            Print("[V56-FREQUENCY-CONFIG] deferredRetention={0} agingPriority={1} ageBoost={2:F3} structuredRecall={3} recallGeometry={4:F3} recallPrz={5:F3} recallConfidence={6:F3}",
            ...
            Print("[V56-RESTORED-KERNEL] canonicalSetup={0} canonicalStandard={1} independentPivotGraph={2} transitionProof={3} m1Rescue={4} rescueBars={5} diversityScheduler={6} executionClock=M1 projectedD=false",
            ...
            Print("[V56-CONVERSION-ARCH] scaleRouteAdmission={0} temporalRescue={1} armedGrace={2} graceMinutes={3} preExecutionRevalidation={4}",
            ...
            Print("[V56-FAMILY-NATIVE] conversion={0} observation={1} principle=PATTERN_IDENTITY_NEQ_EXECUTION_IDENTITY", EnableFamilyNativeConversion, EnableFamilyNativeObservation);
            Print("[V56-COMPLETION-CONTRACT] canonicalContracts={0} familyCompletion={1} windowBars={2} provenLane=ABCD_SHARK_CYPHER_FROZEN",
                EnableCanonicalFamilyContracts, EnableFamilyCompletionContract, FamilyConfirmationWindowBars);
            Print("[V56-STRUCTURAL-GRID-CONTRACT] gridSpanV2={0} structuralInvalidationV2={1} rule=PRZ_LEGALITY_PLUS_RISK_RR_NOT_LEGACY_XA_MINSPAN",
                EnableGridSpanSemanticV2, EnableStructuralInvalidationV2);
            Print("[V56-MATH-GEOMETRY] jointGeometry={0} executionCorridor={1} entryAnchorForensics={2} rule=FAMILY_NATIVE_GEOMETRY_WITHOUT_RISK_RELAXATION",
                EnableFamilyNativeJointGeometry, EnableFamilyNativeExecutionCorridor, EnableEntryAnchorForensics);
            Print("[V56-DETECTOR-ARCH] identityReconstruction={0} boundedPivotGraph={1} maxMicroSkips={2} familyQuota={3} projectedPrz={4} truthLedger={5} rule=DETECT_MANY_EXECUTE_ONE",
            ...
            Print("[V56-CONVERSION-ARCH-V2] qualification={0} router={1} confirmation={2} contextBus={3} gridExecution={4} rule=FAMILY_NATIVE_POST_DETECTOR_CAUSAL_ISOLATION",
            ...
            Print("[V56-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V46_SCALE_CONVERSION_FROZEN",
        ...
        protected override void OnStop()
        {
            ...
            {
                var x = kv.Value;
                Print("[V56-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} nativeTemporalPass={6} armed={7} slotBlocked={8} parked={9} revalidated={10} revalidationRejected={11} basketPlanned={12} leg0={13} leg1={14} leg2={15} leg3={16} basketClosed={17} executed={18} expired={19} rejected={20} invalidated={21}",
            ...
            if (EnableEntryAnchorForensics)
            {
                ...
                {
                    Print("[V56-ENTRY-ANCHOR-FORENSICS] cid={0} setup={1} pattern={2} route={3} scale={4} completion={5} confirm={6} retest={7} completionMfeR={8:F3} completionMaeR={9:F3} confirmMfeR={10:F3} confirmMaeR={11:F3} retestMfeR={12:F3} retestMaeR={13:F3}",
            ...
            if (EnableDetectorTruthLedger)
            {
                foreach (var kv in _detectorTruth.OrderBy(x => x.Key))
                {
                    ...
                    Print("[V56-DETECTOR-TRUTH] pattern={0} stage={1} count={2}", pattern, stage, kv.Value);
                }
            }
            foreach (var kv in _conversionTruth.OrderBy(x => x.Key))
            {
                ...
                string pattern = sep >= 0 ? kv.Key.Substring(0, sep) : "UNKNOWN";
                string stage = sep >= 0 ? kv.Key.Substring(sep + 2) : kv.Key;
                Print("[V56-CONVERSION-TRUTH] pattern={0} stage={1} count={2}", pattern, stage, kv.Value);
            }
            Print("[V56-GRID-CAUSAL-MODE] enabled={0} rule=GRID_CHANGES_ENTRY_DISTRIBUTION_NOT_SIGNAL_THESIS", EnableFibonacciGridExecution);
            Print("[V56-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V56-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
                _alphaQualityRejected, _regimeRejected, _confirmationRejected, _capitalInfeasibleCandidates, _alphaPassed);
            Print("[V56-FREQUENCY-SUMMARY] schedulerDeferred={0} schedulerRecoveredExecutions={1} structuredRecallAdmitted={2} activeDeferred={3}",
                _schedulerDeferred, _schedulerRecoveredExecutions, _structuredRecallAdmitted, _deferredCandidates.Count);
            Print("[V56-INDEPENDENT-SETUP-SUMMARY] duplicateSuppressed={0} uniqueExecuted={1} rescueAdmissions={2} transitionProofRejected={3} independentScaleCandidates={4}",
            ...
            Print("[V56-CONVERSION-SUMMARY] scaleRouteRejected={0} temporalRescueAdmissions={1} armedGraceExtended={2} preExecutionRevalidationRejected={3}",
            ...
            Print("[V56-THROUGHPUT-SUMMARY] slotBlocked={0} parked={1} revalidated={2} revalidationRejected={3} recoveredExecutions={4} nativeTemporalPass={5} hardLifetimeExpired={6} decayRejected={7} avgSlotWaitMin={8:F2} medianSlotWaitMin={9:F2} p90SlotWaitMin={10:F2} avgBasketOccupancyMin={11:F2} missedPositive={12} avoidedNegative={13}",
            ...
            foreach (var p in _profiles.Select(x => x.Name).Distinct().OrderBy(x => x))
            {
                ...
                Print("[V56-FAMILY-CONTRACT-SUMMARY] pattern={0} pass={1} windowReject={2}", p, pass, reject);
            }
            foreach (var kv in _gridPlanRejectReasons.OrderBy(k => k.Key))
                Print("[V56-GRID-REJECT-SUMMARY] reason={0} count={1}", kv.Key, kv.Value);
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V56-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                if (!qualificationPass)
                {
                    ...
                    if (!EnableUniversalHarmonicLiberation)
                    {
                        Reject(record, "PATTERN_QUALITY");
                        continue;
                    }
                    record.LiberationShadowQuality = true;
                    ConversionTruth(signal.PatternName, "RESEARCH_PLANE_QUALITY_OBSERVED");
                    Event(record, "RESEARCH_PLANE_SHADOW_ONLY_" + familyQualificationReason);
                    Reject(record, "RESEARCH_PLANE_NO_CAPITAL_ADMISSION");
                    continue;
                }
                else ConversionTruth(signal.PatternName, "QUALIFICATION_PASS");
                ...
                if (false && EnableUniversalHarmonicLiberation && record.Route == HarmonicRoute.NO_TRADE)
                {
                    record.Route = LiberationDiagnosticRoute(signal, record.Conflict, regime, record.Context);
                    if (record.Route != HarmonicRoute.NO_TRADE)
                    {
                        record.LiberationShadowRoute = true;
                        ConversionTruth(signal.PatternName, "LIBERATION_ROUTE_CONTINUE_" + record.Route);
                    }
                }
                ...
                Print("[V56-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (EnableV56MarketStateEvidence)
                    {
                        bool admitted=EvaluateV56Evidence(c);
                        Print("[V56-EVIDENCE] cid={0} pattern={1} core={2} score={3:F3} expR={4:F3} margin2={5:F3} hazard={6:F3} rPerHour={7:F3} admitted={8}",
                            c.CandidateId,c.Signal.PatternName,c.ChampionCoreProtected,c.EvidenceScore,c.ExpectedNetRProxy,c.CommercialMarginProxy,c.CoreArrivalHazard,c.ExpectedRPerSlotHour,admitted);
                        if(!admitted)
                        {
                            c.ResearchShadowOnly=true;
                            Transition(c,CandidateState.RESEARCH_SHADOW,"EVIDENCE_RESEARCH_SHADOW");
                            ConversionTruth(c.Signal.PatternName,"EVIDENCE_RESEARCH_ONLY");
                            continue;
                        }
                        ConversionTruth(c.Signal.PatternName,c.ChampionCoreProtected?"CHAMPION_CORE_ADMITTED":"EVIDENCE_EXPANSION_ADMITTED");
                    }
        ...
        private void TryScheduleAndExecute()
        {
            ...
            var armed = armedQuery
                .OrderByDescending(c => EnableV56CoreProtectedCapital && c.ChampionCoreProtected ? 1 : 0)
        ...
        private void MarkOpportunityTerminal(CandidateRecord c, string reason)
        {
            ...
            Print("[V56-OPPORTUNITY-LOSS] cid={0} setup={1} pattern={2} route={3} scale={4} reason={5} shadowMfeR={6:F3} shadowMaeR={7:F3} missedPositive={8} avoidedNegative={9}",
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            double[] activeFractions = EnableV56EvidenceGridV4 ? FamilyGridFractionsV4(c, p.GridFractions) : (EnableFamilyGridAlphaCoreV3 ? FamilyGridFractionsV3(c.Signal.PatternName, p.GridFractions) : p.GridFractions);
            double[] activeWeights = EnableV56EvidenceGridV4 ? FamilyGridRiskWeightsV4(activeFractions.Length) : (EnableFamilyGridAlphaCoreV3 ? FamilyGridRiskWeightsV3(c.Signal.PatternName, activeFractions.Length) : p.GridRiskWeights);
            int maxLegs = Math.Min(Math.Min(routeMax, p.MaximumGridLegs), activeFractions.Length);
            ...
            {
                double fraction = activeFractions[leg];
                ...
                double riskWeight = !EnableFibonacciGridExecution ? 1.0 : (leg < activeWeights.Length ? activeWeights[leg] : 0);
            ...
            Print("[V56-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V56-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V56-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void ReconcileAndManageBaskets()
        {
            ...
            foreach (var basket in _baskets.Values.Where(b => b.IsActive).ToList())
            {
                ...
                double stateAwareCancelR = EnableStateAwareGridV3 ? FamilyGridCancelMfeR(basket.Pattern) : GridCancelMfeR;
                if (basket.PeakR >= stateAwareCancelR && pending.Count > 0)
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V56-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V56-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V56-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V56-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V56-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V56-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V56-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V56-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V56-BASKET-CLOSED] basket={0} cid={1} setup={2} pattern={3} subtype={4} route={5} dir={6} plannedLegs={7} filledLegs={8} anchor={9} avgEntry={10} entryImprovePips={11:F3} stop={12} target={13} initialRisk={14:F2} worstRisk={15:F2} mfeR={16:F3} maeR={17:F3} realizedR={18:F3} net={19:F2} reason={20}",
            ...
            Print("[V56-SLOT-OCCUPANCY] basket={0} cid={1} pattern={2} route={3} occupancyMinutes={4:F2} realizedR={5:F3} net={6:F2}",
        ...
        {
            Print("[V56-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
        ...
        private HarmonicRoute RouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (baseRoute == HarmonicRoute.NO_TRADE && EnableStructuredRecallExpansion)
            {
                ...
                if (baseRoute != HarmonicRoute.NO_TRADE)
                {
                    ...
                    Print("[V56-RECALL-ADMIT] pattern={0} direction={1} route={2} conflict={3} geometry={4:F3} prz={5:F3} confidence={6:F3} efficiency={7:F3} atrRatio={8:F3} extensionAtr={9:F3}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionProofGate)
            {
                ...
                if (!actualStructuralTransition)
                {
                    ...
                    Print("[V56-TRANSITION-PROOF-REJECT] pattern={0} conflict={1} transition={2} adxSlope={3:F2}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionStateVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V56-ROUTE-VETO] type=TRANSITION_STATE conflict={0} transition={1} adxSlope={2:F2}",
            ...
            if (baseRoute == HarmonicRoute.EXHAUSTION_REVERSAL && EnableExhaustionEvidenceVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V56-ROUTE-VETO] type=EXHAUSTION_EVIDENCE extensionAtr={0:F3} adxSlope={1:F2}",
        ...
        private OrthogonalContextFeatures BuildOrthogonalContextFeatureBus(PatternSignal s, RegimeSnapshot r)
        {
            ...
            double vwapNow=SessionVwap(_m15Bars,m15,48), vwapPrev=SessionVwap(_m15Bars,Math.Max(20,m15-4),48);
            f.VwapDistanceAtr=a0>0?Math.Abs(s.D.Price-vwapNow)/a0:0;
            f.VwapSlopeAtr=a0>0?(vwapNow-vwapPrev)/a0:0;
            f.AdxLevelNorm=VClamp(((r.AdxH1+r.AdxH4)*.5)/40.0);
            f.AdxSlopeNorm=VClamp(.5+.5*Math.Tanh(r.AdxH1Slope/8.0));
            f.Choppiness=ChoppinessNorm(_m15Bars,m15,14);
            f.BandwidthNorm=BandwidthNorm(_m15Bars,m15,20,a0);
            f.TickVolumePercentile=TickVolumePercentile(_m15Bars,m15,40);
            return f;
        }
        ...
        private double SessionVwap(Bars bars, int end, int lookback)
        {
            if (bars == null || end < 1 || end >= bars.Count) return 0;
            int start=Math.Max(0,end-Math.Max(4,lookback)+1); double pv=0,vol=0;
            for(int i=start;i<=end;i++)
            {
            ...
            return vol>0?pv/vol:bars.ClosePrices[end];
        }

        private double TickVolumePercentile(Bars bars, int end, int lookback)
        {
            if(bars==null||end<2||end>=bars.Count)return .5;
            int start=Math.Max(0,end-Math.Max(5,lookback)+1); double cur=bars.TickVolumes[end]; int n=0,le=0;
            for(int i=start;i<=end;i++){n++;if(bars.TickVolumes[i]<=cur)le++;}
            return n>0?VClamp((double)le/n):.5;
        }

        private double BandwidthNorm(Bars bars,int end,int period,double atr)
        {
            if(bars==null||end<period||end>=bars.Count||atr<=0)return .5;
            double mean=0; for(int i=end-period+1;i<=end;i++)mean+=bars.ClosePrices[i]; mean/=period;
            double v=0; for(int i=end-period+1;i<=end;i++){double d=bars.ClosePrices[i]-mean;v+=d*d;}
            double sd=Math.Sqrt(v/period);
            return VClamp(sd/(atr*2.0));
        }

        private double ChoppinessNorm(Bars bars,int end,int period)
        {
            if(bars==null||end<period+1||end>=bars.Count)return .5;
            double tr=0,hi=double.MinValue,lo=double.MaxValue;
            for(int i=end-period+1;i<=end;i++)
            {
                double prev=bars.ClosePrices[i-1];
                tr+=Math.Max(bars.HighPrices[i]-bars.LowPrices[i],Math.Max(Math.Abs(bars.HighPrices[i]-prev),Math.Abs(bars.LowPrices[i]-prev)));
                hi=Math.Max(hi,bars.HighPrices[i]); lo=Math.Min(lo,bars.LowPrices[i]);
            }
            double range=Math.Max(hi-lo,_symbol.PipSize);
            double ci=100.0*Math.Log10(Math.Max(1.0,tr/range))/Math.Log10(period);
            return VClamp(ci/100.0);
        }

        private bool IsV56ChampionCoreCandidate(CandidateRecord c)
        {
            if(c==null||c.Signal==null||c.Signal.Profile==null)return false;
            if(c.LiberationShadowQuality||c.LiberationShadowRoute)return false;
            bool legacyQual=c.Signal.GeometryQuality>=Math.Max(MinGeometryQuality,c.Signal.Profile.MinGeometry) &&
                            c.Signal.PrzConfluence>=Math.Max(MinPrzConfluence,c.Signal.Profile.MinPrz);
            if(!legacyQual)return false;
            var legacyRoute=RouteSignal(c.Signal,c.Conflict,c.Regime);
            if(legacyRoute==HarmonicRoute.NO_TRADE)return false;
            if(c.NetRR+1e-9<MinimumNetRR)return false;
            return c.ConfirmationScore>=.55;
        }

        private double CoreArrivalHazard(CandidateRecord c)
        {
            DateTime u=Server.Time.ToUniversalTime(); int h=u.Hour;
            double peak=((h>=7&&h<=10)||(h>=13&&h<=16))?.48:.26;
            double trend=c!=null&&c.Regime!=null?.18*VClamp(c.Regime.TrendStrength):0;
            double stability=c!=null&&c.Context!=null?.12*c.Context.Stability:0;
            return VClamp(peak+trend-stability);
        }

        private bool EvaluateV56Evidence(CandidateRecord c)
        {
            if(c==null||c.Signal==null)return false;
            c.ChampionCoreProtected=EnableV56CoreProtectedCapital&&IsV56ChampionCoreCandidate(c);
            if(c.ChampionCoreProtected)
            {
                c.EvidenceAdmitted=true; c.EvidenceScore=1.0; c.ExpectedNetRProxy=Math.Max(.10,c.NetRR*.45); c.CommercialMarginProxy=.50; c.CoreArrivalHazard=0; c.ExpectedRPerSlotHour=c.ExpectedNetRProxy;
                return true;
            }
            var f=c.Context; if(f==null)return false;
            ...
            double slopeFit=c.Signal.Direction==TradeDirection.Buy?VClamp(.5+.5*Math.Tanh(f.VwapSlopeAtr*2.0)):VClamp(.5-.5*Math.Tanh(f.VwapSlopeAtr*2.0));
            double vwapExtreme=VClamp(f.VwapDistanceAtr/2.0), quiet=VClamp(1.0-f.Choppiness), adxFade=VClamp(1.0-f.AdxSlopeNorm);
            double score;
            if(p=="Shark"||p=="Cypher") score=VClamp(.30*c.ContextScore+.20*c.ConfirmationScore+.20*quiet+.15*f.TickVolumePercentile+.15*c.FamilyQualificationScore);
            else if(retr) score=VClamp(.20*c.ContextScore+.15*quiet+.15*slopeFit+.15*f.MicroEfficiency+.15*f.AdxLevelNorm+.10*f.Stability+.10*f.TickVolumePercentile);
            else if(ext) score=VClamp(.20*f.RangeExtreme+.20*vwapExtreme+.15*f.TickVolumePercentile+.15*adxFade+.10*f.LiquidityProximity+.10*(c.FamilyFailedExtension?1:0)+.10*(c.FamilyReclaim?1:0));
            else if(p=="5-0") score=VClamp(.20*(c.Regime!=null&&c.Regime.Transition?1:0)+.20*(c.FamilyFailedExtension?1:0)+.15*(c.FamilyBos?1:0)+.15*(c.FamilyRetest?1:0)+.10*f.TickVolumePercentile+.10*quiet+.10*c.ContextScore);
            else score=VClamp(.20*c.ContextScore+.15*f.MicroEfficiency+.15*quiet+.15*f.TickVolumePercentile+.15*c.ConfirmationScore+.10*f.LiquidityProximity+.10*f.Stability);
            c.EvidenceScore=score;
            double pwin=VClamp(.25+.55*score); pwin=Math.Min(.74,Math.Max(.20,pwin));
            double riskPips=Math.Max(1.0,PriceToPips(Math.Abs(c.GridPlan.EntryAnchor-c.GridPlan.StructuralStop)));
            double costR=Math.Max(0,CurrentSpreadPips()+RoundTurnCommissionPips+SlippageStressPips)/riskPips;
            double rr=Math.Max(MinimumNetRR,c.NetRR);
            c.ExpectedNetRProxy=pwin*rr-(1.0-pwin)-costR;
            c.CommercialMarginProxy=pwin*rr-2.0*(1.0-pwin)-costR;
            c.CoreArrivalHazard=CoreArrivalHazard(c);
            double holdHours=c.Route==HarmonicRoute.EXHAUSTION_REVERSAL?1.25:c.Route==HarmonicRoute.TRANSITION_REVERSAL?1.75:2.25;
            c.ExpectedRPerSlotHour=c.ExpectedNetRProxy/holdHours;
            double minScore=p=="AB=CD"?Math.Max(V56MinEvidenceScore,V56AbcdStandaloneEvidence):V56MinEvidenceScore;
            bool pass=score>=minScore && c.ExpectedNetRProxy>=V56MinExpectedRProxy && c.CommercialMarginProxy>=0 && c.CoreArrivalHazard<=V56MaxCoreArrivalHazard && c.ExpectedRPerSlotHour>0;
            c.EvidenceAdmitted=pass;
            return pass;
        }
        ...
        private HarmonicRoute LiberationDiagnosticRoute(PatternSignal s, MtfConflict conflict, RegimeSnapshot r, OrthogonalContextFeatures f)
        {
            if (s == null || r == null) return HarmonicRoute.NO_TRADE;
            string p=s.PatternName??"";
            bool extension=p=="Alt Bat"||p=="Butterfly"||p=="Crab"||p=="Deep Crab";
            ...
            if(extension)
            {
                if(r.ExtensionAtr>=.45 || (f!=null&&f.RangeExtreme>=.58)) return HarmonicRoute.EXHAUSTION_REVERSAL;
            ...
            if(p=="AB=CD")
            {
                var q=RouteSignal(s,conflict,r);
                if(q!=HarmonicRoute.NO_TRADE)return q;
                if(r.Transition)return HarmonicRoute.TRANSITION_REVERSAL;
            }
            return HarmonicRoute.NO_TRADE;
        }

        private double CoreAlphaPreservationScore(CandidateRecord c)
        {
            if(c==null||c.Signal==null)return 0;
            string p=c.Signal.PatternName??"";
            double family=(p=="Shark"||p=="Cypher")?1.0:(p=="Rat"?.72:(p=="AB=CD"?.55:.35));
            double provenRoute=c.Route==HarmonicRoute.EXHAUSTION_REVERSAL?1.0:(c.Route==HarmonicRoute.TREND_ALIGNED_REVERSAL?.75:.55);
            double quality=VClamp(.45*c.Signal.GeometryQuality+.30*c.Signal.PrzConfluence+.25*c.ConfirmationScore);
            return VClamp(.35*family+.25*provenRoute+.40*quality);
        }

        private double[] FamilyGridFractionsV3(string pattern, double[] legacy)
        {
            if(pattern=="Shark"||pattern=="Cypher") return new[]{0.0,.236};
            if(pattern=="Crab"||pattern=="Deep Crab"||pattern=="Butterfly"||pattern=="Alt Bat") return new[]{0.0,.236,.382};
            if(pattern=="Gartley"||pattern=="Bat"||pattern=="Deep Gartley"||pattern=="Rat") return new[]{0.0,.236,.382,.618};
            if(pattern=="5-0") return new[]{0.0,.236,.382};
            if(pattern=="AB=CD") return new[]{0.0,.236,.382,.618};
            return legacy??new[]{0.0};
        }

        private double[] FamilyGridRiskWeightsV3(string pattern, int count)
        {
            double[] w;
            if(pattern=="Shark"||pattern=="Cypher") w=new[]{.65,.35};
            else if(pattern=="Crab"||pattern=="Deep Crab"||pattern=="Butterfly"||pattern=="Alt Bat") w=new[]{.50,.30,.20};
            else if(pattern=="5-0") w=new[]{.55,.30,.15};
            else w=new[]{3.0/7.0,2.0/7.0,1.0/7.0,1.0/7.0};
            return w.Take(Math.Max(1,Math.Min(count,w.Length))).ToArray();
        }

        private double[] FamilyGridFractionsV4(CandidateRecord c, double[] legacy)
        {
            double[] x=legacy??new[]{0.0,.236,.382,.618};
            int depth=c!=null&&c.ChampionCoreProtected?Math.Min(4,x.Length):(c!=null&&c.EvidenceScore>=.82?Math.Min(4,x.Length):c!=null&&c.EvidenceScore>=.72?Math.Min(3,x.Length):Math.Min(2,x.Length));
            return x.Take(Math.Max(1,depth)).ToArray();
        }

        private double[] FamilyGridRiskWeightsV4(int count)
        {
            if(count<=1)return new[]{1.0};
            if(count==2)return new[]{.60,.40};
            if(count==3)return new[]{.50,.30,.20};
            return new[]{3.0/7.0,2.0/7.0,1.0/7.0,1.0/7.0};
        }

        private double FamilyGridCancelMfeR(string pattern)
        {
            if(pattern=="Crab"||pattern=="Deep Crab"||pattern=="Butterfly"||pattern=="Alt Bat")return .40;
            if(pattern=="Shark"||pattern=="Cypher")return .45;
            if(pattern=="5-0")return .45;
            return .50;
        }
        ...
        private double CandidateRank(CandidateRecord c)
        {
            ...
            if (EnableCoreAlphaPreservation)
            {
                double core = CoreAlphaPreservationScore(c);
                c.CoreAlphaScore = core;
                baseRank += .18 * core;
            }
            if (EnableUniversalHarmonicLiberation && (c.LiberationShadowQuality || c.LiberationShadowRoute))
                return -1000000.0; // Research-plane hypotheses never compete for capital slots.
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V56-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        private void Ledger(CandidateRecord c, CandidateState state, string reason)
        {
            ...
            Print("[V56-EVENT] cid={0} setup={1} pattern={2} subtype={3} scale={4} tf={5} dir={6} state={7} route={8} conflict={9} waitMin={10:F2} reason={11}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V56-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V56-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
    ...
    public enum CandidateState { DETECTED, VALIDATED, ROUTED, WAIT_PRZ, CONFIRMING, RESEARCH_SHADOW, ARMED, SLOT_BLOCKED, PARKED, REVALIDATING, EXECUTABLE, EXECUTED, EXPIRED, REJECTED, INVALIDATED }
    ...
    {
        public double AtrExpansion, AtrVolOfVol, MicroEfficiency, HtfLiquidityDistanceAtr, LiquidityProximity, RangeExtreme, Stability, VwapDistanceAtr, VwapSlopeAtr, AdxLevelNorm, AdxSlopeNorm, Choppiness, BandwidthNorm, TickVolumePercentile;
    }
    ...
    public sealed class CandidateRecord
    {
        ...
        public double AlphaQualityScore, RegimeScore, ContextScore, FamilyQualificationScore, CoreAlphaScore, CapitalMinL0Risk, CapitalMinL0Margin, EvidenceScore, ExpectedNetRProxy, CommercialMarginProxy, CoreArrivalHazard, ExpectedRPerSlotHour;
        public bool LiberationShadowQuality, LiberationShadowRoute, ChampionCoreProtected, ResearchShadowOnly, EvidenceAdmitted;
