...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV57 : Robot
    {
        private const string Version = "HarmonyBot V57 — Regime-Calibrated Universal Harmonic Family Portfolio Engine";
        ...
        [Parameter("V57 Calibrated Capital Admission", DefaultValue = false)]
        public bool EnableV57CalibratedCapitalAdmission { get; set; }

        [Parameter("V57 Family Representative Arbitration", DefaultValue = true)]
        public bool EnableV57FamilyRepresentativeArbitration { get; set; }

        [Parameter("V57 Standalone AB=CD Capital", DefaultValue = false)]
        public bool EnableV57AbcdStandaloneCapital { get; set; }

        [Parameter("V57 Grid Execution", DefaultValue = true)]
        public bool EnableV57GridExecution { get; set; }

        [Parameter("V57 Min Calibrated Lower R", DefaultValue = 0.0, MinValue = -1.0, MaxValue = 1.0)]
        public double V57MinCalibratedLowerBoundR { get; set; }

        [Parameter("V57 Calibration Manifest", DefaultValue = "")]
        public string V57CalibrationManifest { get; set; }
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V57-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V57-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V57-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V57-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            InitializeV57FamilyBooks();
            ParseV57CalibrationManifest();
            Print("[V57-FAMILY-PORTFOLIO] familyBooks={0} virtualParallel={1} calibratedAdmission={2} representatives={3} abcdStandalone={4} calibrationCells={5}",
                EnableV57FamilyBooks, EnableV57VirtualParallelExecution, EnableV57CalibratedCapitalAdmission,
                EnableV57FamilyRepresentativeArbitration, EnableV57AbcdStandaloneCapital, _calibrationCells.Count);
            ...
            Print("[V57-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m1Bars), _profiles.Count);
            Print("[V57-TIMEFRAME-AUDIT] primaryPattern=M15 execution=M1 macro=H4 intermediate=H1 allCompletedBars=true");
            Print("[V57-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
            Print("[V57-ALPHA-CONFIG] qualityObservation={0} legacyRegime={1} legacyEnhancedM1={2} capitalFeasibility={3} transitionVeto={4} exhaustionVeto={5} routeM1Veto={6}",
            ...
            Print("[V57-FREQUENCY-CONFIG] deferredRetention={0} agingPriority={1} ageBoost={2:F3} structuredRecall={3} recallGeometry={4:F3} recallPrz={5:F3} recallConfidence={6:F3}",
            ...
            Print("[V57-RESTORED-KERNEL] canonicalSetup={0} canonicalStandard={1} independentPivotGraph={2} transitionProof={3} m1Rescue={4} rescueBars={5} diversityScheduler={6} executionClock=M1 projectedD=false",
            ...
            Print("[V57-CONVERSION-ARCH] scaleRouteAdmission={0} temporalRescue={1} armedGrace={2} graceMinutes={3} preExecutionRevalidation={4}",
            ...
            Print("[V57-FAMILY-NATIVE] conversion={0} observation={1} principle=PATTERN_IDENTITY_NEQ_EXECUTION_IDENTITY", EnableFamilyNativeConversion, EnableFamilyNativeObservation);
            Print("[V57-COMPLETION-CONTRACT] canonicalContracts={0} familyCompletion={1} windowBars={2} provenLane=ABCD_SHARK_CYPHER_FROZEN",
                EnableCanonicalFamilyContracts, EnableFamilyCompletionContract, FamilyConfirmationWindowBars);
            Print("[V57-STRUCTURAL-GRID-CONTRACT] gridSpanV2={0} structuralInvalidationV2={1} rule=PRZ_LEGALITY_PLUS_RISK_RR_NOT_LEGACY_XA_MINSPAN",
                EnableGridSpanSemanticV2, EnableStructuralInvalidationV2);
            Print("[V57-MATH-GEOMETRY] jointGeometry={0} executionCorridor={1} entryAnchorForensics={2} rule=FAMILY_NATIVE_GEOMETRY_WITHOUT_RISK_RELAXATION",
                EnableFamilyNativeJointGeometry, EnableFamilyNativeExecutionCorridor, EnableEntryAnchorForensics);
            Print("[V57-DETECTOR-ARCH] identityReconstruction={0} boundedPivotGraph={1} maxMicroSkips={2} familyQuota={3} projectedPrz={4} truthLedger={5} rule=DETECT_MANY_EXECUTE_ONE",
            ...
            Print("[V57-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V46_SCALE_CONVERSION_FROZEN",
        ...
        {
            CloseOpenV57ShadowTradesAtStop();
            foreach (var book in _familyBooks.OrderBy(x => x.Key))
            {
                ...
                Print("[V57-FAMILY-BOOK] pattern={0} candidates={1} shadowStarted={2} shadowClosed={3}", book.Key, book.Value.Count, started, closed);
            }
            ...
            {
                var x = kv.Value;
                Print("[V57-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} nativeTemporalPass={6} armed={7} slotBlocked={8} parked={9} revalidated={10} revalidationRejected={11} basketPlanned={12} leg0={13} leg1={14} leg2={15} leg3={16} basketClosed={17} executed={18} expired={19} rejected={20} invalidated={21}",
            ...
            if (EnableEntryAnchorForensics)
            {
                ...
                {
                    Print("[V57-ENTRY-ANCHOR-FORENSICS] cid={0} setup={1} pattern={2} route={3} scale={4} completion={5} confirm={6} retest={7} completionMfeR={8:F3} completionMaeR={9:F3} confirmMfeR={10:F3} confirmMaeR={11:F3} retestMfeR={12:F3} retestMaeR={13:F3}",
            ...
            if (EnableDetectorTruthLedger)
            {
                foreach (var kv in _detectorTruth.OrderBy(x => x.Key))
                {
                    ...
                    Print("[V57-DETECTOR-TRUTH] pattern={0} stage={1} count={2}", pattern, stage, kv.Value);
                }
            }
            Print("[V57-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V57-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
                _alphaQualityRejected, _regimeRejected, _confirmationRejected, _capitalInfeasibleCandidates, _alphaPassed);
            Print("[V57-FREQUENCY-SUMMARY] schedulerDeferred={0} schedulerRecoveredExecutions={1} structuredRecallAdmitted={2} activeDeferred={3}",
                _schedulerDeferred, _schedulerRecoveredExecutions, _structuredRecallAdmitted, _deferredCandidates.Count);
            Print("[V57-INDEPENDENT-SETUP-SUMMARY] duplicateSuppressed={0} uniqueExecuted={1} rescueAdmissions={2} transitionProofRejected={3} independentScaleCandidates={4}",
            ...
            Print("[V57-CONVERSION-SUMMARY] scaleRouteRejected={0} temporalRescueAdmissions={1} armedGraceExtended={2} preExecutionRevalidationRejected={3}",
            ...
            Print("[V57-THROUGHPUT-SUMMARY] slotBlocked={0} parked={1} revalidated={2} revalidationRejected={3} recoveredExecutions={4} nativeTemporalPass={5} hardLifetimeExpired={6} decayRejected={7} avgSlotWaitMin={8:F2} medianSlotWaitMin={9:F2} p90SlotWaitMin={10:F2} avgBasketOccupancyMin={11:F2} missedPositive={12} avoidedNegative={13}",
            ...
            foreach (var p in _profiles.Select(x => x.Name).Distinct().OrderBy(x => x))
            {
                ...
                Print("[V57-FAMILY-CONTRACT-SUMMARY] pattern={0} pass={1} windowReject={2}", p, pass, reject);
            }
            foreach (var kv in _gridPlanRejectReasons.OrderBy(k => k.Key))
                Print("[V57-GRID-REJECT-SUMMARY] reason={0} count={1}", kv.Key, kv.Value);
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V57-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                Print("[V57-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void ProcessNewM1Close()
        {
            ...
            UpdateV57FamilyShadowTrades(i, utc);
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    StartV57FamilyShadowTrade(c, i, utc);
                    ...
                    if (EnableV57CalibratedCapitalAdmission && !V57CalibratedCapitalEligible(c))
                    {
        ...
        private void TryScheduleAndExecute()
        {
            ...
            if (EnableV57FamilyRepresentativeArbitration)
            {
                armedQuery = armedQuery
                    .GroupBy(c => c.Signal == null ? "UNKNOWN" : c.Signal.PatternName)
                    .Select(g => g.OrderByDescending(c => V57FamilyCapitalScore(c, now)).ThenByDescending(c => c.Rank).First());
            }

            var armed = armedQuery
                .OrderByDescending(c => EnableV57FamilyRepresentativeArbitration ? V57FamilyCapitalScore(c, now) :
        ...
        private void MarkOpportunityTerminal(CandidateRecord c, string reason)
        {
            ...
            Print("[V57-OPPORTUNITY-LOSS] cid={0} setup={1} pattern={2} route={3} scale={4} reason={5} shadowMfeR={6:F3} shadowMaeR={7:F3} missedPositive={8} avoidedNegative={9}",
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            int routeMax = !EnableV57GridExecution ? 1 :
            ...
            Print("[V57-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V57-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V57-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V57-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V57-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V57-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V57-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V57-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V57-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V57-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V57-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V57-BASKET-CLOSED] basket={0} cid={1} setup={2} pattern={3} subtype={4} route={5} dir={6} plannedLegs={7} filledLegs={8} anchor={9} avgEntry={10} entryImprovePips={11:F3} stop={12} target={13} initialRisk={14:F2} worstRisk={15:F2} mfeR={16:F3} maeR={17:F3} realizedR={18:F3} net={19:F2} reason={20}",
            ...
            Print("[V57-SLOT-OCCUPANCY] basket={0} cid={1} pattern={2} route={3} occupancyMinutes={4:F2} realizedR={5:F3} net={6:F2}",
        ...
        {
            Print("[V57-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
        ...
        private HarmonicRoute RouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (baseRoute == HarmonicRoute.NO_TRADE && EnableStructuredRecallExpansion)
            {
                ...
                if (baseRoute != HarmonicRoute.NO_TRADE)
                {
                    ...
                    Print("[V57-RECALL-ADMIT] pattern={0} direction={1} route={2} conflict={3} geometry={4:F3} prz={5:F3} confidence={6:F3} efficiency={7:F3} atrRatio={8:F3} extensionAtr={9:F3}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionProofGate)
            {
                ...
                if (!actualStructuralTransition)
                {
                    ...
                    Print("[V57-TRANSITION-PROOF-REJECT] pattern={0} conflict={1} transition={2} adxSlope={3:F2}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionStateVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V57-ROUTE-VETO] type=TRANSITION_STATE conflict={0} transition={1} adxSlope={2:F2}",
            ...
            if (baseRoute == HarmonicRoute.EXHAUSTION_REVERSAL && EnableExhaustionEvidenceVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V57-ROUTE-VETO] type=EXHAUSTION_EVIDENCE extensionAtr={0:F3} adxSlope={1:F2}",
        ...
        private void InitializeV57FamilyBooks()
        {
            if (!EnableV57FamilyBooks) return;
            foreach (var p in _profiles)
                if (!_familyBooks.ContainsKey(p.Name)) _familyBooks[p.Name]=new HashSet<string>();
        }

        private void AddToV57FamilyBook(CandidateRecord c)
        {
            if (!EnableV57FamilyBooks || c==null || c.Signal==null) return;
            HashSet<string> book;
        ...
        private string V57PatternToken(string pattern)
        {
            return (pattern??"UNKNOWN").Replace(" ","_");
        }

        private string V57RegimeBucket(CandidateRecord c)
        {
        ...
        private string V57CalibrationKey(CandidateRecord c)
        {
            ...
            return V57PatternToken(c.Signal.PatternName)+"|"+c.Route+"|"+V57RegimeBucket(c);
        }

        private void ParseV57CalibrationManifest()
        {
            _calibrationCells.Clear();
            if(string.IsNullOrWhiteSpace(V57CalibrationManifest))return;
            foreach(var raw in V57CalibrationManifest.Split(new[]{';'},StringSplitOptions.RemoveEmptyEntries))
            {
        ...
        private CalibrationCell V57CalibrationCell(CandidateRecord c)
        {
            CalibrationCell x;
            return _calibrationCells.TryGetValue(V57CalibrationKey(c),out x)?x:null;
        }
        ...
        private bool V57CalibratedCapitalEligible(CandidateRecord c)
        {
            ...
            return cell.LowerBoundR>V57MinCalibratedLowerBoundR && cell.Samples>0 && c.CalibratedRPerSlotHour>0;
        }

        private double V57FamilyCapitalScore(CandidateRecord c, DateTime now)
        {
            var cell=V57CalibrationCell(c);
            if(cell!=null)
            {
        ...
        private void StartV57FamilyShadowTrade(CandidateRecord c,int i,DateTime utc)
        {
            if(!EnableV57VirtualParallelExecution||c==null||c.Signal==null||_familyShadowTrades.ContainsKey(c.CandidateId))return;
            ...
            {
                Print("[V57-SHADOW-REJECT] cid={0} pattern={1} route={2} regime={3} reason=NO_LEGAL_RR_TARGET",c.CandidateId,c.Signal.PatternName,c.Route,V57RegimeBucket(c));
                return;
            }
            double risk=Math.Abs(entry-stop);
            if(risk<=_symbol.PipSize)return;
            var x=new FamilyShadowTrade{
                CandidateId=c.CandidateId,SetupKey=c.SetupKey,Pattern=c.Signal.PatternName,Route=c.Route,Regime=V57RegimeBucket(c),
            ...
            Print("[V57-SHADOW-START] cid={0} setup={1} pattern={2} route={3} regime={4} entry={5} stop={6} target={7} netTargetR={8:F3}",
                x.CandidateId,x.SetupKey,x.Pattern,x.Route,x.Regime,x.Entry,x.Stop,x.Target,x.NetTargetR);
        }

        private void UpdateV57FamilyShadowTrades(int i,DateTime utc)
        {
            if(!EnableV57VirtualParallelExecution||_familyShadowTrades.Count==0)return;
            ...
            foreach(var x in _familyShadowTrades.Values.Where(z=>!z.Closed).ToList())
            {
                ...
                if(stopHit){CloseV57Shadow(x,utc,-1.0-ModeledCostPips()/Math.Max(1.0,PriceToPips(x.RiskDistance)),"SL_CONSERVATIVE");continue;}
                if(targetHit){CloseV57Shadow(x,utc,x.NetTargetR,"TP");continue;}
                if(utc>=x.ExpiryUtc)
                {
                    ...
                    CloseV57Shadow(x,utc,r,"TTL_MARK_TO_MARKET");
                }
            }
        }

        private void CloseV57Shadow(FamilyShadowTrade x,DateTime utc,double realizedR,string reason)
        {
            ...
            Print("[V57-SHADOW-CLOSED] cid={0} setup={1} pattern={2} route={3} regime={4} mfeR={5:F4} maeR={6:F4} realizedR={7:F4} holdMin={8:F2} reason={9}",
                x.CandidateId,x.SetupKey,x.Pattern,x.Route,x.Regime,x.MfeR,x.MaeR,x.RealizedR,hold,reason);
        }

        private void CloseOpenV57ShadowTradesAtStop()
        {
            ...
            foreach(var x in _familyShadowTrades.Values.Where(z=>!z.Closed).ToList())
            {
                double signed=x.Direction==TradeDirection.Buy?close-x.Entry:x.Entry-close;
                double r=signed/x.RiskDistance-ModeledCostPips()/Math.Max(1.0,PriceToPips(x.RiskDistance));
                CloseV57Shadow(x,utc,r,"BOT_STOP_MARK_TO_MARKET");
            }
        }
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V57-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        {
            _candidateSeq++;
            return "V57-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private void Ledger(CandidateRecord c, CandidateState state, string reason)
        {
            ...
            Print("[V57-EVENT] cid={0} setup={1} pattern={2} subtype={3} scale={4} tf={5} dir={6} state={7} route={8} conflict={9} waitMin={10:F2} reason={11}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V57-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V57-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
