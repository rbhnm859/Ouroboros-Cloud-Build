...
TRADING="${TRADING:-true}"; FAMILYBOOKS="${FAMILYBOOKS:-true}"; SHADOW="${SHADOW:-true}"; CALADMIT="${CALADMIT:-false}"; FAMREP="${FAMREP:-true}"
ABCDLIVE="${ABCDLIVE:-false}"; V57GRID="${V57GRID:-true}"; CALMIN="${CALMIN:-0.0}"; CALMAN="${CALMAN:-}"
...
ALGO="${ALGO:-seal/algo/HarmonyBot_V57_Regime_Calibrated_Universal_Harmonic_Family_Portfolio_RC.algo}"
...
test "$DONE" = 1 || { echo "[V57-WATCHDOG-FAIL] run=$RUN_NAME"; tail -400 "seal/logs/$RUN_NAME.log" || true; exit 20; }
echo "[V57-WATCHDOG] run=$RUN_NAME status=complete"
