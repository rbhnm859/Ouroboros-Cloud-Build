...
s=pathlib.Path(sys.argv[1] if len(sys.argv)>1 else "HarmonyBot-V57/src/HarmonyBotV57.cs").read_text(errors="ignore")
checks={
"identity":"HarmonyBot V57" in s and "class HarmonyBotV57" in s and 'BotPrefix = "HB57"' in s,
...
"detector_truth":"V57-DETECTOR-TRUTH" in s and "DetectorTruth" in s,
...
out={"version":"HarmonyBot V57","architecture":"HARMONIC_FAMILY_IDENTITY_DETECTION_GRAPH_RECONSTRUCTION","checks":checks,"pass":all(checks.values())}
pathlib.Path("V57_ARCHITECTURE_AUDIT.json").write_text(json.dumps(out,indent=2))
