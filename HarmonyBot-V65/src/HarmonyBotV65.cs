...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V65 — Harmonic Thesis Completion Engine";
        ...
        [Parameter("V65 Thesis Max M1 Bars", DefaultValue = 6, MinValue = 6, MaxValue = 6)]
        public int V65ThesisMaxM1Bars { get; set; }
        ...
        private int _v65ThesisCompleted;
        private int _v65ThesisWindowRejected;
        private int _v65ThesisRouteProofRejected;
        ...
        protected override void OnStop()
        {
            ...
            Print("[V65-PRODUCT-SUMMARY] mode={0} thesisCompleted={1} thesisWindowRejected={2} thesisRouteProofRejected={3} runnerAssigned={4} runnerFilled={5} runnerClosed={6} runnerSurvivedCanonical={7} conditionalArmed={8} conditionalRejected={9}",
                V65Variant,_v65ThesisCompleted,_v65ThesisWindowRejected,_v65ThesisRouteProofRejected,
                _v65RunnerAssigned,_v65RunnerFilled,_v65RunnerClosed,_v65RunnerSurvivedCanonicalTp,_v65ConditionalLegArmed,_v65ConditionalLegRejected);
        }
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (V65ThesisCompletionEnabled())
                    {
                        double thesisScore;
                        confirmationPass = UpdateV65ThesisCompletionEvidence(i, c, out thesisScore);
                        c.ConfirmationScore = Math.Max(c.ConfirmationScore, thesisScore);

                        if (!confirmationPass && c.V65ThesisBarsObserved >= Math.Max(1, V65ThesisMaxM1Bars))
                        {
                            _v65ThesisWindowRejected++;
                            Reject(c, "V65_THESIS_COMPLETION_WINDOW_EXHAUSTED");
                            continue;
                        }
                    }
                    ...
                    if (!V65ThesisCompletionEnabled() && !familyContractLane && !confirmationPass && EnablePatternNativeM1Expansion)
                    {
                    ...
                    if (!V65ThesisCompletionEnabled() && !confirmationPass && EnableM1RescueLane)
                    {
                    ...
                    if (!confirmationPass)
                    {
                        Event(c, (V65ThesisCompletionEnabled() ? "V65_THESIS_WAIT_" : "LEGACY_CONFIRMATION_FAILED_") + legacyScore.ToString("F2", CultureInfo.InvariantCulture));
                        continue;
                    }
                    ...
                    if (!V65ThesisCompletionEnabled() && EnableRouteSpecificM1Veto && !RouteSpecificM1EvidencePass(i, c.Signal, c.Route))
                    {
        ...
        private bool UpdateV65ThesisCompletionEvidence(int i, CandidateRecord c, out double score)
        {
            score = 0;
            if (c == null || c.Signal == null || i < 3 || i >= _m1Bars.Count) return false;

            c.V65ThesisBarsObserved++;
            bool completionPass;
            double nativeScore;

            if (IsFamilyCompletionLane(c.Signal.PatternName))
            {
                completionPass = UpdateFamilyCompletionEvidence(i, c, out nativeScore);
            }
            else
            {
                c.NativeM1BarsObserved++;
                completionPass = UpdatePatternNativeM1State(i, c, out nativeScore);
            }

            bool routeProof = RouteSpecificM1EvidencePass(i, c.Signal, c.Route);
            score = Math.Max(nativeScore, c.ConfirmationScore);
            if (routeProof) score = Math.Max(score, .80);

            Print("[V65-THESIS-STATE] cid={0} setup={1} pattern={2} route={3} bar={4}/{5} nativeStage={6} completion={7} routeProof={8} score={9:F3}",
                c.CandidateId, c.SetupKey, c.Signal.PatternName, c.Route, c.V65ThesisBarsObserved,
                V65ThesisMaxM1Bars, c.NativeStage, completionPass, routeProof, score);

            if (completionPass && routeProof)
            {
                _v65ThesisCompleted++;
                Print("[V65-THESIS-COMPLETE] cid={0} setup={1} pattern={2} route={3} bars={4} score={5:F3}",
                    c.CandidateId, c.SetupKey, c.Signal.PatternName, c.Route, c.V65ThesisBarsObserved, score);
                return true;
            }

            if (completionPass && !routeProof)
                _v65ThesisRouteProofRejected++;
        ...
        {
            return string.Equals(V65Variant, "V65_V64_CONTROL", StringComparison.Ordinal);
        }

        private bool V65ProductEnabled()
        {
            return V65ControlEnabled() || string.Equals(V65Variant, "V65_PRODUCT", StringComparison.Ordinal);
        }

        private bool V65ThesisCompletionEnabled()
        {
        ...
        {
            return V65ControlEnabled() || V65ThesisCompletionEnabled();
        }
        ...
        {
            // V65 control preserves V64/V51 semantics; V65 product replaces generic confirmation with a single pattern-native thesis-completion state machine.
            score = 0;
    ...
    public sealed class CandidateRecord
    {
        ...
        public int V65ThesisBarsObserved;
