...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV65 : Robot
    {
        private const string Version = "HarmonyBot V65 — Harmonic Alpha Regime Truth Engine";
        private const string BotPrefix = "HB65";
        ...
        [Parameter("V65 Mode", DefaultValue = "V65_PRODUCT")]
        public string V65Variant { get; set; }

        [Parameter("V65 Runner Minimum R", DefaultValue = 3.0, MinValue = 3.0, MaxValue = 3.0)]
        public double V65RunnerMinimumR { get; set; }

        [Parameter("V65 Alpha Truth Enabled", DefaultValue = true)]
        public bool V65AlphaTruthEnabled { get; set; }

        [Parameter("V65 Minimum Alpha Truth Score", DefaultValue = 0.50, MinValue = 0.35, MaxValue = 0.75)]
        public double V65MinimumAlphaTruthScore { get; set; }

        [Parameter("V65 Extreme Volatility Percentile", DefaultValue = 0.92, MinValue = 0.80, MaxValue = 0.99)]
        public double V65ExtremeVolatilityPercentile { get; set; }

        [Parameter("V65 Shock Ratio", DefaultValue = 1.65, MinValue = 1.20, MaxValue = 2.50)]
        public double V65ShockRatio { get; set; }
        ...
        private int _v65RunnerAssigned;
        private int _v65RunnerFilled;
        private int _v65RunnerClosed;
        private int _v65RunnerSurvivedCanonicalTp;
        private int _v65ConditionalLegArmed;
        private int _v65ConditionalLegRejected;
        ...
        protected override void OnStart()
        {
            if (!V65ModeValid())
            {
                Print("[V65-FATAL] INVALID_MODE {0}", V65Variant);
                Stop();
                return;
            }
            ...
            Print("[V65-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
        ...
        protected override void OnStop()
        {
            ...
            Print("[V65-RISK-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V65-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
            ...
            Print("[V65-PRODUCT-SUMMARY] mode={0} runnerAssigned={1} runnerFilled={2} runnerClosed={3} runnerSurvivedCanonical={4} conditionalArmed={5} conditionalRejected={6}",
                V65Variant,_v65RunnerAssigned,_v65RunnerFilled,_v65RunnerClosed,_v65RunnerSurvivedCanonicalTp,_v65ConditionalLegArmed,_v65ConditionalLegRejected);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                Print("[V65-GATE-PASS] cid={0} setup={1} stage=PatternQualification reason=PATTERN_VALIDATED", record.CandidateId, record.SetupKey);
                ...
                record.AlphaTruthScore = V65AlphaTruthScore(signal, record.Conflict, regime, record.Route);
                record.AlphaRegime = V65AlphaRegime(regime);
                Print("[V65-ALPHA-TRUTH] cid={0} setup={1} pattern={2} route={3} regime={4} score={5:F3} geom={6:F3} prz={7:F3} time={8:F3} pivot={9:F3} atrPct={10:F3} atrRatio={11:F3} efficiency={12:F3} adxH1={13:F2} adxSlope={14:F2} extension={15:F3} conflict={16}",
                ...
                if (V65AlphaTruthEnabled && record.Route != HarmonicRoute.NO_TRADE && !V65AlphaTruthEligible(signal, record.Conflict, regime, record.Route, record.AlphaTruthScore))
                {
                    Reject(record, "ALPHA_TRUTH_REGIME_REJECT");
                    continue;
                }
                ...
                Print("[V65-THESIS] cid={0} setup={1} pattern={2} route={3} geometry={4:F3} prz={5:F3} confirmation={6:F3}",
                ...
                Print("[V65-GATE-PASS] cid={0} setup={1} stage=RegimeRouting reason=ROUTE_{2}", record.CandidateId, record.SetupKey, record.Route);
        ...
        private bool GridPlanReject(CandidateRecord c, string reason)
        {
            ...
            Print("[V65-GATE-REJECT] cid={0} setup={1} stage=RiskPlan reason={2}", c != null ? c.CandidateId : "", c != null ? c.SetupKey : "", reason);
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            int maxLegs;
            if (V65CanonicalGridEnabled())
            {
                V65RouteGridContract(c, out v62Fractions, out v62Weights);
            ...
            {
                double fraction = V65CanonicalGridEnabled() ? v62Fractions[leg] : p.GridFractions[leg];
                ...
                double riskWeight = V65CanonicalGridEnabled() ? v62Weights[leg] : (leg < p.GridRiskWeights.Length ? p.GridRiskWeights[leg] : 0);
            ...
            double targetBasis = V65ConditionalGridEnabled() ? anchor : plan.ExpectedWeightedEntry;
            double target, netRr;
            ...
            if (V65StructuralExitEnabled() && c.Signal.CanonicalTarget2 > 0 &&
                GeometryValid(c.Signal.Direction, targetBasis, plan.StructuralStop, c.Signal.CanonicalTarget2))
            {
                ...
                if (rr >= MinimumNetRR)
                {
                    ...
                    netRr = rr;
                    Print("[V65-STRUCTURAL-EXIT] cid={0} setup={1} target={2} netRR={3:F3}", c.CandidateId, c.SetupKey, target, netRr);
                }
            }

            plan.CanonicalTarget = target;
            plan.ExpectedNetRR = netRr;
            ConfigureV65SelectiveRunner(c, plan);
            c.GridPlan = plan;
            c.SelectedTarget = target;
            c.NetRR = netRr;

            Print("[V65-EXECUTION-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V65-EXECUTION-LEG] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
            ...
            if (V65ConditionalGridEnabled())
            {
                double shadowL3 = c.Signal.Direction == TradeDirection.Buy ? anchor - .618 * executionUnit : anchor + .618 * executionUnit;
                Print("[V65-L3-SHADOW] cid={0} setup={1} pattern={2} route={3} fraction=0.618 price={4} capital=false",
                    c.CandidateId, c.SetupKey, c.Signal.PatternName, c.Route, shadowL3);
            }
            Print("[V65-GATE-PASS] cid={0} setup={1} stage=RiskPlan reason=GRID_PLAN_VALID", c.CandidateId, c.SetupKey);
        ...
        private void ExecuteFibonacciGridPlan(CandidateRecord c)
        {
            ...
            double l0Target = V65LegTarget(l0, plan.CanonicalTarget);
            ...
            Print("[V65-GATE-PASS] cid={0} setup={1} stage=Executed reason=LEG0_FILLED", c.CandidateId, c.SetupKey);
            ...
            if (V65ConditionalGridEnabled())
            {
                ...
                if (firstDeep != null)
                {
                    ...
                    {
                        if (PlaceGridLimit(basket, firstDeep)) { basket.State = FibonacciBasketState.GRID_PENDING; _v65ConditionalLegArmed++; }
                    }
                    else { TransitionLegState(basket, firstDeep, GridLegState.CANCELLED, "CONDITIONAL_GRID_INITIAL_REJECT"); _v65ConditionalLegRejected++; }
                }
            }
            else
            {
        ...
        private bool ExecuteImmediateRunnerLeg(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            double target = V65LegTarget(leg, basket.CanonicalTarget);
        ...
        private bool PlaceGridLimit(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            double legTarget = V65LegTarget(leg, basket.CanonicalTarget);
        ...
        private bool V65ControlEnabled()
        {
            return string.Equals(V65Variant, "V65_V51_CONTROL", StringComparison.Ordinal);
        }

        private bool V65ProductEnabled()
        {
            return string.Equals(V65Variant, "V65_PRODUCT", StringComparison.Ordinal);
        }

        private bool V65ModeValid()
        {
            return V65ControlEnabled() || V65ProductEnabled();
        }

        private bool V65StructuralExitEnabled() { return false; }
        private bool V65CanonicalGridEnabled() { return V65ProductEnabled(); }
        private bool V65ConditionalGridEnabled() { return V65ProductEnabled(); }
        private bool V65RunnerEnabled() { return V65ProductEnabled(); }
        private bool V65CommercialMaxEnabled() { return V65ProductEnabled(); }

        // V65 deliberately uses route-native execution only. Harmonic family identity remains in
        // the Alpha Engine; execution does not create a second pattern-selection system.
        private string V65PolicyFor(CandidateRecord c)
        {
            if (!V65ProductEnabled() || c == null) return "LEGACY_V51";
        ...
        private bool V65PolicyUsesRunner(CandidateRecord c)
        {
            return V65ProductEnabled() && V65PolicyFor(c) == "CONDITIONAL_RUNNER";
        }

        private void V65RouteGridContract(CandidateRecord c, out double[] fractions, out double[] weights)
        {
            string policy = V65PolicyFor(c);
            if (policy == "SINGLE")
            {
                fractions = new[] { 0.0 };
                weights = new[] { 1.0 };
        ...
        private void ConfigureV65SelectiveRunner(CandidateRecord c, FibonacciGridPlan plan)
        {
            if (c == null || c.Signal == null || plan == null || !V65PolicyUsesRunner(c) || c.Route == HarmonicRoute.EXHAUSTION_REVERSAL) return;
            ...
            double minimumRunner = c.Signal.Direction == TradeDirection.Buy ? leg.PlannedPrice + riskDistance * V65RunnerMinimumR : leg.PlannedPrice - riskDistance * V65RunnerMinimumR;
            ...
            leg.RunnerEligible = true; leg.RunnerTarget = target; plan.SelectiveRunnerEnabled = true; plan.RunnerRiskWeight = leg.RiskWeight; _v65RunnerAssigned++;
            Print("[V65-RUNNER-ASSIGN] cid={0} setup={1} route={2} leg=L{3} weight={4:F3} target={5}", c.CandidateId, c.SetupKey, c.Route, leg.Index, leg.RiskWeight, target);
        }

        private double V65LegTarget(FibonacciGridLeg leg, double fallback)
        {
            if (V65CommercialMaxEnabled() && leg != null && leg.RunnerEligible && leg.RunnerTarget > 0) return leg.RunnerTarget;
            return fallback;
        }
        ...
        private bool DeeperLegThesisEligible(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            if (!V65ConditionalGridEnabled()) return notAcceleratingAgainst;
            ...
            Print("[V65-CONDITIONAL-LEG-CHECK] basket={0} leg=L{1} pass={2} impulse={3} structure={4} contained={5} m1Evidence={6}", basket.BasketId, leg.Index, pass, impulse, structureStillValid, penetrationContained, m1ReversalEvidence);
            return pass;
        }
        ...
        private bool EnsurePostFillProtection(Position p, FibonacciBasket basket)
        {
            ...
            double protectionTarget = V65LegTarget(protectionLeg, basket.CanonicalTarget);
        ...
        private void RecordLegFillTelemetry(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            if (leg.RunnerEligible) _v65RunnerFilled++;
            BasketEvent(basket, "GRID_LEG_FILLED_L" + leg.Index);
            if (V65ConditionalGridEnabled() && leg.Fraction > 1e-9) TryArmNextConditionalLeg(basket, leg.Index);
        }

        private void TryArmNextConditionalLeg(FibonacciBasket basket, int filledIndex)
        {
            if (basket == null || !basket.IsActive || !V65ConditionalGridEnabled()) return;
            ...
            { TransitionLegState(basket, next, GridLegState.CANCELLED, "CONDITIONAL_GRID_REVALIDATION_REJECT"); _v65ConditionalLegRejected++; return; }
            if (PlaceGridLimit(basket, next))
            { basket.State = FibonacciBasketState.GRID_PENDING; _v65ConditionalLegArmed++; Print("[V65-CONDITIONAL-LEG-ARMED] basket={0} prior=L{1} next=L{2}", basket.BasketId, filledIndex, next.Index); }
        }
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V65-LEG-ECON] basket={0} setup={1} leg=L{2} fraction={3:F3} weight={4:F3} runner={5} mfeR={6:F3} maeR={7:F3} realizedR={8:F3} net={9:F2} reason={10}", basket.BasketId, basket.Candidate != null ? basket.Candidate.SetupKey : "", l.LegIndex, spec != null ? spec.Fraction : -1, spec != null ? spec.RiskWeight : 0, runner, l.PeakR, l.MaxAdverseR, legRealizedR, p.NetProfit, args.Reason);
                if (runner) { _v65RunnerClosed++; Print("[V65-RUNNER-CLOSED] basket={0} leg=L{1} realizedR={2:F3} net={3:F2} reason={4}", basket.BasketId, l.LegIndex, legRealizedR, p.NetProfit, args.Reason); }
                if (args.Reason.ToString().IndexOf("TakeProfit", StringComparison.OrdinalIgnoreCase) >= 0 && !runner)
                {
                    ...
                    if (openRunner) { _v65RunnerSurvivedCanonicalTp++; Print("[V65-RUNNER-SURVIVED-CANONICAL] basket={0}", basket.BasketId); }
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V65-ECONOMIC-ATTRIBUTION] basket={0} setup={1} pattern={2} route={3} mfeR={4:F3} maeR={5:F3} realizedR={6:F3} capture={7:F3} timeToMfeMin={8:F2} timeToMaeMin={9:F2}", basket.BasketId, setupKey, basket.Pattern, basket.Route, basket.PeakR, basket.MaxAdverseR, realizedR, captureRatio, timeToMfe, timeToMae);
        ...
        private string V65AlphaRegime(RegimeSnapshot r)
        {
            if (r == null) return "UNKNOWN";
            if (r.AtrPercentile >= V65ExtremeVolatilityPercentile || r.AtrRatio >= V65ShockRatio) return "SHOCK";
        ...
        private double V65AlphaTruthScore(PatternSignal s, MtfConflict conflict, RegimeSnapshot r, HarmonicRoute route)
        {
        ...
        private bool V65AlphaTruthEligible(PatternSignal s, MtfConflict conflict, RegimeSnapshot r, HarmonicRoute route, double score)
        {
            if (s == null || r == null) return false;
            if (r.AtrPercentile >= V65ExtremeVolatilityPercentile && r.AtrRatio >= V65ShockRatio) return false;
            ...
            double floor = V65MinimumAlphaTruthScore;
        ...
        {
            // V65 preserves immutable V51 completion semantics in every candidate lane; the V62/V65 stage-aware DAG is observation-only.
            score = 0;
        ...
        {
            _candidateSeq++;
            return "V65-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private string V65GateStage(string reason)
        {
            string r=reason??"";
            if (r.IndexOf("ALPHA_TRUTH",StringComparison.OrdinalIgnoreCase)>=0) return "AlphaRegimeTruth";
        ...
        private void Reject(CandidateRecord c, string reason)
        {
            ...
            Print("[V65-GATE-REJECT] cid={0} setup={1} stage={2} reason={3}",c.CandidateId,c.SetupKey,V65GateStage(reason),reason);
