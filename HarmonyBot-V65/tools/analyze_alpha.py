...
v64=pathlib.Path(sys.argv[1]); v65c=pathlib.Path(sys.argv[2]); legacy=pathlib.Path(sys.argv[3]); dev=pathlib.Path(sys.argv[4]); out=pathlib.Path(sys.argv[5]); out.mkdir(parents=True,exist_ok=True)
...
v65cw={w:load_one(v65c,f"-{w}.json") for w in "ABC"}
...
control_equivalence=signature(v64w)==signature(v65cw)
...
prod["alpha_breakthrough_gate"]=(control_equivalence and prod["dev_stability_gate"] and prod["known_failure_stress_repaired"] and
                                 prod["legacy_preservation_gate"] and prod["net"]>base["net"] and prod["pf"]>=1.50 and
                                 prod["expectancy"]>10 and prod["max_dd_pct"]<=6 and prod["right_tail_gate"] and prod["frequency_expansion_gate"])
...
candidate="V65_PRODUCT" if prod["alpha_breakthrough_gate"] else ""
commercial_ready=bool(candidate and prod["commercial_gate"] and prod["historical_best_gate"])
report={"version":"HarmonyBot V65","architecture":"Harmonic Thesis Completion State Machine","v64_product_control":base,
        "v65_control_dynamic_equivalence":control_equivalence,"legacy_stress_2021_2023":hist,
        "dev_2024h2_2025h2":prod,"historical_best_reference":HIST_BEST,"commercial_minimum":COMM,
...
(out/"V65_ALPHA_FRONTIER.json").write_text(json.dumps(report,indent=2))
