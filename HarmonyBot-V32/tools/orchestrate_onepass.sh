...
WORK="$CONTROL/HarmonyBot-V32/work"
OUT="$CONTROL/HarmonyBot-V32/output"
...
import json,sys,pathlib
p,status,reason=sys.argv[1:4]
x={"version":"HarmonyBot V32.0 — Fibonacci Harmonic Grid Portfolio Execution Engine",
...
pathlib.Path(p).with_suffix(".md").write_text("# HarmonyBot V32.0 Final Decision\n\n**"+status+"**\n\n"+reason+"\n")
print(json.dumps(x,indent=2))
PY
}

echo "== V32 engineering audits =="
cd "$CONTROL"
python3 HarmonyBot-V32/tools/static_audit.py HarmonyBot-V32/src/HarmonyBotV32.cs
python3 HarmonyBot-V32/tools/data_exposure_scan.py
...
echo "== V32 clean build =="
dotnet restore HarmonyBot-V32/HarmonyBotV32.csproj
dotnet build HarmonyBot-V32/HarmonyBotV32.csproj -c Release --no-restore --nologo 2>&1 | tee "$OUT/V32_BUILD.log"
! grep -Eq '(^|[^0-9])error (CS|MSB|NETSDK)[0-9]+' "$OUT/V32_BUILD.log"

ALGO=$(find HarmonyBot-V32/bin/Release -type f -name '*.algo' | head -1)
test -n "$ALGO" && test -s "$ALGO"
cp "$ALGO" "$WORK/seal/algo/HarmonyBot_V32_Internal.algo"
cp HarmonyBot-V32/src/HarmonyBotV32.cs "$OUT/HarmonyBotV32.cs"
sha256sum HarmonyBot-V32/src/HarmonyBotV32.cs "$ALGO" > "$OUT/SHA256SUMS"
...
run_dev() {
  ...
  (
    cd "$WORK"
    RUN_NAME="$run" START_DATE="$start" EVAL_DATE="$eval" END_DATE="$end" CAPITAL=10000 DATA_MODE=m1 RISK=1.0       "$CONTROL/HarmonyBot-V32/tools/run_backtest.sh"
  )
  python3 "$CONTROL/HarmonyBot-V32/tools/audit_report.py"     --report "$WORK/seal/reports/$run.json"     --log "$WORK/seal/logs/$run.log"     --out "$OUT/$window.json"     --window "$window" --years "$years" --capital 10000
}

echo "== V32 EXPOSED architecture verification: one pass only =="
run_dev "V32-DEV-A" "DEV-A" "04/01/2021" "2021-01-11T00:00:00Z" "30/06/2021" 0.5
run_dev "V32-DEV-B" "DEV-B" "01/07/2021" "2021-07-08T00:00:00Z" "31/12/2021" 0.5
run_dev "V32-DEV-C" "DEV-C" "03/01/2022" "2022-01-10T00:00:00Z" "30/06/2022" 0.5
...
if [ "$ENGINEERING_FAIL" = "true" ]; then
  ...
  finalize "ENGINEERING_PIPELINE_FAILURE" "V32 compiled/backtested but the candidate/grid evidence pipeline did not initialize correctly. This is engineering failure, not strategy failure."
  exit 0
fi

if [ "$DEV_PASS" != "true" ]; then
  if [ "$GRID_NO_EDGE" = "true" ]; then
    write_not_run "NOT_RUN" "V32 Development classified FIBONACCI_GRID_NO_EDGE."
    finalize "FIBONACCI_GRID_NO_EDGE" "The pre-registered Fibonacci staged-entry architecture increased drawdown without improving PF, expectancy, MAE efficiency or entry efficiency versus the actual L0 SingleEntryEquivalent. No V32.x rescue is authorized."
  else
    write_not_run "NOT_RUN" "V32 exposed Development hard gate failed."
    finalize "STRATEGY_ARCHITECTURE_LIMITATION" "V32 completed the one-pass exposed DEV-A/B/C architecture verification but failed one or more mandatory Development gates. No threshold/grid/pattern rescue is authorized."
  fi
  exit 0
fi
...
if [ "$AUTHORIZED" != "true" ]; then
  ...
  finalize "HOLD_DATA_GOVERNANCE" "V32 passed exposed Development, but fresh pre-reserved downstream evidence windows are not presently proven untouched. Validation, Commercial100, OOS, walk-forward, sensitivity, cost stress, Monte Carlo and Final Holdout remain locked."
  exit 0
fi
