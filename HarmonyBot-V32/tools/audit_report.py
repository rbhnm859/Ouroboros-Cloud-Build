...
brx=re.compile(r"\[V32-BASKET-CLOSED\]\s+basket=(\S+)\s+cid=(\S+)\s+pattern=(.*?)\s+route=([A-Z_]+)\s+dir=(Buy|Sell)\s+plannedLegs=(\d+)\s+filledLegs=(\d+)\s+anchor=([-0-9.Ee]+)\s+avgEntry=([-0-9.Ee]+)\s+entryImprovePips=([-0-9.]+)\s+stop=([-0-9.Ee]+)\s+target=([-0-9.Ee]+)\s+initialRisk=([-0-9.]+)\s+worstRisk=([-0-9.]+)\s+mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)\s+reason=([^\r\n]+)")
baskets=[]
...
lrx=re.compile(r"\[V32-LEG-CLOSED\]\s+basket=(\S+)\s+cid=(\S+)\s+leg=L(\d+)\s+pos=(\d+)\s+pattern=(.*?)\s+route=([A-Z_]+)\s+dir=(Buy|Sell)\s+mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)\s+reason=([^\r\n]+)")
legs=[]
...
pipe={}
prx=re.compile(r"\[V32-PIPELINE\]\s+pattern=(.*?)\s+detected=(\d+)\s+validated=(\d+)\s+routed=(\d+)\s+prz=(\d+)\s+confirming=(\d+)\s+armed=(\d+)\s+basketPlanned=(\d+)\s+leg0=(\d+)\s+leg1=(\d+)\s+leg2=(\d+)\s+leg3=(\d+)\s+basketClosed=(\d+)\s+executed=(\d+)\s+expired=(\d+)\s+rejected=(\d+)\s+invalidated=(\d+)")
...
for x in h:
    lab=str(x.get("label",""))
    if not lab.startswith("HB32|") or not lab.endswith("|L0"): continue
    parts=lab.split("|")
...
out={"status":"OK","version":"HarmonyBot V32.0","window":a.window,"initial_capital":a.capital,
