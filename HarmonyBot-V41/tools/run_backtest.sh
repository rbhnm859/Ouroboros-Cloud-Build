...
ALGO="${ALGO:-seal/algo/HarmonyBot_V41_Marginal_Alpha_Pattern_Portfolio_RC.algo}"
...
PID=$!; DONE=0
echo "[V41-WATCHDOG] run=$RUN_NAME pid=$PID timeoutSeconds=$BACKTEST_TIMEOUT_SECONDS"
for ((i=0;i<BACKTEST_TIMEOUT_SECONDS;i+=5)); do
 if (( i % 60 == 0 )); then echo "[V41-WATCHDOG] run=$RUN_NAME elapsedSeconds=$i status=running"; fi
...
test "$DONE" = 1 || { echo "[V41-WATCHDOG-FAIL] run=$RUN_NAME"; tail -400 "seal/logs/$RUN_NAME.log" || true; exit 20; }
echo "[V41-WATCHDOG] run=$RUN_NAME status=complete"
