...
root=pathlib.Path(sys.argv[1] if len(sys.argv)>1 else "HarmonyBot-V41/merged")
out=pathlib.Path(sys.argv[2] if len(sys.argv)>2 else "HarmonyBot-V41/final")
out.mkdir(parents=True,exist_ok=True)
fams=["LEGACY_REPLAY","CORE_THESIS","EDGE_RESCUE","FULL_V41"]
...
# Cross-validation diagnostic only: never used to retune the current V41 run.
def vol_bucket(x):
 a=x.get("atr_ratio",0)
...
(out/"V41_CROSS_VALIDATION_DIAGNOSTIC.json").write_text(json.dumps({
 "version":"HarmonyBot V41","scope":"DEV-only diagnostic; not used to retune current run","method":"leave-one-window-out grouped Pattern x Route x Volatility x Efficiency x Lane",
 "results":cv},indent=2))
...
(out/"V41_PATTERN_FUNNEL.json").write_text(json.dumps({"version":"HarmonyBot V41","families":pattern_funnel},indent=2))

eligible=[(A[f]["net"],A[f]["pf"],A[f]["executable_baskets_per_year"],f) for f in fams if A[f]["alpha_promotion_pass"]]
eligible.sort(reverse=True)
winner=eligible[0][-1] if eligible else None

frontier={"version":"HarmonyBot V41","architecture":"CROSS_VALIDATED_MARGINAL_ALPHA_PATTERN_PORTFOLIO",
...
(out/"V41_PERFORMANCE_FRONTIER.json").write_text(json.dumps(frontier,indent=2))
decision={"version":"HarmonyBot V41","development_candidate":winner,"status":frontier["status"],"fresh_validation_used":False,
 "next_stage":"CAPITAL_COMPATIBILITY" if winner else "MARGINAL_ALPHA_DIAGNOSIS",
 "negative_net_frequency_expansion":"PROHIBITED"}
(out/"V41_PROMOTION_DECISION.json").write_text(json.dumps(decision,indent=2))
