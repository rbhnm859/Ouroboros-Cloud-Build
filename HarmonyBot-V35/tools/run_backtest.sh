...
QUALITY="${QUALITY:-true}"; REGIME="${REGIME:-true}"; CONFIRM="${CONFIRM:-true}"; CAPITAL_GATE="${CAPITAL_GATE:-true}"
MIN_ROBUST="${MIN_ROBUST:-0.56}"
ALGO="${ALGO:-seal/algo/HarmonyBot_V35_RegimeAware_Research_RC.algo}"
...
CNAME="v35-$(echo "$RUN_NAME"|tr '[:upper:]_' '[:lower:]-')-$GITHUB_RUN_ID"
docker run --name "$CNAME" -v "$PWD/seal:/work" "$IMAGE" backtest "/work/${ALGO#seal/}"   --ctid="$CTRADER_CTID" --pwd-file=/work/ctrader.pwd --account="$ACCT" --symbol=XAUUSD --period=m1   --start="$START_DATE" --end="$END_DATE" --balance="$BALANCE" --data-mode=m1 --data-dir=/work/data --commission=35 --spread=1   --SymbolName=XAUUSD --TradingEnabled=true --BasketRiskPercent=1.0 --AdaptiveCapitalMode=true   --MinimumSupportedEquity=100 --MicroCapitalThreshold=500 --MaxDrawdownPercent=10 --DailyLossLimitPercent=3   --MaxSpreadPips=60 --RoundTurnCommissionPips=0.5 --SlippageStressPips=0.3 --MinimumNetRR=2.0 --MinStopLossPips=10   --MinFreeMarginRiskMultiple=5 --M15SwingDepth=3 --M15SwingLookback=320 --H1SwingDepth=3 --H4SwingDepth=2   --PortfolioMaxCandidates=8 --CandidateTtlM15Bars=8 --MinGeometryQuality=0.55 --MinPrzConfluence=0.55   --EnableHarmonicRobustnessGate="$QUALITY" --EnableRegimeContextGate="$REGIME"   --EnableEnhancedM1Confirmation="$CONFIRM" --EnableCapitalFeasibilityGate="$CAPITAL_GATE" --MinHarmonicRobustness="$MIN_ROBUST"   --GridCancelMfeR=0.50 --NoMfeProofR=0.15 --NoMfeKillR=0.80 --NoMfeMinAgeMinutes=3   --BreakEvenTriggerR=1.0 --BreakEvenLockR=0.10 --TrailTriggerR=1.50 --TrailDistanceR=0.75   --EvaluationStartUtcIso="$EVAL_DATE" --report="/work/reports/$RUN_NAME.html" --report-json="/work/reports/$RUN_NAME.json"   --exit-on-stop > "seal/logs/$RUN_NAME.log" 2>&1 &
PID=$!; DONE=0
echo "[V35-WATCHDOG] run=$RUN_NAME pid=$PID timeoutSeconds=$BACKTEST_TIMEOUT_SECONDS"
for ((i=0;i<BACKTEST_TIMEOUT_SECONDS;i+=5)); do
 if (( i % 60 == 0 )); then echo "[V35-WATCHDOG] run=$RUN_NAME elapsedSeconds=$i status=running"; fi
...
test "$DONE" = 1 || { echo "[V35-WATCHDOG-FAIL] run=$RUN_NAME"; tail -400 "seal/logs/$RUN_NAME.log" || true; exit 20; }
echo "[V35-WATCHDOG] run=$RUN_NAME status=complete"
