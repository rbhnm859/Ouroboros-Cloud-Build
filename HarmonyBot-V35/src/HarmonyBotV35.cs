...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV35 : Robot
    {
        private const string Version = "HarmonyBot V35.0 — Regime-Aware Harmonic Alpha Architecture";
        ...
        [Parameter("Capital Feasibility Gate", DefaultValue = true)]
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V35-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V35-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V35-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V35-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            Print("[V35-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m1Bars), _profiles.Count);
            Print("[V35-TIMEFRAME-AUDIT] primaryPattern=M15 execution=M1 macro=H4 intermediate=H1 allCompletedBars=true");
            Print("[V35-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
            Print("[V35-ALPHA-CONFIG] harmonicQuality={0} regimeContext={1} enhancedM1={2} capitalFeasibility={3} minRobustness={4:F2}",
                EnableHarmonicRobustnessGate, EnableRegimeContextGate, EnableEnhancedM1Confirmation, EnableCapitalFeasibilityGate, MinHarmonicRobustness);
        }
        ...
        protected override void OnStop()
        {
            ...
            {
                var x = kv.Value;
                Print("[V35-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} armed={6} basketPlanned={7} leg0={8} leg1={9} leg2={10} leg3={11} basketClosed={12} executed={13} expired={14} rejected={15} invalidated={16}",
            ...
            Print("[V35-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V35-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
            ...
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V35-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                if (EnableHarmonicRobustnessGate && !HarmonicRobustnessEligible(signal, record.AlphaQualityScore))
                {
                    ...
                    Reject(record, "HARMONIC_ROBUSTNESS");
                    continue;
                }
                ...
                Print("[V35-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    double score = EnableEnhancedM1Confirmation
                        ? EnhancedM1ConfirmationScore(i, c.Signal, c.Route, c.Regime)
                        : M1ConfirmationScore(i, c.Signal);
                    c.ConfirmationScore = score;
                    double required = EnableEnhancedM1Confirmation
                        ? EnhancedM1Threshold(c.Route)
                        : (c.Route == HarmonicRoute.EXHAUSTION_REVERSAL ? 0.75 : 0.60);
                    if (score < required)
                    {
                        if (EnableEnhancedM1Confirmation) _confirmationRejected++;
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            Print("[V35-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V35-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V35-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V35-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V35-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V35-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V35-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V35-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V35-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V35-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V35-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            double entryImprovementPips = basket.AverageEntry > 0
                ...
                : 0;
            Print("[V35-BASKET-CLOSED] basket={0} cid={1} pattern={2} route={3} dir={4} plannedLegs={5} filledLegs={6} anchor={7} avgEntry={8} entryImprovePips={9:F3} stop={10} target={11} initialRisk={12:F2} worstRisk={13:F2} mfeR={14:F3} maeR={15:F3} realizedR={16:F3} net={17:F2} reason={18}",
        ...
        {
            Print("[V35-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
        ...
        {
            if (!EnableRegimeContextGate)
                return RouteSignalV34(s, conflict, r);

            bool trendAligned = r.TrendDirection == s.Direction;
            bool trendOpposed = r.TrendDirection != TradeDirection.Neutral && r.TrendDirection != s.Direction;
            bool robust = HarmonicRobustnessScore(s) >= Math.Max(.50, MinHarmonicRobustness);
            bool strongTrend = r.TrendStrength >= .55 && r.Efficiency >= .28;
            bool adxRising = r.AdxH1Slope > .50;
            bool healthyVol = r.AtrPercentile >= .08 && r.AtrPercentile <= .95;

            // Avoid catching a falling/rising knife when counter-trend force is still expanding.
            if (trendOpposed && strongTrend && adxRising && r.ExtensionAtr < 1.35)
                return HarmonicRoute.NO_TRADE;

            if (trendAligned && conflict != MtfConflict.CONFLICT && robust && healthyVol &&
                r.Efficiency >= .12 && r.AtrRatio >= .50 && r.AtrRatio <= 1.85)
                return HarmonicRoute.TREND_ALIGNED_REVERSAL;

            bool exhaustion = trendOpposed && robust && r.ExtensionAtr >= 1.35 &&
                r.AdxH1 >= 22.0 && r.AdxH1Slope <= 1.0 && r.AtrPercentile <= .98;
            if (exhaustion && (conflict != MtfConflict.CONFLICT ||
                (s.GeometryQuality >= .75 && s.PrzConfluence >= .75 && s.TimeSymmetry >= .45)))
                return HarmonicRoute.EXHAUSTION_REVERSAL;

            bool transition = conflict == MtfConflict.TRANSITION || r.Transition ||
                r.TrendDirection == TradeDirection.Neutral;
            if (transition && robust && r.TrendStrength <= .75 && healthyVol)
                return HarmonicRoute.TRANSITION_REVERSAL;
        ...
        private double CandidateRank(CandidateRecord c)
        {
            ...
            if (EnableHarmonicRobustnessGate || EnableRegimeContextGate || EnableEnhancedM1Confirmation)
                return .25 * c.AlphaQualityScore + .20 * c.RegimeScore + .15 * mtf +
                       .20 * c.ConfirmationScore + .20 * VClamp(c.NetRR / 3.0);
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V35-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        {
            _candidateSeq++;
            return "V35-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        {
            Print("[V35-EVENT] cid={0} pattern={1} tf={2} dir={3} state={4} route={5} conflict={6} reason={7}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V35-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V35-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
