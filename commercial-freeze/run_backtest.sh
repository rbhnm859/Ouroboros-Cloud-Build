...
ALGO="${ALGO:-seal/algo/HarmonyBotPro_architecture_redesign_rc.algo}"
...
case "$CANDIDATE" in
  R1) MODE=1; KILL=0.85; PROOF=0.15 ;;
  R2) MODE=2; KILL=0.85; PROOF=0.15 ;;
  R3) MODE=3; KILL=0.70; PROOF=0.20 ;;
  *) echo "unknown candidate $CANDIDATE"; exit 2 ;;
esac

CTX=$(python3 - <<PY
print(max(0.0,min(1.0,0.56*float("$THRESH_SCALE"))))
PY
)
CONF=$(python3 - <<PY
print(max(0.0,min(1.0,0.60*float("$THRESH_SCALE"))))
PY
)
