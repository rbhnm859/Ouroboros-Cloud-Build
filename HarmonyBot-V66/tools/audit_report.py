...
prod_rx=re.findall(r"\[V66-PRODUCT-SUMMARY\].*?mode=(\S+)\s+evidenceCompleted=(\d+)\s+evidenceWindowRejected=(\d+)\s+runnerAssigned=(\d+)\s+runnerFilled=(\d+)\s+runnerClosed=(\d+)\s+runnerSurvivedCanonical=(\d+)\s+conditionalArmed=(\d+)\s+conditionalRejected=(\d+)",t)
product={"mode":a.mode,"evidence_completed":0,"evidence_window_rejected":0,"runner_assigned":0,"runner_filled":0,"runner_closed":0,"runner_survived_canonical":0,"conditional_armed":0,"conditional_rejected":0}
if prod_rx:
    z=prod_rx[-1]; ks=list(product); product=dict(zip(ks,[z[0]]+list(map(int,z[1:]))))
...
out={"version":"HarmonyBot V66","mode":a.mode,"window":a.window,"starting_balance":a.balance,"spread":a.spread,"data_snapshot_sha":a.data_sha,
 ...
 "max_dd_pct":float(eq.get("maxEquityDrawdownPercent",0) or 0),"engineering_clean":summary and all(c[k]==0 for k in clean),
 "summary_present":summary,"broker_profile_present":"[V51-BROKER-PROFILE]" in t,
