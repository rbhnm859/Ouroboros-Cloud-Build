...
BALANCE="${BALANCE:-10000}"; SPREAD="${SPREAD:-1}"; V66MODE="${V66MODE:-V66_PRODUCT}"
ALGO="${ALGO:-seal/algo/HarmonyBot_V66_Harmonic_Alpha_Regime_Truth_RC.algo}"
IMAGE="${CTRADER_IMAGE:-ghcr.io/spotware/ctrader-console:5.9.11}"
BACKTEST_TIMEOUT_SECONDS="${BACKTEST_TIMEOUT_SECONDS:-2700}"
case "$V66MODE" in V66_PRODUCT|V66_CONTROL) ;; *) echo "invalid V66 mode $V66MODE" >&2; exit 64;; esac
if [ "$V66MODE" = "V66_PRODUCT" ]; then
  V66_TREND_THESIS=true
  V66_QUALIFIED_RECALL=true
else
  V66_TREND_THESIS=false
  V66_QUALIFIED_RECALL=false
fi
