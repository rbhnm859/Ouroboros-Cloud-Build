...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V66 — Harmonic Evidence Accumulator";
        ...
        [Parameter("V66 Evidence Max M1 Bars", DefaultValue = 6, MinValue = 6, MaxValue = 6)]
        public int V66EvidenceMaxM1Bars { get; set; }
        ...
        private int _v66EvidenceCompleted;
        private int _v66EvidenceWindowRejected;
        ...
        protected override void OnStop()
        {
            ...
            Print("[V66-PRODUCT-SUMMARY] mode={0} evidenceCompleted={1} evidenceWindowRejected={2} runnerAssigned={3} runnerFilled={4} runnerClosed={5} runnerSurvivedCanonical={6} conditionalArmed={7} conditionalRejected={8}",
                V66Variant,_v66EvidenceCompleted,_v66EvidenceWindowRejected,_v66RunnerAssigned,_v66RunnerFilled,_v66RunnerClosed,
                _v66RunnerSurvivedCanonicalTp,_v66ConditionalLegArmed,_v66ConditionalLegRejected);
        }
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (V66EvidenceEngineEnabled())
                    {
                        double evidenceScore;
                        string proof;
                        confirmationPass = UpdateV66EvidenceAccumulator(i, c, out evidenceScore, out proof);
                        c.ConfirmationScore = Math.Max(c.ConfirmationScore, evidenceScore);

                        if (!confirmationPass && c.V66EvidenceBarsObserved >= Math.Max(1, V66EvidenceMaxM1Bars))
                        {
                            _v66EvidenceWindowRejected++;
                            Reject(c, "V66_EVIDENCE_WINDOW_EXHAUSTED");
                            continue;
                        }
                    }
                    ...
                    if (!V66EvidenceEngineEnabled() && !familyContractLane && !confirmationPass && EnablePatternNativeM1Expansion)
                    {
                    ...
                    if (!V66EvidenceEngineEnabled() && !confirmationPass && EnableM1RescueLane)
                    {
                    ...
                    if (!confirmationPass)
                    {
                        Event(c, (V66EvidenceEngineEnabled() ? "V66_EVIDENCE_WAIT_" : "LEGACY_CONFIRMATION_FAILED_") + legacyScore.ToString("F2", CultureInfo.InvariantCulture));
                        continue;
                    }
                    ...
                    if (!V66EvidenceEngineEnabled() && EnableRouteSpecificM1Veto && !RouteSpecificM1EvidencePass(i, c.Signal, c.Route))
                    {
        ...
        private bool UpdateV66EvidenceAccumulator(int i, CandidateRecord c, out double score, out string proof)
        {
            score = 0;
            proof = "";
            if (c == null || c.Signal == null || i < 3 || i >= _m1Bars.Count) return false;

            c.V66EvidenceBarsObserved++;
            ...
            bool bos1 = buy ? cl > ph1 : cl < pl1;
            bool bos2 = buy ? cl > ph2 : cl < pl2;
            bool rejection = buy ? Math.Max(0, Math.Min(o, cl) - l) >= body * .5 : Math.Max(0, h - Math.Max(o, cl)) >= body * .5;
            bool sweep = buy ? l < pl1 : h > ph1;
            bool failedExtension = buy ? (l < pl1 && cl > pl1) : (h > ph1 && cl < ph1);
            ...
            c.V66Deceleration |= deceleration;

            bool location = c.FamilyReclaim || c.FamilyInsidePrz || c.FamilyRetest;
            bool structure = c.FamilyBos || c.TemporalBos2 || c.FamilyDisplacement;
            bool reaction = c.FamilyRejection || c.FamilyFailedExtension || c.FamilySweep || c.V66Deceleration;
            bool trendPass = location && structure && reaction;
            bool exhaustionPass = location && (c.FamilyFailedExtension || c.FamilySweep) && (c.FamilyRejection || c.FamilyBos || c.FamilyDisplacement);
            bool transitionPass = location && c.TemporalBos2 && c.FamilyRejection;
            ...
            if (p == "AB=CD")
            {
                familyPass = c.Route == HarmonicRoute.EXHAUSTION_REVERSAL
                    ? c.V66Deceleration && location && (c.FamilyFailedExtension || c.FamilySweep) && structure
                    : c.V66Deceleration && location && structure;
            }
            else if (p == "Shark" || p == "5-0")
            {
                familyPass = location && (c.FamilyFailedExtension || c.FamilySweep) && structure &&
                             (c.FamilyRetest || c.FamilyDirectional);
            }
            else if (p == "Cypher")
            {
                familyPass = location && c.FamilyRejection && structure;
            }
            else if (retracement)
            {
                familyPass = location && (c.FamilyRejection || c.FamilyFailedExtension) && structure;
            }
            else if (extension)
            {
                familyPass = c.FamilySweep && c.FamilyFailedExtension && location && structure;
            }
            else familyPass = location && structure && reaction;

            bool routePass = c.Route == HarmonicRoute.TREND_ALIGNED_REVERSAL ? trendPass :
                             c.Route == HarmonicRoute.EXHAUSTION_REVERSAL ? exhaustionPass :
                             c.Route == HarmonicRoute.TRANSITION_REVERSAL ? transitionPass : false;

            int evidenceCount = (location ? 1 : 0) + (structure ? 1 : 0) + (reaction ? 1 : 0) +
                                (c.FamilyDirectional ? 1 : 0) + (c.FamilyDisplacement ? 1 : 0);
            score = evidenceCount / 5.0;
            proof = "L" + (location ? "1" : "0") + "S" + (structure ? "1" : "0") + "R" + (reaction ? "1" : "0") +
                    "F" + (familyPass ? "1" : "0") + "P" + (routePass ? "1" : "0");

            Print("[V66-EVIDENCE] cid={0} setup={1} pattern={2} route={3} bar={4}/{5} proof={6} reclaim={7} bos1={8} bos2={9} reject={10} sweep={11} failedExt={12} displacement={13} decel={14} retest={15} score={16:F3}",
                c.CandidateId, c.SetupKey, p, c.Route, c.V66EvidenceBarsObserved, V66EvidenceMaxM1Bars, proof,
                c.FamilyReclaim, c.FamilyBos, c.TemporalBos2, c.FamilyRejection, c.FamilySweep, c.FamilyFailedExtension,
                c.FamilyDisplacement, c.V66Deceleration, c.FamilyRetest, score);

            if (routePass && familyPass)
            {
                _v66EvidenceCompleted++;
                Print("[V66-EVIDENCE-COMPLETE] cid={0} setup={1} pattern={2} route={3} bars={4} proof={5}",
                    c.CandidateId, c.SetupKey, p, c.Route, c.V66EvidenceBarsObserved, proof);
                return true;
            }
            return false;
        }
        ...
        {
            return string.Equals(V66Variant, "V66_V64_CONTROL", StringComparison.Ordinal);
        }

        private bool V66ProductEnabled()
        {
            return V66ControlEnabled() || string.Equals(V66Variant, "V66_PRODUCT", StringComparison.Ordinal);
        }

        private bool V66EvidenceEngineEnabled()
        {
        ...
        {
            return V66ControlEnabled() || V66EvidenceEngineEnabled();
        }
    ...
    public sealed class CandidateRecord
    {
        ...
        public int V66EvidenceBarsObserved;
        public bool V66Deceleration;
