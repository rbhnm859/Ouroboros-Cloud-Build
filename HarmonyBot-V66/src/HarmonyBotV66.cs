...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV66 : Robot
    {
        private const string Version = "HarmonyBot V66 — Trend-Thesis Proof & Positive Marginal Opportunity Commercial Convergence";
        private const string BotPrefix = "HB66";
        ...
        [Parameter("V66 Mode", DefaultValue = "V66_PRODUCT")]
        public string V66Variant { get; set; }

        [Parameter("V66 Runner Minimum R", DefaultValue = 3.0, MinValue = 3.0, MaxValue = 3.0)]
        public double V66RunnerMinimumR { get; set; }

        [Parameter("V66 Alpha Truth Enabled", DefaultValue = true)]
        public bool V66AlphaTruthEnabled { get; set; }

        [Parameter("V66 Minimum Alpha Truth Score", DefaultValue = 0.50, MinValue = 0.35, MaxValue = 0.75)]
        public double V66MinimumAlphaTruthScore { get; set; }

        [Parameter("V66 Extreme Volatility Percentile", DefaultValue = 0.92, MinValue = 0.80, MaxValue = 0.99)]
        public double V66ExtremeVolatilityPercentile { get; set; }

        [Parameter("V66 Shock Ratio", DefaultValue = 1.65, MinValue = 1.20, MaxValue = 2.50)]
        public double V66ShockRatio { get; set; }

        [Parameter("V66 Trend Thesis Proof", DefaultValue = true)]
        public bool V66TrendThesisProofEnabled { get; set; }

        [Parameter("V66 Qualified Recall", DefaultValue = true)]
        public bool V66QualifiedRecallEnabled { get; set; }

        [Parameter("V66 Rejected Opportunity Shadow", DefaultValue = true)]
        public bool V66RejectedOpportunityShadowEnabled { get; set; }
        ...
        private int _v66RunnerAssigned;
        private int _v66RunnerFilled;
        private int _v66RunnerClosed;
        private int _v66RunnerSurvivedCanonicalTp;
        private int _v66ConditionalLegArmed;
        private int _v66ConditionalLegRejected;
        private int _v66DirectionalPersistenceRejected;
        private int _v66TrendM1ProofRejected;
        private int _v66QualifiedRecallAdmitted;
        private int _v66QualifiedRecallExecuted;
        private int _v66RejectedShadowRegistered;
        private int _v66RejectedShadowCompleted;
        private readonly Dictionary<string, RejectedOpportunityShadow> _v66RejectedOpportunityShadows = new Dictionary<string, RejectedOpportunityShadow>();
        ...
        protected override void OnStart()
        {
            if (!V66ModeValid())
            {
                Print("[V66-FATAL] INVALID_MODE {0}", V66Variant);
                Stop();
                return;
            }
            ...
            Print("[V66-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
            ...
            Print("[V66-THESIS-CONFIG] mode={0} trendThesisProof={1} qualifiedRecall={2} rejectedShadow={3} legacyAlphaTruthDecision=false",
                V66Variant, V66TrendThesisProofEnabled, V66QualifiedRecallEnabled, V66RejectedOpportunityShadowEnabled);
        ...
        protected override void OnStop()
        {
            ...
            V66FlushRejectedOpportunityShadows("BOT_STOP");
            Print("[V66-THESIS-SUMMARY] directionalRejected={0} trendM1Rejected={1} recallAdmitted={2} recallExecuted={3} rejectedShadowRegistered={4} rejectedShadowCompleted={5}",
                _v66DirectionalPersistenceRejected, _v66TrendM1ProofRejected, _v66QualifiedRecallAdmitted, _v66QualifiedRecallExecuted,
                _v66RejectedShadowRegistered, _v66RejectedShadowCompleted);
            Print("[V66-RISK-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V66-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
            ...
            Print("[V66-PRODUCT-SUMMARY] mode={0} runnerAssigned={1} runnerFilled={2} runnerClosed={3} runnerSurvivedCanonical={4} conditionalArmed={5} conditionalRejected={6}",
                V66Variant,_v66RunnerAssigned,_v66RunnerFilled,_v66RunnerClosed,_v66RunnerSurvivedCanonicalTp,_v66ConditionalLegArmed,_v66ConditionalLegRejected);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                Print("[V66-GATE-PASS] cid={0} setup={1} stage=PatternQualification reason=PATTERN_VALIDATED", record.CandidateId, record.SetupKey);
                ...
                record.AlphaTruthScore = V66AlphaTruthScore(signal, record.Conflict, regime, record.Route);
                record.AlphaRegime = V66AlphaRegime(regime);
                Print("[V66-ALPHA-TRUTH] cid={0} setup={1} pattern={2} route={3} regime={4} score={5:F3} geom={6:F3} prz={7:F3} time={8:F3} pivot={9:F3} atrPct={10:F3} atrRatio={11:F3} efficiency={12:F3} adxH1={13:F2} adxSlope={14:F2} extension={15:F3} conflict={16}",
                    record.CandidateId, record.SetupKey, signal.PatternName, record.Route, record.AlphaRegime, record.AlphaTruthScore,
                    signal.GeometryQuality, signal.PrzConfluence, signal.TimeSymmetry, signal.PivotQuality,
                    regime.AtrPercentile, regime.AtrRatio, regime.Efficiency, regime.AdxH1, regime.AdxH1Slope, regime.ExtensionAtr, record.Conflict);

                if (V66AlphaTruthEnabled && record.Route != HarmonicRoute.NO_TRADE && !V66AlphaTruthEligible(signal, record.Conflict, regime, record.Route, record.AlphaTruthScore))
                    Event(record, "LEGACY_ALPHA_TRUTH_OBSERVATION_FAIL");

                record.V66DirectionalBiasH1 = V66SignedDirectionalBias(signal.Direction, regime.DiPlusH1, regime.DiMinusH1);
                record.V66DirectionalBiasH4 = V66SignedDirectionalBias(signal.Direction, regime.DiPlusH4, regime.DiMinusH4);
                record.V66SignedEmaSlopeH1 = signal.Direction == TradeDirection.Buy ? regime.EmaSlopeH1Atr : -regime.EmaSlopeH1Atr;
                record.V66SignedEmaSlopeH4 = signal.Direction == TradeDirection.Buy ? regime.EmaSlopeH4Atr : -regime.EmaSlopeH4Atr;
                Print("[V66-THESIS-TELEMETRY] cid={0} setup={1} pattern={2} initialRoute={3} dbH1={4:F4} dbH4={5:F4} emaSlopeH1Atr={6:F4} emaSlopeH4Atr={7:F4} diPlusH1={8:F2} diMinusH1={9:F2} diPlusH4={10:F2} diMinusH4={11:F2}",
                    record.CandidateId, record.SetupKey, signal.PatternName, record.Route,
                    record.V66DirectionalBiasH1, record.V66DirectionalBiasH4, record.V66SignedEmaSlopeH1, record.V66SignedEmaSlopeH4,
                    regime.DiPlusH1, regime.DiMinusH1, regime.DiPlusH4, regime.DiMinusH4);

                if (record.Route == HarmonicRoute.NO_TRADE && V66ProductEnabled() && V66QualifiedRecallEnabled &&
                    V66QualifiedRecallEligible(signal, record.Conflict, regime))
                {
                    record.Route = HarmonicRoute.TREND_ALIGNED_REVERSAL;
                    record.V66QualifiedRecall = true;
                    _v66QualifiedRecallAdmitted++;
                    Print("[V66-QUALIFIED-RECALL] cid={0} setup={1} pattern={2} route={3} reason=SOFT_ROUTER_RECALL dbH1={4:F4} dbH4={5:F4} emaSlopeH1Atr={6:F4}",
                        record.CandidateId, record.SetupKey, signal.PatternName, record.Route,
                        record.V66DirectionalBiasH1, record.V66DirectionalBiasH4, record.V66SignedEmaSlopeH1);
                }
                ...
                if (V66ProductEnabled() && V66TrendThesisProofEnabled && record.Route == HarmonicRoute.TREND_ALIGNED_REVERSAL &&
                    !V66DirectionalTrendEligible(signal, regime))
                {
                    _v66DirectionalPersistenceRejected++;
                    Reject(record, "V66_DIRECTIONAL_PERSISTENCE_REJECT");
                    continue;
                }
                record.V66TrendProofPassed = record.Route != HarmonicRoute.TREND_ALIGNED_REVERSAL ||
                                             !V66ProductEnabled() || !V66TrendThesisProofEnabled ||
                                             V66DirectionalTrendEligible(signal, regime);
                Print("[V66-THESIS] cid={0} setup={1} pattern={2} route={3} geometry={4:F3} prz={5:F3} confirmation={6:F3}",
                    record.CandidateId, record.SetupKey, signal.PatternName, record.Route, signal.GeometryQuality, signal.PrzConfluence, record.ConfirmationScore);
                Print("[V66-GATE-PASS] cid={0} setup={1} stage=RegimeRouting reason=ROUTE_{2}", record.CandidateId, record.SetupKey, record.Route);
        ...
        private void ProcessNewM1Close()
        {
            ...
            V66UpdateRejectedOpportunityShadows(i, utc);
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (V66ProductEnabled() && V66TrendThesisProofEnabled && c.Route == HarmonicRoute.TREND_ALIGNED_REVERSAL &&
                        !RouteSpecificM1EvidencePass(i, c.Signal, c.Route))
                    {
                        _confirmationRejected++;
                        _v66TrendM1ProofRejected++;
                        Event(c, "V66_TREND_M1_REACCELERATION_WAIT");
                        continue;
                    }
        ...
        private void TryScheduleAndExecute()
        {
            ...
            if (winner.State == CandidateState.EXECUTED && winner.V66QualifiedRecall)
                _v66QualifiedRecallExecuted++;
        ...
        private bool GridPlanReject(CandidateRecord c, string reason)
        {
            ...
            Print("[V66-GATE-REJECT] cid={0} setup={1} stage=RiskPlan reason={2}", c != null ? c.CandidateId : "", c != null ? c.SetupKey : "", reason);
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            double[] v62Fractions = null;
            double[] v62Weights = null;
            int maxLegs;
            if (V66CanonicalGridEnabled())
            {
                V66RouteGridContract(c, out v62Fractions, out v62Weights);
                maxLegs = Math.Min(v62Fractions.Length, v62Weights.Length);
            }
            else
            {
                ...
                maxLegs = Math.Min(Math.Min(routeMax, p.MaximumGridLegs), p.GridFractions.Length);
            }
            ...
            {
                double fraction = V66CanonicalGridEnabled() ? v62Fractions[leg] : p.GridFractions[leg];
                ...
                double riskWeight = V66CanonicalGridEnabled() ? v62Weights[leg] : (leg < p.GridRiskWeights.Length ? p.GridRiskWeights[leg] : 0);
            ...
            double targetBasis = V66ConditionalGridEnabled() ? anchor : plan.ExpectedWeightedEntry;
            double target, netRr;
            if (!SelectCanonicalBasketTarget(c.Signal, targetBasis, plan.StructuralStop, out target, out netRr))
                return GridPlanReject(c, "TARGET_RR");

            if (V66StructuralExitEnabled() && c.Signal.CanonicalTarget2 > 0 &&
                GeometryValid(c.Signal.Direction, targetBasis, plan.StructuralStop, c.Signal.CanonicalTarget2))
            {
                double reward = PriceToPips(Math.Abs(c.Signal.CanonicalTarget2 - targetBasis));
                double risk = PriceToPips(Math.Abs(targetBasis - plan.StructuralStop));
                double rr = (reward - ModeledCostPips()) / Math.Max(risk + ModeledCostPips(), 1e-9);
                if (rr >= MinimumNetRR)
                {
                    target = c.Signal.CanonicalTarget2;
                    netRr = rr;
                    Print("[V66-STRUCTURAL-EXIT] cid={0} setup={1} target={2} netRR={3:F3}", c.CandidateId, c.SetupKey, target, netRr);
                }
            }

            plan.CanonicalTarget = target;
            plan.ExpectedNetRR = netRr;
            ConfigureV66SelectiveRunner(c, plan);
            c.GridPlan = plan;
            c.SelectedTarget = target;
            c.NetRR = netRr;

            Print("[V66-EXECUTION-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V66-EXECUTION-LEG] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
            ...
            if (V66ConditionalGridEnabled())
            {
                double shadowL3 = c.Signal.Direction == TradeDirection.Buy ? anchor - .618 * executionUnit : anchor + .618 * executionUnit;
                Print("[V66-L3-SHADOW] cid={0} setup={1} pattern={2} route={3} fraction=0.618 price={4} capital=false",
                    c.CandidateId, c.SetupKey, c.Signal.PatternName, c.Route, shadowL3);
            }
            Print("[V66-GATE-PASS] cid={0} setup={1} stage=RiskPlan reason=GRID_PLAN_VALID", c.CandidateId, c.SetupKey);
        ...
        private void ExecuteFibonacciGridPlan(CandidateRecord c)
        {
            ...
            double l0Target = V66LegTarget(l0, plan.CanonicalTarget);
            double tpPips = PriceToPips(Math.Abs(l0Target - marketEntry));
            if (!BrokerProtectionDistancesValid(c.Signal.Direction, marketEntry, plan.StructuralStop, l0Target))
            {
            ...
            Print("[V66-GATE-PASS] cid={0} setup={1} stage=Executed reason=LEG0_FILLED", c.CandidateId, c.SetupKey);

            var immediateRunner = plan.Legs.Where(x => x.Index > 0 && x.Physical && x.RunnerEligible && Math.Abs(x.Fraction) <= 1e-9).OrderBy(x => x.Index).FirstOrDefault();
            if (immediateRunner != null) ExecuteImmediateRunnerLeg(basket, immediateRunner);

            if (V66ConditionalGridEnabled())
            {
                var firstDeep = plan.Legs.Where(x => x.Index > 0 && x.Physical && x.Fraction > 1e-9 && x.State == GridLegState.PLANNED).OrderBy(x => x.Index).FirstOrDefault();
                if (firstDeep != null)
                {
                    if (DeeperLegThesisEligible(basket, firstDeep))
                    {
                        if (PlaceGridLimit(basket, firstDeep)) { basket.State = FibonacciBasketState.GRID_PENDING; _v66ConditionalLegArmed++; }
                    }
                    else { TransitionLegState(basket, firstDeep, GridLegState.CANCELLED, "CONDITIONAL_GRID_INITIAL_REJECT"); _v66ConditionalLegRejected++; }
                }
            }
            else
            {
                foreach (var leg in plan.Legs.Where(x => x.Index > 0))
                {
                    ...
                    if (!DeeperLegThesisEligible(basket, leg)) { TransitionLegState(basket, leg, GridLegState.CANCELLED, "DEEPER_LEG_THESIS_REJECT"); continue; }
                    if (PlaceGridLimit(basket, leg)) basket.State = FibonacciBasketState.GRID_PENDING;
                }
            }
        }

        private bool ExecuteImmediateRunnerLeg(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            if (basket == null || leg == null || !leg.Physical || !leg.RunnerEligible || leg.Volume <= 0) return false;
            string label = GridLabel(basket.BasketId, leg.Index);
            if (LegAlreadyExists(label)) return false;
            double entry = basket.Direction == TradeDirection.Buy ? _symbol.Ask : _symbol.Bid;
            double slPips = PriceToPips(Math.Abs(entry - basket.StructuralStop));
            double target = V66LegTarget(leg, basket.CanonicalTarget);
            double tpPips = PriceToPips(Math.Abs(target - entry));
            if (slPips < MinStopLossPips || tpPips <= 0 || !BrokerProtectionDistancesValid(basket.Direction, entry, basket.StructuralStop, target))
            { TransitionLegState(basket, leg, GridLegState.REJECTED, "RUNNER_GEOMETRY_REJECT"); return false; }
            double volume = VolumeForRiskBudget(leg.RiskBudget, slPips);
            if (volume <= 0) { TransitionLegState(basket, leg, GridLegState.RISK_REJECTED, "RUNNER_VOLUME_REJECT"); return false; }
            double existingWorst = ActualBasketWorstRisk(basket);
            double runnerWorst = volume * _symbol.PipValue * (slPips + ModeledCostPips());
            if (!double.IsFinite(existingWorst) || existingWorst + runnerWorst > basket.InitialBasketRisk + 1e-8)
            { _gridRiskViolations++; TransitionLegState(basket, leg, GridLegState.RISK_REJECTED, "RUNNER_WORST_RISK"); return false; }
            TransitionLegState(basket, leg, GridLegState.SUBMITTING, "RUNNER_SUBMITTING");
            ...
            if (tr == null || !tr.IsSuccessful || tr.Position == null)
            {
                string code = "RUNNER_ORDER_" + (tr == null ? "NULL" : tr.Error.ToString());
                RecordExecutionError(code, "basket=" + basket.BasketId + ";leg=L" + leg.Index); TransitionLegState(basket, leg, GridLegState.REJECTED, code); return false;
            }
            leg.Volume = volume; leg.PositionId = tr.Position.Id;
            if (!_postFillValidated.Contains(tr.Position.Id)) PostFillSafetyKernel(tr.Position, "RUNNER_RESULT");
            if (!_postFillValidated.Contains(tr.Position.Id) || !PositionStillExists(tr.Position.Id))
            { if (PositionStillExists(tr.Position.Id)) FailClosePosition(tr.Position, basket, "RUNNER_POST_FILL_NOT_VALIDATED"); return false; }
            if (!_positions.ContainsKey(tr.Position.Id)) RegisterFilledPosition(basket, leg, tr.Position);
            RecordLegFillTelemetry(basket, leg); BasketEvent(basket, "RUNNER_FILLED_L" + leg.Index); return true;
        }
        ...
        private bool PlaceGridLimit(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            double legTarget = V66LegTarget(leg, basket.CanonicalTarget);
            double tpPips = PriceToPips(Math.Abs(legTarget - leg.PlannedPrice));
            ...
            if (!BrokerProtectionDistancesValid(basket.Direction, leg.PlannedPrice, basket.StructuralStop, legTarget))
            {
            ...
            var tr = PlaceLimitOrder(tt, SymbolName, leg.Volume, leg.PlannedPrice, label,
                basket.StructuralStop, legTarget, ProtectionType.Absolute, basket.ExpirationUtc,
        ...
        private bool V66ControlEnabled()
        {
            return string.Equals(V66Variant, "V66_CONTROL", StringComparison.Ordinal);
        }

        private bool V66ProductEnabled()
        {
            return string.Equals(V66Variant, "V66_PRODUCT", StringComparison.Ordinal);
        }

        private bool V66ModeValid()
        {
            return V66ControlEnabled() || V66ProductEnabled();
        }
        ...
        private void ConfigureV66SelectiveRunner(CandidateRecord c, FibonacciGridPlan plan)
        {
            if (c == null || c.Signal == null || plan == null || !V66PolicyUsesRunner(c) || c.Route == HarmonicRoute.EXHAUSTION_REVERSAL) return;
            ...
            double minimumRunner = c.Signal.Direction == TradeDirection.Buy ? leg.PlannedPrice + riskDistance * V66RunnerMinimumR : leg.PlannedPrice - riskDistance * V66RunnerMinimumR;
            ...
            leg.RunnerEligible = true; leg.RunnerTarget = target; plan.SelectiveRunnerEnabled = true; plan.RunnerRiskWeight = leg.RiskWeight; _v66RunnerAssigned++;
            Print("[V66-RUNNER-ASSIGN] cid={0} setup={1} route={2} leg=L{3} weight={4:F3} target={5}", c.CandidateId, c.SetupKey, c.Route, leg.Index, leg.RiskWeight, target);
        }

        private double V66LegTarget(FibonacciGridLeg leg, double fallback)
        {
            if (V66CommercialMaxEnabled() && leg != null && leg.RunnerEligible && leg.RunnerTarget > 0) return leg.RunnerTarget;
            return fallback;
        }
        ...
        private void ReconcileAndManageBaskets()
        {
            ...
            foreach (var basket in _baskets.Values.Where(b => b.IsActive).ToList())
            {
                ...
                foreach (var p in positions)
                {
                    ...
                    if (legR > legLedger.PeakR) { legLedger.PeakR = legR; legLedger.PeakUtc = Server.Time.ToUniversalTime(); }
                    if (-legR > legLedger.MaxAdverseR) { legLedger.MaxAdverseR = -legR; legLedger.MaxAdverseUtc = Server.Time.ToUniversalTime(); }
                }
                ...
                if (currentR > basket.PeakR) { basket.PeakR = currentR; basket.PeakUtc = Server.Time.ToUniversalTime(); }
                if (-currentR > basket.MaxAdverseR) { basket.MaxAdverseR = -currentR; basket.MaxAdverseUtc = Server.Time.ToUniversalTime(); }
        ...
        {
            if (leg.Index <= 0 || Math.Abs(leg.Fraction) <= 1e-9) return true;
            ...
            int i = LastClosedIndex(_m1Bars); if (i < 3) return false;
            double close = _m1Bars.ClosePrices[i], prev = _m1Bars.ClosePrices[i - 1];
            ...
            if (!V66ConditionalGridEnabled()) return notAcceleratingAgainst;
            bool structureStillValid = basket.Direction == TradeDirection.Buy ? close > basket.StructuralStop + _symbol.TickSize : close < basket.StructuralStop - _symbol.TickSize;
            bool m1ReversalEvidence = basket.Candidate != null && basket.Candidate.Signal != null && RouteSpecificM1EvidencePass(i, basket.Candidate.Signal, basket.Route);
            double span = Math.Abs(basket.EntryAnchor - basket.StructuralStop);
            bool penetrationContained = span > _symbol.TickSize && (basket.Direction == TradeDirection.Buy ? close >= basket.StructuralStop + .10 * span : close <= basket.StructuralStop - .10 * span);
            bool pass = notAcceleratingAgainst && structureStillValid && penetrationContained && m1ReversalEvidence;
            Print("[V66-CONDITIONAL-LEG-CHECK] basket={0} leg=L{1} pass={2} impulse={3} structure={4} contained={5} m1Evidence={6}", basket.BasketId, leg.Index, pass, impulse, structureStillValid, penetrationContained, m1ReversalEvidence);
            return pass;
        }
        ...
        private bool EnsurePostFillProtection(Position p, FibonacciBasket basket)
        {
            ...
            double protectionTarget = V66LegTarget(protectionLeg, basket.CanonicalTarget);
            ...
            if (!tpAcceptable)
            {
                if (!BrokerTargetDistanceValid(p.TradeType, protectionTarget))
                {
                    _postFillProtectionFailures++;
                    return false;
                }
                var rt = live.ModifyTakeProfitPrice(protectionTarget);
        ...
        private void RecordLegFillTelemetry(FibonacciBasket basket, FibonacciGridLeg leg)
        {
            ...
            { _executionStateViolations++; BasketEvent(basket, "FILL_TELEMETRY_WITHOUT_PROTECTION_L" + leg.Index); return; }
            ...
            if (leg.RunnerEligible) _v66RunnerFilled++;
            BasketEvent(basket, "GRID_LEG_FILLED_L" + leg.Index);
            if (V66ConditionalGridEnabled() && leg.Fraction > 1e-9) TryArmNextConditionalLeg(basket, leg.Index);
        }

        private void TryArmNextConditionalLeg(FibonacciBasket basket, int filledIndex)
        {
            if (basket == null || !basket.IsActive || !V66ConditionalGridEnabled()) return;
            ...
            { TransitionLegState(basket, next, GridLegState.CANCELLED, "CONDITIONAL_GRID_REVALIDATION_REJECT"); _v66ConditionalLegRejected++; return; }
            if (PlaceGridLimit(basket, next))
            { basket.State = FibonacciBasketState.GRID_PENDING; _v66ConditionalLegArmed++; Print("[V66-CONDITIONAL-LEG-ARMED] basket={0} prior=L{1} next=L{2}", basket.BasketId, filledIndex, next.Index); }
        }
        ...
        {
            var p = args.Position;
            if (p == null || p.SymbolName != SymbolName || string.IsNullOrWhiteSpace(p.Label) || !p.Label.StartsWith(BotPrefix + "|", StringComparison.Ordinal)) return;
            PositionLedger l;
            if (!_positions.TryGetValue(p.Id, out l)) { Print("[V51-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason); return; }
            FibonacciBasket basket = null;
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                basket.RealizedNet += p.NetProfit; basket.ClosedLegs++;
                double legRealizedR = l.InitialRiskPips > 0 ? p.Pips / l.InitialRiskPips : 0;
                var spec = basket.Plan.Legs.FirstOrDefault(x => x.Index == l.LegIndex);
                ...
                Print("[V66-LEG-ECON] basket={0} setup={1} leg=L{2} fraction={3:F3} weight={4:F3} runner={5} mfeR={6:F3} maeR={7:F3} realizedR={8:F3} net={9:F2} reason={10}", basket.BasketId, basket.Candidate != null ? basket.Candidate.SetupKey : "", l.LegIndex, spec != null ? spec.Fraction : -1, spec != null ? spec.RiskWeight : 0, runner, l.PeakR, l.MaxAdverseR, legRealizedR, p.NetProfit, args.Reason);
                if (runner) { _v66RunnerClosed++; Print("[V66-RUNNER-CLOSED] basket={0} leg=L{1} realizedR={2:F3} net={3:F2} reason={4}", basket.BasketId, l.LegIndex, legRealizedR, p.NetProfit, args.Reason); }
                if (args.Reason.ToString().IndexOf("TakeProfit", StringComparison.OrdinalIgnoreCase) >= 0 && !runner)
                {
                    ...
                    if (openRunner) { _v66RunnerSurvivedCanonicalTp++; Print("[V66-RUNNER-SURVIVED-CANONICAL] basket={0}", basket.BasketId); }
                    CancelBasketPending(basket, "CANONICAL_TARGET_REACHED");
                }
            }
            _positions.Remove(p.Id); _postFillValidated.Remove(p.Id);
            if (basket != null && !OwnPositions().Any(x=>LabelBasketId(x.Label)==basket.BasketId) && !OwnPendingOrders().Any(x=>LabelBasketId(x.Label)==basket.BasketId))
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            double captureRatio = basket.PeakR > 1e-9 ? realizedR / basket.PeakR : 0;
            ...
            Print("[V66-ECONOMIC-ATTRIBUTION] basket={0} setup={1} pattern={2} route={3} mfeR={4:F3} maeR={5:F3} realizedR={6:F3} capture={7:F3} timeToMfeMin={8:F2} timeToMaeMin={9:F2}", basket.BasketId, setupKey, basket.Pattern, basket.Route, basket.PeakR, basket.MaxAdverseR, realizedR, captureRatio, timeToMfe, timeToMae);
        ...
        private RegimeSnapshot BuildRegimeSnapshot()
        {
            ...
            double h4Atr = Atr(_h4Bars, 14, h4);
            r.ExtensionAtr = h1Atr > 0 ? Math.Abs(_h1Bars.ClosePrices[h1] - refEma) / h1Atr : 0;
            r.EmaSlopeH1Atr = h1Atr > 0 ? (h1e50 - h1e50Prev) / h1Atr : 0;
            r.EmaSlopeH4Atr = h4Atr > 0 ? (h4e50 - h4e50Prev) / h4Atr : 0;
            V66DirectionalMovement(_h1Bars, 14, h1, out r.DiPlusH1, out r.DiMinusH1);
            V66DirectionalMovement(_h4Bars, 14, h4, out r.DiPlusH4, out r.DiMinusH4);
            return r;
        }


        private double V66SignedDirectionalBias(TradeDirection direction, double diPlus, double diMinus)
        {
            double den = Math.Abs(diPlus) + Math.Abs(diMinus) + 1e-9;
            double raw = (diPlus - diMinus) / den;
            return direction == TradeDirection.Buy ? raw :
                   direction == TradeDirection.Sell ? -raw : 0;
        }

        private void V66DirectionalMovement(Bars bars, int period, int end, out double diPlus, out double diMinus)
        {
            diPlus = 0; diMinus = 0;
            if (bars == null || period < 2 || end <= 1 || end >= bars.Count) return;
            ...
            for (int i = start; i <= end; i++)
            {
                ...
                double r0 = bars.HighPrices[i] - bars.LowPrices[i];
                double r1 = Math.Abs(bars.HighPrices[i] - bars.ClosePrices[i - 1]);
                double r2 = Math.Abs(bars.LowPrices[i] - bars.ClosePrices[i - 1]);
                tr += Math.Max(r0, Math.Max(r1, r2));
            }
            if (tr <= 1e-12) return;
        ...
        private bool V66DirectionalTrendEligible(PatternSignal s, RegimeSnapshot r)
        {
            if (s == null || r == null) return false;
            double dbH1 = V66SignedDirectionalBias(s.Direction, r.DiPlusH1, r.DiMinusH1);
            double dbH4 = V66SignedDirectionalBias(s.Direction, r.DiPlusH4, r.DiMinusH4);
            double slopeH1 = s.Direction == TradeDirection.Buy ? r.EmaSlopeH1Atr : -r.EmaSlopeH1Atr;
            return dbH1 > 0 && slopeH1 > 0 && dbH4 >= 0;
        }

        private bool V66QualifiedRecallEligible(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            if (s == null || r == null || !V66DirectionalTrendEligible(s, r)) return false;
            if (conflict == MtfConflict.CONFLICT || conflict == MtfConflict.TRANSITION) return false;
            if (r.AtrRatio < .55 || r.AtrRatio > 1.75) return false;
            return true;
        }

        private bool V66SoftOpportunityReason(string reason)
        {
            string r = reason ?? "";
            return r == "ROUTER_NO_TRADE" ||
                   r == "V66_DIRECTIONAL_PERSISTENCE_REJECT" ||
                   r == "FAMILY_COMPLETION_WINDOW_EXHAUSTED" ||
                   r.IndexOf("CONFIRM", StringComparison.OrdinalIgnoreCase) >= 0 ||
                   r.IndexOf("TTL", StringComparison.OrdinalIgnoreCase) >= 0 ||
                   r.IndexOf("SESSION_EXPIRED", StringComparison.OrdinalIgnoreCase) >= 0;
        }

        private void V66RegisterRejectedOpportunityShadow(CandidateRecord c, string reason)
        {
            if (!V66RejectedOpportunityShadowEnabled || c == null || c.Signal == null || string.IsNullOrWhiteSpace(c.SetupKey)) return;
            if (_executedSetupKeys.Contains(c.SetupKey) || _v66RejectedOpportunityShadows.ContainsKey(c.SetupKey)) return;
            double anchor = c.Signal.Direction == TradeDirection.Buy ? _symbol.Ask : _symbol.Bid;
            double risk = Math.Abs(anchor - c.Signal.StructuralInvalidation);
            if (risk <= _symbol.PipSize) risk = Math.Abs(c.Signal.D.Price - c.Signal.StructuralInvalidation);
            if (risk <= _symbol.PipSize) return;
            DateTime now = Server.Time.ToUniversalTime();
            DateTime expiry = c.ExpiryUtc > now ? c.ExpiryUtc : now.AddMinutes(180);
            _v66RejectedOpportunityShadows[c.SetupKey] = new RejectedOpportunityShadow
            {
                SetupKey = c.SetupKey,
                CandidateId = c.CandidateId,
                Pattern = c.Signal.PatternName,
                ...
                StartUtc = now,
                ExpiryUtc = expiry,
                SoftEligible = V66SoftOpportunityReason(reason),
                Geometry = c.Signal.GeometryQuality,
                Prz = c.Signal.PrzConfluence,
                TimeSymmetry = c.Signal.TimeSymmetry,
                PivotQuality = c.Signal.PivotQuality,
                DirectionalBiasH1 = c.V66DirectionalBiasH1,
                DirectionalBiasH4 = c.V66DirectionalBiasH4
            };
            _v66RejectedShadowRegistered++;
        }

        private void V66UpdateRejectedOpportunityShadows(int i, DateTime utc)
        {
            if (!V66RejectedOpportunityShadowEnabled || i < 0 || i >= _m1Bars.Count) return;
            foreach (var x in _v66RejectedOpportunityShadows.Values.Where(z => z.Active).ToList())
            {
                double fav, adv;
                if (x.Direction == TradeDirection.Buy)
                {
                    fav = (_m1Bars.HighPrices[i] - x.Anchor) / x.RiskDistance;
                    adv = (x.Anchor - _m1Bars.LowPrices[i]) / x.RiskDistance;
                }
                else
                {
                    fav = (x.Anchor - _m1Bars.LowPrices[i]) / x.RiskDistance;
                    adv = (_m1Bars.HighPrices[i] - x.Anchor) / x.RiskDistance;
                }
                x.MfeR = Math.Max(x.MfeR, fav);
                x.MaeR = Math.Max(x.MaeR, adv);
                if (utc >= x.ExpiryUtc)
                    V66CompleteRejectedOpportunityShadow(x, "SHADOW_TTL");
            }
        }

        private void V66CompleteRejectedOpportunityShadow(RejectedOpportunityShadow x, string terminal)
        {
            if (x == null || !x.Active) return;
            x.Active = false;
            _v66RejectedShadowCompleted++;
            Print("[V66-REJECTED-SHADOW] setup={0} cid={1} pattern={2} route={3} dir={4} reason={5} softEligible={6} start={7:o} terminal={8} mfeR={9:F3} maeR={10:F3} geom={11:F3} prz={12:F3} time={13:F3} pivot={14:F3} dbH1={15:F4} dbH4={16:F4}",
                x.SetupKey, x.CandidateId, x.Pattern, x.Route, x.Direction, x.Reason, x.SoftEligible, x.StartUtc, terminal,
                x.MfeR, x.MaeR, x.Geometry, x.Prz, x.TimeSymmetry, x.PivotQuality, x.DirectionalBiasH1, x.DirectionalBiasH4);
        }

        private void V66FlushRejectedOpportunityShadows(string terminal)
        {
            foreach (var x in _v66RejectedOpportunityShadows.Values.Where(z => z.Active).ToList())
                V66CompleteRejectedOpportunityShadow(x, terminal);
        }
        ...
        private string V66AlphaRegime(RegimeSnapshot r)
        {
            if (r == null) return "UNKNOWN";
            if (r.AtrPercentile >= V66ExtremeVolatilityPercentile || r.AtrRatio >= V66ShockRatio) return "SHOCK";
            if (r.Transition || Math.Abs(r.AdxH1Slope) >= 4.0) return "TRANSITION";
            if (r.Efficiency >= .30 && r.AdxH1 >= 22 && r.AdxH4 >= 20) return "DIRECTIONAL";
            if (r.Efficiency <= .16 && r.AdxH1 <= 20) return "CHOP";
            return "BALANCED";
        }

        private double V66AlphaTruthScore(PatternSignal s, MtfConflict conflict, RegimeSnapshot r, HarmonicRoute route)
        {
            if (s == null || r == null || route == HarmonicRoute.NO_TRADE) return 0;
            double geometry = VClamp(.40 * s.GeometryQuality + .30 * s.PrzConfluence + .15 * s.TimeSymmetry + .15 * s.PivotQuality);
            double volatility = 1.0 - VClamp(Math.Max(0, r.AtrPercentile - .70) / .30);
            double stability = 1.0 - VClamp(Math.Abs(r.AdxH1Slope) / 10.0);
            double context = RegimeContextScore(s, conflict, r);
            double routeFit = .50;
            if (route == HarmonicRoute.TREND_ALIGNED_REVERSAL)
                routeFit = VClamp(.45 * r.Efficiency + .30 * Math.Min(1.0, r.AdxH1 / 30.0) + .25 * stability);
            else if (route == HarmonicRoute.EXHAUSTION_REVERSAL)
                routeFit = VClamp(.45 * Math.Min(1.0, r.ExtensionAtr / 2.0) + .30 * stability + .25 * volatility);
            else if (route == HarmonicRoute.TRANSITION_REVERSAL)
                routeFit = VClamp(.40 * stability + .35 * volatility + .25 * (conflict == MtfConflict.TRANSITION ? 1.0 : .55));
            return VClamp(.38 * geometry + .27 * routeFit + .20 * context + .15 * volatility);
        }

        private bool V66AlphaTruthEligible(PatternSignal s, MtfConflict conflict, RegimeSnapshot r, HarmonicRoute route, double score)
        {
            if (s == null || r == null) return false;
            if (r.AtrPercentile >= V66ExtremeVolatilityPercentile && r.AtrRatio >= V66ShockRatio) return false;
            if (route == HarmonicRoute.TREND_ALIGNED_REVERSAL && r.AdxH1Slope >= 6.0 && r.AtrPercentile >= .80) return false;
            if (route == HarmonicRoute.EXHAUSTION_REVERSAL && r.AdxH1Slope > 0.0) return false;
            if (route == HarmonicRoute.TRANSITION_REVERSAL && Math.Abs(r.AdxH1Slope) >= 5.0) return false;
            double floor = V66MinimumAlphaTruthScore;
            // High-quality geometry earns a small, bounded tolerance; regime truth can never be fully compensated by ratios.
            if (s.GeometryQuality >= .78 && s.PrzConfluence >= .78 && s.TimeSymmetry >= .45) floor -= .03;
            return score >= floor;
        }
        ...
        {
            // V66 preserves immutable V51 completion semantics in every candidate lane; the V62/V66 stage-aware DAG is observation-only.
            score = 0;
        ...
        {
            _candidateSeq++;
            return "V66-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private string V66GateStage(string reason)
        {
            string r=reason??"";
            if (r.IndexOf("DIRECTIONAL_PERSISTENCE",StringComparison.OrdinalIgnoreCase)>=0) return "TrendThesisProof";
            if (r.IndexOf("ALPHA_TRUTH",StringComparison.OrdinalIgnoreCase)>=0) return "AlphaRegimeTruthObservation";
            if (r.IndexOf("PATTERN_QUALITY",StringComparison.OrdinalIgnoreCase)>=0) return "PatternQualification";
            if (r.IndexOf("ROUTER",StringComparison.OrdinalIgnoreCase)>=0 || r.IndexOf("REGIME",StringComparison.OrdinalIgnoreCase)>=0) return "RegimeRouting";
            if (r.IndexOf("CONFIRM",StringComparison.OrdinalIgnoreCase)>=0 || r.IndexOf("M1",StringComparison.OrdinalIgnoreCase)>=0) return "Confirmation";
            if (r.IndexOf("SESSION",StringComparison.OrdinalIgnoreCase)>=0) return "Session";
            if (r.IndexOf("SPREAD",StringComparison.OrdinalIgnoreCase)>=0) return "Spread";
            if (r.IndexOf("MARGIN",StringComparison.OrdinalIgnoreCase)>=0) return "Margin";
            if (r.IndexOf("VOLUME",StringComparison.OrdinalIgnoreCase)>=0 || r.IndexOf("CAPITAL",StringComparison.OrdinalIgnoreCase)>=0) return "MinVolumeCapital";
            if (r.IndexOf("RISK",StringComparison.OrdinalIgnoreCase)>=0 || r.IndexOf("GRID",StringComparison.OrdinalIgnoreCase)>=0 || r.IndexOf("STOP",StringComparison.OrdinalIgnoreCase)>=0 || r.IndexOf("TARGET",StringComparison.OrdinalIgnoreCase)>=0) return "RiskExecution";
            return "Other";
        }
        ...
        {
            if (c==null) return;
            V66RegisterRejectedOpportunityShadow(c, reason);
            c.State=CandidateState.REJECTED; c.IsActive=false; _parkedCandidateIds.Remove(c.CandidateId);
            if (EnableCanonicalSetupIdentity && !string.IsNullOrWhiteSpace(c.SetupKey)) _activeSetupOwners.Remove(c.SetupKey);
            c.LastReason=reason; CountPipeline(c.Signal.PatternName).Rejected++;
            Print("[V66-GATE-REJECT] cid={0} setup={1} stage={2} reason={3}",c.CandidateId,c.SetupKey,V66GateStage(reason),reason);
    ...
    public sealed class RegimeSnapshot
    {
        ...
        public double DiPlusH1, DiMinusH1, DiPlusH4, DiMinusH4;
        public double EmaSlopeH1Atr, EmaSlopeH4Atr;
    }
    ...
    public sealed class CandidateRecord
    {
        ...
        public double AlphaQualityScore, AlphaTruthScore, RegimeScore, CapitalMinL0Risk, CapitalMinL0Margin;
        public string AlphaRegime = "UNKNOWN";
        public bool V66QualifiedRecall, V66TrendProofPassed;
        public double V66DirectionalBiasH1, V66DirectionalBiasH4, V66SignedEmaSlopeH1, V66SignedEmaSlopeH4;
    ...
    public sealed class RejectedOpportunityShadow
    {
        public string SetupKey, CandidateId, Pattern, Reason;
        public HarmonicRoute Route;
        public TradeDirection Direction;
        public double Anchor, RiskDistance, MfeR, MaeR;
        public double Geometry, Prz, TimeSymmetry, PivotQuality, DirectionalBiasH1, DirectionalBiasH4;
        public DateTime StartUtc, ExpiryUtc;
        public bool SoftEligible, Active = true;
    }
    ...
    public sealed class PositionLedger
    {
        ...
        public DateTime PeakUtc, MaxAdverseUtc;
    ...
    {
        public int Index;
        public double Fraction, PlannedPrice, RiskWeight, RiskBudget, Volume, PlannedRisk, ModeledCost, MinBrokerRisk, RunnerTarget;
        public long PendingOrderId, PositionId;
        public bool Physical, FillCounted, RunnerEligible;
    ...
    public sealed class FibonacciGridPlan
    {
        ...
        public double BasketRiskAmount, WorstCaseRisk, ExpectedNetRR, VirtualWeightedEntry, EstimatedPhysicalMargin, RunnerRiskWeight;
        public int LogicalLegCount, PhysicalDepth;
        public bool MicroCapitalMode, SelectiveRunnerEnabled;
