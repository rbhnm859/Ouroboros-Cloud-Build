...
def label_basket(label):
    ...
    if len(p)>=3 and p[0] in ("HB52","HB67","HB69"): return p[1]
    return ""
def kv_comment(c):
    z={}
...
KNOWN_FAMILIES={"Gartley","Bat","Alt Bat","Butterfly","Crab","Deep Crab","Deep Gartley","Rat","Cypher","Shark","5-0","AB=CD"}
def family_from_cid(cid):
    s=str(cid or "")
    probes=[
      ("DeepGartley","Deep Gartley"),("DeepCrab","Deep Crab"),("AltBat","Alt Bat"),
      ("Butterfly","Butterfly"),("Gartley","Gartley"),("Cypher","Cypher"),("Shark","Shark"),
      ("Crab","Crab"),("Rat","Rat"),("AB=CD","AB=CD"),("5-0","5-0"),("Bat","Bat")
    ]
    for token,name in probes:
        if f"-{token}-Buy-" in s or f"-{token}-Sell-" in s:
            return name
    return ""
...
for basket,z in groups.items():
    ...
    cid_family=family_from_cid(cid)
    if cid_family and z["pattern"] not in KNOWN_FAMILIES:
        z["pattern"]=cid_family
...
out={
 ...
 "engineering_clean":engineering,"summary_present":bool(summary),"broker_profile_present":("[V69-BROKER-PROFILE]" in t or "[V67-BROKER-PROFILE]" in t or "[V52-BROKER-PROFILE]" in t),
