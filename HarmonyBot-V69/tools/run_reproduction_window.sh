...
C="$PWD/control"; W="$C/HarmonyBot-V69/repro-$TRACK-$WIN"; O="$C/HarmonyBot-V69/repro-out-$TRACK-$WIN"
...
case "$TRACK" in
 V52_BASE)
   ...
   ;;
 V69_A)
   cp "$C/HarmonyBot-V69/dist/HarmonyBot_V69_Family_Positive_Cohort_Commercial_Breakthrough.algo" "$W/seal/algo/"
   RUNNER="$C/HarmonyBot-V69/tools/run_backtest.sh"
   EXTRA=(V69CONTRACTS=false V69FRONTIER=false V69FGRID=false V69EVIDENCE=false V69EXPAND=false V69ATLAS=false V69POSITIVE=false V69SCHED=false MAXDD=10 REPORT_FLUSH_GRACE_SECONDS=8)
   ;;
 *) echo "unknown reproduction track $TRACK"; exit 31;;
esac
chmod +x "$C/HarmonyBot-V52/tools/run_backtest.sh" "$C/HarmonyBot-V69/tools/run_backtest.sh"
N="V69-REPRO-$TRACK-$WIN-B10000"
(
 cd "$W"
