...
if [ "$CONTROL" = "V52" ]; then
  ...
  RUNNER="$C/HarmonyBot-V69/tools/run_backtest.sh"
fi
chmod +x "$RUNNER"
...
(
 ...
 if [ "$CONTROL" = "V52" ]; then
   ...
   IDENT=true BOUNDED=true SKIPS=2 FQUOTA="$FQUOTA" PRJPRZ=false DTRUTH=true \
   ...
   V69CONTRACTS="$CONTRACTS" V69FRONTIER="$FRONTIER" V69FGRID="$FGRID" V69EVIDENCE="$EVIDENCE" V69EXPAND="$EXPAND" \
   V69ATLAS="$ATLAS" V69POSITIVE="$POSITIVE" V69SCHED="$SCHED" MAXDD="$MAXDD" \
   BACKTEST_TIMEOUT_SECONDS=1800 "$RUNNER"
 fi
)
python3 "$C/HarmonyBot-V69/tools/audit_report.py" --report "$W/seal/reports/$N.json" --log "$W/seal/logs/$N.log" \
