...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V69 — Family-Native Regime Survival Commercial Convergence";
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                if (EnableV69PositiveThroughputExpansion && !V69RegimeSurvivalPass(record))
                {
                    _regimeRejected++;
                    Print("[V69-REGIME-SURVIVAL-VETO] cid={0} pattern={1} route={2} regime={3:F3} atrPct={4:F3} adxSlope={5:F3} efficiency={6:F3}",
                        record.CandidateId, record.Signal.PatternName, record.Route, record.RegimeScore,
                        record.Regime.AtrPercentile, record.Regime.AdxH1Slope, record.Regime.Efficiency);
                    Reject(record, "V69_REGIME_SURVIVAL_VETO");
                    continue;
                }
        ...
        private bool V69AbcdStandaloneEligible(PatternSignal s)
        {
            ...
            return s.HarmonicSubtype == "ABCD_EXACT";
        }
        ...
        private HarmonicRoute RouteSignalFamilyNativeV69(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (EnableV69PositiveThroughputExpansion && V69IsSupplyRecoveryFamily(s.PatternName))
            {
                return V69RecoveryRoute(s, conflict, r, c, trendOk, exhaustOk, transitionOk);
            }
        ...
        private bool V69FamilyRouteEvidencePass(int i, CandidateRecord c)
        {
            ...
            if (EnableV69PositiveThroughputExpansion && c.NativeStage < 4) return false;
            if (c.Route == HarmonicRoute.TREND_ALIGNED_REVERSAL)
                return c.FamilyReclaim && (c.FamilyBos || c.FamilyDisplacement) && c.FamilyDirectional;
            if (c.Route == HarmonicRoute.EXHAUSTION_REVERSAL)
                return (c.FamilyFailedExtension || c.FamilySweep) && c.FamilyRejection &&
                       (c.FamilyBos || c.FamilyDisplacement || c.FamilyReclaim);
            if (c.Route == HarmonicRoute.TRANSITION_REVERSAL)
                return c.Signal.PatternName=="5-0" && c.FamilyFailedExtension && c.FamilyBos &&
                       c.FamilyRetest && c.FamilyDirectional && c.FamilyDisplacement;
            return false;
        }

        private bool V69RegimeSurvivalPass(CandidateRecord c)
        {
            if (c==null || c.Signal==null || c.Regime==null) return false;
            var r=c.Regime;
            if (c.RegimeScore < .60) return false;
            if (c.Route==HarmonicRoute.TRANSITION_REVERSAL)
            {
                if (c.Signal.PatternName!="5-0") return false;
                return c.Signal.GeometryQuality>=.72 && c.RegimeScore>=.66 && c.RegimeScore<=.78 &&
                       r.AdxH1Slope<=-6.0 && r.AtrPercentile>=.30 && r.AtrPercentile<=.70 &&
                       r.Efficiency>=.06 && r.Efficiency<=.18;
            }
            if (c.Route==HarmonicRoute.TREND_ALIGNED_REVERSAL)
            {
                if (c.RegimeScore < .65) return false;
                if (r.AdxH1Slope>-8.0 && r.AdxH1Slope<-3.0 && r.Efficiency<.35) return false;
                if (r.AtrPercentile>=.35 && r.AtrPercentile<=.50 && r.Efficiency<.30) return false;
                if (c.Signal.PatternName=="AB=CD" &&
                    (c.Signal.HarmonicSubtype!="ABCD_EXACT" || c.Signal.GeometryQuality<.72 ||
                     c.Signal.PrzConfluence<.62 || c.Signal.Confidence<.62))
                    return false;
                return true;
            }
            if (c.Route==HarmonicRoute.EXHAUSTION_REVERSAL)
            {
                var fc=V69Contract(c.Signal.PatternName);
                double need=fc==null?.70:Math.Max(.70,fc.ExhaustionExtensionAtr*.68);
                return r.ExtensionAtr>=need && r.AdxH1Slope<=.75 && c.RegimeScore>=.60;
            }
            return false;
        }
            ...
            bool trendOk, bool exhaustOk, bool transitionOk)
        {
            ...
            {
                bool selectiveTransition=conflict==MtfConflict.NEUTRAL && s.GeometryQuality>=.72 &&
                    familyRegime>=.66 && familyRegime<=.78 && r.AdxH1Slope<=-6.0 &&
                    r.AtrPercentile>=.30 && r.AtrPercentile<=.70 &&
                    r.Efficiency>=.06 && r.Efficiency<=.18;
                return selectiveTransition ? HarmonicRoute.TRANSITION_REVERSAL : HarmonicRoute.NO_TRADE;
            }
            if(s.PatternName=="Gartley" || s.PatternName=="Bat" || s.PatternName=="Deep Gartley")
                return trendOk && familyRegime>=.65 ? HarmonicRoute.TREND_ALIGNED_REVERSAL : HarmonicRoute.NO_TRADE;
            if(c.Role.Contains("EXTENSION"))
                return c.AllowExhaustion && (terminalExtension || conflictExhaustion) && familyRegime>=.60
                    ? HarmonicRoute.EXHAUSTION_REVERSAL : HarmonicRoute.NO_TRADE;
            if(trendOk && familyRegime>=.65) return HarmonicRoute.TREND_ALIGNED_REVERSAL;
            if(exhaustOk && familyRegime>=.68) return HarmonicRoute.EXHAUSTION_REVERSAL;
            return HarmonicRoute.NO_TRADE;
        }
        ...
        private bool TryBuildV69FamilyGridAtlasPlan(CandidateRecord c)
        {
            ...
            if(c.RegimeScore<.72) maxLegs=Math.Min(maxLegs,2);
            if(c.Route==HarmonicRoute.TRANSITION_REVERSAL) maxLegs=Math.Min(maxLegs,2);
