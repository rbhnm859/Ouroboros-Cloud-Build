...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V69 — Harmonic Family Positive Cohort Commercial Breakthrough";
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (EnableV69PositiveThroughputExpansion && !V69CommercialSurvivalPass(c))
                    {
                        _regimeRejected++;
                        Event(c, "V69_COMMERCIAL_SURVIVAL_VETO");
                        continue;
                    }
        ...
        private double FamilyNativeJointGeometryScore(PatternProfile p, double xab, double abc, double bcd, double xad, double abcd, double xac)
        {
            ...
            if (p.Mode == PatternMode.STANDARD)
            {
                ...
                // V69: family identity is native XAB/ABC/BCD/XAD geometry. AB=CD remains
                // completion/confluence evidence and may not veto an otherwise valid family.
                if (!EnableFamilyIdentityReconstruction)
        ...
        private bool V69CommercialSurvivalPass(CandidateRecord c)
        {
            if (c == null || c.Signal == null || c.Regime == null) return false;
            string p = c.Signal.PatternName ?? "";
            double regime = c.RegimeScore;
            double confirm = c.ConfirmationScore;
            double eff = c.Regime.Efficiency;
            double atrPct = c.Regime.AtrPercentile;

            // This gate uses only information available before entry.  It never reads future
            // MFE/MAE, future volatility or future bars.  Calibration outcomes selected which
            // family/route cohorts require protection; runtime eligibility remains causal.
            if (p == "5-0")
                return c.Route == HarmonicRoute.TRANSITION_REVERSAL &&
                       regime >= .66 && confirm >= .90 &&
                       c.Regime.AdxH1Slope <= -4.0 && eff >= .05 && eff <= .20 &&
                       atrPct >= .25 && atrPct <= .75;

            if (p == "Cypher")
                return c.Route == HarmonicRoute.TREND_ALIGNED_REVERSAL &&
                       regime >= .68 && confirm >= .75 &&
                       eff >= .10 && eff <= .60 && atrPct >= .20 && atrPct <= .85;

            if (p == "Rat")
                return c.Route == HarmonicRoute.TREND_ALIGNED_REVERSAL &&
                       regime >= .64 && confirm >= .75;
            ...
            if (p == "AB=CD")
                return V69AbcdStandaloneEligible(c.Signal) &&
                       regime >= .62 && confirm >= .75;

            // Sparse native families are supply-limited, not quota-forced.  Their own completed
            // four-stage family confirmation plus a modest pre-entry regime floor is required.
            if (V69IsSupplyRecoveryFamily(p))
                return c.NativeStage >= 4 && regime >= .55 && confirm >= .75;

            return regime >= .60 && confirm >= .75;
        }
