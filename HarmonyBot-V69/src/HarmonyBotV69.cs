...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V69 — V52 Exact Family-State Grid Commercial Rebase";
        ...
        [Parameter("V69 Mode", DefaultValue = "V69_FAMILY_STATE_GRID")]
        public string V69Variant { get; set; }

        [Parameter("V69 Suppress Overlap AB=CD", DefaultValue = true)]
        public bool V69SuppressOverlapAbcd { get; set; }

        [Parameter("V69 Family-Owned Routing", DefaultValue = true)]
        public bool V69FamilyOwnedRouting { get; set; }

        [Parameter("V69 Family-Native Grid", DefaultValue = true)]
        public bool V69FamilyNativeGrid { get; set; }
        ...
        private int _v69ParentAbcdSuppressed;
        private int _v69FamilyStateRouted;
        private int _v69FamilyStateRejected;
        private int _v69GridWeightNormalized;
        ...
        protected override void OnStart()
        {
            if (!V69ModeValid())
            {
                Print("[V69-FATAL] INVALID_MODE {0}", V69Variant);
                Stop();
                return;
            }
            ...
            Print("[V69-PRODUCT-CONFIG] mode={0} familyOwnedRouting={1} suppressOverlapAbcd={2} familyNativeGrid={3} exactV52Control={4}",
                V69Variant, V69FamilyOwnedRouting, V69SuppressOverlapAbcd, V69FamilyNativeGrid, V69ExactControlEnabled());
        ...
        protected override void OnStop()
        {
            ...
            Print("[V69-FAMILY-STATE-SUMMARY] mode={0} parentAbcdSuppressed={1} familyStateRouted={2} familyStateRejected={3} gridWeightNormalized={4}",
                V69Variant, _v69ParentAbcdSuppressed, _v69FamilyStateRouted, _v69FamilyStateRejected, _v69GridWeightNormalized);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                record.Route = V69ProductEnabled() && V69FamilyOwnedRouting
                    ? V69FamilyRouteSignal(signal, record.Conflict, regime)
                    : RouteSignal(signal, record.Conflict, regime);
                if (V69ProductEnabled() && record.Route != HarmonicRoute.NO_TRADE)
                {
                    _v69FamilyStateRouted++;
                    Event(record, "V69_FAMILY_STATE_ROUTE_" + record.Route);
                }

                if (record.Route == HarmonicRoute.NO_TRADE)
                {
                    if (EnableRegimeContextGate) _regimeRejected++;
                    if (V69ProductEnabled()) _v69FamilyStateRejected++;
                    Reject(record, V69ProductEnabled() ? "V69_FAMILY_STATE_NO_TRADE" : "ROUTER_NO_TRADE");
                    continue;
                }
                ...
                if (!V69ProductEnabled() && EnableScaleRouteAdmission && signal.PivotScale != M15SwingDepth &&
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            if (V69GridEnabled())
            {
                double logicalWeightSum = plan.Legs.Sum(x => x.RiskWeight);
                if (logicalWeightSum <= 0) return GridPlanReject(c, "V69_INVALID_GRID_WEIGHT_SUM");
                foreach (var l in plan.Legs)
                {
                    l.RiskWeight /= logicalWeightSum;
                    l.RiskBudget = plan.BasketRiskAmount * l.RiskWeight;
                }
                _v69GridWeightNormalized++;
            }
        ...
        private List<PatternSignal> DetectPatternCandidates(Bars bars, int endIndex, int depth, int lookback, int maxCandidates, string timeframe)
        {
            ...
            if (V69ProductEnabled() && V69SuppressOverlapAbcd)
            {
                var parentPreferred = new List<PatternSignal>();
                foreach (var g in hypotheses.GroupBy(BuildSetupGeometryKey))
                {
                    bool hasNamedParent = g.Any(x => x.PatternName != "AB=CD");
                    foreach (var x in g)
                    {
                        if (hasNamedParent && x.PatternName == "AB=CD")
                        {
                            _v69ParentAbcdSuppressed++;
                            DetectorTruth("AB=CD", "PARENT_FAMILY_SUPPRESSED");
                            continue;
                        }
                        parentPreferred.Add(x);
                    }
                }
                hypotheses = parentPreferred;
            }
            ...
            double tolerance, int ttlMinutes, double stopFibBuffer, string anchorRule, string targetPolicy)
        {
            ...
            p.GridRiskWeights = V69GridEnabled()
                ? V69FamilyGridWeights(name, maxLegs)
        ...
        private bool V69ExactControlEnabled()
        {
            return string.Equals(V69Variant, "V69_V52_EXACT_CONTROL", StringComparison.Ordinal);
        }

        private bool V69FamilyStateEnabled()
        {
            return string.Equals(V69Variant, "V69_FAMILY_STATE", StringComparison.Ordinal);
        }

        private bool V69GridEnabled()
        {
            return string.Equals(V69Variant, "V69_FAMILY_STATE_GRID", StringComparison.Ordinal) && V69FamilyNativeGrid;
        }

        private bool V69ProductEnabled()
        {
            return V69FamilyStateEnabled() || V69GridEnabled();
        }

        private bool V69ModeValid()
        {
            return V69ExactControlEnabled() || V69FamilyStateEnabled() || V69GridEnabled();
        }

        private bool V69IsRetracementFamily(string p)
        {
            return p == "Gartley" || p == "Bat" || p == "Deep Gartley" || p == "Rat";
        }

        private bool V69IsExtensionFamily(string p)
        {
            return p == "Alt Bat" || p == "Butterfly" || p == "Crab" || p == "Deep Crab";
        }

        private HarmonicRoute V69FamilyRouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            bool liquidVol = r.AtrRatio >= .50 && r.AtrRatio <= 1.80;

            if (V69IsRetracementFamily(p))
            {
                if (aligned && conflict != MtfConflict.CONFLICT && liquidVol && r.Efficiency >= .14)
                    return HarmonicRoute.TREND_ALIGNED_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            }

            if (V69IsExtensionFamily(p))
            {
                if (opposed && liquidVol && r.ExtensionAtr >= 1.20 && r.AdxH1Slope <= .50)
            ...
            if (p == "Cypher" || p == "Shark")
            {
                if (aligned && conflict != MtfConflict.CONFLICT && liquidVol)
                    return HarmonicRoute.TREND_ALIGNED_REVERSAL;
                if (opposed && liquidVol && r.ExtensionAtr >= 1.20 && r.AdxH1Slope <= .50)
                    return HarmonicRoute.EXHAUSTION_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            ...
            if (p == "AB=CD")
            {
                if (aligned && conflict != MtfConflict.CONFLICT && liquidVol && r.Efficiency >= .18)
                    return HarmonicRoute.TREND_ALIGNED_REVERSAL;
                if (transition && liquidVol && s.GeometryQuality >= .68 && s.PrzConfluence >= .68)
        ...
        private double[] V69FamilyGridWeights(string name, int maxLegs)
        {
            if (maxLegs <= 1) return new[] { 1.0 };
            if (name == "Rat" || name == "Deep Gartley") return new[] { .45, .35, .20 };
            if (name == "Cypher") return new[] { .50, .30, .20 };
            if (name == "Shark" || name == "5-0") return new[] { .65, .35 };
            if (name == "Alt Bat" || name == "Butterfly") return new[] { .55, .30, .15 };
            if (name == "Crab" || name == "Deep Crab") return new[] { .65, .35 };
            if (name == "AB=CD") return new[] { 3.0 / 7.0, 2.0 / 7.0, 1.0 / 7.0, 1.0 / 7.0 };
