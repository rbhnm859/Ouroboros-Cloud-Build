...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV69 : Robot
    {
        private const string Version = "HarmonyBot V69 — Harmonic Family Identity & Detection Graph Reconstruction Kernel";
        private const string BotPrefix = "HB69";
        ...
        [Parameter("Max Drawdown %", DefaultValue = 6.0, MinValue = 1.0, MaxValue = 20.0)]
        public double MaxDrawdownPercent { get; set; }

        [Parameter("Daily Loss Limit %", DefaultValue = 2.0, MinValue = 0.5, MaxValue = 10.0)]
        ...
        [Parameter("V69 Family Trade Contracts", DefaultValue = false)]
        public bool EnableV69FamilyTradeContracts { get; set; }

        [Parameter("V69 Family Detector Frontier", DefaultValue = false)]
        public bool EnableV69FamilyDetectorFrontier { get; set; }

        [Parameter("V69 Family-Native Grid", DefaultValue = false)]
        public bool EnableV69FamilyNativeGrid { get; set; }

        [Parameter("V69 Evidence Route Guard", DefaultValue = false)]
        public bool EnableV69EvidenceRouteGuard { get; set; }

        [Parameter("V69 Controlled Expansion", DefaultValue = false)]
        public bool EnableV69ControlledExpansion { get; set; }

        [Parameter("V69 Family Grid Atlas", DefaultValue = false)]
        public bool EnableV69FamilyGridAtlas { get; set; }

        [Parameter("V69 Positive Throughput Expansion", DefaultValue = false)]
        public bool EnableV69PositiveThroughputExpansion { get; set; }

        [Parameter("V69 Family Opportunity Scheduler", DefaultValue = false)]
        public bool EnableV69FamilyOpportunityScheduler { get; set; }

        [Parameter("V69 Family Survival Veto", DefaultValue = false)]
        public bool EnableV69FamilySurvivalVeto { get; set; }
        ...
        private readonly Dictionary<string, FamilyGridModule> _familyGridAtlas = new Dictionary<string, FamilyGridModule>(StringComparer.OrdinalIgnoreCase);
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V69-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V69-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V69-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V69-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            BuildV69FamilyTradeContracts();
            BuildV69FamilyGridAtlas();
            if (EnableV69FamilyNativeGrid && !EnableV69FamilyGridAtlas) ApplyV69FamilyGridContracts();
            ...
            Print("[V69-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m1Bars), _profiles.Count);
            Print("[V69-TIMEFRAME-AUDIT] primaryPattern=M15 execution=M1 macro=H4 intermediate=H1 allCompletedBars=true");
            Print("[V69-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
            Print("[V69-ALPHA-CONFIG] qualityObservation={0} legacyRegime={1} legacyEnhancedM1={2} capitalFeasibility={3} transitionVeto={4} exhaustionVeto={5} routeM1Veto={6}",
            ...
            Print("[V69-FREQUENCY-CONFIG] deferredRetention={0} agingPriority={1} ageBoost={2:F3} structuredRecall={3} recallGeometry={4:F3} recallPrz={5:F3} recallConfidence={6:F3}",
            ...
            Print("[V69-RESTORED-KERNEL] canonicalSetup={0} canonicalStandard={1} independentPivotGraph={2} transitionProof={3} m1Rescue={4} rescueBars={5} diversityScheduler={6} executionClock=M1 projectedD=false",
            ...
            Print("[V69-CONVERSION-ARCH] scaleRouteAdmission={0} temporalRescue={1} armedGrace={2} graceMinutes={3} preExecutionRevalidation={4}",
            ...
            Print("[V69-FAMILY-NATIVE] conversion={0} observation={1} principle=PATTERN_IDENTITY_NEQ_EXECUTION_IDENTITY", EnableFamilyNativeConversion, EnableFamilyNativeObservation);
            Print("[V69-COMPLETION-CONTRACT] canonicalContracts={0} familyCompletion={1} windowBars={2} provenLane=ABCD_SHARK_CYPHER_FROZEN",
                EnableCanonicalFamilyContracts, EnableFamilyCompletionContract, FamilyConfirmationWindowBars);
            Print("[V69-STRUCTURAL-GRID-CONTRACT] gridSpanV2={0} structuralInvalidationV2={1} rule=PRZ_LEGALITY_PLUS_RISK_RR_NOT_LEGACY_XA_MINSPAN",
                EnableGridSpanSemanticV2, EnableStructuralInvalidationV2);
            Print("[V69-MATH-GEOMETRY] jointGeometry={0} executionCorridor={1} entryAnchorForensics={2} rule=FAMILY_NATIVE_GEOMETRY_WITHOUT_RISK_RELAXATION",
                EnableFamilyNativeJointGeometry, EnableFamilyNativeExecutionCorridor, EnableEntryAnchorForensics);
            Print("[V69-DETECTOR-ARCH] identityReconstruction={0} boundedPivotGraph={1} maxMicroSkips={2} familyQuota={3} projectedPrz={4} truthLedger={5} rule=DETECT_MANY_EXECUTE_ONE",
            ...
            Print("[V69-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V46_SCALE_CONVERSION_FROZEN",
            ...
            Print("[V69-FAMILY-TRADE-CONTRACTS] enabled={0} detectorFrontier={1} familyGrid={2} evidenceGuard={3} controlledExpansion={4} contracts={5} principle=FAMILY_IDENTITY_ROUTE_CONFIRMATION_GRID_SEPARATED",
                EnableV69FamilyTradeContracts, EnableV69FamilyDetectorFrontier, EnableV69FamilyNativeGrid,
                EnableV69EvidenceRouteGuard, EnableV69ControlledExpansion, _familyTradeContracts.Count);
            Print("[V69-GRID-ATLAS] enabled={0} positiveThroughputExpansion={1} opportunityScheduler={2} modules={3} rule=NO_FIXED_FAMILY_QUOTA_EACH_FAMILY_MUST_PROVE_OWN_POSITIVE_CONTRIBUTION",
                EnableV69FamilyGridAtlas, EnableV69PositiveThroughputExpansion, EnableV69FamilyOpportunityScheduler, _familyGridAtlas.Count);
            foreach (var gm in _familyGridAtlas.OrderBy(x => x.Key))
            {
                var g = gm.Value;
                Print("[V69-FAMILY-GRID-MODULE] pattern={0} geometry={1} scale={2:F3} ttlMin={3} cancelMfeR={4:F2} Tlegs={5} Elegs={6} Xlegs={7}",
                    g.Name, g.GeometryBasis, g.GeometryScale, g.PendingTtlMinutes, g.CancelPendingMfeR,
                    g.TrendFractions.Length, g.ExhaustionFractions.Length, g.TransitionFractions.Length);
            }
            foreach (var kv in _familyTradeContracts.OrderBy(x => x.Key))
            {
                var fc = kv.Value;
                Print("[V69-FAMILY-POLICY] pattern={0} role={1} routes=T{2}/E{3}/X{4} confirm={5} gridLegs={6} runner={7} expiryBars={8}",
        ...
        protected override void OnStop()
        {
            ...
            {
                var x = kv.Value;
                Print("[V69-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} nativeTemporalPass={6} armed={7} slotBlocked={8} parked={9} revalidated={10} revalidationRejected={11} basketPlanned={12} leg0={13} leg1={14} leg2={15} leg3={16} basketClosed={17} executed={18} expired={19} rejected={20} invalidated={21}",
            ...
            if (EnableEntryAnchorForensics)
            {
                ...
                {
                    Print("[V69-ENTRY-ANCHOR-FORENSICS] cid={0} setup={1} pattern={2} route={3} scale={4} completion={5} confirm={6} retest={7} completionMfeR={8:F3} completionMaeR={9:F3} confirmMfeR={10:F3} confirmMaeR={11:F3} retestMfeR={12:F3} retestMaeR={13:F3}",
            ...
            if (EnableDetectorTruthLedger)
            {
                foreach (var kv in _detectorTruth.OrderBy(x => x.Key))
                {
                    ...
                    Print("[V69-DETECTOR-TRUTH] pattern={0} stage={1} count={2}", pattern, stage, kv.Value);
                }
            }
            Print("[V69-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V69-ALPHA-SUMMARY] qualityRejected={0} regimeRejected={1} confirmationRejected={2} capitalInfeasible={3} alphaPassed={4}",
                _alphaQualityRejected, _regimeRejected, _confirmationRejected, _capitalInfeasibleCandidates, _alphaPassed);
            Print("[V69-FREQUENCY-SUMMARY] schedulerDeferred={0} schedulerRecoveredExecutions={1} structuredRecallAdmitted={2} activeDeferred={3}",
                _schedulerDeferred, _schedulerRecoveredExecutions, _structuredRecallAdmitted, _deferredCandidates.Count);
            Print("[V69-INDEPENDENT-SETUP-SUMMARY] duplicateSuppressed={0} uniqueExecuted={1} rescueAdmissions={2} transitionProofRejected={3} independentScaleCandidates={4}",
            ...
            Print("[V69-CONVERSION-SUMMARY] scaleRouteRejected={0} temporalRescueAdmissions={1} armedGraceExtended={2} preExecutionRevalidationRejected={3}",
            ...
            Print("[V69-THROUGHPUT-SUMMARY] slotBlocked={0} parked={1} revalidated={2} revalidationRejected={3} recoveredExecutions={4} nativeTemporalPass={5} hardLifetimeExpired={6} decayRejected={7} avgSlotWaitMin={8:F2} medianSlotWaitMin={9:F2} p90SlotWaitMin={10:F2} avgBasketOccupancyMin={11:F2} missedPositive={12} avoidedNegative={13}",
            ...
            foreach (var p in _profiles.Select(x => x.Name).Distinct().OrderBy(x => x))
            {
                ...
                Print("[V69-FAMILY-CONTRACT-SUMMARY] pattern={0} pass={1} windowReject={2}", p, pass, reject);
            }
            foreach (var kv in _gridPlanRejectReasons.OrderBy(k => k.Key))
                Print("[V69-GRID-REJECT-SUMMARY] reason={0} count={1}", kv.Key, kv.Value);
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V69-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                var record = new CandidateRecord
                {
                    ...
                    ExpiryUtc = Server.Time.ToUniversalTime().AddMinutes(15.0 * (EnableV69FamilyTradeContracts ? V69FamilyExpiryBars(signal) : Math.Max(2, Math.Min(CandidateTtlM15Bars, signal.Profile.MaxAgeM15Bars)))),
                ...
                bool familyQualityPass = EnableV69FamilyTradeContracts
                    ? V69FamilyQualityPass(signal)
                ...
                {
                    if (EnableV69FamilyTradeContracts)
                        Print("[V69-FAMILY-QUALITY-REJECT] cid={0} pattern={1} geom={2:F3} prz={3:F3} conf={4:F3} subtype={5}",
                    ...
                    Reject(record, EnableV69FamilyTradeContracts ? "V69_FAMILY_QUALITY" : "PATTERN_QUALITY");
                    continue;
                }
                ...
                record.RegimeScore = EnableV69FamilyTradeContracts ? V69FamilyRegimeScore(signal, record.Conflict, regime) : RegimeContextScore(signal, record.Conflict, regime);
                record.Route = EnableV69FamilyTradeContracts ? RouteSignalFamilyNativeV69(signal, record.Conflict, regime) : RouteSignal(signal, record.Conflict, regime);

                if (record.Route == HarmonicRoute.NO_TRADE)
                {
                    if (EnableRegimeContextGate) _regimeRejected++;
                    if (EnableV69FamilyTradeContracts)
                        Print("[V69-FAMILY-ROUTE-REJECT] cid={0} pattern={1} conflict={2} regimeScore={3:F3} trend={4} transition={5} extAtr={6:F3} atrRatio={7:F3} adxSlope={8:F3} efficiency={9:F3}",
                ...
                if (!EnableV69FamilyTradeContracts && EnableScaleRouteAdmission && signal.PivotScale != M15SwingDepth &&
                ...
                Print("[V69-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                {
                    if (EnableV69PositiveThroughputExpansion ? V69FamilyActivationTouch(i, c) : BarTouchesPrz(i, c.Signal))
                    {
                        c.PrzTouchUtc = utc;
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    bool familyContractLane = EnableV69FamilyTradeContracts || (EnableFamilyCompletionContract && IsFamilyCompletionLane(c.Signal.PatternName));
                    ...
                    if (familyContractLane)
                    {
                        ...
                        double familyScore;
                        confirmationPass = EnableV69FamilyTradeContracts
                            ? UpdateV69FamilyContractEvidence(i, c, out familyScore)
                        ...
                        if (confirmationPass)
                        {
                            ...
                            Event(c, (EnableV69FamilyTradeContracts ? "V69_FAMILY_TRADE_CONTRACT_PASS_" : "FAMILY_COMPLETION_CONTRACT_PASS_") + familyScore.ToString("F2", CultureInfo.InvariantCulture));
                        }
                        else if (c.FamilyConfirmationBarsObserved >= (EnableV69FamilyTradeContracts ? V69FamilyConfirmationWindow(c.Signal) : Math.Max(6, FamilyConfirmationWindowBars)))
                        {
                            IncrementCounter(_familyContractWindowReject, c.Signal.PatternName);
                            if (EnableV69FamilyTradeContracts)
                                Print("[V69-FAMILY-CONFIRM-REJECT] cid={0} pattern={1} stage={2} bars={3} score={4:F3} dir={5} reclaim={6} bos={7} reject={8} sweep={9} failedExt={10} insidePrz={11} disp={12} retest={13}",
                            ...
                            Reject(c, EnableV69FamilyTradeContracts ? "V69_FAMILY_CONFIRMATION_WINDOW_EXHAUSTED" : "FAMILY_COMPLETION_WINDOW_EXHAUSTED");
                            continue;
                        }
                    }
                    ...
                    if (EnableRouteSpecificM1Veto && !(EnableV69FamilyTradeContracts ? V69FamilyRouteEvidencePass(i, c) : RouteSpecificM1EvidencePass(i, c.Signal, c.Route)))
                    {
                    ...
                    if (EnableV69FamilySurvivalVeto && !V69FamilySurvivalPass(c))
                    {
                        Event(c, "V69_FAMILY_SURVIVAL_VETO");
                        Reject(c, "V69_FAMILY_SURVIVAL_VETO");
                        continue;
                    }
        ...
        private void TryScheduleAndExecute()
        {
            ...
            var armed = armedQuery
                .OrderByDescending(c => EnableV69FamilyOpportunityScheduler
                    ? V69FamilyOpportunityScore(c, now)
        ...
        private bool RevalidateCandidateForExecution(CandidateRecord c, DateTime now, bool finalCheck)
        {
            ...
            if (EnableV69FamilySurvivalVeto && !V69FamilySurvivalPass(c)) return false;
        ...
        private void MarkOpportunityTerminal(CandidateRecord c, string reason)
        {
            ...
            Print("[V69-OPPORTUNITY-LOSS] cid={0} setup={1} pattern={2} route={3} scale={4} reason={5} shadowMfeR={6:F3} shadowMaeR={7:F3} missedPositive={8} avoidedNegative={9}",
        ...
        {
            if (EnableV69FamilyGridAtlas)
                return TryBuildV69FamilyGridAtlasPlan(c);
            ...
            Print("[V69-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V69-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V69-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void ReconcileAndManageBaskets()
        {
            ...
            foreach (var basket in _baskets.Values.Where(b => b.IsActive).ToList())
            {
                ...
                double cancelMfe = basket.Plan != null && basket.Plan.CancelPendingAtMfeR > 0
                    ? basket.Plan.CancelPendingAtMfeR : GridCancelMfeR;
                if (basket.PeakR >= cancelMfe && pending.Count > 0)
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V69-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V69-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V69-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V69-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V69-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V69-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V69-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V69-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V69-BASKET-CLOSED] basket={0} cid={1} setup={2} pattern={3} subtype={4} route={5} dir={6} plannedLegs={7} filledLegs={8} anchor={9} avgEntry={10} entryImprovePips={11:F3} stop={12} target={13} initialRisk={14:F2} worstRisk={15:F2} mfeR={16:F3} maeR={17:F3} realizedR={18:F3} net={19:F2} reason={20}",
            ...
            Print("[V69-SLOT-OCCUPANCY] basket={0} cid={1} pattern={2} route={3} occupancyMinutes={4:F2} realizedR={5:F3} net={6:F2}",
        ...
        {
            Print("[V69-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
        ...
        private List<PatternSignal> DetectPatternCandidates(Bars bars, int endIndex, int depth, int lookback, int maxCandidates, string timeframe)
        {
            ...
            if (EnableV69FamilyDetectorFrontier)
            {
                // V69 deliberately removes per-family trading quotas. Supply is ranked by family-native
                ...
                var frontier = hypotheses
                    .OrderByDescending(V69FamilyDetectionPriority)
                    .ThenByDescending(x => x.Confidence)
                    .ThenByDescending(x => x.GeometryQuality)
                    .Take(Math.Max(48, maxCandidates * 6))
                    .ToList();
                foreach (var x in frontier) DetectorTruth(x.PatternName, "V69_FAMILY_FRONTIER_SELECTED");
                return frontier;
            }
        ...
        private HarmonicRoute RouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (baseRoute == HarmonicRoute.NO_TRADE && EnableStructuredRecallExpansion)
            {
                ...
                if (baseRoute != HarmonicRoute.NO_TRADE)
                {
                    ...
                    Print("[V69-RECALL-ADMIT] pattern={0} direction={1} route={2} conflict={3} geometry={4:F3} prz={5:F3} confidence={6:F3} efficiency={7:F3} atrRatio={8:F3} extensionAtr={9:F3}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionProofGate)
            {
                ...
                if (!actualStructuralTransition)
                {
                    ...
                    Print("[V69-TRANSITION-PROOF-REJECT] pattern={0} conflict={1} transition={2} adxSlope={3:F2}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionStateVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V69-ROUTE-VETO] type=TRANSITION_STATE conflict={0} transition={1} adxSlope={2:F2}",
            ...
            if (baseRoute == HarmonicRoute.EXHAUSTION_REVERSAL && EnableExhaustionEvidenceVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V69-ROUTE-VETO] type=EXHAUSTION_EVIDENCE extensionAtr={0:F3} adxSlope={1:F2}",
        ...
        private void BuildV69FamilyTradeContracts()
        {
            ...
            AddV69FamilyContract("Gartley", "RETRACEMENT", true, false, false, "RECLAIM_REJECTION_BOS",
                .53, .53, .55, .14, 1.75, 1.30, 6, .010, false,
                new[] { 0.0, .236, .382, .618 }, new[] { .40, .30, .20, .10 });
            AddV69FamilyContract("Bat", "DEEP_RETRACEMENT", true, false, true, "REJECTION_RECLAIM_DISPLACEMENT",
                .50, .50, .54, .13, 1.75, 1.30, 6, .020, false,
                new[] { 0.0, .236, .382, .618 }, new[] { .42, .30, .18, .10 });
            AddV69FamilyContract("Alt Bat", "EXTENSION", false, true, true, "SWEEP_FAILED_EXTENSION_BOS",
                .52, .52, .56, .12, 1.80, 1.15, 7, .025, false,
                new[] { 0.0, .236, .382 }, new[] { .50, .30, .20 });
            AddV69FamilyContract("Butterfly", "EXTENSION", false, true, true, "SWEEP_RECLAIM_BOS",
                .54, .54, .58, .12, 1.80, 1.15, 7, .025, false,
                new[] { 0.0, .236, .382 }, new[] { .55, .30, .15 });
            AddV69FamilyContract("Crab", "EXTREME_EXTENSION", false, true, true, "SWEEP_FAILED_EXTENSION_DISPLACEMENT",
                .55, .55, .60, .10, 1.85, 1.10, 7, .030, false,
                new[] { 0.0, .236 }, new[] { .65, .35 });
            AddV69FamilyContract("Deep Crab", "EXTREME_EXTENSION", false, true, true, "SWEEP_PRZ_BOS",
                .55, .55, .60, .10, 1.85, 1.10, 7, .030, false,
                new[] { 0.0, .236 }, new[] { .65, .35 });
            AddV69FamilyContract("Deep Gartley", "DEEP_RETRACEMENT", true, false, true, "FAILED_EXTENSION_RECLAIM_BOS",
                .52, .52, .55, .13, 1.75, 1.30, 7, .020, false,
                new[] { 0.0, .236, .382 }, new[] { .50, .30, .20 });
            AddV69FamilyContract("Rat", "RETRACEMENT", true, false, false, "REJECTION_RECLAIM_BOS_DISPLACEMENT",
                .58, .58, .60, .15, 1.75, 1.25, 6, .080, true,
                new[] { 0.0, .236, .382 }, new[] { .50, .30, .20 });
            AddV69FamilyContract("Cypher", "XC_RETRACE", true, true, false, "REJECTION_RECLAIM_BOS_DISPLACEMENT",
                .58, .58, .60, .14, 1.80, 1.20, 6, .050, true,
                new[] { 0.0, .236, .382 }, new[] { .50, .30, .20 });
            AddV69FamilyContract("Shark", "TERMINAL_EXTENSION", true, true, false, "FAILED_EXTENSION_REJECTION_BOS",
                .58, .58, .60, .12, 1.85, 1.10, 6, .100, true,
                new[] { 0.0, .236 }, new[] { .65, .35 });
            AddV69FamilyContract("5-0", "TRANSITION_REVERSAL", false, false, true, "FAILED_EXTENSION_BOS_RETEST_DIRECTIONAL",
                .60, .60, .62, .10, 1.80, 1.20, 7, .000, false,
                new[] { 0.0, .236 }, new[] { .65, .35 });
            AddV69FamilyContract("AB=CD", "COMPLETION_PRIMITIVE", true, false, false, "DECELERATION_FAILED_EXTENSION_DIRECTIONAL_BOS",
                .58, .58, .60, .14, 1.75, 1.25, 6, .040, false,
                new[] { 0.0, .236 }, new[] { .70, .30 });
        }

        private void AddV69FamilyContract(string name, string role, bool trend, bool exhaustion, bool transition,
        ...
        private FamilyTradeContract V69Contract(string pattern)
        {
            FamilyTradeContract c;
            return pattern != null && _familyTradeContracts.TryGetValue(pattern, out c) ? c : null;
        }

        private int V69FamilyExpiryBars(PatternSignal s)
        {
            var c = s == null ? null : V69Contract(s.PatternName);
            int legacy = Math.Max(2, Math.Min(CandidateTtlM15Bars, c == null ? s.Profile.MaxAgeM15Bars : c.ExpiryM15Bars));
            if (!EnableV69PositiveThroughputExpansion || s == null || !V69IsSupplyRecoveryFamily(s.PatternName)) return legacy;
        ...
        private int V69FamilyConfirmationWindow(PatternSignal s)
        {
            var c = s == null ? null : V69Contract(s.PatternName);
            int bars = Math.Max(4, c == null ? FamilyConfirmationWindowBars : c.ConfirmationWindowBars);
            if (EnableV69PositiveThroughputExpansion && s != null && V69IsSupplyRecoveryFamily(s.PatternName))
                bars = Math.Max(bars, (s.PatternName=="Alt Bat" || s.PatternName=="Butterfly" || s.PatternName=="Crab" || s.PatternName=="Deep Crab") ? 10 : 9);
            return bars;
        }

        private bool V69FamilyQualityPass(PatternSignal s)
        {
            if (s == null) return false;
            var c = V69Contract(s.PatternName);
            if (c == null)
            ...
            if (EnableV69PositiveThroughputExpansion && V69IsSupplyRecoveryFamily(s.PatternName))
                return V69RecoveryQualityPass(s, c);
            if (EnableV69ControlledExpansion && s.PatternName == "AB=CD" && s.HarmonicSubtype == "ABCD_NEAR_127")
        ...
        private bool V69AbcdStandaloneEligible(PatternSignal s)
        {
            if (s == null || s.PatternName != "AB=CD") return true;
            if (s.HarmonicSubtype == "ABCD_EXACT") return true;
            if (EnableV69ControlledExpansion && s.HarmonicSubtype == "ABCD_NEAR_127" &&
                s.GeometryQuality >= .72 && s.PrzConfluence >= .72 && s.Confidence >= .68)
                return true;
            return false;
        }

        private double V69AbcdConfluenceScore(PatternSignal s)
        {
            if (s == null) return 0;
        ...
        private double V69FamilyDetectionPriority(PatternSignal s)
        {
            if (s == null) return 0;
            var c = V69Contract(s.PatternName);
            double boost = c == null ? 0 : c.DetectorPriorityBoost;
            double baseScore = .45 * s.Confidence + .30 * s.GeometryQuality + .15 * s.PrzConfluence + .10 * V69AbcdConfluenceScore(s);
        ...
        private double V69FamilyRegimeScore(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            return VClamp(.30 * mtf + .25 * direction + .20 * efficiencyFit + .15 * volFit + .10 * V69AbcdConfluenceScore(s));
        }

        private HarmonicRoute RouteSignalFamilyNativeV69(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            if (s == null || r == null) return HarmonicRoute.NO_TRADE;
            var c = V69Contract(s.PatternName);
            if (c == null) return RouteSignal(s, conflict, r);
            if (!V69FamilyQualityPass(s)) return HarmonicRoute.NO_TRADE;
            if (s.PatternName == "AB=CD" && !V69AbcdStandaloneEligible(s))
            {
                DetectorTruth(s.PatternName, "V69_ABCD_CONFLUENCE_ONLY");
                return HarmonicRoute.NO_TRADE;
            }
            bool recoveryConflictCandidate = EnableV69PositiveThroughputExpansion && V69IsSupplyRecoveryFamily(s.PatternName) &&
            ...
            if (conflict == MtfConflict.CONFLICT && s.PatternName != "Shark" && !recoveryConflictCandidate)
                return HarmonicRoute.NO_TRADE;
            if (r.AtrRatio < (EnableV69PositiveThroughputExpansion ? .34 : .45) || r.AtrRatio > c.MaxAtrRatio + (EnableV69PositiveThroughputExpansion ? .15 : 0))
                return HarmonicRoute.NO_TRADE;
            ...
            if (EnableV69EvidenceRouteGuard)
            {
                ...
                if (EnableV69PositiveThroughputExpansion && s.PatternName == "Cypher") exhaustOk = false;
                if (EnableV69PositiveThroughputExpansion && s.PatternName == "Shark") exhaustOk = false;
            }

            if (EnableV69PositiveThroughputExpansion && V69IsSupplyRecoveryFamily(s.PatternName))
            {
                HarmonicRoute recovered = V69RecoveryRoute(s, conflict, r, c, trendOk, exhaustOk, transitionOk);
            ...
            if (EnableV69ControlledExpansion)
            {
        ...
        private bool UpdateV69FamilyContractEvidence(int i, CandidateRecord c, out double score)
        {
            score = 0;
            ...
            if (EnableV69PositiveThroughputExpansion)
                return V69StageAwareFamilyConfirmation(c, s, directional, reclaim, bos, rejection, sweep,
            ...
            switch (s.PatternName)
            {
                ...
                case "AB=CD":
                    if (!V69AbcdStandaloneEligible(s)) return false;
                    score = (c.FamilyDeceleration ? .20 : 0) + (c.FamilyFailedExtension ? .25 : 0) + (c.FamilyDirectional ? .20 : 0) + (c.FamilyBos ? .25 : 0) + .10 * V69AbcdConfluenceScore(s);
                    return c.FamilyDeceleration && c.FamilyFailedExtension && c.FamilyDirectional && c.FamilyBos && score >= .85;
            }
            return false;
        }

        private bool V69FamilyRouteEvidencePass(int i, CandidateRecord c)
        {
            if (c == null || c.Signal == null) return false;
            if (!V69AbcdStandaloneEligible(c.Signal)) return false;
            var fc = V69Contract(c.Signal.PatternName);
            if (fc == null) return RouteSpecificM1EvidencePass(i, c.Signal, c.Route);
            if (EnableV69PositiveThroughputExpansion) return c.NativeStage >= 4;
        ...
        private double V69FamilyNativeCandidateRank(CandidateRecord c)
        {
            var fc = c == null || c.Signal == null ? null : V69Contract(c.Signal.PatternName);
            ...
            double score = .22 * c.Signal.GeometryQuality + .16 * c.Signal.PrzConfluence + .16 * c.Signal.Confidence +
                           .14 * confirmation + .12 * routeFit + .10 * rr + .10 * V69AbcdConfluenceScore(c.Signal) +
                           fc.DetectorPriorityBoost - agePenalty;
            if (c.Signal.PatternName == "AB=CD" && c.Signal.HarmonicSubtype == "ABCD_LEGACY_BROAD") score -= .35;
            return score;
        }

        private void ApplyV69FamilyGridContracts()
        {
            foreach (var p in _profiles)
            {
                var c = V69Contract(p.Name);
                if (c == null) continue;
                p.GridEnabled = true;
        ...
        private bool V69IsSupplyRecoveryFamily(string pattern)
        {
            return pattern == "Gartley" || pattern == "Bat" || pattern == "Alt Bat" || pattern == "Butterfly" ||
                   pattern == "Crab" || pattern == "Deep Crab" || pattern == "Deep Gartley" || pattern == "5-0";
        }

        private double V69FamilyIdentityGeometryScore(PatternSignal s)
        {
        ...
        private bool V69RecoveryQualityPass(PatternSignal s, FamilyTradeContract c)
        {
            ...
            double nativeGeometry=V69FamilyIdentityGeometryScore(s);
        ...
        private HarmonicRoute V69RecoveryRoute(PatternSignal s, MtfConflict conflict, RegimeSnapshot r, FamilyTradeContract c,
            bool trendOk, bool exhaustOk, bool transitionOk)
        {
            ...
            double familyRegime=V69FamilyRegimeScore(s, conflict, r);
        ...
        private bool V69StageAwareFamilyConfirmation(CandidateRecord c, PatternSignal s,
        ...
        private bool V69RecoveryConfirmationPass(CandidateRecord c, out double score)
        {
            score=0;
            if(c==null || c.Signal==null) return false;
            int core=0;
        ...
        private double V69ActivationDriftRisk(string pattern, HarmonicRoute route)
        {
        ...
        private void BuildV69FamilyGridAtlas()
        {
            ...
            AddV69Grid("Gartley","RISK",1.00,90,.50,.40,.98,2,
                new[]{0.0,.236,.382,.618},new[]{.40,.30,.20,.10},new double[0],new double[0],new[]{0.0,.236,.382},new[]{.50,.30,.20});
            AddV69Grid("Bat","RISK",.95,105,.50,.40,.98,2,
                new[]{0.0,.236,.382,.618},new[]{.40,.30,.20,.10},new double[0],new double[0],new[]{0.0,.236,.382},new[]{.50,.30,.20});
            AddV69Grid("Alt Bat","CD",.30,105,.42,.48,.84,2,
                new double[0],new double[0],new[]{0.0,.08,.18},new[]{.55,.30,.15},new[]{0.0,.08,.18},new[]{.55,.30,.15});
            AddV69Grid("Butterfly","CD",.28,105,.42,.48,.84,2,
                new double[0],new double[0],new[]{0.0,.08,.18,.28},new[]{.50,.28,.14,.08},new[]{0.0,.08,.18},new[]{.55,.30,.15});
            AddV69Grid("Crab","CD",.22,90,.38,.46,.78,2,
                new double[0],new double[0],new[]{0.0,.06,.12},new[]{.60,.28,.12},new[]{0.0,.06,.12},new[]{.60,.28,.12});
            AddV69Grid("Deep Crab","CD",.20,90,.35,.46,.78,2,
                new double[0],new double[0],new[]{0.0,.05,.10},new[]{.65,.25,.10},new[]{0.0,.05,.10},new[]{.65,.25,.10});
            AddV69Grid("Deep Gartley","RISK",.95,105,.50,.40,.98,2,
                new[]{0.0,.236,.382},new[]{.50,.30,.20},new double[0],new double[0],new[]{0.0,.236,.382},new[]{.50,.30,.20});
            AddV69Grid("Rat","D_STOP",.78,90,.50,.36,.90,2,
                new[]{0.0,.18,.382},new[]{.45,.35,.20},new double[0],new double[0],new[]{0.0,.18,.30},new[]{.50,.30,.20});
            AddV69Grid("Cypher","RISK",.90,90,.50,.35,.80,2,
                new[]{0.0,.236,.382},new[]{.50,.33,.17},new[]{0.0,.236},new[]{.65,.35},new[]{0.0,.236,.382},new[]{.50,.33,.17});
            AddV69Grid("Shark","RISK",.85,75,.42,.32,.88,2,
                new[]{0.0,.236},new[]{.65,.35},new[]{0.0,.236},new[]{.65,.35},new[]{0.0,.236},new[]{.65,.35});
            AddV69Grid("5-0","RISK",.90,75,.42,.35,.70,2,
                new double[0],new double[0],new double[0],new double[0],new[]{0.0,.236},new[]{.70,.30});
            AddV69Grid("AB=CD","RISK",1.00,90,.50,.28,.75,2,
                new[]{0.0,.236,.382,.618},new[]{.428571,.285714,.142857,.142858},new double[0],new double[0],new[]{0.0,.236,.382},new[]{.50,.30,.20});
        }

        private void AddV69Grid(string name,string geometryBasis,double scale,int ttl,double cancelMfe,
        ...
        private FamilyGridModule V69GridModule(string pattern)
        {
            FamilyGridModule m;
            return pattern!=null && _familyGridAtlas.TryGetValue(pattern,out m)?m:null;
        }

        private bool V69RouteGrid(FamilyGridModule m,HarmonicRoute route,out double[] fractions,out double[] weights)
        {
            fractions=new double[0];weights=new double[0];
            if(m==null)return false;
            if(route==HarmonicRoute.TREND_ALIGNED_REVERSAL){fractions=m.TrendFractions;weights=m.TrendWeights;}
            else if(route==HarmonicRoute.EXHAUSTION_REVERSAL){fractions=m.ExhaustionFractions;weights=m.ExhaustionWeights;}
            else if(route==HarmonicRoute.TRANSITION_REVERSAL){fractions=m.TransitionFractions;weights=m.TransitionWeights;}
            return fractions!=null && weights!=null && fractions.Length>0 && fractions.Length==weights.Length;
        }

        private double V69GridExecutionUnit(CandidateRecord c,FamilyGridModule m)
        {
        ...
        private bool TryBuildV69FamilyGridAtlasPlan(CandidateRecord c)
        {
            if(c==null||c.Signal==null)return GridPlanReject(c,"V69_NULL_SIGNAL");
            var m=V69GridModule(c.Signal.PatternName);
            if(m==null)return GridPlanReject(c,"V69_NO_FAMILY_GRID_MODULE");
            double[] fractions,weights;
            if(!V69RouteGrid(m,c.Route,out fractions,out weights))return GridPlanReject(c,"V69_ROUTE_NOT_NATIVE_FOR_FAMILY");
            if(c.Signal.PatternName=="AB=CD"&&!V69AbcdStandaloneEligible(c.Signal))return GridPlanReject(c,"V69_ABCD_CONFLUENCE_ONLY");
            if(!_initialCapitalEligible)return GridPlanReject(c,"INITIAL_CAPITAL");

            double anchor=c.Signal.Direction==TradeDirection.Buy?_symbol.Ask:_symbol.Bid;
            double stop=c.Signal.StructuralInvalidation;
            double riskDistance=c.Signal.Direction==TradeDirection.Buy?anchor-stop:stop-anchor;
            if(riskDistance<=PipsToPrice(MinStopLossPips))return GridPlanReject(c,"STOP_DISTANCE");
            double unit=V69GridExecutionUnit(c,m);
            if(!double.IsFinite(unit)||unit<=_symbol.TickSize)return GridPlanReject(c,"V69_INVALID_GRID_UNIT");

            var plan=new FibonacciGridPlan
            {
                CandidateId=c.CandidateId,Pattern=c.Signal.PatternName,Direction=c.Signal.Direction,Route=c.Route,
                EntryAnchor=anchor,StructuralStop=stop,GridDistance=unit,BasketRiskAmount=Account.Equity*BasketRiskPercent/100.0,
                CreatedUtc=Server.Time.ToUniversalTime(),ExpirationUtc=MinDate(c.ExpiryUtc,Server.Time.ToUniversalTime().AddMinutes(m.PendingTtlMinutes)),
                MicroCapitalMode=AdaptiveCapitalMode&&Account.Equity<=MicroCapitalThreshold,
            ...
            for(int leg=0;leg<maxLegs;leg++)
            {
                ...
                plan.Legs.Add(new FibonacciGridLeg{Index=leg,Fraction=f,PlannedPrice=price,RiskWeight=w,RiskBudget=plan.BasketRiskAmount*w,
                    MinBrokerRisk=minRisk,Volume=0,PlannedRisk=0,ModeledCost=0,Physical=false,State=GridLegState.VIRTUAL_ONLY});
            }

            if(plan.Legs.Count==0||plan.Legs[0].Index!=0)return GridPlanReject(c,"V69_NO_LEGAL_L0");
            plan.LogicalLegCount=plan.Legs.Count;
            if(plan.LogicalLegCount<m.MinLogicalLegs && Account.Equity>MicroCapitalThreshold)
                return GridPlanReject(c,"V69_FAMILY_GRID_DEPTH");
            double ws=plan.Legs.Sum(x=>x.RiskWeight);
            if(ws<=0)return GridPlanReject(c,"V69_ZERO_WEIGHT");
            foreach(var l in plan.Legs)l.RiskWeight/=ws;
            if(!ConfigureCapitalExecution(plan)){_capitalRejectedBaskets++;return GridPlanReject(c,"CAPITAL_EXECUTION");}
            var physical=plan.Legs.Where(x=>x.Physical&&x.Volume>0).ToList();
            if(physical.Count==0||physical[0].Index!=0)return GridPlanReject(c,"NO_PHYSICAL_L0");
            double tv=physical.Sum(x=>x.Volume);
            plan.ExpectedWeightedEntry=physical.Sum(x=>x.PlannedPrice*x.Volume)/tv;
            double vd=plan.Legs.Sum(x=>x.RiskWeight/Math.Max(PriceToPips(Math.Abs(x.PlannedPrice-stop)),1e-9));
            plan.VirtualWeightedEntry=vd>0?plan.Legs.Sum(x=>x.PlannedPrice*(x.RiskWeight/Math.Max(PriceToPips(Math.Abs(x.PlannedPrice-stop)),1e-9)))/vd:plan.ExpectedWeightedEntry;
            double target,netRr;
            if(!SelectCanonicalBasketTarget(c.Signal,plan.ExpectedWeightedEntry,plan.StructuralStop,out target,out netRr))return GridPlanReject(c,"TARGET_RR");
            if(netRr<MinimumNetRR)return GridPlanReject(c,"V69_NET_RR");
            plan.CanonicalTarget=target;plan.ExpectedNetRR=netRr;c.GridPlan=plan;c.SelectedTarget=target;c.NetRR=netRr;
            Print("[V69-FAMILY-GRID-PLAN] cid={0} pattern={1} route={2} module={3} unit={4} logicalLegs={5} physicalDepth={6} weighted={7} stop={8} target={9} budget={10:F2} worst={11:F2} rr={12:F3}",
                c.CandidateId,c.Signal.PatternName,c.Route,plan.GridModuleName,unit,plan.LogicalLegCount,plan.PhysicalDepth,
                plan.ExpectedWeightedEntry,stop,target,plan.BasketRiskAmount,plan.WorstCaseRisk,plan.ExpectedNetRR);
            PrintCapitalCompatibilityMatrix(plan);
            return true;
        }

        private bool V69FamilySurvivalPass(CandidateRecord c)
        {
            if(c==null||c.Signal==null||c.Regime==null||c.GridPlan==null) return false;
            var s=c.Signal; var r=c.Regime;
            if(c.NetRR<MinimumNetRR) return false;
            double q=VClamp(.18*s.GeometryQuality+.18*s.PrzConfluence+.16*s.Confidence+
                            .22*c.ConfirmationScore+.13*s.TimeSymmetry+.13*s.PivotQuality);
            bool clean=c.Conflict!=MtfConflict.CONFLICT;
            bool trend=c.Route==HarmonicRoute.TREND_ALIGNED_REVERSAL;
            bool exhaust=c.Route==HarmonicRoute.EXHAUSTION_REVERSAL;
            bool transition=c.Route==HarmonicRoute.TRANSITION_REVERSAL;
            switch(s.PatternName)
            {
                case "Gartley": return trend&&clean&&q>=.58&&c.RegimeScore>=.58&&r.Efficiency>=.10;
                case "Bat": return (trend||transition)&&clean&&q>=.56&&c.RegimeScore>=.58&&r.AtrRatio>=.38&&r.AtrRatio<=1.65;
                case "Alt Bat": return (exhaust||transition)&&q>=.54&&c.RegimeScore>=.56&&r.ExtensionAtr>=.65;
                case "Butterfly": return (exhaust||transition)&&q>=.55&&c.RegimeScore>=.58&&r.ExtensionAtr>=.65;
                case "Crab": return (exhaust||transition)&&q>=.54&&c.RegimeScore>=.58&&r.ExtensionAtr>=.72;
                case "Deep Crab": return (exhaust||transition)&&q>=.55&&c.RegimeScore>=.58&&r.ExtensionAtr>=.72;
                case "Deep Gartley": return (trend||transition)&&clean&&q>=.56&&c.RegimeScore>=.58;
                case "Rat": return trend&&clean&&q>=.60&&c.RegimeScore>=.60;
                case "Cypher": return trend&&clean&&q>=.64&&c.RegimeScore>=.64;
                case "Shark": return trend&&clean&&q>=.58&&c.RegimeScore>=.58;
                case "5-0": return transition&&q>=.70&&c.ConfirmationScore>=.75&&c.RegimeScore>=.62&&
                                   r.AtrPercentile>=.30&&r.AtrPercentile<=.70&&r.Efficiency>=.06&&r.Efficiency<=.20;
                case "AB=CD": return s.HarmonicSubtype=="ABCD_EXACT"&&trend&&clean&&q>=.66&&
                                     c.ConfirmationScore>=.65&&c.RegimeScore>=.60;
                default: return false;
            }
        }

        private double V69FamilyOpportunityScore(CandidateRecord c,DateTime now)
        {
            if(c==null||c.Signal==null)return double.MinValue;
            double baseScore=EnableOpportunityDecayRanking?OpportunityScore(c,now):c.Rank;
            var m=V69GridModule(c.Signal.PatternName);
            if(m==null)return baseScore;
            double rr=VClamp(c.NetRR/4.0),conf=VClamp(c.ConfirmationScore);
            double geometry=VClamp((c.Signal.GeometryQuality+c.Signal.PrzConfluence+c.Signal.Confidence)/3.0);
            double feasible=c.GridPlan!=null&&c.GridPlan.PhysicalDepth>0?Math.Min(1.0,c.GridPlan.PhysicalDepth/3.0):0;
            return baseScore+.16*rr+.12*conf+.10*geometry+.06*feasible;
        }
        ...
        private bool V69FamilyActivationTouch(int i, CandidateRecord c)
        {
            if (i < 0 || i >= _m1Bars.Count || c == null || c.Signal == null) return false;
            var s=c.Signal;
            if (BarTouchesPrz(i,s)) return true;
            if (!V69IsSupplyRecoveryFamily(s.PatternName)) return false;
            ...
            if(touch)
                Print("[V69-FAMILY-ACTIVATION-CORRIDOR] cid={0} pattern={1} tol={2:F5} przLow={3} przHigh={4}",
        ...
        {
            if (EnableV69FamilyTradeContracts) return V69FamilyNativeCandidateRank(c);
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V69-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        {
            _candidateSeq++;
            return "V69-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private void Ledger(CandidateRecord c, CandidateState state, string reason)
        {
            ...
            Print("[V69-EVENT] cid={0} setup={1} pattern={2} subtype={3} scale={4} tf={5} dir={6} state={7} route={8} conflict={9} waitMin={10:F2} reason={11}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V69-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V69-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
    ...
    public sealed class FamilyGridModule
    {
        public string Name;
        public string GeometryBasis;
    ...
    public sealed class FibonacciGridPlan
    {
        ...
        public double CancelPendingAtMfeR;
        public string GridModuleName;
