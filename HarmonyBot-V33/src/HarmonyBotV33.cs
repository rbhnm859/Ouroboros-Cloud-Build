...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV33 : Robot
    {
        private const string Version = "HarmonyBot V33.0 — Fibonacci Harmonic Grid Portfolio Execution Engine";
        private const string BotPrefix = "HB33";
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V33-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V33-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V33-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V33-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            Print("[V33-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m1Bars), _profiles.Count);
            Print("[V33-TIMEFRAME-AUDIT] primaryPattern=M15 execution=M1 macro=H4 intermediate=H1 allCompletedBars=true");
            Print("[V33-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
        }
        ...
        protected override void OnStop()
        {
            ...
            {
                var x = kv.Value;
                Print("[V33-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} armed={6} basketPlanned={7} leg0={8} leg1={9} leg2={10} leg3={11} basketClosed={12} executed={13} expired={14} rejected={15} invalidated={16}",
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            Print("[V33-GRID-PLAN] cid={0} pattern={1} route={2} legs={3} anchor={4} weighted={5} stop={6} target={7} budget={8:F2} worst={9:F2} netRR={10:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V33-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} volume={5} risk={6:F2} cost={7:F2}",
        ...
        private void ExecuteFibonacciGridPlan(CandidateRecord c)
        {
            ...
            foreach (var leg in plan.Legs.Where(x => x.Index > 0))
            {
                if (!DeeperLegThesisEligible(basket, leg))
                {
                    ...
                    BasketEvent(basket, "DEEPER_LEG_THESIS_REJECT_L" + leg.Index);
                    continue;
                }
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V33-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V33-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            double entryImprovementPips = basket.AverageEntry > 0
                ...
                : 0;
            Print("[V33-BASKET-CLOSED] basket={0} cid={1} pattern={2} route={3} dir={4} plannedLegs={5} filledLegs={6} anchor={7} avgEntry={8} entryImprovePips={9:F3} stop={10} target={11} initialRisk={12:F2} worstRisk={13:F2} mfeR={14:F3} maeR={15:F3} realizedR={16:F3} net={17:F2} reason={18}",
        ...
        {
            _basketSeq++;
            return "HB33-" + SymbolName + "-" + Server.Time.ToUniversalTime().ToString("yyyyMMdd", CultureInfo.InvariantCulture) + "-" +
        ...
        {
            Print("[V33-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V33-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        {
            _candidateSeq++;
            return "V32-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        {
            Print("[V33-EVENT] cid={0} pattern={1} tf={2} dir={3} state={4} route={5} conflict={6} reason={7}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V33-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V33-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
