...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V44 — Cross-Validated Marginal Alpha Admission & Pattern Portfolio Engine";
        ...
        [Parameter("Pattern Alpha Firewall", DefaultValue = true)]
        public bool EnablePatternAlphaFirewall { get; set; }
        ...
        private void AdvanceCandidateLifecycleOnBars(Bars bars, int i, DateTime utc, string executionTf, bool allowArm)
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                // V44 alpha firewall: all families remain detected, validated and routed.
                // AB=CD is shadow-only because V42 DEV attribution showed negative marginal alpha.
                if (EnablePatternAlphaFirewall && string.Equals(c.Signal.PatternName, "AB=CD", StringComparison.Ordinal))
                {
                    AddCounterfactualShadow(c, "PATTERN_ALPHA_FIREWALL_ABCD", utc);
                    Reject(c, "PATTERN_ALPHA_FIREWALL_ABCD");
                    continue;
                }
        ...
        private void TryScheduleAndExecute()
        {
            ...
            if (EnableOpportunityAuctionWindow && OpportunityAuctionMinutes > 0)
            {
        ...
        private List<PatternSignal> DetectPatternCandidates(Bars bars, int endIndex, int depth, int lookback, int maxCandidates, string timeframe)
        {
            ...
            if (EnableCanonicalSetupDedupe)
                ordered = ordered.GroupBy(BuildSetupGeometryKey)
                    .Select(g => g.OrderByDescending(s => s.Confidence).ThenByDescending(s => s.GeometryQuality).First())
        ...
        private double OpportunityCostRank(CandidateRecord c)
        {
            ...
            return edge * Math.Max(.10, readiness) * urgency * Math.Max(.10, structural) * routePrior *
