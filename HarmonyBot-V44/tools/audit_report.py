...
def last_counts(pattern,names):
 q=re.findall(pattern,t)
 z={k:0 for k in names}
 if q:
...
ac=last_counts(r"\[V44-ATTRIBUTION-SUMMARY\].*?observed=(\d+)\s+legacyRankFirstReleased=(\d+)\s+followThroughObserved=(\d+)\s+thesisFailureExits=(\d+)",
 ["attribution_observed","legacy_rank_first_released","follow_through_observed","thesis_failure_exits"])
mc=last_counts(r"\[V44-MARGINAL-ALPHA-SUMMARY\].*?coreAdmissions=(\d+)\s+rescueEvaluated=(\d+)\s+rescueAdmissions=(\d+)\s+rescueRejected=(\d+)",
 ["core_admissions","rescue_evaluated","rescue_admissions","rescue_rejected"])
xc=last_counts(r"\[V44-CROSS-REGIME-SUMMARY\].*?rejected=(\d+)\s+opportunityLossObserved=(\d+)\s+hypothesisResolutions=(\d+)",
 ["cross_regime_rejected","opportunity_loss_observed","hypothesis_resolutions"])
tc=last_counts(r"\[V44-THESIS-SUMMARY\].*?thesisRejected=(\d+)\s+trendPersistenceRejected=(\d+)\s+gridAttributedFailures=(\d+)\s+gridL0Recoveries=(\d+)",
 ["thesis_rejected","trend_persistence_rejected","grid_attributed_failures","grid_l0_recoveries"])
...
# Correct V44 event parser: the V42 analyzer accidentally searched for literal backslashes.
...
thesis_outcomes=[]
trx=re.compile(r"\[V44-THESIS-OUTCOME\]\s+cid=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+thesis=(\S+)\s+relation=(\S+)\s+regimeClass=(\S+)\s+persistenceHazard=([-0-9.]+)\s+reversalPlausibility=([-0-9.]+)\s+resumption=([-0-9.]+)\s+temporal=([-0-9.]+)\s+net=([-0-9.]+)")
for q in trx.finditer(t):
 thesis_outcomes.append({"cid":q.group(1),"pattern":q.group(2),"route":q.group(3),"thesis":q.group(4),"relation":q.group(5),
 "regime_class":q.group(6),"persistence_hazard":float(q.group(7)),"reversal_plausibility":float(q.group(8)),
 "resumption":float(q.group(9)),"temporal":float(q.group(10)),"net":float(q.group(11))})

grid_failures={}
gfr=re.compile(r"\[V44-GRID-FAIL\]\s+cid=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+thesis=(\S+)\s+reason=(\S+)")
for q in gfr.finditer(t):
 k=q.group(2)+"|"+q.group(3)+"|"+q.group(4)+"|"+q.group(5)
 grid_failures[k]=grid_failures.get(k,0)+1

grid_recoveries=[]
grr=re.compile(r"\[V44-GRID-RECOVERY\]\s+candidate=(\S+)\s+mode=(\S+)\s+volume=([-0-9.]+)\s+risk=([-0-9.]+)\s+margin=([-0-9.]+)")
for q in grr.finditer(t):
 grid_recoveries.append({"cid":q.group(1),"mode":q.group(2),"volume":float(q.group(3)),"risk":float(q.group(4)),"margin":float(q.group(5))})
...
"broker_profile_present":"[V44-BROKER-PROFILE]" in t,"evidence_events":len(re.findall(r"\[V44-EVIDENCE\]",t)),
...
"thesis_outcomes":thesis_outcomes,"grid_failures":grid_failures,"grid_recoveries":grid_recoveries,
**fc,**ec,**ac,**mc,**xc,**tc,**c}
