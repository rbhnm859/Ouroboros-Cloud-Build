...
freq=re.findall(r"\[V44-FREQUENCY-SUMMARY\].*?schedulerDeferred=(\d+)\s+schedulerRecoveredExecutions=(\d+)\s+structuredRecallAdmitted=(\d+)\s+activeDeferred=(\d+).*?routeDeferred=(\d+)\s+routeRecovered=(\d+)",t)
...
ex=re.findall(r"\[V44-EXECUTION-SUMMARY\].*?evidenceExecutable=(\d+)\s+survivalWaits=(\d+)\s+opportunityArbitrations=(\d+)\s+riskRenormalizations=(\d+)\s+riskRejects=(\d+)\s+shadowTargets=(\d+)\s+shadowStops=(\d+)\s+shadowUnresolved=(\d+)",t)
...
attr=re.findall(r"\[V44-ATTRIBUTION-SUMMARY\].*?observed=(\d+)\s+legacyRankFirstReleased=(\d+)\s+followThroughObserved=(\d+)\s+thesisFailureExits=(\d+)",t)
...
mar=re.findall(r"\[V44-MARGINAL-ALPHA-SUMMARY\].*?coreAdmissions=(\d+)\s+rescueEvaluated=(\d+)\s+rescueAdmissions=(\d+)\s+rescueRejected=(\d+)",t)
