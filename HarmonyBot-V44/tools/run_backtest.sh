...
THESISROUTING="${THESISROUTING:-true}"; PERSISTENCE="${PERSISTENCE:-true}"; RESUMPTION="${RESUMPTION:-true}"
GRIDATTR="${GRIDATTR:-true}"; GRIDRECOVERY="${GRIDRECOVERY:-true}"; CONTMIN="${CONTMIN:-0.60}"; COUNTERMIN="${COUNTERMIN:-0.60}"
ALGO="${ALGO:-seal/algo/HarmonyBot_V44_Cross_Regime_Positive_Alpha_Portfolio_RC.algo}"
...
docker run --name "$CNAME" -v "$PWD/seal:/work" "$IMAGE" backtest "/work/${ALGO#seal/}" \
 ...
 --EnablePatternHypothesisSet="$HYPOTHESIS" --EnableStructuralContextVector="$STRUCTCTX" \
 --EnablePatternTemporalStateMachine="$TEMPORAL" --EnableCrossRegimeAlphaAdmission="$CROSSALPHA" \
 --EnableRobustAlphaDensity="$DENSITY" --EnableEventDrivenOpportunityAuction="$EVENTAUCTION" --EnableOpportunityLossLedger="$OPPLOSS" \
 --ConditionalAlphaFloor="$ALPHAFLOOR" --AbcdTrendAdmissionFloor="$ABCDTRENDFLOOR" \
 --EnableThesisConsistentRouting="$THESISROUTING" --EnableTrendPersistenceHazard="$PERSISTENCE" \
 --EnableContinuationResumptionProof="$RESUMPTION" --ContinuationMinResumptionScore="$CONTMIN" --CountertrendMinPlausibility="$COUNTERMIN" \
 --EnableGridFailureAttribution="$GRIDATTR" --EnableGridL0PhysicalRecovery="$GRIDRECOVERY" \
