...
s=pathlib.Path(sys.argv[1] if len(sys.argv)>1 else "HarmonyBot-V68/src/HarmonyBotV68.cs").read_text(errors="ignore")
...
out={"version":"HarmonyBot V68","families":families,"checks":checks,"pass":all(checks.values())}
pathlib.Path("V68_DETECTOR_CONTRACT_TEST.json").write_text(json.dumps(out,indent=2))
