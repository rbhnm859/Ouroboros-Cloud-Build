...
rx=re.compile(r"\[V68-BASKET-CLOSED\].*?cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+subtype=(\S+)\s+route=(\S+)\s+dir=(\S+).*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)\s+reason=(\S+)")
...
fn_rx=re.findall(r"\[V68-FAMILY-NATIVE-SUMMARY\].*?parentAbcdSuppressed=(\d+)\s+familyRouteRejected=(\d+)\s+nativeConfirmed=(\d+)\s+nativeExpired=(\d+)\s+gridWeightNormalized=(\d+)",t)
owned_rx=re.findall(r"\[V68-FAMILY-OWNED-SUMMARY\].*?familyOwnedRouted=(\d+)\s+jointQualityAdmitted=(\d+)\s+gridRiskRebalanced=(\d+)",t)
fn={"parent_abcd_suppressed":0,"family_route_rejected":0,"native_confirmed":0,"native_expired":0,"grid_weight_normalized":0}
prevent_rx=re.findall(r"\[V68-RISK-PREVENTION-SUMMARY\].*?preventedRiskRejections=(\d+)",t)
prevented_risk_rejections=int(prevent_rx[-1]) if prevent_rx else 0
if fn_rx:
    z=fn_rx[-1]
    for k,v in zip(fn.keys(),z): fn[k]=int(v)
owned={"family_owned_routed":0,"joint_quality_admitted":0,"grid_risk_rebalanced":0}
if owned_rx:
    z=owned_rx[-1]
    for k,v in zip(owned.keys(),z): owned[k]=int(v)

grid_rx=re.findall(r"\[V68-GRID-PLAN\].*?budget=([-0-9.]+)\s+worst=([-0-9.]+)\s+margin=([-0-9.]+)\s+netRR=([-0-9.]+)",t)
...
out={
 "version":"HarmonyBot V68","mode":a.mode,"window":a.window,"starting_balance":a.balance,"spread":a.spread,"data_snapshot_sha":a.data_sha,
 ...
 "engineering_clean":clean,"broker_profile_present":"[V68-BROKER-PROFILE]" in t,
 ...
 "grid_plans":len(grid),"v68_family_native":fn,"v68_family_owned":owned,"prevented_risk_rejections":prevented_risk_rejections,**c
}
