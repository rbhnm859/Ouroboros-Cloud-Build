...
MODE="${V68MODE:-V68_PRODUCT}"
BALANCE="${BALANCE:-10000}"
SPREAD="${SPREAD:-1}"
ALGO="${ALGO:-seal/algo/HarmonyBot_V68_V52_Family_Owned_Routing_Grid_Commercial_Breakthrough_RC.algo}"
IMAGE="${CTRADER_IMAGE:-ghcr.io/spotware/ctrader-console:5.9.11}"
BACKTEST_TIMEOUT_SECONDS="${BACKTEST_TIMEOUT_SECONDS:-2700}"
case "$MODE" in V68_PRODUCT|V68_V52_CONTROL) ;; *) echo "invalid V68 mode $MODE" >&2; exit 64;; esac
if [ "$MODE" = "V68_PRODUCT" ]; then
  V68_CONFIRM_BARS=8
else
  V68_CONFIRM_BARS=6
fi
