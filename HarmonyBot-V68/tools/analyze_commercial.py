...
def rows():
    ...
    for p in pathlib.Path(a.root).rglob("*.json"):
        ...
        if isinstance(d,dict) and "basket_outcomes" in d and d.get("version")=="HarmonyBot V68": out.append(d)
    return out
...
def agg(xs,years):
    ...
    utils=[float(x.get("avg_grid_risk_utilization",0) or 0) for x in xs if float(x.get("avg_grid_risk_utilization",0) or 0)>0]
    minrr=[float(x.get("min_grid_net_rr",0) or 0) for x in xs if float(x.get("min_grid_net_rr",0) or 0)>0]
    return {"baskets":len(b),"frequency":len(b)/years if years else 0,"net":sum(v),
            ...
            "executing_family_count":len(famsum),
            "avg_grid_risk_utilization":statistics.mean(utils) if utils else 0,
            "min_grid_net_rr":min(minrr) if minrr else 0}

R=rows(); o={"version":"HarmonyBot V68","phase":a.phase,"source_rows":len(R)}
if a.phase=="dev":
    by={(x["mode"],x["window"]):x for x in R}
    wins=["L1","L2","L3","D1","D2","D3"]
    miss=[f"{m}:{w}" for m in ["V68_V52_CONTROL","V68_PRODUCT"] for w in wins if (m,w) not in by]
    if miss: raise SystemExit("missing evidence "+",".join(miss))
    CL=agg([by[("V68_V52_CONTROL",w)] for w in ["L1","L2","L3"]],1.5)
    PL=agg([by[("V68_PRODUCT",w)] for w in ["L1","L2","L3"]],1.5)
    PD=agg([by[("V68_PRODUCT",w)] for w in ["D1","D2","D3"]],1.5)
    control_ok=CL["baskets"]>0 and CL["clean"]
    throughput=PL["frequency"]>=80 and PL["frequency"]>=CL["frequency"]*.85
    legacy_quality=(PL["net"]>max(0,CL["net"]) and PL["pf"]>max(1.0,CL["pf"]) and PL["expectancy"]>max(0,CL["expectancy"]) and
                    PL["all_positive"] and PL["clean"])
    diversity_legacy=(PL["abcd_share"]<=.35 and PL["dominant_family_share"]<=.45 and PL["positive_family_count"]>=3 and PL["executing_family_count"]>=5)
    diversity_dev=(PD["abcd_share"]<=.35 and PD["dominant_family_share"]<=.45 and PD["positive_family_count"]>=3 and PD["executing_family_count"]>=5)
    ...
    grid_ok=(PL["avg_grid_risk_utilization"]>=.90 and PL["avg_grid_risk_utilization"]<=1.000001 and
             PD["avg_grid_risk_utilization"]>=.90 and PD["avg_grid_risk_utilization"]<=1.000001 and
             PL["min_grid_net_rr"]>=2 and PD["min_grid_net_rr"]>=2)
    candidate=bool(control_ok and throughput and legacy_quality and diversity_legacy and diversity_dev and current_commercial and grid_ok)
    o.update({"control_legacy":CL,"product_legacy":PL,"product_current_dev":PD,
      "control_gate":control_ok,"throughput_gate":throughput,"legacy_quality_gate":legacy_quality,
      "family_diversification_legacy":diversity_legacy,"family_diversification_dev":diversity_dev,
      "current_commercial_gate":current_commercial,"grid_efficiency_risk_gate":grid_ok,
      "double_v52_net_stretch":PD["net"]>=5804.54,"candidate":candidate,
      "status":"VALIDATION_CANDIDATE" if candidate else "HOLD_WITH_EVIDENCE",
      "windows":{"control":{w:by[("V68_V52_CONTROL",w)] for w in wins},"product":{w:by[("V68_PRODUCT",w)] for w in wins}}})
    ...
    diversity=(A["abcd_share"]<=.35 and A["dominant_family_share"]<=.45 and A["positive_family_count"]>=3)
    candidate=bool(len(base)==2 and A["all_positive"] and A["pf"]>=2 and A["expectancy"]>=20 and A["win_rate"]>=.5 and A["max_dd_pct"]<=6 and A["clean"] and stress and diversity)
    o.update({"base":A,"stress_all_positive_clean":stress,"diversity":diversity,"candidate":candidate,"status":"CAPITAL_CANDIDATE" if candidate else "HOLD_VALIDATION"})
