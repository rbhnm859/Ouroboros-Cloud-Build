...
namespace cAlgo.Robots
{
    ...
    {
        private const string Version = "HarmonyBot V68 — V52 Family-Owned Routing Grid Commercial Breakthrough";
        ...
        [Parameter("V68 Mode", DefaultValue = "V68_PRODUCT")]
        public string V68Variant { get; set; }

        [Parameter("V68 Family Native Confirmation Bars", DefaultValue = 8, MinValue = 4, MaxValue = 10)]
        public int V68FamilyNativeConfirmationBars { get; set; }

        [Parameter("V68 Suppress Parent AB=CD", DefaultValue = true)]
        public bool V68SuppressParentAbcd { get; set; }

        [Parameter("V68 Normalize Grid Risk", DefaultValue = true)]
        public bool V68NormalizeGridRisk { get; set; }

        [Parameter("V68 Family-Owned Routing", DefaultValue = true)]
        public bool V68FamilyOwnedRouting { get; set; }

        [Parameter("V68 Named Family Joint Quality", DefaultValue = true)]
        public bool V68NamedFamilyJointQuality { get; set; }

        [Parameter("V68 Rebalance Grid Risk Budget", DefaultValue = true)]
        public bool V68RebalanceGridRiskBudget { get; set; }
        ...
        private int _v68ParentAbcdSuppressed;
        private int _v68FamilyRouteRejected;
        private int _v68FamilyNativeConfirmed;
        private int _v68FamilyNativeExpired;
        private int _v68GridWeightNormalized;
        private int _v68PreventedRiskRejections;
        private int _v68FamilyOwnedRouted;
        private int _v68NamedFamilyJointQualityAdmitted;
        private int _v68GridRiskRebalanced;
        ...
        protected override void OnStart()
        {
            if (!V68ModeValid())
            {
                Print("[V68-FATAL] INVALID_MODE {0}", V68Variant);
                Stop();
                return;
            }
            ...
            Print("[V68-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V52_FAMILY_IDENTITY_THROUGHPUT",
            ...
            Print("[V68-PRODUCT-CONFIG] mode={0} familyNativeBars={1} parentAbcdSuppression={2} normalizeGridRisk={3} abcdRole=COMPLETION_PRIMITIVE_FALLBACK",
                V68Variant, V68FamilyNativeConfirmationBars, V68SuppressParentAbcd, V68NormalizeGridRisk);
            Print("[V68-FAMILY-OWNED-CONFIG] familyOwnedRouting={0} namedFamilyJointQuality={1} rebalanceGridRisk={2} standaloneIndicatorAlpha=false",
                V68FamilyOwnedRouting, V68NamedFamilyJointQuality, V68RebalanceGridRiskBudget);
        }
        ...
        protected override void OnStop()
        {
            ...
            Print("[V68-FAMILY-NATIVE-SUMMARY] parentAbcdSuppressed={0} familyRouteRejected={1} nativeConfirmed={2} nativeExpired={3} gridWeightNormalized={4}",
                _v68ParentAbcdSuppressed, _v68FamilyRouteRejected, _v68FamilyNativeConfirmed, _v68FamilyNativeExpired, _v68GridWeightNormalized);
            Print("[V68-FAMILY-OWNED-SUMMARY] familyOwnedRouted={0} jointQualityAdmitted={1} gridRiskRebalanced={2}",
                _v68FamilyOwnedRouted, _v68NamedFamilyJointQualityAdmitted, _v68GridRiskRebalanced);
            Print("[V68-RISK-PREVENTION-SUMMARY] preventedRiskRejections={0}", _v68PreventedRiskRejections);
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                bool patternQualityOk = V68ProductEnabled() && V68NamedFamilyJointQuality
                    ? V68PatternQualityEligible(signal)
                ...
                record.Route = V68ProductEnabled() && V68FamilyOwnedRouting
                    ? V68FamilyRouteSignal(signal, record.Conflict, regime)
                    : RouteSignal(signal, record.Conflict, regime);
                if (V68ProductEnabled() && V68FamilyOwnedRouting && record.Route != HarmonicRoute.NO_TRADE)
                {
                    _v68FamilyOwnedRouted++;
                    Event(record, "V68_FAMILY_OWNED_ROUTE_" + record.Route);
                }
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    if (V68ProductEnabled())
                    {
                        c.NativeM1BarsObserved++;
                        double nativeScore;
                        confirmationPass = V68FamilyNativeConfirmationPass(i, c, out nativeScore);
                        c.ConfirmationScore = Math.Max(c.ConfirmationScore, nativeScore);
                        if (confirmationPass)
                        {
                            _v68FamilyNativeConfirmed++;
                            _nativeTemporalPass++;
                            CountPipeline(c.Signal.PatternName).NativeTemporalPass++;
                            Event(c, "V68_FAMILY_NATIVE_CONFIRM_" + nativeScore.ToString("F2", CultureInfo.InvariantCulture));
                        }
                        else if (c.NativeM1BarsObserved >= Math.Max(4, V68FamilyNativeConfirmationBars))
                        {
                            _v68FamilyNativeExpired++;
                            Reject(c, "V68_FAMILY_NATIVE_CONFIRMATION_EXPIRED");
                            continue;
                        }
                    }
                    ...
                    if (!V68ProductEnabled() && !familyContractLane && !confirmationPass && EnablePatternNativeM1Expansion)
                    {
                        c.NativeM1BarsObserved++;
                        double nativeScore;
                    ...
                    if (!V68ProductEnabled() && !confirmationPass && EnableM1RescueLane)
                    {
        ...
        private void TryScheduleAndExecute()
        {
            ...
            if (EnableDiversityScheduler)
                armedQuery = armedQuery.GroupBy(c => string.IsNullOrWhiteSpace(c.SetupKey) ? c.CandidateId : c.SetupKey)
                    ...
                    {
                        var q = g.ToList();
                        if (V68ProductEnabled() && q.Any(x => x.Signal != null && x.Signal.PatternName != "AB=CD"))
                            q = q.Where(x => x.Signal != null && x.Signal.PatternName != "AB=CD").ToList();
                        return q.OrderByDescending(c => EnableOpportunityDecayRanking ? OpportunityScore(c, now) : c.Rank).First();
                    });
        ...
        private bool TryBuildFibonacciGridPlan(CandidateRecord c)
        {
            ...
            if (V68ProductEnabled() && V68NormalizeGridRisk)
            {
                double weightSum = plan.Legs.Sum(x => x.RiskWeight);
                if (weightSum <= 0) return GridPlanReject(c, "INVALID_GRID_WEIGHT_SUM");
                foreach (var l in plan.Legs)
                {
                    l.RiskWeight /= weightSum;
                    l.RiskBudget = plan.BasketRiskAmount * l.RiskWeight;
                }
                _v68GridWeightNormalized++;
            }
            ...
            if (V68ProductEnabled() && V68RebalanceGridRiskBudget && !plan.MicroCapitalMode)
                V68RebalancePhysicalGridRisk(plan);
        ...
        private void V68RebalancePhysicalGridRisk(FibonacciGridPlan plan)
        {
            if (plan == null || plan.Legs == null || plan.Legs.Count == 0) return;
            for (int round = 0; round < 3; round++)
            {
                double remaining = plan.BasketRiskAmount - plan.WorstCaseRisk;
                if (remaining <= Math.Max(.01, plan.BasketRiskAmount * .005)) break;
                var physical = plan.Legs.Where(x => x.Physical && x.Volume > 0).ToList();
                if (physical.Count == 0) break;
                double wsum = physical.Sum(x => Math.Max(0, x.RiskWeight));
                if (wsum <= 0) break;
                bool changed = false;

                foreach (var l in physical.OrderByDescending(x => x.RiskWeight).ThenBy(x => x.Index))
                {
                    double slPips = PriceToPips(Math.Abs(l.PlannedPrice - plan.StructuralStop));
                    if (slPips <= 0) continue;
                    double currentWorst = l.PlannedRisk + l.ModeledCost;
                    double share = Math.Max(0, l.RiskWeight) / wsum;
                    double targetWorst = currentWorst + remaining * share;
                    double newVolume = VolumeForRiskBudget(targetWorst, slPips);
                    if (newVolume <= l.Volume + 1e-9) continue;

                    double newRisk = newVolume * _symbol.PipValue * slPips;
                    double newCost = newVolume * _symbol.PipValue * ModeledCostPips();
                    double extraWorst = (newRisk + newCost) - currentWorst;
                    if (extraWorst <= 0 || plan.WorstCaseRisk + extraWorst > plan.BasketRiskAmount + 1e-8) continue;

                    double oldMargin = EstimatedMargin(plan.Direction, l.Volume);
                    double newMargin = EstimatedMargin(plan.Direction, newVolume);
                    double marginDelta = Math.Max(0, newMargin - oldMargin);
                    if (Account.FreeMargin - (plan.EstimatedPhysicalMargin + marginDelta) <
                        plan.BasketRiskAmount * MinFreeMarginRiskMultiple) continue;

                    l.Volume = newVolume;
                    l.PlannedRisk = newRisk;
                    l.ModeledCost = newCost;
                    plan.WorstCaseRisk += extraWorst;
                    plan.EstimatedPhysicalMargin += marginDelta;
                    changed = true;
                    _v68GridRiskRebalanced++;
                }
                if (!changed) break;
            }
        }
        ...
        private List<PatternSignal> DetectPatternCandidates(Bars bars, int endIndex, int depth, int lookback, int maxCandidates, string timeframe)
        {
            ...
            if (V68ProductEnabled() && V68SuppressParentAbcd)
            {
                var filtered = new List<PatternSignal>();
                foreach (var g in hypotheses.GroupBy(BuildSetupGeometryKey))
                {
                    bool hasParent = g.Any(x => x.PatternName != "AB=CD");
                    foreach (var x in g)
                    {
                        if (hasParent && x.PatternName == "AB=CD")
                        {
                            _v68ParentAbcdSuppressed++;
                            DetectorTruth("AB=CD", "PARENT_FAMILY_SUPPRESSED");
                            continue;
                        }
                        filtered.Add(x);
                    }
            ...
            var familyFair = hypotheses
                ...
                .SelectMany(g => g.OrderByDescending(x => x.Confidence).ThenByDescending(x => x.GeometryQuality)
                    .Take(V68FamilyDetectionQuotaFor(g.Key, quota)));

            var fair = V68ProductEnabled()
            ...
            double tolerance, int ttlMinutes, double stopFibBuffer, string anchorRule, string targetPolicy)
        {
            ...
            p.GridRiskWeights = V68ProductEnabled()
                ? V68FamilyGridWeights(name, maxLegs)
        ...
        private double[] V68FamilyGridWeights(string name, int maxLegs)
        {
        ...
        private bool V68ProductEnabled()
        {
            return string.Equals(V68Variant, "V68_PRODUCT", StringComparison.Ordinal);
        }

        private bool V68ControlEnabled()
        {
            return string.Equals(V68Variant, "V68_V52_CONTROL", StringComparison.Ordinal);
        }

        private bool V68ModeValid()
        {
            return V68ProductEnabled() || V68ControlEnabled();
        }

        private bool V68IsRetracementFamily(string p)
        {
            return p == "Gartley" || p == "Bat" || p == "Deep Gartley" || p == "Rat";
        }

        private bool V68IsExtensionFamily(string p)
        {
            return p == "Alt Bat" || p == "Butterfly" || p == "Crab" || p == "Deep Crab";
        }

        private bool V68FamilyRouteEligible(PatternSignal sig, HarmonicRoute route, MtfConflict conflict, RegimeSnapshot r)
        {
            if (!V68ProductEnabled() || sig == null || r == null) return true;
            if (V68FamilyOwnedRouting) return route != HarmonicRoute.NO_TRADE;
        ...
        private int V68FamilyDetectionQuotaFor(string pattern, int defaultQuota)
        {
            if (!V68ProductEnabled()) return defaultQuota;
            if (string.Equals(pattern, "AB=CD", StringComparison.Ordinal)) return 1;
            return Math.Max(1, defaultQuota);
        }
        ...
        private bool V68PatternQualityEligible(PatternSignal s)
        {
            if (s == null || s.Profile == null) return false;
            double minG = Math.Max(MinGeometryQuality, s.Profile.MinGeometry);
            double minP = Math.Max(MinPrzConfluence, s.Profile.MinPrz);
            if (s.PatternName == "AB=CD")
                return s.GeometryQuality >= minG && s.PrzConfluence >= minP;

            bool strict = s.GeometryQuality >= minG && s.PrzConfluence >= minP;
            double jointFloor = Math.Max(.55, 0.50 * (minG + minP) - .03);
            bool joint = s.Confidence >= jointFloor &&
                         s.GeometryQuality >= Math.Max(.42, minG - .10) &&
                         s.PrzConfluence >= Math.Max(.42, minP - .10) &&
                         s.TimeSymmetry >= .35 && s.PivotQuality >= .30;
            if (!strict && joint) _v68NamedFamilyJointQualityAdmitted++;
            return strict || joint;
        }

        private HarmonicRoute V68FamilyRouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            bool sessionVol = r.AtrRatio >= .50 && r.AtrRatio <= 1.90;

            if (V68IsRetracementFamily(p))
            {
                if (aligned && conflict != MtfConflict.CONFLICT && sessionVol && r.Efficiency >= .14)
                    return HarmonicRoute.TREND_ALIGNED_REVERSAL;
                if ((p == "Rat" || p == "Deep Gartley") && transition && sessionVol &&
                    s.Confidence >= .62 && s.PrzConfluence >= .55)
            ...
            if (V68IsExtensionFamily(p))
            {
                if (opposed && sessionVol && r.ExtensionAtr >= 1.25 && r.AdxH1Slope <= .50 &&
                    s.Confidence >= .60)
                    return HarmonicRoute.EXHAUSTION_REVERSAL;
                if (transition && sessionVol && s.Confidence >= .62)
                    return HarmonicRoute.TRANSITION_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            }
            ...
            if (p == "Cypher")
            {
                if (aligned && conflict != MtfConflict.CONFLICT && sessionVol && r.Efficiency >= .16)
                    return HarmonicRoute.TREND_ALIGNED_REVERSAL;
                if (opposed && sessionVol && r.ExtensionAtr >= 1.20 && r.AdxH1Slope <= .25)
                    return HarmonicRoute.EXHAUSTION_REVERSAL;
                if (transition && sessionVol) return HarmonicRoute.TRANSITION_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            }

            if (p == "Shark")
            {
                if (opposed && sessionVol && r.ExtensionAtr >= 1.35 && r.AdxH1Slope <= 0 &&
                    s.Confidence >= .62)
                    return HarmonicRoute.EXHAUSTION_REVERSAL;
                if (transition && sessionVol && s.Confidence >= .64)
                    return HarmonicRoute.TRANSITION_REVERSAL;
                if (aligned && sessionVol && r.Efficiency >= .24 && r.AdxH1Slope >= 0)
                    return HarmonicRoute.TREND_ALIGNED_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            }
            ...
            if (p == "5-0")
            {
                if (opposed && sessionVol && r.ExtensionAtr >= 1.40 && r.AdxH1Slope <= 0)
                    return HarmonicRoute.EXHAUSTION_REVERSAL;
                if (transition && sessionVol && s.Confidence >= .64)
                    return HarmonicRoute.TRANSITION_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            ...
            if (p == "AB=CD")
            {
                if (aligned && conflict != MtfConflict.CONFLICT && sessionVol && r.Efficiency >= .20 &&
                    s.GeometryQuality >= .60 && s.PrzConfluence >= .58)
                    return HarmonicRoute.TREND_ALIGNED_REVERSAL;
                if (transition && sessionVol && s.Confidence >= .68)
                    return HarmonicRoute.TRANSITION_REVERSAL;
                return HarmonicRoute.NO_TRADE;
            }
        ...
        private double V68FamilyThesisFit(CandidateRecord c)
        {
            ...
            double mtf = c.Conflict == MtfConflict.ALIGNED ? 1.0 :
                         ...
                         c.Conflict == MtfConflict.TRANSITION ? .70 : .55;
            if (V68IsRetracementFamily(p))
                return VClamp(.35 * mtf + .30 * c.Regime.Efficiency +
                              .20 * c.ConfirmationScore + .15 * c.Signal.PrzConfluence);
            if (V68IsExtensionFamily(p) || p == "Shark" || p == "5-0")
            {
                double terminality = VClamp(c.Regime.ExtensionAtr / 2.0);
                double decel = VClamp(.50 - c.Regime.AdxH1Slope / 10.0);
                return VClamp(.30 * terminality + .25 * decel + .25 * c.ConfirmationScore +
                              .20 * c.Signal.PrzConfluence);
            }
            if (p == "Cypher")
                return VClamp(.30 * mtf + .25 * c.Regime.Efficiency + .25 * c.ConfirmationScore +
                              .20 * c.Signal.GeometryQuality);
            return VClamp(.30 * mtf + .25 * c.ConfirmationScore + .25 * c.Signal.GeometryQuality +
                          .20 * c.Signal.PrzConfluence);
        }
        ...
        private HarmonicRoute RouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (!V68FamilyRouteEligible(s, baseRoute, conflict, r))
            {
                _v68FamilyRouteRejected++;
                Print("[V68-FAMILY-ROUTE-REJECT] pattern={0} route={1} conflict={2} trend={3} extensionAtr={4:F3} efficiency={5:F3}",
                    s.PatternName, baseRoute, conflict, r.TrendDirection, r.ExtensionAtr, r.Efficiency);
                return HarmonicRoute.NO_TRADE;
            }
        ...
        private bool V68FamilyNativeConfirmationPass(int i, CandidateRecord c, out double score)
        {
            score = 0;
            if (i < 3 || i >= _m1Bars.Count || c == null || c.Signal == null) return false;
            var sig = c.Signal;
            ...
            if (p == "Gartley")
            {
                score = (c.FamilyReclaim ? .35 : 0) + ((c.FamilyBos || c.FamilyDisplacement) ? .35 : 0) +
                        (c.FamilyDirectional ? .20 : 0) + (c.FamilyRejection ? .10 : 0);
                return c.FamilyReclaim && (c.FamilyBos || c.FamilyDisplacement) && c.FamilyDirectional;
            }
            if (p == "Bat")
            {
                score = (c.FamilyReclaim ? .30 : 0) + ((c.FamilyFailedExtension || c.FamilyRejection) ? .30 : 0) +
                        ((c.FamilyBos || c.FamilyDisplacement) ? .30 : 0) + (c.FamilyDirectional ? .10 : 0);
                return c.FamilyReclaim && (c.FamilyFailedExtension || c.FamilyRejection) &&
                       (c.FamilyBos || c.FamilyDisplacement);
            }
            if (p == "Deep Gartley")
            {
                score = (c.FamilyReclaim ? .30 : 0) + (c.FamilyFailedExtension ? .30 : 0) +
                        ((c.FamilyBos || c.FamilyDisplacement) ? .30 : 0) + (c.FamilyDirectional ? .10 : 0);
                return c.FamilyReclaim && c.FamilyFailedExtension && (c.FamilyBos || c.FamilyDisplacement);
            }
            if (p == "Rat")
            {
                score = (c.FamilyReclaim ? .30 : 0) + ((c.FamilyFailedExtension || c.FamilyRejection) ? .25 : 0) +
            ...
            if (p == "Alt Bat")
            {
                score = ((c.FamilySweep || c.FamilyFailedExtension) ? .30 : 0) + (c.FamilyReclaim ? .30 : 0) +
                        (c.FamilyDisplacement ? .25 : 0) + (c.FamilyDirectional ? .15 : 0);
                return (c.FamilySweep || c.FamilyFailedExtension) && c.FamilyReclaim &&
                       c.FamilyDisplacement && c.FamilyDirectional;
            }
            if (p == "Butterfly")
            {
                score = (c.FamilySweep ? .30 : 0) + ((c.FamilyReclaim || c.FamilyInsidePrz) ? .25 : 0) +
                        ((c.FamilyBos || c.FamilyDisplacement) ? .30 : 0) + (c.FamilyDirectional ? .15 : 0);
                return c.FamilySweep && (c.FamilyReclaim || c.FamilyInsidePrz) &&
                       (c.FamilyBos || c.FamilyDisplacement) && c.FamilyDirectional;
            }
            if (p == "Crab" || p == "Deep Crab")
            {
                score = (c.FamilyFailedExtension ? .30 : 0) + (c.FamilyReclaim ? .30 : 0) +
                        (c.FamilyDisplacement ? .25 : 0) + (c.FamilyDirectional ? .15 : 0);
                return c.FamilyFailedExtension && c.FamilyReclaim && c.FamilyDisplacement;
            }
            if (p == "Shark")
            {
                score = ((c.FamilySweep || c.FamilyFailedExtension) ? .30 : 0) + (c.FamilyReclaim ? .30 : 0) +
            ...
            if (p == "Cypher")
            {
                score = (c.FamilyReclaim ? .35 : 0) + ((c.FamilyBos || c.FamilyDisplacement) ? .30 : 0) +
                        (c.FamilyDirectional ? .20 : 0) + ((c.FamilyRejection || c.FamilyFailedExtension) ? .15 : 0);
                return c.FamilyReclaim && (c.FamilyBos || c.FamilyDisplacement) && c.FamilyDirectional;
            }
            if (p == "5-0")
            {
                score = (c.FamilyFailedExtension ? .30 : 0) + ((c.FamilyRetest || c.FamilyReclaim) ? .25 : 0) +
                        ((c.FamilyBos || c.FamilyDisplacement) ? .30 : 0) + (c.FamilyDirectional ? .15 : 0);
                return c.FamilyFailedExtension && (c.FamilyRetest || c.FamilyReclaim) &&
                       (c.FamilyBos || c.FamilyDisplacement);
            }
            if (p == "AB=CD")
            {
        ...
        private double CandidateRank(CandidateRecord c)
        {
            ...
            double familyFit = V68ProductEnabled() ? V68FamilyThesisFit(c) : .50;
            double baseRank = .20 * c.Signal.GeometryQuality + .13 * c.Signal.PrzConfluence + .12 * mtf + .10 * regime +
                              .18 * c.ConfirmationScore + .12 * VClamp(c.NetRR / 3.0) + .15 * familyFit;
