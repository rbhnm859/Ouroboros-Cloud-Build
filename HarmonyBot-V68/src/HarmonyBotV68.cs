...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV68 : Robot
    {
        private const string Version = "HarmonyBot V68 — Harmonic Family Identity & Detection Graph Reconstruction Kernel";
        private const string BotPrefix = "HB68";
        ...
        [Parameter("V68 Family Trade Contracts", DefaultValue = false)]
        public bool EnableV68FamilyTradeContracts { get; set; }

        [Parameter("V68 Family Detector Frontier", DefaultValue = false)]
        public bool EnableV68FamilyDetectorFrontier { get; set; }

        [Parameter("V68 Family-Native Grid", DefaultValue = false)]
        public bool EnableV68FamilyNativeGrid { get; set; }

        [Parameter("V68 Evidence Route Guard", DefaultValue = false)]
        public bool EnableV68EvidenceRouteGuard { get; set; }

        [Parameter("V68 Controlled Expansion", DefaultValue = false)]
        public bool EnableV68ControlledExpansion { get; set; }

        [Parameter("V68 Family Grid Atlas", DefaultValue = false)]
        public bool EnableV68FamilyGridAtlas { get; set; }

        [Parameter("V68 Positive Throughput Expansion", DefaultValue = false)]
        public bool EnableV68PositiveThroughputExpansion { get; set; }

        [Parameter("V68 Family Opportunity Scheduler", DefaultValue = false)]
        public bool EnableV68FamilyOpportunityScheduler { get; set; }
        ...
        protected override void OnStart()
        {
            ...
            if (_symbol == null)
            {
                Print("[V68-FATAL] SYMBOL_NOT_FOUND {0}", SymbolName);
                Stop();
                return;
            }
            ...
            if (!BarsObjectsReady())
            {
                Print("[V68-FATAL] TIMEFRAME_OBJECT_LOAD_FAILED");
                Stop();
                return;
            }
            ...
            if (!BarsReady())
                Print("[V68-WARMUP-PENDING] H4={0} H1={1} M15={2} M1={3}",
            ...
            {
                Print("[V68-FATAL] DST_TIMEZONE_UNAVAILABLE");
                Stop();
                return;
            }
            ...
            BuildV68FamilyTradeContracts();
            BuildV68FamilyGridAtlas();
            if (EnableV68FamilyNativeGrid && !EnableV68FamilyGridAtlas) ApplyV68FamilyGridContracts();
            ...
            Print("[V68-START] version={0} symbol={1} H4={2} H1={3} M15={4} M1={5} profiles={6}",
                Version, SymbolName, Count(_h4Bars), Count(_h1Bars), Count(_m15Bars), Count(_m1Bars), _profiles.Count);
            Print("[V68-TIMEFRAME-AUDIT] primaryPattern=M15 execution=M1 macro=H4 intermediate=H1 allCompletedBars=true");
            Print("[V68-SESSION-AUDIT] london={0} newYork={1} dstAware=true", _londonTz.Id, _newYorkTz.Id);
            Print("[V68-ALPHA-CONFIG] qualityObservation={0} legacyRegime={1} legacyEnhancedM1={2} capitalFeasibility={3} transitionVeto={4} exhaustionVeto={5} routeM1Veto={6}",
            ...
            Print("[V68-FREQUENCY-CONFIG] deferredRetention={0} agingPriority={1} ageBoost={2:F3} structuredRecall={3} recallGeometry={4:F3} recallPrz={5:F3} recallConfidence={6:F3}",
            ...
            Print("[V68-RESTORED-KERNEL] canonicalSetup={0} canonicalStandard={1} independentPivotGraph={2} transitionProof={3} m1Rescue={4} rescueBars={5} diversityScheduler={6} executionClock=M1 projectedD=false",
            ...
            Print("[V68-CONVERSION-ARCH] scaleRouteAdmission={0} temporalRescue={1} armedGrace={2} graceMinutes={3} preExecutionRevalidation={4}",
            ...
            Print("[V68-FAMILY-NATIVE] conversion={0} observation={1} principle=PATTERN_IDENTITY_NEQ_EXECUTION_IDENTITY", EnableFamilyNativeConversion, EnableFamilyNativeObservation);
            Print("[V68-COMPLETION-CONTRACT] canonicalContracts={0} familyCompletion={1} windowBars={2} provenLane=ABCD_SHARK_CYPHER_FROZEN",
                EnableCanonicalFamilyContracts, EnableFamilyCompletionContract, FamilyConfirmationWindowBars);
            Print("[V68-STRUCTURAL-GRID-CONTRACT] gridSpanV2={0} structuralInvalidationV2={1} rule=PRZ_LEGALITY_PLUS_RISK_RR_NOT_LEGACY_XA_MINSPAN",
                EnableGridSpanSemanticV2, EnableStructuralInvalidationV2);
            Print("[V68-MATH-GEOMETRY] jointGeometry={0} executionCorridor={1} entryAnchorForensics={2} rule=FAMILY_NATIVE_GEOMETRY_WITHOUT_RISK_RELAXATION",
                EnableFamilyNativeJointGeometry, EnableFamilyNativeExecutionCorridor, EnableEntryAnchorForensics);
            Print("[V68-DETECTOR-ARCH] identityReconstruction={0} boundedPivotGraph={1} maxMicroSkips={2} familyQuota={3} projectedPrz={4} truthLedger={5} rule=DETECT_MANY_EXECUTE_ONE",
            ...
            Print("[V68-THROUGHPUT-ARCH] persistentQueue={0} serialHandoff={1} nativeM1={2} nativeBars={3} decayRanking={4} hardLifetimeMinutes={5} alphaKernel=V46_SCALE_CONVERSION_FROZEN",
            ...
            Print("[V68-FAMILY-TRADE-CONTRACTS] enabled={0} detectorFrontier={1} familyGrid={2} evidenceGuard={3} controlledExpansion={4} contracts={5} principle=FAMILY_IDENTITY_ROUTE_CONFIRMATION_GRID_SEPARATED",
                EnableV68FamilyTradeContracts, EnableV68FamilyDetectorFrontier, EnableV68FamilyNativeGrid,
                EnableV68EvidenceRouteGuard, EnableV68ControlledExpansion, _familyTradeContracts.Count);
            Print("[V68-GRID-ATLAS] enabled={0} positiveThroughputExpansion={1} opportunityScheduler={2} modules={3} rule=NO_FIXED_FAMILY_QUOTA_EACH_FAMILY_MUST_PROVE_OWN_POSITIVE_CONTRIBUTION",
                EnableV68FamilyGridAtlas, EnableV68PositiveThroughputExpansion, EnableV68FamilyOpportunityScheduler, _familyGridAtlas.Count);
            foreach (var gm in _familyGridAtlas.OrderBy(x => x.Key))
            {
                var g = gm.Value;
                Print("[V68-FAMILY-GRID-MODULE] pattern={0} geometry={1} scale={2:F3} ttlMin={3} cancelMfeR={4:F2} Tlegs={5} Elegs={6} Xlegs={7}",
            ...
            {
                var fc = kv.Value;
                Print("[V68-FAMILY-POLICY] pattern={0} role={1} routes=T{2}/E{3}/X{4} confirm={5} gridLegs={6} runner={7} expiryBars={8}",
        ...
        protected override void OnStop()
        {
            ...
            {
                var x = kv.Value;
                Print("[V68-PIPELINE] pattern={0} detected={1} validated={2} routed={3} prz={4} confirming={5} nativeTemporalPass={6} armed={7} slotBlocked={8} parked={9} revalidated={10} revalidationRejected={11} basketPlanned={12} leg0={13} leg1={14} leg2={15} leg3={16} basketClosed={17} executed={18} expired={19} rejected={20} invalidated={21}",
            ...
            if (EnableEntryAnchorForensics)
            {
                ...
                {
                    Print("[V68-ENTRY-ANCHOR-FORENSICS] cid={0} setup={1} pattern={2} route={3} scale={4} completion={5} confirm={6} retest={7} completionMfeR={8:F3} completionMaeR={9:F3} confirmMfeR={10:F3} confirmMaeR={11:F3} retestMfeR={12:F3} retestMaeR={13:F3}",
            ...
            if (EnableDetectorTruthLedger)
            {
                foreach (var kv in _detectorTruth.OrderBy(x => x.Key))
                {
                    ...
                    Print("[V68-DETECTOR-TRUTH] pattern={0} stage={1} count={2}", pattern, stage, kv.Value);
                }
            }
            Print("[V68-SUMMARY] candidates={0} baskets={1} openLedgers={2} executionErrors={3} gridRiskViolations={4} duplicateGridLegs={5} orphanPendingOrders={6} stopWideningViolations={7} gapThroughInvalidations={8} gapThroughSurvivors={9} unprotectedSurvivors={10} postFillProtectionFailures={11} actualBasketRiskViolations={12} executionStateViolations={13} virtualGridFills={14} microModeBaskets={15} capitalRejectedBaskets={16} marginRiskViolations={17}",
            ...
            Print("[V68-CONVERSION-SUMMARY] scaleRouteRejected={0} temporalRescueAdmissions={1} armedGraceExtended={2} preExecutionRevalidationRejected={3}",
            ...
            Print("[V68-THROUGHPUT-SUMMARY] slotBlocked={0} parked={1} revalidated={2} revalidationRejected={3} recoveredExecutions={4} nativeTemporalPass={5} hardLifetimeExpired={6} decayRejected={7} avgSlotWaitMin={8:F2} medianSlotWaitMin={9:F2} p90SlotWaitMin={10:F2} avgBasketOccupancyMin={11:F2} missedPositive={12} avoidedNegative={13}",
            ...
            foreach (var p in _profiles.Select(x => x.Name).Distinct().OrderBy(x => x))
            {
                ...
                Print("[V68-FAMILY-CONTRACT-SUMMARY] pattern={0} pass={1} windowReject={2}", p, pass, reject);
            }
            foreach (var kv in _gridPlanRejectReasons.OrderBy(k => k.Key))
                Print("[V68-GRID-REJECT-SUMMARY] reason={0} count={1}", kv.Key, kv.Value);
            foreach (var kv in _executionErrorReasons.OrderBy(k => k.Key))
                Print("[V68-EXECUTION-ERROR-SUMMARY] code={0} count={1}", kv.Key, kv.Value);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                var record = new CandidateRecord
                {
                    ...
                    ExpiryUtc = Server.Time.ToUniversalTime().AddMinutes(15.0 * (EnableV68FamilyTradeContracts ? V68FamilyExpiryBars(signal) : Math.Max(2, Math.Min(CandidateTtlM15Bars, signal.Profile.MaxAgeM15Bars)))),
                ...
                bool familyQualityPass = EnableV68FamilyTradeContracts
                    ? V68FamilyQualityPass(signal)
                ...
                record.RegimeScore = EnableV68FamilyTradeContracts ? V68FamilyRegimeScore(signal, record.Conflict, regime) : RegimeContextScore(signal, record.Conflict, regime);
                record.Route = EnableV68FamilyTradeContracts ? RouteSignalFamilyNativeV68(signal, record.Conflict, regime) : RouteSignal(signal, record.Conflict, regime);
                ...
                if (!EnableV68FamilyTradeContracts && EnableScaleRouteAdmission && signal.PivotScale != M15SwingDepth &&
                ...
                Print("[V68-ALPHA-CANDIDATE] cid={0} pattern={1} quality={2:F3} regime={3:F3} conflict={4} route={5} adxH1={6:F2} adxH4={7:F2} adxSlope={8:F2} atrPct={9:F3} efficiency={10:F3} capitalFeasible={11} minL0Risk={12:F4}",
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    bool familyContractLane = EnableV68FamilyTradeContracts || (EnableFamilyCompletionContract && IsFamilyCompletionLane(c.Signal.PatternName));
                    ...
                    if (familyContractLane)
                    {
                        ...
                        double familyScore;
                        confirmationPass = EnableV68FamilyTradeContracts
                            ? UpdateV68FamilyContractEvidence(i, c, out familyScore)
                        ...
                        if (confirmationPass)
                        {
                            ...
                            Event(c, (EnableV68FamilyTradeContracts ? "V68_FAMILY_TRADE_CONTRACT_PASS_" : "FAMILY_COMPLETION_CONTRACT_PASS_") + familyScore.ToString("F2", CultureInfo.InvariantCulture));
                        }
                    ...
                    if (EnableRouteSpecificM1Veto && !(EnableV68FamilyTradeContracts ? V68FamilyRouteEvidencePass(i, c) : RouteSpecificM1EvidencePass(i, c.Signal, c.Route)))
                    {
        ...
        private void TryScheduleAndExecute()
        {
            ...
            var armed = armedQuery
                .OrderByDescending(c => EnableV68FamilyOpportunityScheduler
                    ? V68FamilyOpportunityScore(c, now)
        ...
        private void MarkOpportunityTerminal(CandidateRecord c, string reason)
        {
            ...
            Print("[V68-OPPORTUNITY-LOSS] cid={0} setup={1} pattern={2} route={3} scale={4} reason={5} shadowMfeR={6:F3} shadowMaeR={7:F3} missedPositive={8} avoidedNegative={9}",
        ...
        {
            if (EnableV68FamilyGridAtlas)
                return TryBuildV68FamilyGridAtlasPlan(c);
            ...
            Print("[V68-GRID-PLAN] cid={0} pattern={1} route={2} logicalLegs={3} physicalDepth={4} micro={5} anchor={6} weighted={7} virtualWeighted={8} stop={9} target={10} budget={11:F2} worst={12:F2} margin={13:F2} netRR={14:F3}",
            ...
            foreach (var leg in plan.Legs)
                Print("[V68-GRID-LEG-PLAN] cid={0} leg=L{1} fraction={2:F3} price={3} weight={4:F6} physical={5} volume={6} budget={7:F2} risk={8:F2} minBrokerRisk={9:F2} state={10}",
        ...
        private void PrintCapitalCompatibilityMatrix(FibonacciGridPlan plan)
        {
            ...
            Print("[V68-CAPITAL-COMPAT] cid={0} logicalLegs={1} riskOnlyPhysicalDepths={2}", plan.CandidateId, plan.LogicalLegCount, matrix);
        }
        ...
        private void CancelAllOwnPending(string reason)
        {
            ...
            Print("[V68-PENDING-CANCEL-ALL] reason={0}", reason);
        }
        ...
        private void PostFillSafetyKernel(Position p, string source)
        {
            ...
            {
                Print("[V68-POST-FILL-DEDUPE] pos={0} source={1}", p.Id, source);
                return;
            }
        ...
        private void PostFillSafetyKernelCore(Position p, string source)
        {
                ...
                leg.State == GridLegState.REJECTED || leg.State == GridLegState.RISK_REJECTED)
            {
                ...
                Print("[V68-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next=FILLED_UNVERIFIED reason=LATE_FILL_AFTER_TERMINAL_STATE",
            ...
            Print("[V68-POST-FILL-AUDIT] basket={0} leg=L{1} pos={2} source={3} entry={4} stop={5} target={6} actualWorst={7:F4} budget={8:F4} protected=true",
        ...
        private void TransitionLegState(FibonacciBasket basket, FibonacciGridLeg leg, GridLegState next, string reason)
        {
            ...
            {
                _executionStateViolations++;
                Print("[V68-STATE-VIOLATION] basket={0} leg=L{1} prior={2} next={3} reason={4}", basket == null ? "" : basket.BasketId, leg.Index, prior, next, reason);
            }
            leg.State = next;
        ...
        private void PrintBrokerCapabilityProfile()
        {
            ...
            Print("[V68-BROKER-PROFILE] symbol={0} equity={1:F2} freeMargin={2:F2} minVolume={3} step={4} maxVolume={5} pipValue={6} tickValue={7} minSL={8} minTP={9} minDistanceType={10} minMarginBuy={11:F2} minMarginSell={12:F2} minimumSupportedEquity={13:F2} adaptiveCapital={14} microThreshold={15:F2} initialCapitalEligible={16}",
        ...
        private void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            {
                Print("[V68-LEG-CLOSED] pos={0} basket=UNKNOWN net={1:F2} reason={2}", p.Id, p.NetProfit, args.Reason);
                return;
            }
            ...
            if (_baskets.TryGetValue(l.BasketId, out basket))
            {
                ...
                Print("[V68-LEG-CLOSED] basket={0} cid={1} leg=L{2} pos={3} pattern={4} route={5} dir={6} mfeR={7:F3} maeR={8:F3} realizedR={9:F3} net={10:F2} reason={11}",
        ...
        private void CloseBasketLedger(FibonacciBasket basket, string reason)
        {
            ...
            Print("[V68-BASKET-CLOSED] basket={0} cid={1} setup={2} pattern={3} subtype={4} route={5} dir={6} plannedLegs={7} filledLegs={8} anchor={9} avgEntry={10} entryImprovePips={11:F3} stop={12} target={13} initialRisk={14:F2} worstRisk={15:F2} mfeR={16:F3} maeR={17:F3} realizedR={18:F3} net={19:F2} reason={20}",
            ...
            Print("[V68-SLOT-OCCUPANCY] basket={0} cid={1} pattern={2} route={3} occupancyMinutes={4:F2} realizedR={5:F3} net={6:F2}",
        ...
        {
            Print("[V68-BASKET-EVENT] basket={0} cid={1} pattern={2} route={3} state={4} reason={5}",
        ...
        private List<PatternSignal> DetectPatternCandidates(Bars bars, int endIndex, int depth, int lookback, int maxCandidates, string timeframe)
        {
            ...
            if (EnableV68FamilyDetectorFrontier)
            {
                // V68 deliberately removes per-family trading quotas. Supply is ranked by family-native
                ...
                var frontier = hypotheses
                    .OrderByDescending(V68FamilyDetectionPriority)
                    .ThenByDescending(x => x.Confidence)
                    .ThenByDescending(x => x.GeometryQuality)
                    .Take(Math.Max(48, maxCandidates * 6))
                    .ToList();
                foreach (var x in frontier) DetectorTruth(x.PatternName, "V68_FAMILY_FRONTIER_SELECTED");
                return frontier;
            }
        ...
        private HarmonicRoute RouteSignal(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            if (baseRoute == HarmonicRoute.NO_TRADE && EnableStructuredRecallExpansion)
            {
                ...
                if (baseRoute != HarmonicRoute.NO_TRADE)
                {
                    ...
                    Print("[V68-RECALL-ADMIT] pattern={0} direction={1} route={2} conflict={3} geometry={4:F3} prz={5:F3} confidence={6:F3} efficiency={7:F3} atrRatio={8:F3} extensionAtr={9:F3}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionProofGate)
            {
                ...
                if (!actualStructuralTransition)
                {
                    ...
                    Print("[V68-TRANSITION-PROOF-REJECT] pattern={0} conflict={1} transition={2} adxSlope={3:F2}",
            ...
            if (baseRoute == HarmonicRoute.TRANSITION_REVERSAL && EnableTransitionStateVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V68-ROUTE-VETO] type=TRANSITION_STATE conflict={0} transition={1} adxSlope={2:F2}",
            ...
            if (baseRoute == HarmonicRoute.EXHAUSTION_REVERSAL && EnableExhaustionEvidenceVeto)
            {
                ...
                {
                    _regimeRejected++;
                    Print("[V68-ROUTE-VETO] type=EXHAUSTION_EVIDENCE extensionAtr={0:F3} adxSlope={1:F2}",
        ...
        private void BuildV68FamilyTradeContracts()
        {
            ...
            AddV68FamilyContract("Gartley", "RETRACEMENT", true, false, false, "RECLAIM_REJECTION_BOS",
                .53, .53, .55, .14, 1.75, 1.30, 6, .010, false,
                new[] { 0.0, .236, .382, .618 }, new[] { .40, .30, .20, .10 });
            AddV68FamilyContract("Bat", "DEEP_RETRACEMENT", true, false, true, "REJECTION_RECLAIM_DISPLACEMENT",
                .50, .50, .54, .13, 1.75, 1.30, 6, .020, false,
                new[] { 0.0, .236, .382, .618 }, new[] { .42, .30, .18, .10 });
            AddV68FamilyContract("Alt Bat", "EXTENSION", false, true, true, "SWEEP_FAILED_EXTENSION_BOS",
                .52, .52, .56, .12, 1.80, 1.15, 7, .025, false,
                new[] { 0.0, .236, .382 }, new[] { .50, .30, .20 });
            AddV68FamilyContract("Butterfly", "EXTENSION", false, true, true, "SWEEP_RECLAIM_BOS",
                .54, .54, .58, .12, 1.80, 1.15, 7, .025, false,
                new[] { 0.0, .236, .382 }, new[] { .55, .30, .15 });
            AddV68FamilyContract("Crab", "EXTREME_EXTENSION", false, true, true, "SWEEP_FAILED_EXTENSION_DISPLACEMENT",
                .55, .55, .60, .10, 1.85, 1.10, 7, .030, false,
                new[] { 0.0, .236 }, new[] { .65, .35 });
            AddV68FamilyContract("Deep Crab", "EXTREME_EXTENSION", false, true, true, "SWEEP_PRZ_BOS",
                .55, .55, .60, .10, 1.85, 1.10, 7, .030, false,
                new[] { 0.0, .236 }, new[] { .65, .35 });
            AddV68FamilyContract("Deep Gartley", "DEEP_RETRACEMENT", true, false, true, "FAILED_EXTENSION_RECLAIM_BOS",
                .52, .52, .55, .13, 1.75, 1.30, 7, .020, false,
                new[] { 0.0, .236, .382 }, new[] { .50, .30, .20 });
            AddV68FamilyContract("Rat", "RETRACEMENT", true, false, false, "REJECTION_RECLAIM_BOS_DISPLACEMENT",
                .58, .58, .60, .15, 1.75, 1.25, 6, .080, true,
                new[] { 0.0, .236, .382 }, new[] { .50, .30, .20 });
            AddV68FamilyContract("Cypher", "XC_RETRACE", true, true, false, "REJECTION_RECLAIM_BOS_DISPLACEMENT",
                .58, .58, .60, .14, 1.80, 1.20, 6, .050, true,
                new[] { 0.0, .236, .382 }, new[] { .50, .30, .20 });
            AddV68FamilyContract("Shark", "TERMINAL_EXTENSION", true, true, false, "FAILED_EXTENSION_REJECTION_BOS",
                .58, .58, .60, .12, 1.85, 1.10, 6, .100, true,
                new[] { 0.0, .236 }, new[] { .65, .35 });
            AddV68FamilyContract("5-0", "TRANSITION_REVERSAL", false, false, true, "FAILED_EXTENSION_BOS_RETEST_DIRECTIONAL",
                .60, .60, .62, .10, 1.80, 1.20, 7, .000, false,
                new[] { 0.0, .236 }, new[] { .65, .35 });
            AddV68FamilyContract("AB=CD", "COMPLETION_PRIMITIVE", true, false, false, "DECELERATION_FAILED_EXTENSION_DIRECTIONAL_BOS",
                .58, .58, .60, .14, 1.75, 1.25, 6, .040, false,
                new[] { 0.0, .236 }, new[] { .70, .30 });
        }

        private void AddV68FamilyContract(string name, string role, bool trend, bool exhaustion, bool transition,
        ...
        private FamilyTradeContract V68Contract(string pattern)
        {
        ...
        private bool V68FamilyQualityPass(PatternSignal s)
        {
            ...
            var c = V68Contract(s.PatternName);
            if (c == null)
        ...
        private bool V68AbcdStandaloneEligible(PatternSignal s)
        {
            ...
            if (EnableV68ControlledExpansion && s.HarmonicSubtype == "ABCD_NEAR_127" &&
                s.GeometryQuality >= .72 && s.PrzConfluence >= .72 && s.Confidence >= .68)
                return true;
            return false;
        }

        private double V68AbcdConfluenceScore(PatternSignal s)
        {
            if (s == null) return 0;
        ...
        private double V68FamilyDetectionPriority(PatternSignal s)
        {
            if (s == null) return 0;
            var c = V68Contract(s.PatternName);
            double boost = c == null ? 0 : c.DetectorPriorityBoost;
            double baseScore = .45 * s.Confidence + .30 * s.GeometryQuality + .15 * s.PrzConfluence + .10 * V68AbcdConfluenceScore(s);
        ...
        private double V68FamilyRegimeScore(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            ...
            return VClamp(.30 * mtf + .25 * direction + .20 * efficiencyFit + .15 * volFit + .10 * V68AbcdConfluenceScore(s));
        }

        private HarmonicRoute RouteSignalFamilyNativeV68(PatternSignal s, MtfConflict conflict, RegimeSnapshot r)
        {
            if (s == null || r == null) return HarmonicRoute.NO_TRADE;
            var c = V68Contract(s.PatternName);
            if (c == null) return RouteSignal(s, conflict, r);
            if (!V68FamilyQualityPass(s)) return HarmonicRoute.NO_TRADE;
            if (s.PatternName == "AB=CD" && !V68AbcdStandaloneEligible(s))
            {
            ...
            if (EnableV68EvidenceRouteGuard)
            {
            ...
            if (EnableV68ControlledExpansion)
            {
        ...
        private bool UpdateV68FamilyContractEvidence(int i, CandidateRecord c, out double score)
        {
            score = 0;
            ...
            switch (s.PatternName)
            {
                ...
                case "AB=CD":
                    if (!V68AbcdStandaloneEligible(s)) return false;
                    score = (c.FamilyDeceleration ? .20 : 0) + (c.FamilyFailedExtension ? .25 : 0) + (c.FamilyDirectional ? .20 : 0) + (c.FamilyBos ? .25 : 0) + .10 * V68AbcdConfluenceScore(s);
                    return c.FamilyDeceleration && c.FamilyFailedExtension && c.FamilyDirectional && c.FamilyBos && score >= .85;
            }
            return false;
        }

        private bool V68FamilyRouteEvidencePass(int i, CandidateRecord c)
        {
        ...
        private double V68FamilyNativeCandidateRank(CandidateRecord c)
        {
            var fc = c == null || c.Signal == null ? null : V68Contract(c.Signal.PatternName);
            ...
            double score = .22 * c.Signal.GeometryQuality + .16 * c.Signal.PrzConfluence + .16 * c.Signal.Confidence +
                           .14 * confirmation + .12 * routeFit + .10 * rr + .10 * V68AbcdConfluenceScore(c.Signal) +
                           fc.DetectorPriorityBoost - agePenalty;
            if (c.Signal.PatternName == "AB=CD" && c.Signal.HarmonicSubtype == "ABCD_LEGACY_BROAD") score -= .35;
            return score;
        }

        private void ApplyV68FamilyGridContracts()
        {
            foreach (var p in _profiles)
            {
                var c = V68Contract(p.Name);
                if (c == null) continue;
                p.GridEnabled = true;
        ...
        private bool V68IsSupplyRecoveryFamily(string pattern)
        {
        ...
        private FamilyGridModule V68GridModule(string pattern)
        {
            FamilyGridModule m;
            return pattern!=null && _familyGridAtlas.TryGetValue(pattern,out m)?m:null;
        }

        private bool V68RouteGrid(FamilyGridModule m,HarmonicRoute route,out double[] fractions,out double[] weights)
        {
        ...
        private bool TryBuildV68FamilyGridAtlasPlan(CandidateRecord c)
        {
            if(c==null||c.Signal==null)return GridPlanReject(c,"V68_NULL_SIGNAL");
            var m=V68GridModule(c.Signal.PatternName);
            if(m==null)return GridPlanReject(c,"V68_NO_FAMILY_GRID_MODULE");
            double[] fractions,weights;
            if(!V68RouteGrid(m,c.Route,out fractions,out weights))return GridPlanReject(c,"V68_ROUTE_NOT_NATIVE_FOR_FAMILY");
            if(c.Signal.PatternName=="AB=CD"&&!V68AbcdStandaloneEligible(c.Signal))return GridPlanReject(c,"V68_ABCD_CONFLUENCE_ONLY");
            if(!_initialCapitalEligible)return GridPlanReject(c,"INITIAL_CAPITAL");
            ...
            double unit=V68GridExecutionUnit(c,m);
            if(!double.IsFinite(unit)||unit<=_symbol.TickSize)return GridPlanReject(c,"V68_INVALID_GRID_UNIT");
            ...
            if(netRr<MinimumNetRR)return GridPlanReject(c,"V68_NET_RR");
            plan.CanonicalTarget=target;plan.ExpectedNetRR=netRr;c.GridPlan=plan;c.SelectedTarget=target;c.NetRR=netRr;
            Print("[V68-FAMILY-GRID-PLAN] cid={0} pattern={1} route={2} module={3} unit={4} logicalLegs={5} physicalDepth={6} weighted={7} stop={8} target={9} budget={10:F2} worst={11:F2} rr={12:F3}",
        ...
        private double V68FamilyOpportunityScore(CandidateRecord c,DateTime now)
        {
        ...
        {
            if (EnableV68FamilyTradeContracts) return V68FamilyNativeCandidateRank(c);
        ...
        private void RecordExecutionError(string code, string detail)
        {
            ...
            Print("[V68-EXECUTION-ERROR] code={0} detail={1}", code, detail ?? "");
        }
        ...
        {
            _candidateSeq++;
            return "V68-" + _candidateSeq.ToString("D8", CultureInfo.InvariantCulture) + "-" +
        ...
        private void Ledger(CandidateRecord c, CandidateState state, string reason)
        {
            ...
            Print("[V68-EVENT] cid={0} setup={1} pattern={2} subtype={3} scale={4} tf={5} dir={6} state={7} route={8} conflict={9} waitMin={10:F2} reason={11}",
        ...
        private void WarmupBars(Bars bars, int minimum, string name)
        {
            ...
            while (bars.Count < minimum && loops < 32)
            {
                ...
                {
                    Print("[V68-WARMUP-ERROR] tf={0} count={1} error={2}", name, bars.Count, ex.Message);
                    break;
                }
                loops++;
                Print("[V68-WARMUP] tf={0} added={1} count={2}", name, added, bars.Count);
                if (added <= 0) break;
            }
        }
