...
namespace cAlgo.Robots
{
    ...
    public class HarmonyBotV61 : Robot
    {
        private const string Version = "HarmonyBot V61 — Champion-Core Fibonacci Grid Commercial Rebase";
        private const string BotPrefix = "HB61";
        ...
        protected override void OnStop()
        {
            ...
            Print("[V61-SUMMARY] variant={0} regimeVetoed={1} ratExpansionAdmitted={2} abcdCapitalRejected={3} runnerAssigned={4}",
                V61Variant, _v61RegimeVetoed, _v61RatExpansionAdmitted, _v61AbcdCapitalRejected, _v61RunnerAssigned);
        }
        ...
        private void ProcessNewM15Close()
        {
            ...
            foreach (var signal in detected)
            {
                ...
                if (!V61ExactControlEnabled() && signal.PatternName == "AB=CD")
                {
                    ...
                    Reject(record, "V61_ABCD_STANDALONE_CAPITAL_OFF");
                    continue;
                }
        ...
        private void ProcessNewM1Close()
        {
            ...
            foreach (var c in _candidates.Values.Where(x => x.IsActive).ToList())
            {
                ...
                if (c.State == CandidateState.CONFIRMING)
                {
                    ...
                    {
                        _v61RegimeVetoed++;
                        Reject(c, "V61_REGIME_SURVIVAL_VETO");
                        continue;
                    }
        ...
        private bool RevalidateCandidateForExecution(CandidateRecord c, DateTime now, bool finalCheck)
        {
            ...
            {
                _v61RegimeVetoed++;
                Event(c, "V61_REGIME_SURVIVAL_REVALIDATION_VETO");
                return false;
            }
        ...
        private bool V61CanonicalGridEnabled()
        {
            return string.Equals(V61Variant, "V61_CHAMPION_FIB_GRID", StringComparison.Ordinal) ||
                   string.Equals(V61Variant, "V61_GRID_REGIME_SURVIVAL", StringComparison.Ordinal) ||
                   string.Equals(V61Variant, "V61_COMMERCIAL_MAX", StringComparison.Ordinal);
        }

        private bool V61RegimeSurvivalEnabled()
        {
            return string.Equals(V61Variant, "V61_GRID_REGIME_SURVIVAL", StringComparison.Ordinal) ||
                   string.Equals(V61Variant, "V61_COMMERCIAL_MAX", StringComparison.Ordinal);
        }

        private bool V61CommercialMaxEnabled()
        {
            return string.Equals(V61Variant, "V61_COMMERCIAL_MAX", StringComparison.Ordinal);
        }
