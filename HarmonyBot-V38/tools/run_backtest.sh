...
ALGO="${ALGO:-seal/algo/HarmonyBot_V38_M15_M5_Evidence_Execution_RC.algo}"
...
CNAME="v38-$(echo "$RUN_NAME"|tr '[:upper:]_' '[:lower:]-')-$GITHUB_RUN_ID"
...
PID=$!; DONE=0
echo "[V38-WATCHDOG] run=$RUN_NAME pid=$PID timeoutSeconds=$BACKTEST_TIMEOUT_SECONDS"
for ((i=0;i<BACKTEST_TIMEOUT_SECONDS;i+=5)); do
 if (( i % 60 == 0 )); then echo "[V38-WATCHDOG] run=$RUN_NAME elapsedSeconds=$i status=running"; fi
...
test "$DONE" = 1 || { echo "[V38-WATCHDOG-FAIL] run=$RUN_NAME"; tail -400 "seal/logs/$RUN_NAME.log" || true; exit 20; }
echo "[V38-WATCHDOG] run=$RUN_NAME status=complete"
