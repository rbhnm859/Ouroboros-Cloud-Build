...
BALANCE="${BALANCE:-10000}"; ALGO="${ALGO:-seal/algo/HarmonyBot_V36_Dynamic_Reroute_RC.algo}"
...
CNAME="v36-reroute-$(echo "$RUN_NAME"|tr '[:upper:]_' '[:lower:]-')-$GITHUB_RUN_ID"
docker run --name "$CNAME" -v "$PWD/seal:/work" "$IMAGE" backtest "/work/${ALGO#seal/}" \
 ...
 --EnableExhaustionEvidenceVeto=true --EnableRouteSpecificM1Veto=false --EnableDeferredCandidateRetention=true \
 --EnableFrequencyAgingPriority=true --CandidateAgeRankBoost=0.08 --EnableStructuredRecallExpansion=true \
 --RecallMinGeometry=0.72 --RecallMinPrz=0.72 --RecallMinConfidence=0.68 --EnableDynamicRouteReevaluation=true \
