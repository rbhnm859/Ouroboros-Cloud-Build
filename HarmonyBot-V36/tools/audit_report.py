...
rx=re.compile(r"\[V36-BASKET-CLOSED\].*?mfeR=([-0-9.]+)\s+maeR=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
pat=(r"\[V36-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
freq=re.findall(r"\[V36-FREQUENCY-SUMMARY\].*?schedulerDeferred=(\d+)\s+schedulerRecoveredExecutions=(\d+)\s+structuredRecallAdmitted=(\d+)\s+activeDeferred=(\d+)",t)
fc={"scheduler_deferred":0,"scheduler_recovered_executions":0,"structured_recall_admitted":0,"active_deferred":0}
if freq:
 z=list(map(int,freq[-1]))
 for k,n in zip(fc,z): fc[k]=n
eq=d.get("equity",{}); clean_keys=["execution_errors","grid_risk_violations","duplicate_grid_legs","orphan_pending_orders",
"stop_widening_violations","gap_through_survivors","unprotected_survivors","post_fill_protection_failures",
"actual_basket_risk_violations","execution_state_violations","margin_risk_violations"]
...
"broker_profile_present":"[V36-BROKER-PROFILE]" in t,
"transition_veto_events":len(re.findall(r"\[V36-ROUTE-VETO\] type=TRANSITION_STATE",t)),
"exhaustion_veto_events":len(re.findall(r"\[V36-ROUTE-VETO\] type=EXHAUSTION_EVIDENCE",t)),
"m1_veto_events":len(re.findall(r"ROUTE_SPECIFIC_M1_VETO",t)),
"recall_admit_events":len(re.findall(r"\[V36-RECALL-ADMIT\]",t)),
"scheduler_defer_events":len(re.findall(r"SCHEDULER_DEFERRED_KEEP_ALIVE",t)),
"mean_mfe_r":statistics.mean([x["mfe"] for x in b]) if b else 0,
"mean_mae_r":statistics.mean([x["mae"] for x in b]) if b else 0,**fc,**c}
