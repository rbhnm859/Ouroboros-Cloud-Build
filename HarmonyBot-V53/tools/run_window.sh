...
test -s "$W/seal/data.snapshot.sha" || { echo "[V53-DATA-SNAPSHOT-MISSING] $W/seal/data.snapshot.sha"; exit 41; }
...
python3 "$C/HarmonyBot-V53/tools/audit_report.py" --report "$W/seal/reports/$N.json" --log "$W/seal/logs/$N.log" --out "$O/$FAM-$WIN.json" --window "$WIN" --family "$FAM" --years .5 --balance 10000 --data-sha "$DATA_SHA"
