...
s=pathlib.Path(sys.argv[1] if len(sys.argv)>1 else "HarmonyBot-V53/src/HarmonyBotV53.cs").read_text(errors="ignore")
checks={
"identity":"HarmonyBot V53" in s and "class HarmonyBotV53" in s and 'BotPrefix = "HB53"' in s,
...
"detector_truth":"V53-DETECTOR-TRUTH" in s and "DetectorTruth" in s,
"conversion_truth":"V53-CONVERSION-TRUTH" in s and "ConversionTruth" in s,
...
out={"version":"HarmonyBot V53","architecture":"FAMILY_NATIVE_CONVERSION_AND_GRID_CAUSAL_ATTRIBUTION","checks":checks,"pass":all(checks.values())}
pathlib.Path("V53_ARCHITECTURE_AUDIT.json").write_text(json.dumps(out,indent=2))
