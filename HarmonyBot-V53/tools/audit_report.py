...
pat=(r"\[V53-SUMMARY\].*?executionErrors=(\d+)\s+gridRiskViolations=(\d+)\s+duplicateGridLegs=(\d+)\s+"
...
evrx=re.compile(r"\[V53-EVENT\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+subtype=(\S+)\s+scale=(\d+)\s+tf=(\S+)\s+dir=(\S+)\s+state=(\S+)\s+route=(\S+)\s+conflict=(\S+)\s+waitMin=([-0-9.]+)\s+reason=(.*)$",re.M)
events=[]
...
oprx=re.compile(r"\[V53-OPPORTUNITY-LOSS\]\s+cid=(\S+)\s+setup=(\S+)\s+pattern=(.*?)\s+route=(\S+)\s+scale=(\d+)\s+reason=(\S+)\s+shadowMfeR=([-0-9.]+)\s+shadowMaeR=([-0-9.]+)\s+missedPositive=(True|False)\s+avoidedNegative=(True|False)")
opp=[]
...
sorx=re.compile(r"\[V53-SLOT-OCCUPANCY\].*?pattern=(.*?)\s+route=(\S+)\s+occupancyMinutes=([-0-9.]+)\s+realizedR=([-0-9.]+)\s+net=([-0-9.]+)")
...
truth_rx=re.compile(r"\[V53-DETECTOR-TRUTH\]\s+pattern=(.*?)\s+stage=(\S+)\s+count=(\d+)")
truth={}
for q in truth_rx.finditer(t):
 truth.setdefault(q.group(1),{})[q.group(2)]=int(q.group(3))
out["detector_truth"]=truth
conv_rx=re.compile(r"\[V53-CONVERSION-TRUTH\]\s+pattern=(.*?)\s+stage=(\S+)\s+count=(\d+)")
conv={}
for q in conv_rx.finditer(t): conv.setdefault(q.group(1),{})[q.group(2)]=int(q.group(3))
out["conversion_truth"]=conv
gmeta_rx=re.compile(r"\[V53-BASKET-CLOSED\].*?cid=(\S+).*?plannedLegs=(\d+)\s+filledLegs=(\d+).*?entryImprovePips=([-0-9.]+)")
...
out["grid_execution_enabled"]="[V53-GRID-CAUSAL-MODE] enabled=True" in t
